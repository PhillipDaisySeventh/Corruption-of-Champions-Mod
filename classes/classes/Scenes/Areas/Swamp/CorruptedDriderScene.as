...
//*Victory Rape: PC Penis goes in Vagina (done)
		private function winDriderPCDickInSpiderCunt():void
		{
			...
			outputText("Eager to plumb the depths of this greedy spider's snatch, you practically tear your way out of your " + player.armorName + " in your hurry to get naked.  Through it all ");
...
//Part 2 of the above
		private function winDriderPCDickInSpiderCunt2():void
		{
			...
			outputText(images.showImage("cDrider-win"));
			...
			outputText("You collapse into the dirt, not even caring that it stinks of dried cum and girl-jizz.  You've completely emptied out every speck of sperm from your " + player.ballsDescriptLight() + ", and you even kept going for a while after that.  It wasn't until " + player.sMultiCockDesc() + " went soft at last that you found enough peace to lie down.  ");
...
//*Victory Rape: Ride Ovi Vaginal (done)
		private function victoryVSDriderRideOviVaginal():void
		{
			...
			player.slimeFeed();
			outputText("Eyeing the small lump of the spider's retracted ovipositor, you resolve to have a little fun with it, even if it means giving the spider exactly what she wants in a way that she doesn't.  You tear off your " + player.armorName + " while the drider watches with ");
...
//*Victory Rape: Ride Ovi Anal sex-insenitive (done)
		public function victoryVSDriderRideOviAnal():void
		{
			...
			if (player.hasCock()) {
				outputText("  The warm organ brushes past your prostate, the slippery caress making " + player.sMultiCockDesc() + " dribble a ");
				if (player.cumQ() < 500) outputText("drop");
				else outputText("stream");
				outputText(" of pre-cum down its length.");
			}
			if (player.hasVagina()) {
				outputText("  Your " + player.vaginaDescript(0) + " quivers");
				if (player.wetness() >= 3) outputText(" and drips");
				outputText(" sympathetically with the sensation, and your " + player.clitDescript() + " peeks from the wet lips.");
			}
...
//(please view the spinneret as a spider-horsecock because for all intents and purposes that is really what it is) - shambadibs (1.5-inch width max because i'm racist against buur-bous american penis) (also probably a corruption minimum this is a pretty dick move) (I GET IT)
		private function victoryVSDriderStickDickInSpinneret():void
		{
			...
			if (player.biggestTitSize() >= 3) outputText("  The semi-sticky stuff soaks into the cleavage of your " + player.allBreastsDescript() + ", the feeling surprisingly enjoyable.  You even knead the web in a bit, biting your bottom lip from the sensation.");
			...
			if (player.balls > 0) outputText(", even giggling a little as your " + player.ballsDescriptLight() + " slap against her nose");
			outputText(".\n\n");

			outputText(images.showImage("cDrider-win-oral"));
			...
			outputText("\n\n");
			outputText("Your lust sated, you disengage from her with a little pop, leaving her to lick her lips happily and groan unintelligible babble.  You wipe your " + player.cockDescript(x) + " off on her hair to clean it off a bit, then redress and head out.  With a glance at the snoozing drider over your shoulder, you head out of the smelly swamp and start back towards camp.");
			player.orgasm('Dick');
			dynStats("cor", .5);
			combat.cleanupAfterCombat();
		}
...
//*Loss Vaginal Impregnation and forced BJ - Symphonie (done)
		private function loseVSDriderAndVaginallyImpreggleOvi():void {
			...
			if (!player.isNaga()) outputText("pull your own " + player.feet() + " apart");
			...
			if (!player.isTaur()) outputText("Her fingers then move to the sides of your head, lacing webbing through your hair and sticking it firmly to the ground. More vulnerable than ever, you can't lift your head without ripping your hair from your scalp in chunks.");
				...
				if (player.biggestTitSize() > 4) outputText("With pain and then a pop, your " + player.allBreastsDescript() + " squash out to the sides as she presses your torso into your knees.  ");
			...
			if (player.hasCock() && monster.nipplesPierced > 0) outputText("  Chains brush over " + player.sMultiCockDesc() + " as her chest passes your crotch.");
			outputText("  Her fangs trail teasingly along your belly, her fingers crawling up your sides.");
			if (player.biggestTitSize() > 3) outputText("  Venom dribbles down the cleavage of your " + player.biggestBreastSizeDescript() + " as she approaches your vulnerable face.  Her own quartet of matronly tits comes to rest on them as her torso appears over their horizon.");
			outputText("\n\n");
		...
		private function loseVSDriderAndVaginallyImpreggleOviEnding():void {
			spriteSelect(SpriteDb.s_drider);
			clearOutput();
			outputText(images.showImage("cDrider-loss-vag"));
...
//*Loss Ride & Impregnate Butt (done)
		private function loseVSDriderAndGetFuckedANDAnalImpreggegity():void
		{
			...
			clearOutput();
			if (player.lust >= player.maxLust()) outputText("Unable to resist the desire coursing through your body, you strip off your " + player.armorName + " and start stroking " + player.sMultiCockDesc() + ", displaying it in the hope that it will draw the drider's attention, while at the same time just trying to get yourself off and attain release.");
			else {
				outputText("You slump down, too beat up to mount an effective defense, let alone stay up on your " + player.feet() + ".  The drider skitters over to you and effortlessly peels you out of your " + player.armorName + ", exposing your ");
				if (player.lust100 < 50) outputText("soft");
...
//Summary: what goes up must come down
		...
		{
			spriteSelect(SpriteDb.s_drider);
			outputText(images.showImage("birth-cDrider-butt"));
			...
			if (player.hasCock()) outputText(player.SMultiCockDesc() + " stays limp, but drools a heavy flow of semen, puddling up a nice thick batch of your pleasure alongside your goo-pile.  ");
			if (player.hasVagina()) {
				...
				outputText(" into the dirt, getting off along with your " + player.assholeDescript() + ".  ");
			}
		...
		public function driderPregVagBirth():void
		{
			...
			if (player.biggestTitSize() >= 2) {
				outputText("One drider girl cautiously makes her way over to you.  Her legs tickle your stomach as she climbs up to one of your " + player.breastDescript(0) + ".  Grasping the flesh tightly in her tiny hands she begins to suckle at your " + player.nippleDescript(0) + ".");
...
//Win Drider Male Anal (OH BABY)
		private function buttFuckADriderOhBaby():void
		{
			...
			outputText("Finally done, you pull out of the drider, collapsing on her spider-back with a contented sigh.  Completely insensate after the anal ravaging you just gave her, the drider poses no threat as you leisurely gather " + player.sMultiCockDesc() + " back into your " + player.armorName + " and lower yourself off her cum-soaked abdomen.  You give her a final, playful ass slap as you dismount before heading back to camp.");
		...
		private function driderVagSmartFuck():void
		{
			...
			outputText(images.showImage("cDrider-win-collapsedSpidergirl"));
...
//[No] 
		...
		private function driderTiedUpPartThree():void
		{
			...
			outputText(images.showImage("cDrider-sex-tiedUp"));
		...
		private function driderTiedUpPartFour():void
		{
			...
			var y:Number = x + 1;
			clearOutput();
			outputText(images.showImage("cDrider-sex-tiedUp-end"));
