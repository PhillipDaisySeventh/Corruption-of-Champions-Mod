...
//*Victory Cowgirl
		private function victoryCowgirlRidingOnSpiderBoi():void
		{
			...
			outputText("Without even waiting for him to finish, you go back to your cock-riding, the sensation of his trembling, drug-fueled erection tickling at all the right places.  Inside your " + player.vaginaDescript(0) + ", that wonderful, foreskin-clad member bastes in its own spooge, soaking up the sloppy spunk while you use it as your personal, living dildo.  You ");
			...
			outputText("It takes some time to regain your strength with all the orgasmic aftershocks rolling through your " + player.vaginaDescript() + ".  Once you've mastered yourself, you climb off your conquest and get dressed, ");
...
//*Summary: Male only scene for maximum prostate pounding - possible random choice between two positions, plain doggy style or the 'lucky' one, where he will curl you over yourself and fuck you so you can watch his foreskinned spiderboy cock slam your asshole -while- you drip all over your face.
		private function spiderBoyLossMaleButtfuck():void
		{
			...
			outputText(", displaying surprising strength as he holds your " + player.assholeDescript() + " at waist height.  You look up at the gloating arachnid ");
			...
			outputText("  His only response is to squeeze tight on your " + player.buttDescript() + " and press forward until his pre-cum-soaked foreskin is glazing your rim in preparation for the coming penetration.  ");
			...
			else outputText("quite easy due to how well-stretched your " + player.assholeDescript() + " is.  Watching his fat little cock disappear into your roomy backside actually brings a smile to your lips.");
			...
			if (player.cumQ() < 500) outputText(player.SMultiCockDesc() + " bounces on your belly, leaking one last drop from its tip, utterly draining every drop from your " + player.ballsDescriptLight() + ".");
			else outputText(player.SMultiCockDesc() + " explodes again, dumping every remaining drop from your " + player.ballsDescriptLight() + " directly onto your " + player.chestDesc() + " and belly.");
...
//*Summary: Vagoozles only!
		private function loseToSpiderBoyVagFucked():void
		{
			...
			outputText("  He pounces you, lightning fast, quick enough that you don't even react until he's on top of you.  Chitin-clad legs brush your " + player.hipDescript() + ", the hard knees sinking deep into the swampy loam.  Grinning happily, the spider-boy leans over you and sucks a " + player.nippleDescript(0) + " ");
			...
			outputText("You arch your back from the sensation, lifting your " + player.chestDesc() + " to provide him better access.  ");
			...
			outputText("  The monstrous man handles your body with ease, playing your " + player.nippleDescript(0) + " like a finely tuned instrument, and you pant out little");
			...
			outputText("The spider-morph's eyes seem to grow purple in the dim swamp light as he lifts himself away from your " + player.chestDesc() + ", looking you in the eye.  \"<i>I knew you wanted this,</i>\" he says, rubbing his floppy foreskin against your entrance hard enough for you to feel the stiffness of the member hidden within the loose sheath.  His solid-black shaft feels wonderful against your ");
			if (player.wetness() >= 4) outputText("soaked");
			else if (player.wetness() >= 2) outputText("wet");
			else outputText("moist");
			outputText(" mons, teasing against your vulva until your " + player.clitDescript() + " emerges from its hood, ");
			if (player.getClitLength() >= 3) outputText("frotting against him");
			...
			outputText(", ");
			if (player.hasCock()) outputText(player.sMultiCockDesc() + " becomes so full and hard it feels like it could burst, ");
			outputText("and your " + player.clitDescript() + " ");
			if (player.getClitLength() >= 4) outputText("bounces up and down on your belly with each beat of your heart");
