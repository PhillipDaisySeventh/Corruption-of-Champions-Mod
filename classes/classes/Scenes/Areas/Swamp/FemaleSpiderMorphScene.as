...
package classes.Scenes.Areas.Swamp
{
	...
	public class FemaleSpiderMorphScene extends BaseContent implements TimeAwareInterface
	{
		...
		private function defeatFemale():void
		{
			...
			if (!getGame().inCombat) {
				...
				if (player.hasCock()) {
					...
					outputText("  Meanwhile, your " + player.vaginaDescript(0) + " ");
			...
			//(Combat Intro: HP)
				...
				outputText("You drop to the ground, utterly defeated and unable to resist the cruel spider-kin's intentions any longer.  She kicks you in the side, the hard carapace of her foot digging into you quite painfully.  Rolling onto your back from the impact, you look up at her with fear in your eyes as she deftly removes your equipment.  She turns about, takes a slow, sensuous step back, and begins wiggling her sizable backside at you, almost teasing you with it.  You take the moment to get a better look at her, and your eyes trace up her lissome, carapace-coated legs to her heart-shaped ass before finally settling on the bobbing, heavy weight of her abdomen.\n\n");
			...
			//(Combat Intro: Lust Loss)
				...
				outputText("You drop to the ground and begin to furiously finger your " + player.vaginaDescript(0) + ", arching your " + player.hipDescript() + " as the aching need for sexual contact overpowers your inhibitions.  ");
				if (player.hasCock()) outputText(player.SMultiCockDesc() + " flop about lewdly, leaving trails of pre-cum everywhere that they touch.  ");
			...
			outputText("You wriggle underneath your spidery dominatrix, helplessly caught within her web while she torments each of your " + player.nippleDescript(0) + "s.  Your lust builds ever higher, and while your loins ache for the slightest touch, she tactfully avoids your ");
			...
			outputText("Throughout it all, the fingertip in your " + player.vaginaDescript() + " sits idle, not even bothering to provide the slightest stimulation.  Her lack of attention leaves you to thrust and push at your restraints futilely while your face is fucked by the sweet, sweet spider-puss.  You cry out in frustration, the scream of anguish only vibrating the dominatrix's clit that much harder, prolonging her orgasm even further.  The puffy black lips squelch noisily on your " + player.faceDescript() + " a few moments longer, before they finally depart with a messy scchhhhhlick.\n\n");
		...
		private function spiderMorphFemaleRidesACawk():void
		{
			...
			if (player.biggestTitSize() >= 2) outputText("dragging over your own " + player.nippleDescript(0) + "s");
		...
		private function femaleSpiderMorphTooBigWebRape():void
		{
			...
			//(COMBAT LOSS)
				...
				if (player.lust >= player.maxLust()) outputText("stick your hands into your groin, fondling " + player.oMultiCockDesc() + " with need");
		...
		private function fSpiderMorphRape():void
		{
			...
			if (player.getClitLength() < 4) outputText(", bumping and gliding against the black spider-cooch's sopping lips.");
			...
			if (player.cockTotal() > 1) {
				outputText("Throughout it all, " + player.sMultiCockDesc() + " spews wasted seed over her leg");
				if (player.cumQ() >= 400) {
		...
		private function fSpiderMorphRapeDude():void
		{
			...
			outputText(", " + player.cockDescript(x) + " twitching");
			if (player.cockTotal() > 1) {
			...
			outputText("The soon-to-be expecting mother comes down from her orgasm first, regaining control of herself while your " + player.cockDescript(x) + " is giving the last of its gifts to her pussy.  Her hands roam over her ");
		...
		private function evilSpiderGirlVictoryAnal():void
		{
			...
			outputText("Of course you will.  You answer her query by withdrawing from the velvet tunnel and planting your " + player.cockDescript(x) + " squarely at the spider-morph's tight asshole.  She yelps in surprise and tries to scrabble away, but you hold her fast, leaning on her to push her tits and face into the loamy swamp dirt.  Pressing on, you mount her, thrusting your " + player.cockDescript(x) + " deep into her dark hole");
			if (y >= 0) outputText(", while your " + player.cockDescript(y) + " slips into her warmed-up cunt.");
			...
			if (y >= 0) outputText(", but it's still tighter than the sloppy spider-cunt that keeps rhythmically devouring your " + player.cockDescript(y));
			outputText(".\n\n");
