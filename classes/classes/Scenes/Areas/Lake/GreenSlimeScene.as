...
private function serviceLowCorruption():void
{
	...
	//Picture is here
	outputText(images.showImage("greenslime-oral"));
	//Big peoples!
	if (player.tallness >= 82)
	{
...
private function serviceLowCorruptionHighLust():void
{
	...
	{
		outputText("As you rub the creature's shaft you toy with the idea of inserting the thing into your " + player.vaginaDescript(0) + ", biting your lip slightly and sliding forward a bit to rub yourself against the slime's " + monster.cockDescriptShort(0) + ".  You moan a bit and lose yourself for a moment, caught between desire ");
		...
		if (player.vaginalCapacity() <= 18) outputText("  On top of that, you are unsure if the thick, " + UnitSystem.footAndAHalfCompound() + " long appendage would even fit inside of you.");
		outputText("\n\n"); //this line present to ensure proper paragraph structure
	}	
	//Penises foreplay
	if (player.cockTotal() > 0) {
		//Multicock folk get in their own way
		if (player.cockTotal() > 1) outputText("Your own erect " + player.multiCockDescriptLight() + " rub against the creature's as you work, getting in the way somewhat.  ");
		//Big single cocked folk get in their own way too!
		else if (player.cocks[0].cockThickness >= 36) outputText("Your own erect " + player.cockDescript(0) + " rubs against the creature's as you work, getting in the way somewhat.  ");
		...
		else outputText("You feel your " + player.multiCockDescriptLight() + " begin to throb and entertain the idea of trying to penetrate it somehow.  You turn a bit and look around the creature for an orifice, but find nothing.  After a few frustrated moments of searching you give up and decide to handle things yourself.");
		outputText("\n\n");
	}
	//Next PG
	//Picture is here
	...
	if (player.tallness < 82)
	{
		...
		if (player.gender == 1 || (player.gender == 3 && rand(2) == 0))
		{
			...
			if (player.cocks.length > 1) 
			{
				...
				if (player.vaginas.length > 0) outputText("Your " + player.vaginaDescript(0) + " quivers and shares in the sensation as you grind yourself against the slime.  ");
				//Create cumz
			...
			else 
			{
				...
				if (player.cocks[0].cockLength <= 30)
				{
					...
					if (player.vaginas.length > 0)
					{
						outputText("Your " + player.vaginaDescript(0) + " quivers and shares in the sensation as you grind yourself against the slime.  ");
					}
					outputText("The creature reaches its peak as well, and a thick, powerful gout of more green fluid shoots from the slime's cock and into your waiting palm.  You shudder as your orgasm seems to drag on for longer than usual, and notice that the slime's body regains some of its definition for a moment before its cock quickly recedes back into its body.");
				}
				//FUGGIN' 'UGE PRICKS!
				else if (player.cocks[0].cockLength >=31)
				{
					...
					outputText("Instinctively you hold onto the creature's own shaft for support as you do and with a final mighty pulse it also comes, spraying its thin green fluid in a tall gout into the air.  You feel it splash onto your back and " + player.hairDescript() + " as you gasp for breath, before almost falling over as the beast's erection rapidly recedes into its own body.");
				}
			}
			//Newline PG
			outputText("\n\n");
		...
		else if (player.gender == 2 || player.gender == 3)
		{
			...
			//Big clitties
			if (player.getClitLength() >= 2) outputText("You moan and lean back slightly as you grip your " + player.clitDescript() + " between your thumb and forefinger, biting your lip at the intense sensations as you rub your thumb over your " + player.clitDescript() + " and shuddering in anticipation and delight.  ");
			//Small clitties
			else outputText("You moan and lean back slightly as you start to flick your " + player.clitDescript() + ", the slime's fluid providing excellent lubrication and a luxurious warming feeling as you make small circles around your nub.  ");
			...
			{
				outputText("You gasp a little as the slime's massive arms reach up from behind you to cup your " + player.allBreastsDescript() + ", and the soft material ");
				//Tig ol' bitties
	...
	else if (player.tallness >= 82)
	{
		...
		if (player.gender == 1 || (player.gender == 3 && rand(2) == 0))
		{
			...
			else
			{
				...
				//Compare cocks
				if (player.cocks[0].cockType == CockTypesEnum.HUMAN) outputText("a near exact replica of your own " + player.cockDescript(0) + "!  ");
			...
			if (player.vaginas.length > 0)
			{
				outputText("The intense sensations ripple through your entire body, and your " + player.vaginaDescript(0) + "grows wet with arousal.  You push against the creature and rhythmically slide back and forth, its soft, moist body conforming to your folds and creating a wonderfully arousing sensation.");
		...
		else if (player.gender == 2 || player.gender == 3)
		{
			...
			if (player.breastRows.length > 0 && player.biggestTitSize() > 0)
			{
				outputText(player.allBreastsDescript() + ", and you begin to grope yourself as you grip the creature's shaft and stroke it from crown to root.  As the pulsing of the beast's mass becomes more pronounced so does it's appearance, and it takes on the distinct look of a human penis.  Grinning you pull it towards you a bit and rapidly stroke the shaft with one hand while working the cock's head with the other, making the slime buck slightly, pulsing within your hands.  Without warning, its wide arms reach up, one reaching across your breasts while the other drapes across your thighs, the hand going for your " + player.vaginaDescript(0) + ".  ");
				//Cant cover all of tig ol' bitties
				if (player.biggestTitSize() > 8) outputText("Its hand expands, struggling to cover all of your " + player.allBreastsDescript() + ", ");
				//Middling ta-tas get covered
				else if (player.biggestTitSize() > 3) outputText("Its hand expands and gently covers your " + player.allBreastsDescript() + ", ");
				//Small tits?  Pwned.
			...
			if (player.cockTotal() > 0)
			{
				...
				{
					outputText("You begin to moan as the creature rubs the " + player.vaginaDescript(0) + " beneath your " + player.multiCockDescript() + ", working you with incredible skill, but sadly ignoring your needy ");
					//multi!
			...
			{
				//since players without breasts are wholly surprised here, this needs to be acknowledged.
				if (player.biggestTitSize() == 0 || player.breastRows.length == 0) outputText("You begin to moan as the creature rubs your " + player.vaginaDescript(0) + ", working you with incredible skill, but what comes next is an utter surprise, making you unconsciously wring the head of the slime's cock.  ");
				//Clit engulph'ed!
				outputText("The creature's smooth, gel-like body envelopes your " + player.clitDescript() + " and slowly pulsates, stroking it from multiple angles at once and making you gasp with pleasure.  You're torn with the urges to both thrust your hips towards it to seek more pleasure and to draw away from the sheer intensity of the sensations, and settle for helplessly moaning as your hands unthinkingly tighten and squeeze around the head of the creature's cock, causing it to release a massive stream of green fluid across your chest and body.");
			}
			//New PG, CUM!
...
private function maleRapesOoze():void {
	...
	if (rand(2) == 0)
	{
		...
		else outputText("Shrugging, you decide to go with the closest thing available and walk towards its head, pulling out your " + player.cockDescript(0) + " as you do so.  You kneel over the thing's head and stroke your penis lightly to ensure it is fully ready,  ");
		//Balls ftw
		if (player.balls > 1) outputText("anointing the slime's face with your " + player.sackDescript() + " as you do.\n");
		//Ball-less bitches
		...
		//Balls ftw
		if (player.balls > 1) outputText(" as your " + player.ballsDescriptLight() + " slap against its face");
		...
		if (player.vaginas.length > 0) outputText("  Your " + player.vaginaDescript(0) + " begins to grow wet as you repeatedly force yourself into the creature's throat.  You take a single deep stroke and bottom out into the creature, grinding your moist femininity against the soft, sensual material of the creature's face.");
		//New PG for cumming!
		outputText("\n\n");
		...
		//vaginal orgasm text
		if (player.vaginas.length > 0) outputText("  This is quickly followed by a quivering between your nether-lips as your " + player.vaginaDescript(0) + " quivers in orgasm shortly afterwards.");
	...
	else
	{
		...
		if (player.cockTotal() > 1)
		{
			...
			outputText(" of your" + player.multiCockDescriptLight() + " as you grab the slime and roughly try to turn it over.  Surprisingly it maintains its cohesion and flops onto its side, its erection flopping wetly onto the ground.\n");
		}
		//Single dicks
		...
		//Picture is here
		outputText(images.showImage("greenslime-rape-anally"));
		...
		else if (player.cocks[0].cockLength >= 60) outputText("Your penis keeps going deeper and deeper, through the chest, neck, and head, before finally stretching the slime as far as it needs to go to accommodate the full length of your " + player.cockDescript(0) + ".");
		...
		//vaginal arousal text
		if (player.vaginas.length > 0) outputText("Your " + player.vaginaDescript(0) + " grows wet as you work the thing's hole, every slap of your hips against the thing's ass sending a pang of pleasure through your nethers.  ");
		...
		if (player.cockTotal() > 1) outputText("your " + player.multiCockDescriptLight() + " explodes into orgasm, your load filling the space inside the creature and spraying in thick streams across its back.");
...
private function femaleRapesOoze():void {
	...
	{
		outputText(" your " + player.vaginaDescript(0) + " growing wet and your " + player.cockDescript(0) + " growing firm as you imagine it inside you.  ");
	}
	else
	{
	...
	//Picture is here
	outputText(images.showImage("greenslime-rape-vagoo"));
	outputText("The thing's cock, though rigid, somehow manages to squish and expand wherever needed to conform perfectly to your insides, adapting to your " + player.vaginaDescript(0) + " with ease as you ride it.");
	//penis arousal text
	if (player.cockTotal() > 0)
	{
		if (player.cockTotal() == 1)
		{
			outputText("  Your " + player.cockDescript(0) + " grows hard as you are penetrated, and with the creature handling the movement you reach down to jack yourself off.");
		}
		else
		{
	...
	if (player.cockTotal() > 0)
	{
		...
		{
			outputText("Your " + player.cockDescript(0) + " explodes into orgasm and releases thick streams of semen onto both you and the creature, your mind blanking at the release.  You vaguely notice your ejaculate sinking into the creature's body and getting absorbed as you give yourself to pleasure.  ");
		}
	}
...
private function oozeButtRapesYou():void 
{
	...
	if (player.cockTotal() > 0)
	{
		...
		//Multi!
		else outputText("Your " + player.multiCockDescript() + " quickly grows erect under the creature's embrace as it pistons into your ass, and before long you stand at full attention.  You weakly rub at a different cock with each hand as it pumps you, and quickly approach an orgasm.  ");
	}
	//Vaginas!
 	if (player.vaginas.length > 0) outputText("Your " + player.vaginaDescript(0) + " grows wet as the creature continues to assault you, and your cleft begins to ache for a soft touch.  As if sensing your thoughts the creature obligingly wraps a hand around and under you and begins alternating between sliding along your gash and stroking your " + player.clitDescript() + ".");
 	//New PG
	outputText("\n\n");
	...
	if (player.vaginas.length > 0) 
	{
		...
		//vaginal-only orgasm
		else outputText("The creature finally plunges a pair of fingers into your " + player.vaginaDescript(0) + ", the sudden shock finally pushing you to orgasm.  ");
	}
	//mangasm
...
private function oozeRapesYouVaginally():void
{
	...
	if (player.cor <= 20)
	{
		...
		if (player.cockTotal() > 0)
		{
			...
			if (player.cockTotal() > 1)
			{
				outputText("Your " + player.multiCockDescript() + " rapidly grows erect under this assault and begins to throb.  Having now released yourself to instinct you reach to jerk off, ");
			...
			//Single dick
			else outputText("Your " + player.cockDescript(0) + " rapidly grows erect under this assault and begins to throb.  Having now released yourself to instinct you reach to jerk off, rapidly stroking your " + player.cockDescript(0) + " in time with the creature's movements.  ");
		}
		...
		{
			//Multi
			if (player.cockTotal() > 1) outputText("Your " + player.multiCockDescript() + " climaxes as well, sending rope after rope of hot, thick cum onto your face and chest.");
			//Single
	...
	else
	{
		...
		{
			//multi
			if (player.cockTotal() > 1) outputText("Your " + player.multiCockDescript() + " climaxes as well, and you open your mouth as you spray rope after rope of hot, thick jism onto your face and chest.  You relish its salty taste, continuing to stroke yourself even as you come.  ");
			//single cock
...
internal function rapeOozeWithMilk():void {
	...
	{ 
		outputText("manipulating your " + player.clitDescript() + ". ");
	}
	//[If Herm- 
	...
	//[If Genderless- 
		...
		outputText("\n\n");
		outputText("The barrage of pleasurable feelings causes you to fall over onto your " + player.buttDescript() + " and just soak in them, your hands ");
	}
	...
	//[if female- 
	else if (player.gender == 2) outputText("rapidly entering it in and out of your " + player.vaginaDescript(0) + " by way of your fingers.  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you release your juices into it.  You flick it off your fingers ");
	//[if herm- 
