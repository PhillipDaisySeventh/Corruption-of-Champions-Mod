	...
	public class MinotaurScene extends BaseContent 
	{
		...
		private function rapeMinotaurTentacle():void {
			...
			if (player.vaginas.length > 0) { //free pussy!
				...
				temp = 2;
				outputText("You're feeling a little kinky so you step in front of him, offering him a little bit more, pushing yourself down onto his meaty dick.  You slowly push down, enjoying every " + UnitSystem.literalInch() + " as it travels down your " + player.assholeDescript() + ", the pre leaking from it acting as a lubricant as his tentacle-wrapped cock works its way inside you.  ");
				//Ass cum
				outputText("Eventually, the moment is right.  You smile as you feel a boiling heat building in your groin, surging through your " + player.multiCockDescript() + " in a tide of white-hot pleasure.  You groan and shiver, lost in the pleasure until you begin to calm down.  ");
		...
		private function getOralRapedByMinotaur():void {
			...
			outputText("With no warning the minotaur yanks your " + player.legs() + " off of the ledge and grabs your waist. He shoves you down hard on his shaft while slamming his hips forward. Your eyes go wide in shock as the minotaur rams the massive tip of its cock past your mouth and into your throat. Grunting and moaning the minotaur forces its cock into your throat, " + UnitSystem.literalInch() + " by " + UnitSystem.literalInch() + " as it grinds you around its shaft. Your throat bulges as the first ridge around the minotaur's cock approaches your lips.\n\n");
		...
		private function takeMinoCumDirectly():void {
			...
			if (player.analCapacity() < monster.cockArea(0)) //too tight!
				 outputText("You have to push harder and harder to get it inside you, even with the copious lube's drug-like affects relaxing your sphincter.  The penetration is slow, but " + UnitSystem.literalInch() + " by " + UnitSystem.literalInch() + " you force it inside you.");
		...
		private function minoCumAddiction(raw:Number = 10):void { player.minoCumAddiction(raw); }
		...
		private function minoMutual():void { //Added flag to kFlags for this at #2337
			clearOutput();
			spriteSelect(SpriteDb.s_minotaur);
		...
		private function minoMutualPt2(x:Number):void { //pt2-1
			clearOutput();
			//Mino Worship 
		...
		private function minoMutualPt2Frot(x:Number):void { //pt2-2
			clearOutput();
		...
		private function minoMutualPt3(x:Number):void { //pt3-1
			clearOutput();
		...
		public function joinBeingAMinoCumSlutSexPart():void {
			clearOutput();
		...
		public function joinBeingAMinoCumSlutPartII():void {
			clearOutput();
			...
			outputText("The bull groans, biting his lip as a third, and likely final, orgasm rips through him.  His hips buck upwards, his cock flaring up and out of your mammaries.  Ropes of immaculate silver seed jet from his cumslit, arcing up into the air several " + UnitSystem.literalFeet() + " before splattering down on your heads.  Wasting no time, you slip your lips around the flare, gulping down mouthful after mouthful of the sweet man-milk.  Even though it's his third load of the hour, it's just as big as the others, and soon your find you can't swallow any more; your cum-laden belly just won't allow it.\n");
