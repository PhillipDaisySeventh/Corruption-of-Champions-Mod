...
package classes.Scenes.Areas.Mountain
{
	...
	public class HellHoundScene extends BaseContent
	{
		...
		public function hellhoundRapesPlayer():void
		{
			...
			outputText("The heads of the hellhound grin with desire as he rises onto his hind legs. As the hellhound stands before you, your eyes widen at the sight of its equipment: a pair of " + UnitSystem.displayInchWithHyphenTextually(8) + " long black rods, one above the other, with a sack containing a quad of flaming balls underneath. As the hound swaggers closer, you gulp at the thought of what those fiery balls are going to be pumping into you. You are helpless against the hellhound as he pushes you onto your ");
			if (player.lowerBody.type == LowerBody.HOOFED) outputText("hocks and knees");
...
//HELLHOUND MASTER+ BAD ENDZZZZZ
		public function HellHoundMasterEncounter():void
		{
			...
			if (flags[kFLAGS.HELLHOUND_MASTER_PROGRESS] == 0) {
				...
				outputText("As you're wandering the mountains, you once again feel an odd presence in the back of your mind.  You're sure it is the hellhound master once again.  You look at the pink crystal in your possession, and wonder if you should exchange it for the promised hellfire.  Do you let the presence take over and give away the Lethicite?");
...
//choose yes (C)
		...
		{
			clearOutput();
			outputText(images.showImage("hellhound-crazyPresences"));
...
//Regular Lethicite
		private function giveLethicitePiece():void
		{
			...
			outputText(images.showImage("hellhound-hellfire"));
...
//Merae's Lethicite
		private function giveALLTHELETHICITES():void
		{
			...
			if (player.dogCocks() >= 2) {
				...
				outputText(images.showImage("badend-hellhound-male"));
				...
				outputText(images.showImage("badend-hellhound-female"));
		...
		private function badEndPartTwo():void {
			clearOutput();
			outputText(images.showImage("badend-hellhound-cerberus"));
		...
		public function hellHoundPropahRape():void
		{
			...
			//--- IF CORRUPTION < 20 ---
			if (player.cor < 20) { //Disregards tolerance perk, otherwise it'll break.
			...
			//--- ELSE (CORRUPTION >= 20) ---
				...
				//--- ELSE (CORRUPTION >= 40) ---
				outputText(images.showImage("hellhound-win-vag"));
				...
				if (player.hasCock()) {
					...
					//--- IF ONE PENIS ---
					if (player.totalCocks() == 1) outputText("  Soon your own " + player.cockDescript(0) + " reacts, quickly growing into a full throbbing erection. You leave your member alone, and it keeps flapping against the hellhound's belly while you ride him.");
					//--- ELSE ---
					else outputText("  Soon " + player.sMultiCockDesc() + " reacts, quickly growing into a throbbing erection.  You leave your members alone, and they flap against the hellhound's belly while you ride him.");
				}
				//--- IF PC HAS BALLS ---
				...
				if (player.totalCocks() > 0) {
					// --- IF ONE PENIS ---
					if (player.cockTotal() == 1) outputText("Your " + player.cockDescript(0) + " twitches as it ejects its sticky load.");
					//--- ELSE ---
					else outputText("Your " + player.multiCockDescriptLight() + " twitch as they eject their sticky loads.");
