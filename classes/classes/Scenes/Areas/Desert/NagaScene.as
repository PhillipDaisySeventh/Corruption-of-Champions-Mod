...
public function nagaEncounter():void {
	...
	if (player.lowerBody.type == LowerBody.NAGA) {
		...
		//[If player has fangs]
		if (player.face.type == Face.SNAKE_FANGS) outputText("your own.");
		//[player has no fangs]
		...
		if (player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
			...
			if (player.cockArea(0) > 35) {
				...
				outputText("You moan in pleasure as the naga takes the tip of your member into her mouth and starts to suck. You can feel her deft tongue licking every " + UnitSystem.literalInch() + " of your cock head.\n\n");
			...
			//[Cock isn't too big]
				...
				//[player has one dick] 
				if (player.totalCocks() == 1) outputText("She carefully lines it up with your member and starts to tease the tip before gently inserting the first few " + UnitSystem.literalInches() + ". ");
				//[player has two dicks]
				if (player.totalCocks() == 2) outputText("She carefully lines it up with both of your members and starts to tease their tips before gently inserting the first few " + UnitSystem.literalInches() + ". ");
				//[player has three or more dicks]
				if (player.totalCocks() >= 3) outputText("She carefully lines it up with one of your members and starts to tease the tip before gently inserting the first few " + UnitSystem.literalInches() + ". ");
...
private function gooNagaRape():void {
	...
	if (player.cor > 50) {
		//(if corrupt male)
		if (player.gender == 1) {
			outputText("your " + player.cockDescript(0) + " growing harder at the thought.");
		}
		//(if corrupt female)
...
    outputText("You decide that you've played with her enough and withdraw your hand, now wet with her juices, from her pussy. A hiss escapes the naga's lips and she tries to bring your hand back to her needy cunt. You pull your hand out of her grip and instead shift your gooey form over her lips and start to push some of your lower half into her. She starts to coo as you shove the first few " + UnitSystem.literalInches() + " in, her hips thrusting into the air. You continue to push more of your slimy form into her, sliding deeper and deeper into her as her walls convulse around you.\n\n");
	outputText("Soon you feel yourself hit a wall as your body hits the opening of the naga's cervix. A stray thought crosses your mind as you think that maybe this is far enough, ");
	//(if corrupt)
...
    
	...
	//(if female)
		...
		outputText("your clit rubbing against the naga's pleasure buzzer with each downward thrust.");
	} 
	//(if herm) 
	else if (player.gender == 3) {
		if (player.cockTotal() == 1) outputText("your " + player.cockDescript(0));
	...
	//(if genderless)
		...
		outputText("your bare crotch hitting the naga's pleasure buzzer with each downward thrust.");
	}
...
    outputText("\n\nYou continue to relentlessly push yourself into the naga, stretching her wide.  ");
	...
	if (player.gender == 1) {
		...
		//(normal) 
		if (player.cumQ() < 250) outputText("you come all over the naga's stomach.");
		else if (player.cumQ() < 1000) outputText("you come all over the naga's face and stomach.");
		//(if THE LEVEYS HAVE BROKE RUN FOR THE HILLS) 
		else outputText("you come violently, covering the naga in a layer of your sperm.");
	}
	//(if female)
	else if (player.gender == 2) {
	...
	if (player.gender == 3) {
		...
		else outputText(" completely covering the naga in a layer of your spooge.");
	}
	//(if genderless)
	if (player.gender == 0) {
		outputText("You cry out and shudder as you feel wave after wave of the naga's orgasm rushing over your body, bringing you to a strange orgasmic bliss of your own.");
	} 
	outputText("  For a moment you just sit there in post orgasmic bliss, the walls of the naga convulsing around your gooey half. You decide that the snake woman has had enough and slowly withdraw yourself from her abused love canal. You gather up your things and head back to your camp, leaving the naga lying in the sands.");
...
private function nagaVictoryMale():void {
	...
	if (player.totalCocks() > 0) {
		...
		if (player.isTaur()) {
			...
			//[SUPER CUM]
			else outputText("you explode with huge fountains of cum spurting from your " + player.cockDescript(0) + ", gushing out from where the two of you are joined to rain on the sands below, ");
		...
		//Uncorrupted  
			...
			if (player.cockArea(0) <= monster.vaginalCapacity()) {
				...
				//[Spermageddon]
				else outputText("your " + player.cockDescript(0) + " erupting, blasting a violent torrent of semen deep inside her petite frame, filling her up completely as both your voices rise in screams of pleasure.");
			}
			//[If cock too large]
			else {
				...
				outputText("  The naga removes your " + player.cockDescript(0) + " from her mouth");
				//[lots of cum OR more]
	...
	     	}
			...
			outputText("When you awaken, the naga is long gone. All that remains of your encounter is a depression in the landscape");
			//[lots of jizz] 
...
   			//(Loss by Lust)
			if (monster.lust >= monster.maxLust()) {
				...
				//(If PC is Herm i.e. Has a cock(s)){
				if (player.gender == 3) outputText(" and your " + player.multiCockDescriptLight() + " constantly stimulated by the tail wrapped around your body");
...
internal function nagaFUCKSJOOOOOO():void {
	...
	//too)
		...
		outputText("You hear the hiss of the naga's laughter as she slithers over to your prone form. Her long tail twists around you and tightens suddenly, pinning your arms to your sides");
		//(if centaur)
		if (player.isTaur()) 
		{ 
		...
		//(if defeated by lust)
		if (player.lust >= player.maxLust()) outputText("all your resistance seems to do is make your " + player.cockDescript(0) + " harder.");
		//if defeated by KO)
		...
		if (player.cor > 75) outputText("  The thought somehow makes your " + player.cockDescript(0) + " throb even more.");
		outputText("\n\n");
...
     
		...
		//(Cock too big)
			...
			//(if PC has balls)
			if (player.balls > 0) {
				outputText("She shifts one of her hands down from your " + player.cockDescript(0) + ", to your " + player.ballsDescriptLight() + ", cupping them in her slick palm.  ");
			}
			...
			//OH SHIT MOAR CUM
			//(Normal)
			if (player.cumQ() < 250) outputText("The naga manages to hold the warm cum within her mouth, only causing her cheeks to bulge a little.");
			//(Lots of cum)
			else if (player.cumQ() < 1000) outputText("The naga's cheeks balloon as her mouth is filled with your hot, gooey sperm, forced to swallow some of it as her cheeks reach their full capacity.");
			//(Cumception, we need to go deeper)
			else outputText("Hot semen blasts from the naga's nose as her mouth is completely filled in seconds. Much of your warm cum is forced down her throat and into her stomach, causing it to distend slightly within her thin frame.");
...
    		//(If you've been a bad boy during victory scenes)
		...
		//(if uncorrupted)
		else outputText("and while you know you probably won't like it, your " + player.vaginaDescript(0) + " says otherwise.  ");
...
   		
		...
		//(If you are a good girl)
			...
			outputText("You do your best to follow the naga's example, but you are not sure why she is doing it.  ");
		} 
...
    	outputText("The naga lifts you from her body with her tail, placing you onto the warm sand where she had been laying. She moves her own body above yours, her tail buried deep inside your " + player.vaginaDescript(0) + " the entire time. She holds you in place with one hand, her other hand moving to resume playing with your " + player.biggestBreastSizeDescript() + ". She moves her head in, her hot breath washing over your other breast, and looks up into your eyes for a moment. She smiles, giggles, and then wraps her mouth around your " + player.nippleDescript(0) + ", letting her slender tongue tickle your sensitive skin there.");
		//(If fuckable nipples)
		if (player.hasFuckableNipples()) {
			outputText(  "It isn't long before she decides to take it one step further, pushing her tongue inside your " + player.nippleDescript(0) + " and wriggling it around inside. Meanwhile, her other hand begins to work its way into your other " + player.nippleDescript(0) + ", finger-fucking it for all it's worth.");
		} 
		//(If lactating)
		if (player.biggestLactation() > 1) {
...
private function eggUpANagaSpiderLike():void {
	...
	outputText("\n\nJust below where her human skin fades into scales, you uncover the naga's slick hole.  Your breath quickens again as you try to line yourself up with it, your dripping egg-tube smearing its viridian lube across her scaly underside.  You manage to slip a few " + UnitSystem.literalInches() + " in, but she hasn't ceased her squirming, even for a moment.");
