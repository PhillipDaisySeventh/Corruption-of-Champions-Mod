...
//►[Keep Hidden]
		private function keepHidden():void
		{
			...
			//►(If Over 41 - Male)
				...
				outputText("\n\nYou're almost ready to blow your load when a hermaphrodite demon with what must easily be a " + UnitSystem.displayInchEstimateWithHyphen(20) + " prick steps up to the plate.");
			...
			//►If Over 41 - Female
				...
				outputText("\n\nThe demon looks surprised briefly, then smiles devilishly as a sudden realization strikes.  Massaging his pendulous breasts with one hand, the demon begins to tease the ant's pussy with his demonic cock, a monstrous member at least " + UnitSystem.displayInchesEstimate(20) + " in length, dragging the huge head up and down the girl's entrance.  The ant, alarmed, begins to protest with a series of clicks and frantic flails, but when one of the female demons waiting on the sidelines sees the open mouth, she steps up and slides her cunt across the ant's face, quieting her.  Taking his cue from the demoness, the teasing hermaphrodite suddenly shoves his demonic dick, balls deep, into the ant's wet pussy.  You can see the stomach of the ant contort as the demon starts to pump, and both demons slap their hands together in a high five, clearly pleased with themselves.  The herm quickly yields to the pleasure, setting his jaw and pounding rapidly, and you slip several fingers inside yourself as you imagine what his cock must feel like.");
...
//After whuppin' demons!
		internal function consolePhylla():void
		{
			clearOutput();
			spriteSelect(SpriteDb.s_phylla);
			outputText(images.showImage("ant-desesrt-console-phylla"));
			...
			outputText("\n\nSlowly, she crawls out from the back of the cart and stands facing you.  You guess she's about " + UnitSystem.display("five and half-ish feet", UnitSystem.displayHeight2EstimateTextually(5.5)) + " tall, and you can now make out her pair of perfectly firm B-cup breasts.  Curiously, her nipples are hard and stick out at least " + UnitSystem.halfAnInch() + ".  The interest excited by your examination must show on your face, as she blushes and quickly covers her breasts with her two larger arms - while the smaller ones hold her loin cloth in place.");
...
//(Explore the Desert)
		private function enterTheColony():void
		{
			...
			outputText(images.showImage("ant-desert-colony"));
			...
			//[(corruption < 40)
			if (player.isPureEnough(40)) outputText("\"<i>'Champion', you've been sent to this land to resist the demons and your efforts thus far, insignificant though they be, have not escaped my gaze.  ");
			...
			outputText("\n\nChylla stands upright, and you see her muscles strain to lift her large abdomen.  Surprisingly, she's easily over " + UnitSystem.displayHeight2Estimate(10) + " tall and is an impressive and very regal figure even with her distended backside - though, you suspect, not an agile one.");
...
//►[Leave]
		...
		{
			clearOutput();
			outputText(images.showImage("ant-desert-coliseum"));
			//►[Fight #1]
...
//Good End / Waifu Content
		...
		{
			clearOutput();
			outputText(images.showImage("ant-desert-kissing"));
			spriteSelect(SpriteDb.s_phylla);
...
//►Male Continuation
		...
		{
			clearOutput();
			outputText(images.showImage("ant-desert-first-male-vagsex"));
...
//PURE ENDING!
		...
		{
			clearOutput();
			if (!player.canOvipositSpider()) outputText(images.showImage("ant-desert-male-first-pure-vagsex"));
			var x:int = player.cockThatFits(phyllaCapacity());
			if (x < 0) x = player.smallestCockIndex();
...
//Corruption greater than 75 (Corrupt Ending):
		...
		{
			clearOutput();
			outputText(images.showImage("ant-desert-male-first-corrupt-vagsex"));
...
//Cunnilingus Scene: (Triggered if PC does NOT have a dick that is suitable for vaginal sex with Phylla.)
		...
		{
			clearOutput();
			outputText(images.showImage("ant-desert-male-first-cunning"));
...
//►Female Continuation
		...
		{
			clearOutput();
			outputText(images.showImage("ant-desert-female-first-boobsucking"));
		...
		{
			clearOutput();
			outputText(images.showImage("ant-desert-female-first-scissoring"));
...
//If PC Herm/Has (a) cock(s):
		...
		{
			clearOutput();
			if (player.smallestCockArea() > phyllaCapacity()) outputText(images.showImage("ant-desert-female-first-scissoring-with-bigcock"));
			else outputText(images.showImage("ant-desert-female-first-scissoring-with-cock"));
...
//If PC returns to colony after telling her to stay with her mother:
		...
		public function introductionToPhyllaFollower():void
		{
			...
			clearOutput();
			outputText(images.showImage("ant-camp-follower-intro"));
			phyllaSprite();
...
//First Time Blowjob:
		private function phyllaBeeeJays():void
		{
			...
			outputText(images.showImage("ant-camp-give-bj"));
...
//Pure BJ:
		...
		{
			clearOutput();
			outputText(images.showImage("ant-camp-give-pure-bj-cum"));
			var x:int = player.smallestCockIndex();
...
//Corrupt BJ Ending
		private function corruptPhyllaEndings():void
		{
			clearOutput();
			var x:int = player.smallestCockIndex();
			outputText(images.showImage("ant-camp-give-corrupt-bj-cum"));
...
//\"<i>Use Dick</i>\"
		private function dickPhylla():void
		{
			...
			clearOutput();
			outputText(images.showImage("ant-camp-vagsex"));
...
//Straight Sex (Lesbian/Fisting) - Written
		...
		{
			clearOutput();
			outputText(images.showImage("ant-camp-female-fisting"));
...
//While Giving Birth (Male) - Written
		...
		{
			clearOutput();
			outputText(images.showImage("ant-desert-male-preg-vagsex"));
...
//While Giving Birth (Female) - Written
		...
		{
			clearOutput();
			outputText(images.showImage("ant-camp-female-preg-"));
			...
			outputText("\n\nClearly not understanding your question was more playful than real, you cut her off by giving her boob a soft squeeze.  Both of you are clearly surprised when a white jet of milk practically fires out of her breast like a cannon ball, landing on the floor " + UnitSystem.aFewFeet() + " away.");
...
//Orgy w/ Colony (Male) - Written
		private function orgyWithDatColonyCorruptDudes():void
		{
			...
			clearOutput();
			outputText(images.showImage("ant-camp-mle-orgy"));	
			...
			outputText("\n\nEmbarrassed for her children, Phylla looks away as your eyes instantly lock on to their genitals.  \"<i>Well...  I mean, they're... all... gifted... like y-you asked,</i>\"  Phylla says, more to the floor than to you.  They certainly are, you think to yourself, as you visually feast on their cocks!  All of them are erect, and a few even have beads of pre-cum running down their slightly pointed, human-looking shafts.  You would guess all of them have about " + UnitSystem.displayInchesEstimateRange(10, "-", 15) + " of long, hard man-meat.  Two testicles swing beneath their long cocks.  Their balls vary in size, but have mostly human-like proportions compared to their cocks.  You walk over to one and slide your hand down his shaft just to see how he reacts.  A low muffled moan escapes his clenched teeth as he does his best to ignore your hand.");
...
//You tell Phylla you're interested in 'inspecting' your children.
		...
		{
			clearOutput();
			outputText(images.showImage("ant-camp=female-orgy"));
			...
			outputText("\n\nYou and Phylla almost break apart as a male wriggles his way between you two and shoves his " + UnitSystem.displayInches(14) + " cock interveningly between your two cunts. You hear Phylla coo at this sudden intruder.");
...
//LICK THAT!
		private function lickThatAntButt():void
		{
			clearOutput();
			phyllaSprite();
			outputText(images.showImage("ant-camp-lay-egg-licked"));
...
//DON'T LICK THAT!
		private function dontLickAntButt():void
		{
			clearOutput();
			phyllaSprite();
			outputText(images.showImage("ant-camp-lay-egg-not-licked"));
...
//Phylla lays Drider eggs
		private function phyllaLaysSomeDriderEggs():void
		{
			...
			outputText(images.showImage("ant-desert-lay-drideregg"));
...
//Mount Phylla:
		...
		{
			clearOutput();
			outputText(images.showImage("ant-camp-ant-mounted-by-drider"));
...
//►[Appearance]
		private function phyllaPearance():void
		{
			...
			//When Laying Eggs
				...
				outputText("Phylla is a " + UnitSystem.displayHeight2WithHyphen(5, 8) + " tall ant morph with a very small, feminine frame covered in highly defined muscles.  She has four arms, two that attach at her shoulders and two slightly smaller ones that attach at her serratus anterior on her stomach.  Both sets of her arms and human-like legs are covered in plates of semi-reflective brown chitin armor.  Her interlocking joints remind you of how insect joints connect to one another. Her abdomen is currently engorged with eggs and is about " + UnitSystem.displayFeetEstimate(7) + " long!  It is covered in large interlocking plates of skin colored exoskeleton and every minute or two you watch Phylla moan as it contracts and relaxes, popping out another egg.  Despite the fact that she has to carry the weight of an abdomen, her hips are very girly.  Her well toned butt is rarely seen as her abdomen covers it most of the time.  She has a sharp, slightly feminine face and large almond shaped emerald-green eyes with no hint of pupils or irises.  Her mouth is surprisingly large considering how thin her lips are.  When she smiles at you, you see a single row of human like teeth with typical human canines.  Her short brown hair is usually matted and covered in dirt, although on rare occasions she styles it so that two long bangs frame her face leaving the rest to flow around her neck.");
				outputText("\n\nShe has a pair of DD-cup breasts, with a single " + UnitSystem.displayInches(0.5, 1) + " nipple on each breast.  Bursts of milk occasionally spurt from her swollen areolas in time with her contractions.");
				outputText("\n\nShe has one extremely tight pussy placed between her legs, with a " + UnitSystem.displayInchWithHyphen(2) + " clitoris.  A small stream of lubricant constantly flows down her legs, her lips slightly parted.");
				outputText("\n\nShe has one asshole placed between her well toned butt-cheeks, right where it belongs.");
...
//Sex > [Egg Phylla]
		private function eggDatBitch():void
		{
			...
			{
				outputText(images.showImage("ant-camp-ant-mounted-by-drider"));
				outputText("While Phylla appears to be sexually sated, the heaviness in your spider abdomen begs for release.\n\n");				
			}
			//First Time:
