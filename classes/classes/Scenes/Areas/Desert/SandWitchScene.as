...
//const EGG_WITCH_COUNTER:int = 588;
		...
		public function encounter():void {
			...
			unlockCodexEntry("Sand Witches", kFLAGS.CODEX_ENTRY_SANDWITCHES, true, true);
...
internal function sandwitchRape():void {
	...
	if (player.lust >= player.maxLust()) {
		...
		if (player.hair.color.indexOf("sandy blonde") != -1) {
			...
			if (player.biggestLactation() > 0) {
				outputText("A strong pressure builds in your chest, painful in its intensity.  You yank down your top as ");
				if (player.biggestLactation() < 2) outputText("powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarrassed with your increased milk production.  ");
		...
		else 
		{
			...
			if (player.cockTotal() == 1) {
				...
				if (player.dogCocks() > 0 && player.cocks[0].knotMultiplier > 1.4) knotSandwitch();
					...
					outputText("\n\nThe sand witch screams in savage denial as you plant your seed deep within her body, your corruption no doubt making it extra potent. You gaze into her eyes, reveling in her expression of horror. Without uttering a word, you continue to fuck her, making sure your seed is well-entrenched within her womb, feeling it splatter and bubble around your " + player.cockDescript(0) + ".  You wonder if she will remember the pleasure her body felt today, and if she will be more likely to drop to her knees before you than to fight... ");
					outputText("Finally finished, you pull your " + player.cockDescript(0) + " from one of her well-used slits, your " + player.cockDescript(0) + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ");
					player.orgasm('Dick');
				}
			}
			//Multiprick
			if (player.cockTotal() > 1) {
				...
				outputText("Using the corruption in your blood, you place your " + player.foot() + " upon the budding shoots, and wipe them from existence. The sand witch looks shocked at your display of power, only turning you on more. You undo your loin cloth and grope her nethers, only to find a surprise. She has " + num2Text(player.cocks.length + 1) + " loose wet cunts and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping heads of your " + player.multiCockDescriptLight() + " to her nethers, slowly easing into her depths.  ");
				outputText("Thrusting deep, you hit bottom, and marvel at the sensations as you feel the heads of your " + player.multiCockDescriptLight() + " rub against the end of her wet pussies, only making your desire to penetrate increase. As though responding to your will, your " + player.multiCockDescriptLight() + " grow just a tiny bit more in length and thickness.  ");
				outputText("With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervixes and into her wombs. Pulling the heads of your " + player.multiCockDescriptLight() + " from her wombs, you rub and rut at the outside of her cunts, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ");
				...
				//A few dicks...
				outputText("\n\nThe sand witch screams in savage denial as you plant your seed deep within her body, your corruption no doubt making it extra potent. You gaze into her eyes, reveling in her expression of horror. Without uttering a word, you continue to fuck her, making sure your seed is well-entrenched within her wombs, feeling it splatter and bubble around your " + player.multiCockDescriptLight() + ".  You wonder if she will remember the pleasure her body felt today, and if she will be more likely to drop to her knees before you than to fight... ");
				outputText("Finally finished, you pull your " + player.multiCockDescriptLight() + " from her well used slits, your " + player.multiCockDescriptLight() + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ");
			}
			combat.cleanupAfterCombat();
		}
	}
...
private function sandwitchSex():void {
	spriteSelect(SpriteDb.s_sandwich);
	clearOutput();
	outputText(images.showImage("sandwitch-male-win-vagsex"));
...
private function sandwitchCentaurBoning():void {
	...
	if (player.biggestTitSize() > 3) {
		outputText("As you approach, her eyes drift up to your " + player.allBreastsDescript() + " and she ");
		//[win via HP]
	...
	if (player.biggestLactation() > 1) {
		outputText("Her eyes widen when she sees the beads of milk already dripping from your " + player.nippleDescript(0) + "s as her hand drops between her legs and she begins to slowly masturbate.");
	}
	outputText("\n\n");
	//[Corruption < 70]
	if (player.cor < 70) {
		...
		//[has breasts]
		if (player.biggestTitSize() > 0) outputText("; your " + player.nippleDescript(0) + "s  become stiff");
		//[cock only]
		...
		//[has breasts]
		if (player.biggestTitSize() > 0) outputText("fondle your " + player.allBreastsDescript() + " and ");
		outputText("tease your " + player.nippleDescript(0) + "s, and you can feel her grinding herself against your strong shoulders. ");
		...
		if (player.biggestTitSize() > 0) {
			//[lactating]
			if (player.biggestLactation() >= 1) {
				outputText("Your own " + player.nippleDescript(0) + "s  are leaking as well, ");
				//[light-med lactation]
...
private function SWCentaurMore(argument:Number):Boolean {
	...
	if (argument === 2) {
		...
		if (player.cockTotal() > 0 || (player.gender === 3 && rand(2) === 0)) {
			...
			//[2+ cocks]
			if (player.cockTotal() >= 2) {
				outputText("The feeling of your " + player.multiCockDescriptLight() + " touching each other through the narrow layers of her body causes you to grunt in pleasure, experiencing the sensation anew during every thrust. ");
			}
			...
			outputText(".  With a satisfied groan, you pull out and let your " + player.multiCockDescriptLight() + " dribble the last remnants of your cum over the ravished witch. Satisfied, you ride off into the desert.");
		}
		//[% no cocks]
		else {
			outputText("Finished with your games ");
			//[has cunt] 			
			outputText(images.showImage("sandwitch-taur-win-get-cunning-fingered"));
			if (player.hasVagina()) outputText("and your " + player.vaginaDescript(0) + " dripping with desire");
			...
			//[has cunt]
			if (player.hasVagina()) {
				outputText(player.vaginaDescript(0) + ", she fingers herself even harder than you had been a moment ago.  Her lips lock unto your " + player.clitDescript() + " and she suckles on it, drawing a pained gasp from you and causing your pussy juices to gush over her face. You feel ");
				//[cunt size is small]
				...
				outputText("pushing deep into your " + player.vaginaDescript(0) + ", and you're surprised as ");
				//[anus size is small]
			...
			//[does not has cunt]
				...
				outputText(player.assholeDescript() + ", she fingers herself even harder than you had been a moment ago.  Her tongue drives into your " + player.assholeDescript() + ", licking enthusiastically as she squeezes her nipples. ");
				//[anus size is small]
...
private function knotSandwitch():void {
	spriteSelect(SpriteDb.s_sandwich);
	clearOutput();
	outputText(images.showImage("sandwitch-male-win-dogcock-vagsex"));
    ...
    if (player.cumQ() >= 25 && player.cumQ() < 100) outputText("Your cum expands her pussy, stretching her out and leaving your " + player.cockDescript(0) + " swimming in your spunk, trapped within her tight-stretched walls by your massive knot.");
...
private function rapeSandwitchMultis():void {
	spriteSelect(SpriteDb.s_sandwich);
	clearOutput();
	outputText(images.showImage("sandwitch-male-win-multidick-vagsex"));
...
private function missingoSex():void {
	spriteSelect(SpriteDb.s_sandwich);
	clearOutput();
	outputText(images.showImage("sandwitch-silly-missingosex"));
...
private function missingoSex5():void {
	...
	outputText(images.showImage("coc-title"));
...
public function sandwitchGetsDildoed():void {
	clearOutput();
	outputText(images.showImage("sandwitch-u-win-dildoher"));
...
private function eggwitchForeplay():void {
	clearOutput();
	outputText(images.showImage("sandwitch-bee-drider-give-foreplay"));
...
private function getToFuckingWithZeEggsInWitch():void {
	clearOutput();
	outputText(images.showImage("sandwitch-spread-her-leg"));
...
public function sammitchBirthsDriders():void {
	...
	if (player.isPureEnough(60)) helpZeWithBirfBabies();
...
public function witchBirfsSomeBees():void {
	clearOutput();
	//Occurs 8 days (or moar appropriate time) after fertile oviposition; triggered when exploring the Desert
	outputText(images.showImage("sandwitch-give-birth-bee"));
...
private function sandwitchSpanking():void {
	...
	outputText(images.showImage("sandwitch-get-spank"));
