...
// TIMES_MET_CHAMELEON:int = 561;
		...
		private function loseToChameleonWithCockAnBallsAnCunt():void
		{
			...
			spriteSelect(SpriteDb.s_chameleon);
			outputText(images.showImage("chameleon-herm-lose-footjob-taildick"));
		...
		private function dudesLoseToChamChams():void
		{
			...
			spriteSelect(SpriteDb.s_chameleon);
			outputText(images.showImage("chameleon-male-lose-facesit-vagsex"));
			outputText("\n\nRecognizing that you're no danger, her skin shifts back into its natural " + monster.skin.tone + " coloration as she walks towards you.  She swings her hips seductively as she splashes slowly through the water and licks her lips with a loud smack of her tongue, then reaches out and forcefully pulls your " + player.armorName + " above your waist before you can even try to resist her advances, and pushes you back against a tree.  With a few quick pulls of fabric she's stripped her own clothes, and she tosses her thong over a low-hanging branch.  The other band of silk she uses to quickly bind your wrists.  Were you in any shape to fight back you could pull free, but you're too ");
		...
		{
			spriteSelect(SpriteDb.s_chameleon);
			outputText(images.showImage("chameleon-female-lose-facesit-tailfuck"));
		...
		public function defeatChameleonGirl():void
		{
			...
			//-Win by HP
				...
				outputText("Too weak to continue fighting, the chameleon girl drops to her knees, exhausted.  Her skin returns to its usual " + monster.skin.tone + ", unable to maintain the camouflage.");
		...
		private function fuckDatChameleonAsACoolGuyGirlHerm():void
		{
			...
			outputText(images.showImage("chameleon-herm-win-tailjob-handjob"));
		...
		private function manFucksChameleonWithBiggishWang():void
		{
			...
			spriteSelect(SpriteDb.s_chameleon);
			var x:int = player.biggestCockIndex();
			outputText(images.showImage("chameleon-male-win-get-blowjob"));
		...
		private function femaleHasWinSexWithChamCham():void
		{
			clearOutput();
			spriteSelect(SpriteDb.s_chameleon);
			outputText(images.showImage("chameleon-female-win-fingering"));
		...
		private function giveTheChameleonASuccubiMilk():void
		{
			clearOutput();
			spriteSelect(SpriteDb.s_chameleon);
			outputText(images.showImage("chameleon-male-win-succumilk-titfuck"));
		...
		private function doseDatChameleonWithLustAndSensitivityDrafts():void
		{
			...
			if (!player.hasCock() || player.cockArea(player.smallestCockIndex()) > monster.vaginalCapacity()) {	
				outputText(images.showImage("chameleon-male-win-fingering"));
			...
			//(else if at least one cock of appropriate size)
				...
				outputText(images.showImage("chameleon-male-win-vagsex"));
