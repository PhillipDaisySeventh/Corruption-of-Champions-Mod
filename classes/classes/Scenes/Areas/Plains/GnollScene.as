		...
		public function getRapedByGnoll():void
		{
			...
			if (player.cockTotal() > 0) {
				...
				if (player.thickestCockThickness() > 2) {
					//(if cockTotal>1)
					if (player.cockTotal() > 1) outputText("The gnoll fishes into your " + player.multiCockDescriptLight() + " and quickly snaps up the thickest one.  ");
					//(if cockTotal=1)
					else outputText("The gnoll's hands dart down to your crotch and grabs " + player.cockDescript(0) + ".  ");
			...
			//(if breastRating>C)
			if (player.biggestTitSize() >= 3) outputText("  Your " + player.allBreastsDescript() + " bounce up and down as the gnoll grinds her anatomy into yours, slick with sweat under the hot sun.  Your " + player.nippleDescript(0) + "s tighten and flush as your whole body submits to the rough fuck administered by the hyena girl.");
			//(if cockTotal>0)
			if (player.cockTotal() > 0) {
...
//DickDownClit
		private function dickDownGnollClit():void
		{
			...
			else outputText("Soon you feel the end of her clitoral tunnel butting up against your crotch.  You are as far in as you'll get.  You look down the length of her strange anatomy, the tube bulging thicker to accommodate your " + player.cockDescript(x) + ".  There are still " + UnitSystem.literalInches() + " between your cockhead and the end of her clit-dick.\n\n");

			outputText("Now you begin to roll your hips in a steady rhythm, running your dick up and down her long clitoral tunnel.  It's like the deepest, tightest cunt you've ever had, gripping every " + UnitSystem.literalInch() + " of you.");
			//(if balls>0)
			if (player.balls > 0) outputText("  It doesn't take long before your " + player.ballsDescriptLight() + " begin twitching, pumping cum down the length of your encased cock.");
			...
			//(if cumQ()<25)
			if (player.cumQ() < 25) outputText("Your " + player.cockDescript(x) + " convulses, pumping down the length of her clitoris.  She squirms and howls beneath you, hissing for more.");
			//(if cumQ()>25 AND cumQ()<500)
			else if (player.cumQ() < 500) outputText("Your " + player.cockDescript(x) + " convulses, pumping cum into her.  Her clitoris bulges even more as the jism backs up.  The warm, wet, sticky insides swirl around your dick.");
			//(if cumQ()>500)
			else outputText("Your " + player.cockDescript(x) + " convulses, pouring cum into her.  Her clitoris bulges even more as the jism backs up, the skin pulling tight and shiny as the pressure builds.  Finally gouts of cum begin spurting back out the tip of her clitoris, splashing against your crotch.");
		...
		private function dickInGnollAss():void
		{
			...
			if (rand(2) == 0) {
				...
				if (player.totalCocks() > 1 && player.cockArea(x) < monster.analCapacity() && y != -1) {
					...
					//(if cockTotal>3)
					if (player.cockTotal() > 3 && player.cockArea(0) + player.cockArea(1) + player.cockArea(2) < monster.analCapacity()) outputText("  From there it's some time before her anus is stretched wide enough for more.  By now you can feel your orgasm on the horizon, so you're less careful with your fourth " + player.cockDescript(y) + ".  You slap it into the bundle of dicks and then ram it home.");
					//(if cockTotal>4)
					...
					//(if cockTotal>6)
					if (player.cockTotal() > 6 && player.cockArea(0) + player.cockArea(1) + player.cockArea(2) + player.cockArea(4) + player.cockArea(5) + player.cockArea(6) < monster.analCapacity()) outputText("  And the next, until all of your " + player.multiCockDescriptLight() + " are inside her wide-spread ass.");
					outputText("\n\n");
				}
				...
				outputText("The gnoll lets out a squeal that sounds halfway like a maniacal laugh, sliding forward in the dirt a little.  Your " + player.cockDescript(x) + " throbs wonderfully, fully engulfed by the tight passage of the gnoll's anus");
				...
				//Med CumQ):
					...
					outputText("Gripping her sides tightly, you let out a bellowing moan that echos through the grasslands, " + player.sMultiCockDesc() + " swelling noticeably as you pump thick ribbons of spunk into the gnoll's innards");
				...
				//(High CumQ):
					...
					outputText("You bear down on the limber hyena girl with all your force, groaning through clenched teeth as " + player.sMultiCockDesc() + " distends with the pressure of your virile load.  Her puckered anus stretches around the swell of your seed, and she lets out a shuddering, laughing moan as her belly begins to distend with your thick jism.  The involuntary spasms of her rectum milk your " + player.cockDescript(x) + " for every drop, clenching your member tightly");
					if (player.cockTotal() > 1) {
...
//SuckHerClit
		private function suckGnollClit():void
		{
			...
			outputText("You're only too happy to oblige her, slowly and carefully swallowing the strange member down your throat.  " + capitalizeFirstLetter(UnitSystem.literalInch()) + " by " + UnitSystem.literalInch() + " you draw it deeper into yourself, eliciting increasing groans from the hyena girl.  Finally your nose buries itself in the rough, curly hair of her crotch, your chin rubbing up against the fake scrotum formed out of her distorted labia.\n\n");
...
//TakeHerClit
		private function takeGnollClit():void
		{
			...
			outputText("When you can get your feet under yourself again, you stand up, letting the now-shrunken clitoris fall out of your " + player.vaginaDescript(0) + " with a wet plop.  You leave her there on the sun-caked ground and head back to camp.");
