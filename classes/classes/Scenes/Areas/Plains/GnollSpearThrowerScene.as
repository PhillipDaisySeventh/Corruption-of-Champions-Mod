...
//Female Gnoll.  First Page.
		public function gnoll2Encounter():void
		{
			...
			if (flags[kFLAGS.HAVE_ENCOUNTERED_GNOLL_PLAINS] == 0) {
				...
				outputText("The only warning you have is when your ears catch the barest of whispers, giving you time to jump to the side, just as a long wooden javelin whistles through the air where your head would have been.  You whirl around to see a massive hyena woman standing a few " + UnitSystem.literalYards() + " away.  The tall, muscular woman ");
...
//<Hyena Victorious – Anal> 
		public function hyenaSpearLossAnal():void
		{
			...
			//<Paragraph for Non-Taurs>
				...
				if (player.balls > 0) outputText("clamping onto your " + player.sackDescript() + " and squeezing until ");
				else if (player.hasVagina()) outputText("grabbing your " + player.clitDescript() + " until ");
				else outputText("plunging into your " + player.assholeDescript() + " and pinching until ");
			...
			outputText("Sharp talons press against your back, leaving eight lines of sensation as they trace down your body and along your " + player.hipDescript() + ".  You feel the heat of her breath just above your ear.  \"<i>The queens of the savanna demand submission.  You are only welcome in these lands so long as you pay tribute to us in the most primal of ways.</i>\"  ");
			//<For characters with non-naga tails>
			if (player.tail.type > Tail.NONE) outputText("One furred paw wraps around your tail before pulling up sharply, exposing your " + player.assholeDescript() + " to the hot, savanna air.  Sharp fangs clamp onto the tail, holding it up to rob you of any remaining modesty.");
			...
			outputText("Clawed hands slide around your sides to roughly squeeze your " + player.chestDesc() + ", using the painful hold to brace her body.  The hardened shaft vanishes as her hips pull back suddenly.  The barest touches against your entrance is all the warning you have before she shoves the " + UnitSystem.displayInchWithHyphenTextually(2) + " thick hyena meat roughly forward, stretching your poor hole wide around the sudden intruder.");
			...
			//<Male> 
			if (player.hasCock()) outputText("Your cum spurts from your " + player.multiCockDescriptLight() + ", spraying your seed over the dry ground, giving life to the savanna.");
			//<Females>
			else if (player.hasVagina()) outputText("Your own juices flow freely from your " + player.vaginaDescript() + ", coating your thighs and dripping onto the dry grass below you.");
...
//<Hyena Defeat - Fellatio>
		private function hyenaVictoryRapeFellatio():void
		{
			...
			if (player.cocks[x].cockLength > 7) outputText("The tip of your member presses hard against the back of her mouth for a long moment until she swallows, letting your " + player.cockDescript(x) + " slide into her tight throat.  ");
			...
			//<Cock 0 is 7\" or greater> 
			if (player.cocks[x].cockLength >= 7) outputText("The muzzle starts to pull away from your " + player.cockDescript(x) + " until the tip finally slips from her throat, letting her gasp for breath.  ");
