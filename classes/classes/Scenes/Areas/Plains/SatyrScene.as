		...
		*/
		...
		private function consensualSatyrFuck(loc:int = 0):void {
			...
			if (player.inte > 60 && player.lust < player.maxLust()) {
				...
				addButton(0, "Trick Him", trickZeSatyr).hint("Pretend to drink while the satyr drinks himself till he gets smashed.");
			...
			if (player.lib > 60) {
				...
				addButton(2, "Skip Foreplay", skipForeplay).hint("Why wait? Get to the good part!");
			...
			if (player.lust >= 33) {
				...
				addButton(3, "Drink & Sex", drinkAndSex).hint("Have just one more drink and get into some good sexy times.");
			} else addDisabledButton(3, "Drink & Sex", "You are not lusty enough to rush things.");
			
			//What should you do?
			//[Trick him] [Keep Drinking] [Skip Foreplay] [Leave]
			addButton(1, "Keep Drinking", keepDrinking).hint("Keep drinking until you pass out and be at the mercy of the satyr.");
			addButton(4, "Leave", leavePartySatyr).hint("Decline and attempt to leave. However, you're not sure the satyr will be more than willing to let you go.");
		}
		...
		private function keepDrinking():void {
			...
			outputText(images.showImage("satyr-hangover"));
		...
		private function leavePartySatyr():void {
			...
			if (rand(2) == 0) {
				...
				outputText("\n\nDespite moments having passed, you turn back to see the satyr continuing to drink and not seeming to mind the fact that you have left. Seems like you're safe for the time being and you make your trip back to the camp without incidents.");
		...
		internal function defeatASatyr():void {
			...
			if (player.lust >= 33 && !player.isGenderless()) {
				...
				if (player.hasCock() && player.cockThatFits(monster.analCapacity()) >= 0) {
					addButton(0, "FuckHisButt", malesTakeAdvantageOfSatyrs).hint("Teach the satyr a good lesson and ram his butt hard!", "Fuck His Butt");
				}
				if (player.hasVagina()) {
					addButton(1, "Ride Face", femaleTakesAdvantageOfSatyr).hint("Sit on the satyr and make him lick you good down there.");
					if (player.biggestTitSize() >= 4 && player.armor == armors.LMARMOR) {
						addButton(2, "Ride Face", (player.armor as LustyMaidensArmor).lustyMaidenPaizuri).hint("Take advantage of the properties of your special armour and get the spider-morph to give you a good titfuck.", "Bikini Titfuck/Paizuri");;
					}
				}
			}
