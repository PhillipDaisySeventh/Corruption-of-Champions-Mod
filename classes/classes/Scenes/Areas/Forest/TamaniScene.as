...
private function tamaniFemaleEncounter():void {
	...
	clearOutput();
	outputText(images.showImage("tamani-encounter"));
	outputText("A goblin leaps out from behind a rock outcropping.  She keeps her arms folded across her " + tamaniChest() + " and glares at you.  The little thing is only about " + UnitSystem.displayHeight2EstimateTextually(4) + " tall, with pink and black dyed hair cut into a cute little 'do.  The greenish-gray skin of her breasts bulges out around her arms, supported by a few leather straps, amplifying her cleavage.  Her cunt lips are pierced multiple times, inflamed, and slightly parted.  There really isn't any clothing on her to hide them, just more of the ever-present straps wrapping around her thighs.\n\n");
...
private function tamaniFemaleYes():void {
	spriteSelect(SpriteDb.s_tamani);
	clearOutput();
	outputText(images.showImage("tamani-dDildo"));
	...
	doNext(tamaniFemaleItem);
}
private function tamaniFemaleItem():void {
	clearOutput();
	outputText(images.showImage("item-dDildo"));
...
private function tamaniMaleFirstEncounter():void {
	...
	clearOutput();
	outputText(images.showImage("tamani-encounter"));
	outputText("A goblin leaps out from behind a rock outcropping.  For something so small, she has a lot of curves.  She advances towards you, rolling her hips in a suggestive way, immediately diverting your blood-flow to your crotch.  The little thing is only about " + UnitSystem.displayHeight2EstimateTextually(4) + " tall, with pink and black dyed hair cut into a cute little 'do.  The greenish-gray skin of her breasts jiggles pleasantly with every step, supported by a few leather straps, amplifying her cleavage.  Her cunt lips are pierced multiple times, inflamed, and slightly parted.  There really isn't any clothing on her to hide them, just more of the ever-present straps wrapping around her thighs.\n\n");
...
private function tamaniFirstTimeConsentual():void {
	...
	tamaniKnockUp();
	clearOutput();
	outputText(images.showImage("tamani-fuck"));
	...
	outputText("Her hips sway seductively as she approaches with her right hand dipping into the moist honeypot of her sex.  You disrobe, throwing your " + player.armorName + " to the side before you reach down and lift her up, pressing her curvy body against your ");
	...
	if (player.balls > 0) outputText("Your " + player.ballsDescriptLight() + " swell with seed, spurring your desire to new heights.");
	...
	if (player.cockArea(0) <= 90) {
		...
		if (player.balls > 0) outputText(" and drips from your " + player.ballsDescriptLight());
		...
		outputText("It takes a moment to put your " + player.armorName + " back on and make ready to leave, but somehow you know this isn't the last you've seen of this goblin.");
		//[CORRUPT]
	...
	//(TOO BIG)
		...
		if (player.balls > 0 && player.hoursSinceCum > 200) outputText(", visibly draining your " + player.ballsDescriptLight() + " down to their normal size");
...
internal function tamaniSexLetHer():void {
	...
	tamaniKnockUp();
	clearOutput();
	outputText(images.showImage("tamani-letHer-fuck"));
	//[lost combat]
	if (getGame().inCombat) {
		//Taurs
		if (player.isTaur()) {
			if (player.HP < 1) outputText("You stumble, legs weaving drunkenly as you try to keep your feet.  ");
			else outputText("You stumble, legs weaving drunkenly as " + player.sMultiCockDesc() + " ache and drool with need.  ");
	...
	if (player.isTaur()) {
		...
		//If >18 Inches 
			...
			//High cum: 
				...
				if (player.balls > 0) outputText("Your " + player.ballsDescriptLight() + " ache and you feel very thirsty indeed.");
	...
	//Not-taurs
		...
		if (player.cockArea(0) <= 90) {
			...
			if (player.balls > 0) outputText("Your " + player.ballsDescriptLight() + " twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.");
			...
			if (player.cumQ() < 250)outputText(".  I really enjoy our fucks " + player.mf("stud","hun") + ", and I can't wait to 'bump' into you again.");
				...
				if (player.balls > 0) outputText(player.ballsDescriptLight() + " fill back up with cum, ok?");
				else outputText(player.cockDescript(0) + " fills back up with cum, ok?");
			}
			outputText("</i>\"\n\n");
			//Combat end: 
			if (getGame().inCombat) {
		...
		//(Doesnt fit)
			...
			if (player.balls > 0) outputText("lifts up your " + player.ballsDescriptLight());
			else outputText("leans forwards, looking between your legs");
			outputText(", and places the dildo's leaking tip against your " + player.assholeOrPussy() + ".  You feel it stretching you out, reacting with your body's own fluids to swell up inside you like a pervert's balloon as Tamani pushes it further and further inside you.");
			if (player.hasVagina()) {
				outputText("  ");
			...
			outputText("The kinky goblin dominatrix on your " + player.cockDescript(0) + " leans even further forwards, angling the dildo in your " + player.assholeOrPussy() + " upwards to squeeze your ");
...
internal function tamaniSexLost():void {
	...
	if (player.balls > 0) outputText("cups your " + player.ballsDescriptLight() + " tightly");
	...
	outputText(images.showImage("tamani-loss-fuck"));
...
internal function tamaniSexWon():void {
	...
	if (player.cockArea(x) <= 90) {
		...
		outputText("It's too tempting of a target to resist.  You open your " + player.armorName + " and allow your " + player.multiCockDescriptLight() + " to flop free.  You're already hard from the enticing display, and in a moment you're pressing against her lust-slicked pussy");
	...
	//Too big? Jerk off with feet and bukkake
		...
		if (player.cumQ() >= 250) outputText("  You continue working your " + player.cockDescript(x) + " with her supple feet, watching your dripping spooge froth and bubble as you continue to pump more onto the sassy wench.");
...
private function tamaniPregnantEncounter():void {
	spriteSelect(SpriteDb.s_tamani);
	clearOutput();
	outputText(images.showImage("tamani-preggo-encounter"));
...
private function tamaniPregnantFuck():void {
	spriteSelect(SpriteDb.s_tamani);
	clearOutput();
	outputText("Tamani's eyes light up and she pounces you, somehow managing to jump up and latch onto your chest despite the weight of her burgeoning pregnancy.  The two of you overbalance as her weight carries you to the ground, flat on your back.   Your " + player.buttDescript() + " hurts a little from the impact but it's the last thing on your mind.  More important are the milk-dribbling twin mounds pressing tightly against your ");
	if (player.biggestTitSize() >= 2) outputText(player.allBreastsDescript());
	else outputText("chest");
	outputText(".\n\n");
	...
	outputText(images.showImage("tamani-preggo-fuck"));
	...
	//(Doesn't Fit)
		...
		else outputText("jacking the base of your " + player.cockDescript(0) + " with her feet");
...
private function tamaniPoopsOutBabies():void {
	spriteSelect(SpriteDb.s_tamani);
	clearOutput();
	outputText(images.showImage("tamani-encounter-preggo"));
...
internal function getRapedByTamaniYouHypnoSlut():void {
	...
	outputText(images.showImage("tamani-willing-sex"));
	//[FITS]
	if (primary >= 0) {
		...
		if (cocks > 1) {
			...
			outputText("She moans and pulls your " + player.cockDescript(secondary) + " into her mouth, running her tongue around the tip and sliding her palm along the underside.");
		}
		...
		if (cocks > 0) outputText(" around your " + player.cockDescript(secondary));
		...
		if (player.cumQ() < 50) outputText(", but brief, and after a few moments of spasming pleasure, the flood of cum slows to a trickle.");
			...
			else outputText("  It lasts seemingly forever, and by the time you're done, the two of you are standing in a puddle " + UnitSystem.literalInches() + " deep.");
		}
		if (cocks > 1) {
	...
	//[DOESNT FIT]
		...
		outputText(player.cockDescript(0) + ".  Her warmth spreads over the " + player.cockHead() + " of your " + player.cockDescript(0) + ", but will go no further.  You tug harder on her bouncing backside, but you're simply too large to fit inside your wife's beautiful pussy.  She turns around and places a small hand ");
		if (player.biggestTitSize() < 1) outputText("on your chest ");
		else outputText("between your " + player.breastDescript(0) + " ");
...
internal function tamaniBeaten():void {
	...
	outputText("Tamani exhales, \"<i>Eyes up here, " + player.mf("stud","breeder") + ".  I want to watch your face while you fuck me,</i>\" in a tone that promises delights beyond imagining.  \"<i>Put it in my pussy and give me your babies.  There's nothing like fucking a woman who'll take your cock straight into her womb and let you bathe her eggs with cum");
...
private function postTamaniRemoval():void {
	outputText(images.showImage("tamani-satchel"));
