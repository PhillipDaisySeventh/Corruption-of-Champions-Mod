	...
	public class BeeGirlScene extends BaseContent
	{
		...
		public function beeEncounter():void
		{
			...
			if (player.hasKeyItem("Traveler's Guide") >= 0 && player.inte / 2 > rand(40)) {
				...
				menu();
				doYesNo(beeEncounterSelect, camp.returnToCampUseOneHour);
			}
				...
				switch (attitude) {
					...
					default: //Any other attitude options lead to the classic bee encounter
						...
						if ((player.lib100 + player.corAdjustedDown() < 140) || rand(2) == 0) {
							...
							menu();
							addButton(0, "Fight", fightTheBeeGirl).hint("No, the bee-girl is not going to take you today!");
							addButton(1, "Talk", beeTalk).hint("Try to talk to the bee-girl. What could go wrong in the entirety of Mareth?");
		...
		private function beeEncounterAsBeeMorphMaleOrGiantCock(isBeeMorph:Boolean):void {
			...
			if ((player.lib100 + player.corAdjustedDown() < 140) || rand(2) == 0) {
				...
				menu();
				addButton(0, "Fight", fightTheBeeGirl).hint("No, the bee-girl is not going to take you today!");
				addButton(1, "Sex", beeSexForCocks).hint("Try to talk to the bee-girl. What could go wrong in the entirety of Mareth?");
				addButton(4, "Leave", camp.returnToCampUseOneHour);
			}
		...
		private function beeEncounterAsBeeMorphFemale():void {
			...
			menu();
			addButton(0, "Fight", fightTheBeeGirl).hint("No, the bee-girl is not going to take you today!");
			addButton(1, "Play", beeMaidenPlay).hint("Sure, that could be so fun! Take up the offer and engage in some bee play.");
			addButton(4, "Leave", beeEncounterAsBeeMorphFemaleLeave);
		}
		...
		private function beeEncounterAfraid():void {
			...
			menu();
			addButton(0, "Fight", fightTheBeeGirl).hint("No, the bee-girl is not going to take you today!");
			addButton(1, "Have Sex", beeEncounterAfraidFirstTimeSex).hint("You could take up the offer on sex now that she won't lay her eggs into you.");
			addButton(4, "Leave", camp.returnToCampUseOneHour);
		}
		...
		private function beeEncounterAfraidRepeat():void {
			...
			menu();
			addButton(0, "Fight", fightTheBeeGirl).hint("No, the bee-girl is not going to take you today!");
			addButton(1, "Have Sex", beeEncounterAfraidRepeatSex).hint("You could take up the offer on sex now that she won't lay her eggs into you.");
			addButton(4, "Leave", camp.returnToCampUseOneHour);
		}
		...
		private function beeEncounterAfraidRepeatSex():void {
			...
			else outputText("slipping her finger inside your " + player.assholeDescript() + "till you’re ready");
			...
			if (player.analCapacity() < 25) {
				...
				outputText("Your experience in this world has well prepared your " + player.assholeDescript() + " for the knotted appendage.  Its lubricated surface gives nothing but pleasure to your used rear, and both you and your insectoid lover give nothing but moans of pleasure as each new bump passes into your body.\n\n");
			}
		...
		private function beeEncounterDisgusted():void {
			...
			menu();
			addButton(0, "Fight", fightTheBeeGirl).hint("You could fight the bee-girl but why would you hurt her? She doesn't seem that interested in you. What do you want? Get some chitin so you can make some armour?");
			addButton(4, "Leave", camp.returnToCampUseOneHour);
		}
		...
		private function beeEncounterDuty():void {
			...
			menu();
			addButton(0, "Fight", fightTheBeeGirl).hint("You could engage in a battle against the bee-girl but for what purpose? Gathering bee chitin for armour?");
			addButton(1, "Talk", beeEncounterDutyTalk).hint("See if the bee-girl is up for talking.");
			addButton(4, "Leave", camp.returnToCampUseOneHour);
		}
		...
		private function beeEncounterSheFearsYou():void {
			...
			menu();
			addButton(0, "Fight", fightTheBeeGirl).hint("You could fight the bee-girl but why would you hurt her? She already fears you. What do you want? Get some chitin so you can make some armour? You cruel monster.");
			addButton(4, "Leave", camp.returnToCampUseOneHour);
		}
		...
		private function beeEncounterSheBeatsYouRegularly():void {
			...
			if ((player.lib100 + player.corAdjustedDown() < 70) || rand(4) == 0) { //Chance to avoid raaaaeeeeep
				...
				menu();
				addButton(0, "Fight", fightTheBeeGirl).hint("No, the bee-girl is not going to take you today!");
				addButton(1, "Talk", beeEncounterSheBeatsYouRegularlyTalk).hint("See if the bee-girl is up for talking.");
				addButton(4, "Leave", camp.returnToCampUseOneHour);
			}
			else {
		...
		private function beeEncounterWithWorms():void { //If she won't fuck infested players after combat then she shouldn't fuck them here either
			...
			outputText("She lifts off, hovering " + UnitSystem.aFewFeet() + " off the ground.  <i>“Good luck getting rid of thozzze thingzzz.  When you do, come find me and I'll fill you zzzo full of lovely eggzzz,”</i> she promises, her fingers idly stroking her sex.  She shakes her head, deliberately getting control of herself and forcing her fingers away from her slit, then flies up through a hole in the canopy.\n\n");
		...
		private function beeEncounterClassicSex(postCombat:Boolean = true):void {
			...
			if (player.isTaur()) {
				...
				if (player.longestCockLength() >= 100 || player.gender == 0) {
					...
					if (player.gender == 1) {
						...
						outputText("your engorged " + player.multiCockDescriptLight() + " and jerk it off violently, cumming again and again as the bee-girl violates your " + player.assholeDescript() + ".  ");
						//[Without testicles:
					...
					//[Herm (bypasses the bucking scene):
						...
						outputText("You grasp your engorged " + player.multiCockDescriptLight() + " and jerk it off violently, cumming again and again as the bee-girl violates your " + player.assholeDescript() + " and your " + player.vaginaDescript(0) + " pumps out enough femcum to drench your hind legs.  ");
						//[Without testicles:
						if (player.balls == 0) outputText("With your cock quivering and spasming and your untended to " + player.vaginaDescript(0) + " squirting repeatedly, you continue to lap at the honey covered tree until the vibrating bee-girl finishes.  ");
						//[With testicles:
						else outputText("With your cock quivering and spasming semen erupts from the tip.  ");
						//[Little-normal cum amount:
						if (player.cumQ() < 100) outputText("You spurt out more cum than you thought your " + player.ballsDescriptLight() + " could hold. White speckles the base of the tree and tiny puddles form underneath you as you groan in satisfaction.\n\n");
					...
					outputText("After hours of her forcefully filling your hole, her stinger pumping what feels like " + UnitSystem.literalGallons() + " of her bee-seed into your bowels, she finally pulls out. She smiles and lies down on your back and your legs finally give out from exhaustion.\n\n");
				...
				//Route 1 
					...
					//[Male:
					if (player.gender == 1) outputText("Your inability to reach your engorged " + player.multiCockDescriptLight() + " quickly becomes a problem as the pleasure from the bee-maiden's scent courses through you. You ache to be tended to, gushing out pre.");
					//[Female:
					else if (player.gender == 2) outputText("Your inability to reach your " + player.vaginaDescript(0) + " quickly becomes a problem as the pleasure from the bee maiden's scent courses through you, it starts reflexively rippling, trying to milk the cock it so desperately wants while your " + player.clitDescript() + " quivers.");
					//[Herm:
					...
					outputText("The stimulation grows too painful and you begin to buck wildly. The bee-girl holds on desperately, her stinger still lodged in your " + player.assholeDescript() + " and pumping fluids. She seems to realize the problem though and attempts to solve it.  ");
					//[Male:
					if (player.gender == 1) {
						outputText(images.showImage("beegirl-loss-male"));
						outputText("Her legs wrap around your body and her 'feet' clasp onto your " + player.multiCockDescriptLight() + ". The strangeness of the sensation doesn't register though as just the contact is enough to send you over the edge.  ");
						//[Without testicles:
						if (player.balls == 0) outputText("You cum violently, your " + player.multiCockDescriptLight() + " quivering and spasming.  ");
						//[With testicles:
						else outputText("You cum violently, your " + player.multiCockDescriptLight() + " quivering and spasming while semen erupts from the tip.  ");
						//[Little-normal cum amount:
					...
					//[Female:
						...
						outputText("Her legs curl around your hind legs and her 'feet' begin rubbing your " + player.vaginaDescript(0) + ". The strangeness of the sensation doesn't register though as just the contact is enough to send you over the edge.  ");
						//[Tiny-normal clit:
						if (player.getClitLength() < 3) outputText("Her awkward ministrations to your " + player.clitDescript() + " cause a gush of fluids from your " + player.vaginaDescript(0) + " and you let out a lewd moan of satisfaction.  ");
						//[Cock-like clit:
						else outputText("She quickly finds your " + player.clitDescript() + " and begins to jerk it off like a cock.  The sensation is incredible and you moan lewdly as your " + player.vaginaDescript(0) + " gushes fluid.  ");
					...
					//[Herm:
						...
						if (rand(2) == 0) {
							...
							outputText("Thinking you still not satisfied, she pulls her 'feet' up slightly and she begins to rub your " + player.vaginaDescript(0) + ".  ");
						...
						//[Same as female followed by:]
							...
							outputText("Thinking you're still not satisfied, she lowers her 'feet' slightly and she begins to rub your " + player.multiCockDescriptLight() + ".  ");
					...
					outputText("After hours of her forcefully filling your hole, her stinger pumping what feels like " + UnitSystem.literalGallons() + " of her bee-seed into your bowels while awkwardly dealing with your needs; she finally pulls out. She smiles and lies down on your back and your legs finally give out from exhaustion.\n\n");
			...
			//non-dicked version
				...
				if (player.vaginas.length > 0) outputText(" as your hand is guided to your " + player.vaginaDescript(0));
				outputText(".\n\n");
				//Cum text
				if (player.vaginas.length > 0) outputText("Your fingers dance over your clit as you lap up her honey");
				else outputText("Your fingers explore every " + UnitSystem.literalInch() + " of both your bodies as you lap up her honey");
				...
				if (player.vaginas.length > 0) outputText(", the constant aphrodisiac honey you're mindlessly devouring, and your fingers' instinctual ravaging of your " + player.vaginaDescript(0) + ", ");
		...
		private function beeDroneBadEndPart3():void {
			clearOutput();
			spriteSelect(SpriteDb.s_bee_girl);
			outputText("When you enter into the queen’s chamber, your senses are assaulted with a multitude of erotic images, sounds, and scents.  There are dozens of bees all in various states of debauchery and sexual escapades.  The females are all very much like your companion, with voluptuous thin bodies, while the males are built a bit different.  While their main bodies are still fairly thin, and they generally have cute androgynous faces, their main feature would be their massive manhoods, which are all easily " + UnitSystem.displayFeetEstimateRangeTextually(3, " to ", 4) + " long, and at least " + UnitSystem.displayInchesEstimateTextually(5) + " in diameter.\n\n");
			...
			outputText("Ah yes, the queen.  The centerpiece of all of this sexual diorama is the queen herself, and everything about her is simply huge.  She stands at least " + UnitSystem.displayHeight2EstimateTextually(12) + " tall, probably more.  She has a beautiful but not necessarily delicate build.  If you had to describe her, a slightly thicker scaled up version of your bee companion wouldn’t be too far off when describing everything but her face and abdomen.  Her face is, motherly but stern, regal but still kind.  Just looking into her deep black eyes is enough to set your mind at peace, and the sight of her luscious black lips smiling down at you sends your heart fluttering.  Atop her head is a massive amount of hair done up in an elaborate set of braids, bands, and knots.  Her luscious and beautifully styled black and yellow hair is probably the most beautiful hair you’ve ever seen.\n\n");
			outputText("Her other major feature is her abdomen, and it is probably the most notable part of her.  The thing is nearly as tall as the bee queen’s humanoid body, and just as wide.  It extends at least " + UnitSystem.displayFeetEstimateTextually(30) + " out from the queen’s body.  There are two major factors about her abdomen.  First, is that it is ringed with numerous slits of varying sizes, and given that the majority of the male bees are eagerly and frantically thrusting their massive manhoods into these slits, you’d guess that they’re her vaginas.  Second, is the handmaiden that is currently being screwed by a fairly large phallic stinger on the end of it.  Though, admittedly the " + UnitSystem.displayInchWithHyphenTextually(18) + " long by " + UnitSystem.display("two-and-a-half-inch", UnitSystem.displayInchWithHyphenTextually(2.5)) + " wide stinger pales in comparison to the phalluses that the male bees have.\n\n");
		...
		private function beeEncounterRefusedHerEggs():void {
			...
			switch (attitude) {
				case BEE_GIRL_TALKED_AND_LEFT_TWICE:
					...
					menu();
					addButton(0, "Afraid", beeEncounterRefusedHerEggsAfraid).hint("You have the fear of eggs being put inside you. The mere thoughts of eggs hatching inside you and seeing all those bees crawl out of you make you shiver with fear.");
					addButton(1, "Disgusted", beeEncounterRefusedHerEggsDisgusted).hint("The thought of eggs disgust you, you do not want any more of those offers.");
					addButton(2, "Duty", beeEncounterRefusedHerEggsDuty).hint("You have a duty as a Champion and you cannot give it up.");
					addButton(4, "Leave", beeEncounterRefusedHerEggsLeave).hint("Screw it, you're outta here!");
					return;
...
*/
		...
		private function beeMaidenFertileBeeBadEndPart3():void {
			...
			outputText("Her other major feature is her abdomen, and it is probably the most notable part of her.  The thing is nearly as tall as the bee queen’s humanoid body, and just as wide.  It extends at least " + UnitSystem.displayFeetEstimateTextually(30) + " out from the queen’s body.  There are two major factors about her abdomen.  First, it's ringed with numerous slits of varying sizes, and given that the majority of the male bees are eagerly and frantically thrusting their massive manhoods into these slits, you’d guess that they’re her vaginas.  Second, there is a handmaiden that is currently being screwed by a fairly large phallic stinger on the end of it.  Though, admittedly the " + UnitSystem.displayInchWithHyphenTextually(18) + " long by " + UnitSystem.display("two-and-a-half-inch", UnitSystem.displayInchWithHyphenTextually(2.5)) + " wide stinger pales in comparison to the phalluses that the male bees have.\n\n");
			...
			if (player.isTaur()) {
				...
				if (player.HP < 1) {
					...
					//[Female:
						...
						outputText("They seem to understand your problem though and, more out of a desire to not get tossed off than to help you with your needs, begin to kick at where your balls ought to be with their " + UnitSystem.literalFeet() + ". A handful of blows land on your " + player.clitDescript() + " and you cum hard, squirting behind you with surprising force.\n\n");
					...
					//[Herm:
						...
						outputText("They seem to understand your problem though and, more out of a desire to not get tossed off than to help you with your needs, begin to kick and punch at your " + player.vaginaDescript(0));
				...
				//Lust:
					...
					if (player.gender == 1 && player.balls > 0) {
						...
						outputText("She continues pumping into you as you pass out, " + player.sMultiCockDesc() + " and " + player.ballsDescriptLight() + " aching.  The combined scent of cum and honey completely overwhelms you.");
					}
					//[Male without testicles:
				...
				if ((player.hasStatusEffect(StatusEffects.ParalyzeVenom) || player.hasStatusEffect(StatusEffects.lustvenom)) && !sexed && player.cockTotal() == 1) {
					outputText("The bee-girl stands directly over you, looking down at you with a triumphant cute smile across her face, her eyes surveying every " + UnitSystem.display("inch", "milli" + UnitSystem.meter()) + " of your body. All you can do is lay there looking up at her fine figure and a profile of her ample breasts with a knowing smile on your face. She crouches down on all fours, just out of your reach, and begins to rhythmically shake her extended abdomen like a child's toy.\n\n");
		...
		public function beeGirlPCVictory(hpVictory:Boolean):void
		{
			...
			if (player.hasCock()) {
				addButton(0, "Use Cock", rapeTheBeeGirlWithADick).hint("You could fuck her with " + player.oMultiCockDesc() + ".", "Use Cock");
			}
			if (player.cockTotal() > 1) {
				addButton(1, "Use Cocks", rapeTheBeeMultiCockStuff).hint("You could use more than one of your " + player.multiCockDescriptLight() + " on her.", "Use Cocks");
			}
			if (player.hasVagina()) {
				addButton(2, "Use Vagina", rapeABeeGirlWithYourVagina).hint("You could make her get off your " + player.vaginaDescript() + ".", "Use Vagina");
			}
			if (player.isHerm()) {
				addButton(3, "Herm Style", futaRapesBeeGirl).hint("You could try to please both your 'male' and 'female' halves on the bee.", "Herm Style");
			}
			if (player.hasKeyItem("Deluxe Dildo") >= 0) {
				addButton(4, "Dildo Rape", beeGirlsGetsDildoed).hint("You could play with your toy.", "Dildo Rape");
			}
			if (player.hasStatusEffect(StatusEffects.Feeder) || player.lactationQ() >= 500) {
				addButton(5, "B. Feed", milkAndHoneyAreKindaFunny).hint("You could have some relief.", "Breastfeed");
			}
			if (player.isNaga() && player.face.type == Face.SNAKE_FANGS) {
				addButton(6, "Naga", corruptNagaBitchesRapeABee).hint("You could focus on your snakelike, 'naga' attributes.", "Naga");
			}
			if ((player.isCorruptEnough(75) || player.findPerk(PerkLib.Pervert) >= 0 || player.findPerk(PerkLib.Sadist) >= 0)
				...
				addButton(7, "Self-Egg", beeGirlRapeForTheDistinguishedGentleman).hint("You could play with her a bit and try to make her lay eggs into herself.", "Self-Egg");
			}
			if (player.canOvipositSpider() && (player.face.type == Face.SNAKE_FANGS || player.face.type == Face.SPIDER_FANGS)) {
				addButton(8, "LayYourEggs", layEggsInABeeSpiderLike).hint("You could dose her with venom and lay YOUR eggs in her.", "Lay Your Eggs");
			}
		...
		private function rapeTheBeeMultiCockStuff():void
		{
			...
			if (player.cockTotal() >= 5 && player.biggestTitSize() > 2
				...
				outputText(images.showImage("beegirl-win-male"));
		...
		private function rapeTheBeeGirlWithADick():void
		{
			...
			if (player.isTaur()) {
				...
				if (player.cockArea(x) <= monster.vaginalCapacity()) {
					...
					//No testicles:\
					else outputText("Her other hand starts pulling it open and soon her tongue joins them, sliding in and out of your ass delicately. Your " + player.cockDescript(x) + " quivers with anticipation and you swish your tail happily.");
					outputText("\n\n");
					...
					outputText("The sensation is incredible; her muscles spasm and milk your " + player.cockDescript(x) + " while the object in your " + player.assholeDescript() + " (which you think is probably her ovipositor) begins pumping fluids into you, all while your genitals are vibrated unceasingly.");
					//Has testicles:
					if (player.balls > 0) outputText("  Your " + player.ballsDescriptLight() + " really feel spectacular, the vibrations causing the hot spunk inside to roil about.");
					...
					//has testicles: 
					if (player.balls > 0) outputText(", and your " + player.ballsDescriptLight() + " constrict");
					outputText(".\n\n");
				...
				//Cock too long to penetrate her:
					...
					outputText("Your " + player.cockDescript(x) + " feels wonderful though, pumping out fluid and swelling with more and more blood.");
					//Has testicles: 
					if (player.balls > 0) outputText("  The vibrations do a number on your " + player.ballsDescriptLight() + " too, as the semen stored within starts to shake and dance about inside.");
					outputText("  The thing in your " + player.assholeDescript() + " (which you believe is her ovipositor) is vibrating along with her as it pumps fluid inside, and you feel yourself puckering in pleasure.");
					...
					//Other cocks: 
					else outputText("  Her tongue wraps around your " + player.cockDescript(x) + " and slides up and down the small area it can reach before flicking about at the tip of your urethra. Her expert licking feels amazing and it's not long before your feel the familiar sensation of orgasm creeping up on you.");
					outputText("\n\n");

					outputText("With all the combined sensations your " + player.cockDescript(x) + " begins to unload. ");
					//With testicles, masturbatable cock:
					if (player.balls > 0 && player.tallness * (5 / 6) < player.cocks[player.longestCock()].cockLength) {
						outputText("Your " + player.ballsDescriptLight() + " contract and propel your seed down your member toward the fresh air. It fires out of you and hits the bee's flower, splattering it in white.");
						//Huge cum amount: 
					...
					//With testicles, other cocks:
						...
						outputText("Your " + player.ballsDescriptLight() + " contract and propel your seed down your member toward the waiting bee-girl. It fires out of you and hits her face, splattering it in white.");
						//Huge cum amount: 
					...
					//Without testicles: 
					else outputText("Your " + player.assholeDescript() + " tightens uncontrollably around its intruder, making the vibrations feel even stronger there. Your member jerks as you orgasm and a few droplets of semen land on the bee maiden.");
				...
				if (player.isTaur()) outputText("as your " + player.cockDescript(x) + " hardens under your belly.");
				else outputText("as you disrobe, revealing your " + player.cockDescript(x) + " to her.");
				outputText("  Wasting no time, you get down to the business of ");
				if (player.cockTotal() == 1) outputText("penetrating her slick snatch, forcing in " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + ".  ");
				if (player.cockTotal() == 2) outputText("cramming your " + player.multiCockDescript() + " into her, double-stuffing the poor bee-slut with your double dongs.  ");
				if (player.cockTotal() >= 3) outputText("cramming two out of your " + num2Text(player.cockTotal()) + " " + player.multiCockDescript() + "s into her, double-stuffing the poor bee-slut with double the dicks.  ");
				...
				if (player.gender == 3 && !player.isTaur()) outputText("  You reach down and finger your " + player.vaginaDescript(0) + " roughly, lost in the throes of your orgasm.");
				outputText("\n\n");
				//wind down
			...
			//Male 2
				...
				outputText("Letting go of her arms, you grip the base of her abdomen for leverage and let her arms and legs slide around you as your pace increases. You feel her abdomen pressing against your rear as you drive into her, the pressure in your " + player.ballsDescriptLight() + " building as her pussy tightens around you. With a groan you cum, your hot seed gushing inside of her. She clenches you tightly, her pussy milking your " + player.cockDescript(x) + " for every ounce, until the sticky white fluid is pouring from around your " + player.cockDescript(x) + ". Exhausted from the battle and the fuck, she relaxes and collapses on the ground, panting heavily and half-conscious.");
				player.orgasm('Dick');
				combat.cleanupAfterCombat();
			}
		}
...
//FEMALE sometimes herm
		private function rapeABeeGirlWithYourVagina():void
		{
			...
			if (player.isTaur()) {
				...
				//[Fuckable nipples: 
				if (player.hasFuckableNipples()) outputText("  Not even repeated penetration of your " + player.nippleDescript(0) + "s seems to bring the relief you need.");
				outputText("\n\n");
				outputText(images.showImage("beegirl-win-female"));
				...
				//[Breasts >D: 
				if (player.biggestTitSize() > 4) outputText("Her legs release your head and she falls back, knocking into your " + player.allBreastsDescript() + " and causing them to jiggle lewdly. You grab her before she hits the ground, surprised at how light she is.");
				//[Breasts <=C: 
				...
				outputText("  Her body stiffens as she realizes her situation and with your aid manages to right herself. By now the pressure in your loins is unbearable and you're breathing hard. Your " + player.nippleDescript(0) + "s ache just as badly as your " + player.assholeDescript());
				...
				outputText("She's not done yet though, as her finger is quickly replaced with a long, wet tongue. It writhes about inside you while cum continues to spray out of your " + player.vaginaDescript());
				if (player.hasLongTail() > 0) outputText(" and your flailing tail is held firmly in place by her hand");
				outputText(". Her other hand slides into your " + player.vaginaDescript() + " and scrapes around inside, causing an explosion of pleasure inside you. Your love canal grabs her and pulls her arm deeper inside you, your equine muscles making her powerless to resist.");
				...
				outputText(images.showImage("beegirl-win-female"));
				outputText("Firmly grasping her thighs at the joining of her smooth carapace and soft skin, you force them open, revealing the source of her irresistible scent.   She buzzes pitifully in protest as you disrobe, revealing your " + player.vaginaDescript(0));
...
//FUTA Fallback
		private function futaRapesBeeGirl():void
		{
			...
			//BALLZ
			if (player.balls >= 2) {
				if (player.ballSize < 5) outputText("Your " + player.ballsDescriptLight() + " droop into her eyes, blinding and humiliating her in equal measure.  ");
			...
			outputText("Happy with your victim's compliance, you grind harder into her face.  Allowing your hands a bit of freedom, you begin to stroke your already erect " + player.cockDescript(0));
			...
			if (player.cumQ() >= 100) {
				outputText(" More and more pre begins to drip from your " + player.cockDescript(0) + ", soaking your ");
				if (player.balls >= 2) outputText(player.ballsDescriptLight() + " and ");
				outputText("the bee-bitch's face.  ");
			}
			if (player.breastRows.length > 0) {
				if (player.biggestTitSize() >= 2) outputText("You free a hand to tend to your " + player.allBreastsDescript() + " and aching nipples, alternating between squeezing your female flesh and tugging on your erect nipples.  ");
			}
			outputText("\n\n");
			//New PG
			outputText("You moan in ecstasy, your " + player.vaginaDescript(0) + " clamping against her tongue and lips like a vice.  She responds by pulling it from your uterus and swirling it around the inside of your " + player.vaginaDescript(0) + " to stimulate the walls of your trembling love-canal.  ");
			if (player.breastRows.length > 0) {
			...
			if (player.cumQ() < 25) {
				outputText("You erupt, your " + player.cockDescript(0) + " pulsing and spraying ribbons of cum.  You milk your dick hard, pistoning your hand up and down mercilessly, trying to squeeze out every drop.  You watch each burst splatter into the bee-girl's hair and grin cruelly.  ");
			...
			if (player.cumQ() >= 25 && player.cumQ() < 250) {
				...
				if (!player.isTaur()) outputText("you rise up off her, shivering as her slender and squirming tongue retracts from your " + player.vaginaDescript(0) + ".");
			...
			if (player.cumQ() >= 500) {
				outputText("Your " + player.cockDescript(0) + " erupts like a volcano, shooting a constant thick stream of cum that pulses with each clench of your pelvic muscles.  The pressure only seems to increase as your urethra tries to deal with the huge load your body is producing.  ");
...
//(can replace normal rape victory scenes if corruption>75, and strength>60, and while player has naga tongue, dick, vagina, or d-cup or larger breasts)
		private function beeGirlRapeForTheDistinguishedGentleman():void
		{
			...
			else if (player.hasVagina()) outputText("rub your " + player.vaginaDescript(0));
			else outputText("tweak your " + player.nippleDescript(0) + "s");
		...
		private function beeAlternate():void
		{
			...
			if (rand(2) == 0) {
				...
				outputText("After falling to the ground, the bee-girl looks up in dazed fear as you expose your " + player.multiCockDescriptLight() + ", her eyes watching as you throb with lust at the scared and helpless look on her face.");
				//New PG
				outputText("\n\n");
				...
				outputText(images.showImage("beegirl-win-male"));
				outputText("At first, taking her more than satisfies you, but an idea strikes as you watch her face become distracted with each body-shaking thrusts of your hips.  Grabbing your remaining " + player.cockDescript(y) + " you push it against the already-impaled member, and on the next thrust you drive them both in.");
				...
				if (player.cocks.length >= 3) {
					...
					if (player.cocks.length == 4) outputText("  The " + player.cockDescript(3) + " not inside her shakes wildly as you cum; jets of your semen let loose over her abused frame, coating her in white.");
					if (player.cocks.length > 4) outputText("  The " + player.multiCockDescriptLight() + "s not inside her shake wildly as you cum; jets of your semen let loose over her abused frame, coating her in white.");
				}
				//New PG
				outputText("\n\n");
				...
				if (!player.isTaur()) outputText("stuffing your " + player.multiCockDescriptLight() + " inside your " + player.armorName);
				else outputText("your glorious but spent " + player.multiCockDescriptLight() + " dangling under your belly,");
...
//Naga on Bee Scene
		...
		private function nagaRapesPt2TheExtremeContinuationOfAwesome():void
		{
			...
			if (player.gender == 1 || (player.gender == 3 && rand(3) == 0)) {
				...
				outputText(" aching for release, and you tear off your " + player.armorName + " just in time as ");
				...
				outputText("your " + player.multiCockDescriptLight() + ", slurping soundly and driving you wild; ");
				//[if player has testicles] 
				if (player.balls > 0) outputText("Not while her hands cup your " + player.sackDescript() + " and knead tenderly, infusing you with the craving for release; ");
				else outputText("not ");
				outputText("even when she takes your " + player.cockDescript(0) + " to the hilt, her eyes straining up in their sockets. You watch " + UnitSystem.displayInches(player.cocks[0].cockLength) + " of " + player.cockDescript(0) + " disappear inside her mouth, and the vibrations in her throat tell you that you'd hear her cooing if it wasn't packed tight.\n\n");
				...
				//[if player has testicles]
				if (player.balls > 0) outputText("your " + player.sackDescript() + " clenches as ");
			...
			//[Player is female]
				...
				//[if clit size => 1"]: 
				if (player.getClitLength() > 1) outputText("  Emergent from its sheath, she uses the base of her tongue as a bottom lip and begins to suck you off like a cock, unceasing in her lingual assault on the inside of your canal.");
				outputText("\n\n");
			...
			//[Player is herm]
				...
				outputText("Touching a single index finger to the underside of your captive's chin, you pull with the slightest of forces and watch as she is dutifully drawn to your " + player.multiCockDescriptLight() + ", running her lengthy prehensile tongue up and down ");
				if (player.cockTotal() > 1) outputText("each of your shafts");
				else outputText("your shaft");
				outputText(". As the pulsing movement inside your body draws you slowly closer to orgasm, the bee-bitch uses her other hand to draw your " + player.cockDescript(0) + " down to an angle so it slides neatly into her throat");
				//[if Player cock => 12\"]
...
//Play standard victory text
		...
		{
			clearOutput();
			outputText("You stand over the defeated bee, sizing up your latest catch.  She watches you fearfully as your gaze slides down her prone form, taking in every " + UnitSystem.literalInch() + " of her body.  Your eyes stop over her abdomen as you notice the girl's dripping, barely concealed ovipositor.  A wide");
