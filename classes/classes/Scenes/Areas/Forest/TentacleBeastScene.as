...
public function encounter():void {
	...
	if (player.gender == 0 && rand(3) == 0 && !player.isNaga() && !player.isTaur() && !player.isGoo()) {
		...
		outputText(images.showImage("tentaclebeast-sad-monster"));
	...
	//Combat starter
	outputText(images.showImage("monster-tentaclebeast"));
...
private function futaTentacleBadEnd():void {
	...
	outputText(images.showImage("badend-tentaclebeast"));
...
internal function tentacleLossRape():void {
	...
	if (player.countCocksOfType(CockTypesEnum.HORSE) > 0 && player.cocks[0].cockLength > 15 && player.cocks[0].cockThickness >= 3) 
	{
		...
		if (player.balls > 0) outputText(", and you feel your " + player.ballsDescript() + " rise and drop in unison to the muscular contractions pumping freshly made cum into position for release");
		outputText(".\n\n");
		outputText(images.showImage("tentaclebeast-loss-horse"));
	...
	//Centaur madness!
		...
		if (player.hasCock()) {
			if (player.cockArea(player.smallestCockIndex()) <= 50) {
				outputText("More aphrodisiac-toxin pours into you, causing " + player.sMultiCockDesc() + " to expand.  ");
			...
			// has cock, not engulfable: 
				...
				//[With testicles: 
				if (player.balls > 0) outputText("The sucking reaches all the way to your " + player.ballsDescriptLight() + ", a spectacularly strange sensation that nevertheless feels wonderful.");
				outputText("\n\n");
			}
		}
		outputText(images.showImage("tentaclebeast-loss-taur"));
		// has vagina: 
		if (player.hasVagina()) {
			outputText("A squirming tentacle forces its way inside your " + player.vaginaDescript(0) + ", undulating and squirming as it works its way deeper and deeper.  Your body responds by pumping out more fluid, making the passage of the monstrous thing easier.");
		...
		if (player.ass.analLooseness >= 4) {
			outputText("Your " + player.assholeDescript() + " makes an inviting target for the squirming mass and it's quick to capitalize.  A particularly bulbous appendage slides deep inside, roiling about in a way that not even your well-trained hole has been treated to.");
		...
		if (player.hasCock() && player.cumQ() < 1500 && player.ass.analLooseness < 4) {
			outputText("Just as you think it's over, another tentacle rams into your " + player.assholeDescript() + " and begins roughly massaging your prostate as it swells massively, causing another surge of cum to leave you, and another, and another.");
		...
		if (player.hasVagina()) {
			outputText("Just as you think it's over, a tentacle rams into your " + player.assholeDescript() + " and begins to swell massively, causing another surge of girlcum to leave you, and another, and another.");
			player.buttChange(40,true,true,false);
	...
	{
		player.slimeFeed();
		outputText(images.showImage("tentaclebeast-loss-herm"));
		...
		if (player.cockTotal() > 1) outputText(player.multiCockDescriptLight() + ", now begging for release, and clamps down around your pubic mound, fully encapsulating your members.  ");
...
private function tentacleRapeContinuation():void {
	...
	if (player.gender == 1) {
		...
		outputText(images.showImage("tentaclebeast-loss-female-sec-part"));
		...
		outputText(images.showImage("tentaclebeast-loss-herm-sec-part"));
		if (player.cockTotal() == 1)
		{
...
private function tentacleRapeContinuationForFemales():void {
	clearOutput();
	spriteSelect(SpriteDb.s_tentacleMonster);
	outputText(images.showImage("tentaclebeast-loss-female-third-part"));
