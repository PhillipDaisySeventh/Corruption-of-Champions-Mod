	...
	public function execEncounter():void {
		...
		protected function firstWildHuntChase(waited:Boolean = false):void {
			...
			if (waited == false) outputText("You stumble your way through the woods, but no matter which way you turn, you are greeted by bone-chilling fog.  Soon, canine snarls come from all sides.  You’re surrounded.");
				...
				if (player.cor >= 40) {
					if (player.hasCock() && !player.hasVagina()) outputText("  You can't help but stiffen, yourself, at the sight of their eagerness.");
					else if (player.hasVagina() && !player.hasCock()) outputText("  As the air grows thick with their musk, your pussy grows wet, despite your best efforts.");
					...
					else outputText("  Your asshole twitches in response, aching to be filled.");
				}
			}
			doNext(firstWildHuntChaseII);
		}
		private function firstWildHuntChaseII(waited:Boolean = false):void {
			...
			else outputText("“<i>I’m afraid you’re going to have to learn to run faster if you hope to elude me in the future.  I do hope you won’t bore me.</i>”\n\n");
		...
		protected function repeatWildHuntCaught(pScore:int):void {
			...
			outputText(images.showImage("wildhunt-chase"));
			...
			if (player.bunnyScore() >= 4 || player.kitsuneScore() >= 4 || player.harpyScore() >= 4 || pScore > 100) doNext(repeatWildHuntAWinnerIsYou);
			else {
...
//				addButton(1, "FIGHT!", fightTheHounds); //if I have time
		...
		public function repeatWildHuntGivenToTheHounds():void {
			...
			if (player.hasVagina()) outputText(images.showImage("wildhunt-chased-female"));
			else outputText(images.showImage("wildhunt-chased-male"));
		...
		public function repeatWildHuntGivenToTheHoundsII():void {
			...
			outputText("You groan around the Hound’s dick as you feel a pressure against your [asshole].  The beast squeezes your [ass] cheeks as he shoves his " + UnitSystem.footCompound() + "-long doggie cock into your rear. ");
			...
			player.gems -= gemLoss; outputText("<b>You’ve lost " + gemLoss + " gems.</b>\n\n");
		...
		public function takeCaninePepper():void {
			clearOutput();
			outputText(images.showImage("item-cPepper"));
			inventory.takeItem(consumables.CANINEP, camp.returnToCampUseOneHour);
		}
		public function takeFoxBerry():void {
			clearOutput();
			outputText(images.showImage("item-fBerry"));
			inventory.takeItem(consumables.FOXBERY, camp.returnToCampUseOneHour);
		}
		public function takeNeonPinkEgg():void {
			clearOutput();
			outputText(images.showImage("item-egg-neon"));
		...
		protected function surrenderToTheHounds():void {
			...
			outputText(images.showImage("wildhunt-surrender-I"));
			...
			else if (!player.hasVagina() && !player.hasCock()) outputText("  The smooth curve of your crotch ripples and bulges, and a cherry-red tip pushes out from your fur.  The wind around you picks up, blowing across your new, smooth doggie prick as it pushes out.  The overwhelming sensation has you shuddering, and you tilt your head back and howl.");
		...
		protected function surrenderToTheHoundsIII():void {
			clearOutput();
...
//			doNext(5025); //find out the gameover shits
		...
		protected function predatoryPrey():void {
			...
			if (player.tou < player.str) dynStats("toughness+", 1, "fatigue+", 10, "health+", 100, "lust=", 0); //+10 Fatigue, +1 Toughness, 100 hp healed
			else (dynStats("strength+", 1, "fatigue+", 10, "health+", 100, "lust=", 0)); //+10 Fatigue, +1 Strength, 100 hp healed
			player.orgasm('Generic');
			player.slimeFeed();
			menu();
			doNext(camp.returnToCampUseOneHour);
		...
		protected function howDareYouII():void {
			clearOutput();
			if (player.hasVagina() && !player.hasCock()) {
				...
				outputText(images.showImage("wildhunt-revenge-female"));
				...
				outputText(images.showImage("wildhunt-revenge-male"));
		...
		protected function encounterPrincessGwynn():void {
			...
			if (player.hasVagina()) addButton(2, "Eat Me", gwynnNomsDaCunts);
		...
		protected function gwynnGetsButtfuxed():void {
			...
			outputText("You grin and grip her ass, pushing your slippery cock into her pink bud.  She’s a lot looser than she was before, and you slide easily into her.  She purrs as you sink in, " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + ", your hands gripping her small, plush ass.  It seems like her time spent with the monsters in the woods has stretched her out immensely.  You just hope she won’t be too loose.\n\n");
			...
			dynStats("sen-", 2, "lus=", 0); //sensitivity -2
			player.orgasm('Dick');
			menu();
			doNext(camp.returnToCampUseOneHour);
		}
		...
		protected function gwynnNomsDaCunts():void {
			...
			dynStats("sen-", 2, "lib+", 2, "lus=", 0); //sensitivity -2, libido +2
		...
		protected function gwynnGetsDickmilked():void {
			...
			outputText("Her dick is already rigid, " + UnitSystem.aFoot() + " long and mottled pink and white, like clouds at dawn.  You kneel down next to her and grasp it slowly, your fingers finding natural handholds along her triple prepuce rings.\n\n");
			...
			dynStats("lus+", 20, "lib+", 2); //lust +20, libido +2
			menu();
			doNext(camp.returnToCampUseOneHour);
		}
		...
		protected function gwynnGibsGifts():void {
			...
			else inventory.takeItem(consumables.PRNPKR, takeGoldenRing);
		}
