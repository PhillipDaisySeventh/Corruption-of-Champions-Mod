package classes.Scenes.Areas.HighMountains {
	...
	public class MinotaurMobScene extends BaseContent implements TimeAwareInterface {
		...
		public function MinotaurMobScene() { CoC.timeAwareClassAdd(this); }
		...
		public function timeChange():Boolean {
			...
			if (flags[kFLAGS.MINOTAUR_SONS_GROWUP_COUNTER] > 30) flags[kFLAGS.MINOTAUR_SONS_GROWUP_COUNTER] = 30;
			if (flags[kFLAGS.MINOTAUR_SONS_GROWUP_COUNTER] < 0) flags[kFLAGS.MINOTAUR_SONS_GROWUP_COUNTER] = 0;
			//Countdown for son growing up
			if (flags[kFLAGS.MINOTAUR_SONS_GROWUP_COUNTER] > 0) {
				flags[kFLAGS.MINOTAUR_SONS_GROWUP_COUNTER]--;
				//Hit zero, move kid to grown up pile!
				if (flags[kFLAGS.MINOTAUR_SONS_GROWUP_COUNTER] <= 0 && flags[kFLAGS.MINOTAUR_SONS_PENDING] > 0) {
...
public function meetMinotaurSons():void {
	...
	if (flags[kFLAGS.TIMES_ENCOUNTERED_MINOTAUR_MOB] == 1) { //first meeting
		outputText(images.showImage("monster-minomob"));
		if (!player.minotaurAddicted()) { //Non-Addicted
			...
			outputText("Three minotaurs have appeared from behind the rust-red rocks, arranged in a crude half-circle with you at the center.  Two of them are huge, powerfully-built bulls, stomping their hooved feet and snorting idly as they circle you.  Judging from the similar shapes and sizes of their muzzles and eyes, you'd guess they were brothers.  The last one, however, is everything his brothers aren't.  Where they have huge, hulking muscles, his are soft and barely visible.  He's comparatively tiny, only a little over " + UnitSystem.displayHeight2EstimateTextually(5) + " tall, and everything from his expression to his dainty steps reeks of shyness and femininity.  You find yourself wondering if he's even a real minotaur.\n\n");
		...
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] < 20) {
			if (!player.minotaurAddicted()) { //Non Addicted Repeat
				...
				if (player.minotaurNeed()) {
					...
					doNext(submitToMinotaurMob); //Same as 'submit'
					return;
				}
			...
			if (!player.minotaurAddicted()) { //Repeat Tribe-Size Non-Addict
				...
				if (player.minotaurNeed()) {
					...
					doNext(submitToMinotaurMob); //SUBMIT!
					return;
				}
...
private function negotiate():void {
	clearOutput();
	spriteSelect(SpriteDb.s_minotaurSons);
	outputText(images.showImage("minomob-negotiate"));
...
private function runFromMinotaurs():void {
	...
	outputText(".  ");
	if ((player.canFly() && player.spe > rand(40)) || (!player.canFly() && player.spe > rand(60))) { //ESCAPE!
		outputText("A furry arm nearly catches your " + player.leg() + ", but you slip free and quickly escape your lusty brood.");
		doNext(camp.returnToCampUseOneHour);
	}
	else { //FAIL:
		if (player.gender == 3) outputText(images.showImage("minomob-falling-herm"));
		else if (player.gender == 2) outputText(images.showImage("minomob-falling-female"));
		else outputText(images.showImage("minomob-falling-male"));
...
internal function minotaurDeFeet():void {
	...
	if (player.HP < 1) //HP
		outputText("You've been completely defeated.  Bruised and battered, your overtaxed body is barely able to move, and you slump down on the ground.  The minotaurs stomp closer, smiling in anticipation of what's to come.  Mouth agape, you stare while they discard their tiny, pre-cum soaked loincloths and reveal the massive, distinctive cocks that make their breed so famous.  One of the rags lands squarely on your face, by chance or cruel intent, and you gasp in surprise.  Like a lightning bolt shooting down your spine, arousal slams into you with thunderous force, rocking you back as you shake the sticky cloth from your face.  All thoughts of resistance are completely washed away by the tide of your aching need, and you spread your " + player.legs() + " to make yourself an easier target.   Lent strength by the arousal coursing through your veins, you tear off your " + player.armorName + " and wait anxiously for whatever the beast-men have planned.\n\n");
	else { //Lust
		...
		if (player.hasCock()) outputText("  " + player.SMultiCockDesc() + " hardens and dangles above the ground, leaking in sympathy with your hungry quim.  Of course it will likely be ignored - rutting minotaurs have little use for someone else's dick.");
...
private function nonAddictMinotaurGangBang():void {
	spriteSelect(SpriteDb.s_minotaurSons);
	outputText("The minotaurs step closer, their hooves kicking up small clouds of dust as they approach your prone body.  You lift your head, nose twitching, and breathe their scent deeply while casting a coy look at the closest of the mob.  He smiles and squeezes his fingers around your jaw, pulling your mouth into a pouty 'o'.  His other hand holds the heavy mass of his stiff maleness - a turgid, flared shaft over " + UnitSystem.displayFeetEstimateTextually(2) + " long with three prominent ridges along its length.  You shiver and lick your lips unconsciously, tasting the sweet smell of his pre in the air as it inches closer.\n\n");
	if (player.isTaur()) {
	...
	outputText(capitalizeFirstLetter(UnitSystem.literalInch()) + " by " + UnitSystem.literalInch() + ", the double dicks penetrate further, bulging your neck and gut around the growing flares.  You can actually feel their leaky, drug-like pre-cum burbling out to fill your belly and slip into your intestines.  It gives you a nice, pain-numbing high that makes it easy to handle the plus-sized members currently lifting you from the ground. Their hands move to grab at your midriff, helping to steady your aerial form between them.  As you're leveled at waist height, the last few " + UnitSystem.literalInches() + " of the throbbing, drug-leaking cocks push inside your holes.  Their two sweaty ballsacks clap against your " + player.buttDescript() + " and chin simultaneously.\n\n");
	...
	flags[kFLAGS.BONUS_ITEM_AFTER_COMBAT_ID] = consumables.MINOCUM.id; //force cum bottle loot!
	player.knockUp(PregnancyStore.PREGNANCY_MINOTAUR, PregnancyStore.INCUBATION_MINOTAUR, 75); //preggers chance!
...
private function analSpearSemiPregMinotaurGangbang():void { //SEMI PREG [> 120 && <= 216]
	spriteSelect(SpriteDb.s_minotaurSons);
	...
	if (player.hasFuckableNipples()) { //nipcunts
	...
	else if (player.biggestLactation() >= 1) { //lactation, no nipcunts
...
internal function victoryMinotaurGang():void {
	...
	if (flags[kFLAGS.SFW_MODE] > 0) {
		clearOutput();
		outputText("You smile in satisfaction as the last minotaur collapses, unable to continue fighting.");
		combat.cleanupAfterCombat();
		return;
	}
	if (monster.HP < 1) { //HP
		outputText(images.showImage("minomob-fallen-hp"));
		outputText("The last minotaur sinks to his knees, wobbling back and forth while fighting to stay upright. He gives up and slumps down onto his back, mooing in pain.  You exult in your victory as you look across the defeated beast-men.  Though they're strong and aided by potent natural aphrodisiacs, they were no match for you.  Amusingly enough, they're still hard, even in their semi-conscious state. The horse-like mino-cocks lift their loincloths to stab at the sky, dripping moisture.\n\n");
	}
	else { //Lust
		outputText(images.showImage("minomob-fallen-lust"));
		outputText("The last minotaur sinks to his knees, pulling aside his loincloth with thread-rending strength to expose the pulsating, needy shaft to the air.  He begins to masturbate himself just like his brother, spilling his pre-seed over the ground into the messy, growing puddle.  The smell hangs thick in the air, but you've won and kept enough of your wits about you to walk away if you want.\n\n");
	}
	if (player.lust < 33) dynStats("lus=", 33, "scale", false); //+lust regardless
	else dynStats("lus", 20);
	outputText("Your body is burning up, buzzing with growing lust from the obscenity going on " + UnitSystem.aFewFeet() + " away from you.  What do you do?");
...
private function victoryMinotaurGangTitFuck():void {
	...
	//FORK: Small or Large
	if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] >= 10) { //LARGE
		...
		outputText("You awaken in camp, still a little woozy and naked as the day you were born.  Your " + player.armorName + " lies " + UnitSystem.aFewFeet() + " away.  Judging by how dizzy you are, it hasn't been long since your mountain tryst.  You wash the dried cum from your " + player.skinFurScales() + " and gear before you get dressed.  Wow... that was nice.");
	}
	else { //SMALL
...
private function victoryAllThePenetrationsMinotaurGangBang():void {
	...
	if (player.vaginalCapacity() >= 60) outputText("It slips into your " + player.vaginaDescript(0) + " with ease, nestling into your accommodating opening as if it were a second home.");
	else if (player.vaginalCapacity() >= 40) outputText("It slips into your " + player.vaginaDescript(0) + " ever so slowly, gradually widening your quim until it's able to handle its size.");
	else outputText("It stretches your " + player.vaginaDescript(0) + " with painful force as it penetrates you, forcibly widening your quim until it's able to nestle its sheath against your vulva.");
	outputText("  It pulsates wetly inside you with each beat of the minotaur's hammering heart.");
	player.cuntChange(60,true,true,false); //CUNT CHECK, REGISTER 4! And hymens are out of stock again!
	...
	if (player.analCapacity() < 40) outputText("Your " + player.assholeDescript() + " is so tight that he's forced to push and release over and over.  The painful penetration drags on and on as he widens your hole to accept his monstrous cock.");
	else if (player.analCapacity() < 60) outputText("Your " + player.assholeDescript() + " slowly widens to accept the cock that's being deposited into its depths.  You find yourself somewhat grateful that you've been stretched enough to prevent this from being painful.");
	else outputText("Your " + player.assholeDescript() + " devours the cock easily.  The only thing even close to a tight fit is the feeling of your pucker being stretched at his very base.");
	...
	outputText("\n\n");
	if (player.hasFuckableNipples() && flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) { //nipplefuck PG:
	...
	if (player.hasCock()) //dicks
		outputText(player.SMultiCockDesc() + " hangs limply on one of your boys' bellies, dribbling weakly at your need, but there's not any way for it to get involved in the crowd of orgiastic bodies.\n\n");
	...
	outputText("and your ass squeezes down on the minotaur penis inside it all at once.  Overwhelmed by their slutty mother's thrashing form and constricting holes, your sons grab you and take control, fucking you wildly.  ");
	outputText("You bounce around like a child's toy, utterly at the mercy of your muscular offspring.\n\n");
	outputText("The one in your ass gets off first, basting your colon in his juices.  Narcotic spooge slops from your " + player.assholeDescript() + " as he overfills the poor rectal opening, but you get more than enough inside you to make you woozy and light-headed - as if your orgasm hadn't already.  Spunk erupts in your " + player.vaginaDescript() + " with equal fervor, packing itself into your womb with incredible force.  ");
	outputText("You can feel the four balls behind you shrinking as their cargo empties into your pussy and ass, stuffing your hind-end.\n\n");
	outputText("Flaring wide inside your throat, the minotaur cock in your mouth begins to cum as well, egged on by your shaking, orgasmic motions and the sight of your " + player.buttDescript() + " being so thoroughly stuffed.  Heavy globs of cum blast directly into your stomach, one after another, until you're completely packed. ");
	if (player.hasFuckableNipples() && flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) {
		if (flags[kFLAGS.ADULT_MINOTAUR_OFFSPRINGS] > 4) outputText("Meanwhile, your " + player.nippleDescript(0) + "s get a similar feeling from the studs arranged around them");
		else outputText("Meanwhile, your " + player.nippleDescript(0) + " gets a similar feeling from the stud pumping away at it");
...
private function forceMinitaurToGiveOral(choice:Number = 0):void {
	...
	if (choice == 0) { //CUNTPUSSIEFUCKER 
		...
		if (player.isTaur()) {
			...
			outputText("spread your " + player.vaginaDescript() + " wide with your fingers, exposing the glistening pinkness of your womanhood.");
		}
		outputText("  Your " + player.clitDescript() + " slowly peeks out from its hood");
		if (player.getClitLength() >= 8) outputText(", until the huge size of it is revealed");
		else if (player.getClitLength() >= 3) outputText(", until the cock-like length of your feminine organ is revealed");
		...
		outputText(".\n\nThe girly 'monster' crawls forward, ");
		...
		outputText(images.showImage("minomob-won-pussysucker"));
		...
		outputText("\n\nYou begin to undulate your " + player.hipDescript() + " into the minitaur's bestial muzzle, grinding and groaning in joy at the forced cunnilingus.  ");
		if (player.getClitLength() >= 6) outputText("He jacks your huge clit like a cock, and the incredible stimulation is too much for you to take.");
	...
	else { //PENISCOCKSUCKERFUCK
		outputText("start sucking Mommy's hard cock.</i>\"  To emphasize your point, you ");
		if (player.isTaur()) outputText("flex your " + player.multiCockDescriptLight() + " to slap against your belly.");
		else outputText("stroke your " + player.multiCockDescriptLight() + " with one hand.");
		outputText("  " + player.SMultiCockDesc() + " ");
		if (player.lust100 < 50) outputText("grows hard");
		...
		if (player.isTaur()) outputText("disappearing under your hind-legs as he attends to his newly commanded duty.");
		else outputText("looking up at you longingly as he leans in towards " + player.oMultiCockDesc() + ".");
		outputText("  His big brown eyes meet yours as he opens his muzzle and lets the full length of his tongue flop free.  It's roughly " + UnitSystem.aFootAndAHalf() + " long fully extended, and " + player.sMultiCockDesc() + " twitches as you imagine what it must feel like.  Thankfully, he does not keep you waiting long.\n");
		outputText(images.showImage("minomob-won-cocksucker"));
		outputText("The serpent-like tongue snakes ");
		if (player.balls > 0) outputText("around your " + player.sackDescript() + " before curling ");
		outputText("around the base of your " + player.multiCockDescriptLight() + ".  It circles up ");
		...
		if (player.hasVagina()) outputText("Humming happily, the minitaur slips a digit inside your " + player.vaginaDescript() + ", and while you didn't ask for it, the added stimulation only heightens your need.  ");
...
private function victoryBJNippleFuckMinotaurGang():void {
	...
	outputText(images.showImage("minomob-nipplefuck"));
	...
	if (player.biggestTitSize() <= 2) outputText("a few " + UnitSystem.literalInches());
	else if (player.biggestTitSize() <= 4) outputText(UnitSystem.literalInches());
	else if (player.biggestTitSize() <= 8) outputText("most of " + UnitSystem.aFoot());
	else if (player.biggestTitSize() <= 14) outputText("over " + UnitSystem.aFoot());
	else if (player.biggestTitSize() <= 30) outputText("nearly " + UnitSystem.displayFeetEstimateTextually(2));
	...
	outputText(".  He shyly scratches at his head as he daintily lifts his loincloth, stiffing up until his erection is bobbing before your " + player.faceDescript() + ".  \"<i>Mom, could you... lick it please?  It hurts.</i>\"  You smirk and open wide, giving him a perfect view of your slowly-undulating tongue and glistening lips.\n\n");
...
private function minotaurGangBadEnd():void { //Bad End Scene:
	clearOutput();
	spriteSelect(SpriteDb.s_minotaurSons);
	...
	outputText(images.showImage("minotaur-gangbadend"));
	...
	outputText("You give the tiny beast-man's heavy balls a loving squeeze and jerk back, letting the swollen dick pop out of your oral prison and hang there, wet with saliva.");
	dynStats("lus", 25, "lib", 5, "sen", 15);
	doNext(minotaurGangBadEndPart2A);
}
private function minotaurGangBadEndPart2A():void { //part2-1
	clearOutput();
	outputText(images.showImage("minotaur-gangbadend-part2a"));
	outputText("Shaking with a need of your own, you rise and bend ");
...
private function minotaurGangBadEndPart2B():void { //part2-2
	clearOutput();
	outputText(images.showImage("minotaur-gangbadend-part2b"));
...
private function minotaurGangBadEndPart3A():void { //part3-1
	clearOutput();
	spriteSelect(SpriteDb.s_minotaurSons);
	outputText(images.showImage("badend-minomob-I"));
	...
	dynStats("lus", 35, "sen", 15);
	doNext(minotaurGangBadEndPart3B);
}
private function minotaurGangBadEndPart3B():void { //part3-2
	clearOutput();
	...
	outputText(images.showImage("badend-minomob-II"));
...
private function disciplineEldestMinotaurSon():void {
	...
	outputText(images.showImage("minomob-discipline-eldest"));
