package classes.Scenes.Areas.HighMountains
{
	...
	public class Minerva extends Monster
	{
		...
		private function minervaBite():void
		{
			...
			var damage:int = int(str + game.highMountains.minervaScene.minervaSparIntensity() * 1.85 + 85);
...
//Flying kick:
		private function minervaKnowsKungfu():void
		{
			...
			var damage:int = int(str + weaponAttack + (game.highMountains.minervaScene.minervaSparIntensity() * 1.5) + 100);
			damage = player.reduceDamage(damage);
			spe -= 70;
			if (player.getEvasionRoll()) {
...
//Halberd stab:
		private function minervaUsesHalberdStab():void
		{
			...
			var damage:int = int(str + weaponAttack);
			damage = player.reduceDamage(damage);
...
//Halberd CHOP:
		private function minervaUsesHalberdCHOP():void
		{
			...
			var damage:int = int(str + 40 + (weaponAttack * 2));
			damage = player.reduceDamage(damage);
...
//White Fire
		private function kiteFire():void
		{
			...
			var damage:int = int(10 + (inte / 3 + rand(inte / 2)) * ((game.highMountains.minervaScene.minervaSparIntensity() / 30) + 1) * 1.5);
			player.takeDamage(damage, true);
			combatRoundOver();
		}
