...
//*If genderless, rape options are Finger Her*
		public function harpyVictoryuuuuu():void
		{
			...
			if (player.lust >= 33) {
				...
				addButton(2, "Oral", WinOnHarpyAndOralRape).hint("Those lips are inviting, but her drugged lipstick can be dangerous.");
				if (player.hasVagina()) {
					if (!player.isNaga()) addButton(3, "Scissor", harpyScissorSurprise).hint("Get into some girl-on-girl activity with the harpy.");
					if (player.getClitLength() >= 3.5) addButton(4, "Clit Fuck", clitFuckAHarpy).hint("Fuck the harpy with your big clit.");
				}
			}
...
//Requires pussy or cock small enough for harpy!
		private function harpyLossLust():void
		{
			...
			if (player.hasVagina() && x < 0) {
				...
				if (player.hasPlainSkin()) outputText(" and quickly alleviating the pain by brushing their wings across your hide");
				outputText(".  You instantly stop struggling, no matter how insane the sensation is. One wrong move and you'd lance yourself on their talons even if the harpies did not intend it. It's hard to lie still however. Every " + UnitSystem.literalInch() + " of your body is beset by the hair-raising scratch of their razor sharp nails");
				if (player.hasPlainSkin()) outputText(", always followed by a soothing brush of their feathers");
				outputText(". They line the curve of your " + player.breastDescript(0) + ", brush around your aurela, and harden your " + player.nippleDescript(0) + "s. They trace your inner thigh, and not even your " + player.feet() + " are safe. It does not tickle, but your nerves are lit up like a Christmas tree and a tear wells up in your eyes as you're forced to lie still in the face of such a soft, yet overpowering sensation.\n\n");
					outputText(images.showImage("harpy-loss-female"));
				...
				if (player.hasPlainSkin()) outputText(" across your reddening skin");
			...
			//(Reminds me of what my own Succubus does in my novel)
				...
				outputText("They drop you on purpose, only for you to be caught again mid-air by one of the harpies. Using the momentum, your sadistic savior then throws you around like a sling for others to catch, which they do crying with joy, before passing you along again with dizzying speeds and heights. The ground below rushes towards and away from you in rapid succession. After five times, they squabble and two of them nearly pull you apart between them. It is then however, that a third gets an idea. Using the fact that your body is suspended between two others pretty stably, she flies into you and latches on by crossing her legs and arms around your hips and waist. There, high up in the sky and in intimate embrace, the other two harpies still quarreling, she smiles at you while straddling you. In an unknown language of whistles and chirps, she assures you it's going to be all right and reaches underneath her to fondle your cock. With the warmth of her body so near to yours and now armed with wings of your own in the form of the harpy on top of you, it works. Held between three digits, your " + player.cockDescript(x) + " assumes its erect shape. It makes the latched harpy give a pleased warble, and with a quick slip, she pushes her womanhood against it until you sink into her.\n\n");
					outputText(images.showImage("harpy-loss-male"));
				...
				outputText("The ground rapidly approaches, but the harpy fucking you doesn't even seem to notice. She simply continues to dry hump, a stupored stare of sex in her eyes. You try to alert her, but as she starts squeezing your cock within the hawk-like confines of her birdhole, she doesn't even attempt to flap her wings. With just " + UnitSystem.aFewFeet() + " remaining, you finally orgasm and release your load into her eager cunt, yelling and flailing your arms as textured details of the mountain's granite ground rushes towards you. As you fill her up with your last desperate throes, it seems to do it for the bird-lady. With the last " + UnitSystem.displayFeetEstimateTextually(10) + " remaining, she disconnects, spreads her wings and uses the velocity to tumble forcefully and hurl you away horizontally. With a faint arc, you skip across the floor for over " + UnitSystem.aDozenFeet() + ".\n\n");
...
//No genderless folks.
		private function harpyDamageLoss():void
		{
			...
			if (player.hasCock() && x >= 0) {
				...
				//[(Male/shemale)
				if (!player.hasVagina()) {
					outputText("The hierarchy is eventually settled, it seems, because two of those heavy-hipped bird babes wrap their lips around your " + player.cockDescript(x) + ", sucking and pecking at the hot flesh");
					if (player.balls > 0) outputText(", craning their necks to slurp and massage your " + player.ballsDescriptLight() + " eagerly, trying to coax up your cum production as much as they can");
					...
					//(If breasts)
					if (player.biggestTitSize() >= 1) outputText("her petite bosom grinding across your " + player.allBreastsDescript() + ", her nipples tickling at your " + player.nippleDescript(0) + ", ");
					outputText("her golden lips glinting in the light as they make for your own.\n\n");
				}
				//[(Herm)
				else {
					outputText("The hierarchy is settled between the warring bird-girls, and two of them grasp onto your thighs with their clawed hands, those gold-lipped mouths starting to hungrily suckle and taste every " + UnitSystem.literalInch() + " of your " + player.cockDescript(x) + ", their squawking, feathered heads occasionally diving down to probe their short, stubby tongues into your " + player.vaginaDescript(0) + ", sending sharp spikes of pleasure rippling through your body, ");
					//(If balls)
					if (player.balls > 0) outputText("their hands moving up to grope and fondle your " + player.ballsDescriptLight() + ", massaging them to boost your cum production as much as they can, ");
					...
					//(if breasts)
					if (player.biggestTitSize() >= 2) outputText(" her toned chest and pert breasts squishing your " + player.breastDescript(0) + " beneath them");
					outputText(".\n\n");
				}
				//Apply harpy status.
			...
			//[(Female) 
				...
				//{(If breasts)
				if (player.biggestTitSize() >= 2) outputText(", her petite chest squashing your " + player.breastDescript(0) + " against hers");
				outputText(".\n\n");
			...
			if (x >= 0 && player.cockTotal() == 1) {
				...
				if (player.balls > 0) outputText(", her huge feathery bottom coming to rest on your " + player.ballsDescriptLight());
				outputText(". Even as you come to bear this tremendous pleasure, you feel your hands wrenched carelessly from the big butt that's grinding against your face, and a small feathery pair of hands guides them to their goal.  Your fingers slip into the hot, clutching tightness of two harpy holes, the sensations made even more vivid by the blinding faceful of ass.  A sharp-clawed foot steps dominantly on your navel, digging the claws into your " + player.skin.desc + ". The two harpies whose asses are pinning you to the ground start to bounce their full weight brutally on your already broken body in unison.  The sheer, vast weight of their hips batters every " + UnitSystem.literalInch() + " of you, twin feathery punchbags slamming into your cheeks and onto your crotch. Were it not for the hot, moist hole that's coating your mouth, you would be screaming in pain");
				if (player.balls >= 0) {
					outputText(".  Your " + player.ballsDescriptLight() + " bounce with such force as to fetch the cock-speared harpy a stinging slap on the rear with every brutal, mechanical fuck");
					//{(If herm)
					if (player.gender == 3) outputText(" before swinging back to spanking your " + player.vaginaDescript(0) + " wetly.");
					else outputText(".");
				}
			...
			//[(Female)
				...
				outputText("Their hands find their way to your " + player.vaginaDescript(0) + ", and two sets of fingers start to explore your lush, velvety depths.  They seem intent on making their captive quiver and scream in ecstasy beneath the stifling birdy bottom. The riding harpy leans down, ");
				//{(If breasts)
			...
			if (x >= 0 && player.hasCock()) {
				...
				//{(If herm)
				if (player.gender == 3) outputText(", and the number of times your " + player.vaginaDescript(0) + " has dripped your sweet feminine cum over your thighs");
			...
			//[(Female)
				...
				//{(If lactating)
				if (player.biggestLactation() >= 1) outputText("  Their bellies seem a little pudgier than when they'd started, a result of the endless drinking from your " + player.nippleDescript(0) + "s.  ");
		...
		private function victoryHarpyGetsHerPussyRaped():void
		{
			...
			outputText("Wrapping your arms around those immense, motherly hips, you drag her rump up so that it sways and jiggles in the air, forcing the big-butted bird-girl into the doggy position. Her cheeks burn red with embarrassment, but the way she is jiggling and shaking her blubbery bottom at you tells you she wants it just as much as you do. Finally getting a really good view of it, your eyes widen as you run your hands over every " + UnitSystem.literalInch() + " of her bouncy rear - it's more like standing behind a horse than a harpy! An ass this fine isn't to be wasted, and so you lean down and bury your " + player.faceDescript() + " between those luxurious orbs, planting your mouth firmly over her drooling pussy and tasting her sweet juices, suckling and kissing around her beautifully-formed clit and breathing her intoxicating feminine scent in with enjoyment, letting it fill your lungs as your expert ministrations coax a plethora of moans and subtle squawks from your partner.\n\n");
				outputText(images.showImage("harpy-win-male"));
			...
			outputText("You slowly slip your " + player.multiCockDescriptLight() + " into her hot, clutching depths, " + UnitSystem.literalInch() + " by " + UnitSystem.literalInch() + ", seeming to take an eternity as all the sensations of her rough, ribbed pussy ");
			...
			if (player.gender == 1) {
				...
				if (player.biggestTitSize() < 1) {
					outputText("Taking care not to waste your opportunity with an immense butt by cumming too soon, you slowly start to push and pull her fat, quivering cheeks up and down your " + player.cockDescript(x) + ", feeling her rough walls and clutching, gasping convulsions massage your manhood");
				...
				//[(Male w/ breasts)
					...
					outputText(" your " + player.cockDescript(x));
					if (player.totalCocks() > 1) outputText(" and " + player.cockDescript(y));
			...
			outputText("You decide to give her what she so clearly wants and plant both your hands onto her massive bottom, squashing and groping it around your impaling " + player.multiCockDescriptLight() + " as you speed up the pace of your thrusts, driving the full length of your " + player.cockDescript(x));
			if (player.totalCocks() > 1) outputText(" and " + player.cockDescript(y));
			outputText(" into her grinding, overwhelming hole");
			if (player.totalCocks() > 1) outputText("s");
			outputText(".  ");
			//{(If balls)
			if (player.balls > 0) outputText("Your " + player.sackDescript() + " slaps wetly into her immense, wobbling thighs.  ");
		...
		private function winAndRapeHarpyAnally():void
		{
			...
			outputText("Not wishing to keep her fantastic butt waiting, you grasp onto her shoulders and pull her back with you, laying back against the hard ground with the squawking avian slut grinding and bouncing her bean-bag ass against your " + player.multiCockDescriptLight() + ".  Blood flows through your " + player.cockDescript(x) + " as it is sandwiched between her hot, caressing, luxurious ass cheeks, and you grind the head brutally against her impossibly tight pucker, which seems to make her panic a bit. She starts squawking and flapping her arm-wings in a vain attempt to escape your grasp, and the imminent butt-fucking she is about to receive! You smirk to yourself as the fluids from her womanhood drool and spill over your entrapped " + player.cockDescript(x) + ", your slowly-pumping " + player.hipDescript() + " smearing her own sticky sexual juices over those downy butt cheeks.\n\n");
				outputText(images.showImage("harpy-win-male"));
			outputText("The bird-girl's attempts to escape only turn you on more, and you grab HARD onto those gelatinous orbs, guiding your " + player.cockDescript(x) + " into her gloriously tight and hot rear entrance; it feels harsh and rough, practically scratching every " + UnitSystem.literalInch() + " of your member as it's driven hard against her resisting inner walls.  She's far tighter and hotter than any human girl could possibly be! Your " + player.cockDescript(x) + " pushes so viciously and painfully into her clenching tight depths that the harpy squeals and cries out, trying to fly away from you again.  However, she only succeeds in clenching and waving her enormous ass, sending viciously sharp sensations through your blood-engorged dick. " + capitalizeFirstLetter(UnitSystem.literalInch()) + " by " + UnitSystem.literalInch() + " you force your " + player.cockDescript(x) + " into her, her movements getting more frantic as you plumb her most lustful and forbidden hole.  Finally, you bottom out and stop for breath, her powerful, jiggling thighs pressing against yours.");
			//(if balls)
			if (player.balls > 0) outputText("  Your " + player.ballsDescriptLight() + " come to rest against her thick, wiggly rear.");
			outputText("\n\n");
			...
			//[(Herms/Shemales) 
				...
				if (player.gender == 3) {
					...
					//{(If balls)
					if (player.balls > 0) outputText("  She even makes your " + player.ballsDescript() + " slap against your " + player.vaginaDescript(0) + ", a hot, moist SPLAT accompanying every tiny motion the harpy makes.");
				}
				outputText("\n\n");
			}
			outputText("With an ear-splitting squawk, the impaled harpy's drooling pussy absolutely explodes.  It fires huge streams of her sticky feminine cum onto your ever-thrusting " + player.hipDescript() + ", her muscles locking in ecstasy, before the exhausted bird-girl collapses over you, ");
			//(if breasts)
		...
		private function WinOnHarpyAndOralRape():void
		{
			...
			outputText("You stand threateningly over the helpless harpy, stroking yourself through your " + player.armorName + " before grasping onto her chin, turning it this way and that, inspecting your prize thoroughly. Those golden lips glisten in the light as she trills feebly, her eyes looking soulfully up at you.\n\n");
				if (player.hasCock()) outputText(images.showImage("harpy-win-male-oral"));
			...
			//(if pussy)
			if (player.hasVagina()) {
				outputText(", fluids seeping from your plush " + player.vaginaDescript(0) + " all the while");
			}
			//(if cock)
			if (player.hasCock()) {
				outputText(", ");
				...
				if (player.totalCocks() > 1) outputText("each of ");
				outputText("your " + player.multiCockDescriptLight() + " jumps to attention, throbbing and begging to be touched");
			}
			...
			if (player.cockTotal() == 1) {
				...
				//(If balls) 
				if (player.balls > 0) outputText("  Her hands creep down to softly squeeze and massage your " + player.ballsDescriptLight() + ", the harpy's eager head bobbing back and forth along the sweet tasting shaft of your " + player.cockDescript(0) + ", sending waves of powerful, overwhelming pleasure resonating through your body.");
				outputText("\n\n");
			}
			//[(Male, Shemale, Herm, multicock)
				...
				if (player.biggestTitSize() >= 2) {
					outputText(", and your " + player.allBreastsDescript() + " jiggle slightly from the sheer intensity of the pins-and-needles feeling coating your body.  Your " + player.nippleDescript(0) + "s perk up and harden.");
				...
				outputText("  Surging down your body, an alien sensation focuses on your crotch, the pinpricks conglomerating on your " + player.multiCockDescriptLight() + ", and you fall to your knees, hunched over and quivering from the powerful orgasmic sensations. The big-bottomed harpy creeps forward on her knees; her eyes gaze expressively at your face. One arm reaches out to wrap her sweet hand around your " + player.cockDescript(0) + ". As she pumps it gently, the pleasure is magnified tenfold by the lingering effects of those treacherous golden lips.");
				//(If balls)
				if (player.balls > 0) outputText("  She moves that hand to softly squeeze and pet at your " + player.ballsDescriptLight() + ", massaging those bloated cum-factories with expertise. You can feel a distinct rumble as your " + player.sackDescript() + " really ups its production, almost feeling heavier.");
				//(If herm)
			...
			if (player.gender == 2) {
				...
				//(If breasts)
				if (player.biggestTitSize() >= 2) outputText("You grasp onto your " + player.allBreastsDescript() + " violently, kneading and tweaking your " + player.nippleDescript(0) + " in a horny haze as the harpy's fingers tease around your pussy.  ");
			...
			if (player.cockTotal() == 1) {
				...
				//(if herm)
				if (player.gender == 3) outputText(", her other hand doing the exact same to your own " + player.vaginaDescript(0) + ", digging those perfect fingers deep into the wet, hyper-sensitive flesh, spreading those lips wide");
				outputText(".  Her mouth greedily gobbles up " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of your " + player.cockDescript(0) + ", until at last she bottoms out, pressing her nose against your groin, those tainted lips tingling all the way down, increasing the intensity of the pleasure by the second. It takes a surprisingly short time of the harpy bobbing her head, that stubby tongue wetly caressing the sweet shaft of your " + player.cockDescript(0) + ", before you let out a vast, quaking moan, blasting a much larger load than normal into her gentle throat. She doesn't stop though, quite the opposite in fact, as her attentions to your dick became even more aggressive. Her submissive side all but vanishes as her golden lips brush up and down the hot shaft of that " + player.cockDescript(0) + ".  ");
				//(If balls) 
				if (player.balls > 0 && player.cocks[0].cockLength <= 16) outputText("She even deep throats you, taking your length into her expertly-controlled mouth, that tongue creeping out to tickle at your " + player.ballsDescriptLight() + ".  ");
			...
			if (player.cockTotal() > 1) {
				...
				//(if balls)
				if (player.balls > 0) outputText(", dribbling all over your " + player.sackDescript());
				...
				//(If herm)
				if (player.gender == 3) outputText("  Her fingers never stop their sweet teasing of your " + player.vaginaDescript(0) + ", using all four fingers on her hand to plumb your velvet depths.");
...
//{New page}
		private function harpyScissorSurprisePtII():void
		{
			...
			outputText("\n\nGrinning at the girl, you shift so that your legs intertwine, steadying yourself as your crotch hovers above her own.  Your firm grip on her leg allows you to spread her thighs even further, your knee blocking her from closing her legs.  She's writhing madly by now, trying to break your grip and do something to quench the need between her legs, most of the feathers on her thighs having become a sodden mess.  Gently thrusting your hips, your slick lips brush tantalisingly across the harpy's own, drawing out a soft sigh from your own mouth and a gasp of pleasure from the other party.  Releasing her leg, you reposition it so that it is now trapped by your own. You move a hand to her breasts and start pinching and flicking the hard nubs of her nipples, teasing her with the lack of attention to her pussy.  She moans and quivers, clenching her teeth as she tries desperately not to cry out.  You move your hips again, putting a little more force behind it this time and fully grinding your crotches together.  The feeling of her wet lips slipping across your own, your stiff button bumping into hers, is overpowering.  You moan loudly, but it is completely muffled as your partner arches her back and cries out like the little slut she is, her body spasming wildly.  Was that all it took to push her over the edge?  Unluckily for her, you're not even close to finishing yet.");
				outputText(images.showImage("harpy-win-female"));
...
//Fuck a harpy with the players throbbing clit.
		...
		{
			clearOutput();
			outputText("Eyeing the hapless bird-woman up and down, you feel your lusts kindling.  Moisture seeps from your " + player.vaginaDescript(0) + " as you watch the harpy ");
			...
			outputText("\n\nYou drop her taloned toes to the side and slowly remove your " + player.armorName + ", forcing the harpy to watch and wait.  You expose your [chest] and puffy nipples first, shimmying from side to side as you wiggle out of your bottoms, pivoting to shake your [butt] while your rigid clit is freed from confinement.  When you turn back around and nearly bop her on the nose with your " + player.clitDescript() + ", the bird-girl's surprise is complete.  To your amusement, she spends a full ten seconds just staring at it, half in surprise and half in disbelief. You break her reverie by smearing it across her lips, gathering some of the trademark harpy lip-gloss upon your faux girlcock.");
			...
			outputText("\n\nSinking your fingers into her fleshy backside- that posterior promised land- you pull her closer, sliding your slick girl-buzzer through the enveloping buttcheeks for a quick grind.  Her soft cheeks squish from a fleshy crevasse to a swaddling sheath, encircling your girth. You ride the harpy's delightfully wobbling ass, squeezing the rippling curves into a cunt-tight embrace with your hands while you gleefully hump her heart-shaped bottom like you were breaking in a fresh sex-toy.  Delicate feminine fingers with well-manicured nails thumb through your " + player.vaginaDescript(0) + " shortly after, lubricated with another woman's juices.  The harpy's hand slides back and forth through the twin twats, knuckles tickling over her clit and your base, fingernails teasing the flowering folds of your labias, digits probing the tightening wetness of both passages.  She's stopped fighting against your lascivious attentions, at last.  She seems to have given in completely: her every effort seems dedicated to getting you both off.");
			outputText("\n\nYou pull out and release the gelatinous backside, tiring of the foreplay.  It wobbles dangerously, the crack and cheeks gilded with leftover gold that rubbed off your " + player.clitDescript() + ".  The feathery rump waves back and forth as the harpy jills herself off, oblivious to the respite. As you watch that hypnotizing ass, you know you're going to have to fuck this girl until she can't walk.");
				outputText(images.showImage("harpy-win-female"));
			outputText("\n\nLunging forward, you grab hold of the thick-hipped harlot and aim your " + player.clitDescript() + " for her velvet-rimmed muff.  Your aim is true, and you slide right in, displacing probing fingers as you go.  It feels so good that your " + player.legs() + " do give out this time, but you merely fall forward, simply flopping against your victim's prodigious backside.  Your " + player.nippleDescript(0) + " dig into her wings and the feathers tickle your face, but she proves adequate leverage for your pleasure-blasted body to continue its assault.  Once you adjust to the tingling bliss radiating into your " + player.vaginaDescript(0) + ", you grab hold of your ruffled avian friend by her tits, shamelessly groping the palm-sized orbs as your [hips] begin to thrusting in shallow, gradual motions, sinking deeper and deeper by the moment.");
			outputText("\n\nNow [clit]-deep in bird-pussy, you let your body fuck in autopilot, a bestial urgency giving ferocity to your motions.  Barely conscious of your movements, you slam your " + player.hipDescript() + " against that glorious butt again and again.  Ripples of kinetic energy bounce through each cheek with every impact, short gasps of excitement soon slipping from both of you, high pitched and needy.  You bite your lip to keep from screaming, focusing yourself on the pulsing heat and nether-drenching moisture of your feathered clit-sock.");
			if (player.wetness() < 3) outputText("  Runnels of your lubricant are dripping down your " + player.legs() + " in streams,");
			else if (player.wetness() < 4) outputText("  Streams of lubricant are soaking down your " + player.legs() + " in waves,");
			else outputText("  Squirts of lubricant are gushing down your " + player.legs() + " to puddle on the floor,");
			outputText(" evidence of just how hungry your clenching, empty hole is.");
			outputText("\n\nThe harpy is primarily focused on fondling her own lil' button, but she strokes the base of your girlish dipstick from time to time in her tight, rapid vibrations.  She squawks again, a high pitched trill that rings in your ears, and her pussy clenches vice-tight around your " + player.clitDescript() + ".  The harpy is cumming; the tight, squeezing motions of her glittering, gold-painted cunt push you past your limits.  Your tongue lolls out while your hips twitch atop the fat-bottomed slut, pumping your over-sensitive rod in and out of the clutching channel with wild abandon.");
