...
package classes.Scenes.Monsters
{
	...
	public class ImpScene extends BaseContent
	{
		...
		public function impVictory():void {
			...
			addDisabledButton(4, "Use Condom", "This scene requires you to have fitting cock and sufficient arousal. It can't accommodate taurs.", "Use Condom");
			...
			if (pc.lust >= 33) {
				...
				if (pc.hasVagina()) {
					if (pc.isTaur())
						addButton(1, "Group Vaginal", centaurGirlOnImps).hint("Being a taur, you look like you could do with multiple imps to get you off. Call the imps.");
					else 
			...
			if (pc.lactationQ() >= 500 || player.hasStatusEffect(StatusEffects.Feeder)) addButton(3, "Breastfeed", areImpsLactoseIntolerant).hint("Give him a good fill of your milk. See if the imp is lactose tolerant.");
			if (pc.canOvipositBee()) addButton(5, "Oviposite", putBeeEggsInAnImpYouMonster).hint("Use your ovipositor and lay some eggs in his butt.");
		...
		private function sprocketImp():void {
			...
			//Variable cocktext
			if (player.cocks[0].cockType == CockTypesEnum.HUMAN || player.cocks[0].cockType == CockTypesEnum.DEMON || player.cocks[0].cockType.Index > 4) outputText("your " + player.cockDescript(0) + ", rubbing the tip of his prick against your own, ");
			else if (player.hasKnot(0)) outputText("your " + player.cockDescript(0) + ", rubbing the tip of his prick against your point, ");
			else if (player.cocks[0].cockType == CockTypesEnum.HORSE) outputText("your " + player.cockDescript(0) + ", rubbing the tip of his prick against your flared head, ");
			...
			if (player.cocks.length > 2) outputText("The rest of your " + player.multiCockDescriptLight() + " twitch and release their seed at the same time, creating a shower of spunk that rains down on both you and the imp, coating both of your bodies.  ");
			if (player.biggestLactation() >= 1) outputText("At the same time, milk bursts from your " + player.nippleDescript(0) + "s, splattering him in the face.  You feel a sick sort of triumph as you get him back for cumming inside you.  ");
			//Vagoooz
		...
		private function centaurGirlOnImps():void {
			clearOutput();
			outputText(images.showImage("player-centaur-female"));
			...
			if (player.hasVagina()) {
				...
				player.cuntChange(monster.cockArea(0), true, true);
				player.buttChange(monster.cockArea(0), true, true);
				outputText("\n\n");
		...
		private function centaurOnImpMale(vape:Boolean = false):void {
			...
			if (x < 0) x = 0;
			if (vape) clearOutput();
			if (player.gender == 3) outputText(images.showImage("player-centaur-herm"));
			else outputText(images.showImage("player-centaur-male"));
			...
			if (player.cockTotal() > 1 && player.cocks[player.biggestCockIndex()].cockLength >= 24) {
				...
				//<< Cor <50>>
				if (player.cor < 50) {
					outputText("You slowly undulate your " + player.cockDescript(player.biggestCockIndex()) + " against the creature's mouth, delighting in its eager tongue.  ");
					//<<GoTo I3 then return>> 
				...
				//<< 1 or 2 cocks, Cor 50+>>
					...
					//<<Cor 80+>> 
						...
						outputText("You groan in pleasure and slide your " + player.cockDescript(player.biggestCockIndex()) + " even deeper down the creature's throat, until you can feel its head against your ");
						//<<if balls>>
						if (player.balls > 0) outputText(player.ballsDescriptLight() + ".\n\n");
						...
						if (player.hasFuckableNipples()) outputText("Shoving your fingers deep into your " + player.nippleDescript(0) + "s");
						else outputText("With a fierce tug on your " + player.nippleDescript(0) + "s");
						outputText("you begin to cum deep and directly into the imp's stomach and " + monster.assholeDescript() + ".  ");
						//<<cum multiplier: lots>>
				...
				//<< 3+ cocks, Cor 80+>>
					...
					if (player.biggestTitSize() >= 0) {
						...
						//<<nips have pussies>> 
						if (player.hasFuckableNipples()) outputText(" and fingering your " + player.nippleDescript(0) + "s");
		...
		private function centaurOnImpFemale(vape:Boolean = false):void {
			...
			if (player.vaginas[0].vaginalLooseness <= Vagina.LOOSENESS_GAPING) {
				...
				//Not virgin fucking flavors
					...
					//<<At Dicksize>>
					if (player.vaginalCapacity() >= monster.cockArea(0) && player.vaginalCapacity() <= monster.cockArea(0)*1.25) outputText("It makes a pleased sound as it slides deeply into your " + player.vaginaDescript(0) + ".  ");
			...
			//<<Gaping>>
				...
				if (player.cor >= 50 && player.biggestTitSize() >= 2) {
					...
					//<<w/ nip-pussies>>
					if (player.hasFuckableNipples()) outputText(" and shoving your fingers into your " + player.nippleDescript(0) + ".  ");
					else outputText(".  ");
					//<</Breasts>>
				}
				...
				if (player.analCapacity() >= 35) {
					outputText("When the imp realizes it cannot go any further you feel its hands against your asshole, and your eyes go wide in realization of what it is planning on doing.  Lubed up by your now drooling juices, the fist pushes hard into your " + player.assholeDescript() + ", shoving past your ring-muscles.  ");
		...
		*/
		...
		private function centaurOnImpResults(iNum:Number):void {
			...
			if (iNum == 1) {
				...
				if (player.cumQ() >= 250) {
					...
					outputText("When your " + player.cockDescript(x) + " finally emerges a torrent of cum follows out of the distended hole and covering the back of the creature's legs.  ");
					//<<I1_1>>
					//<<2 cocks>>
			...
			if (iNum == 3) {
				...
				if (player.biggestTitSize() >= 2) {
					outputText("As the sensations intensify you reach up and begin massaging your " + player.breastDescript(0) + " and playing with your " + player.nippleDescript(0) + "s.  ");
					//<<(breasts cont.) nips have pussies>> 
					if (player.hasFuckableNipples()) {
						//<<nip-pussies and milk>>
						if (player.biggestLactation() >= 1) outputText("Milk streams out from your " + player.nippleDescript(0) + "s as if they had been recently filled with dripping cum.  ");
						else outputText("Your fingers slide faster and faster into your " + player.nippleDescript(0) + "s even as the imp begins to stroke itself under you.  ");
					}
					//No pussies
					else {
						...
						if (player.biggestLactation() > 0) {
							...
							//<<else>>
							else outputText("Milk pours out of your " + player.breastDescript(0) + " and streams down your body.  ");
						}//<</milk>>
					}
				}//<</Breasts>>
				return;
		...
		private function areImpsLactoseIntolerant():void {
			clearOutput();
			outputText(images.showImage("monster-imp"));
			outputText("You advance on the masturbating imp, baring your " + player.allBreastsDescript() + " and swinging them from side to side. The little creature watches them, mesmerized as he masturbates his " + UnitSystem.footCompound() + "-long erection.\n\n");
			...
			outputText("You feel the imp's mighty gulps start to slow down until he lets out a sigh of relief. While imps may be small, they're very hungry creatures. Your " + player.nippleDescript(0) + " slips out of the imp's mouth, and you gently lay it down on the ground. It gives a few gentle burps before dozing off; you can see that the imp's erection has retracted, and its belly has expanded significantly. You smile to yourself and, feeling fully satisfied, you stand up.");
		...
		public function impGangabangaEXPLOSIONS(loss:Boolean = false):void {
			...
			if ((Math.sqrt(player.inte + player.spe) >= rand(16) || rand(3) == 0) && !loss) {
				outputText("The imps stand anywhere from " + UnitSystem.displayFeetEstimateRangeTextually(2, " to ", 4) + " tall, with scrawny builds and tiny demonic wings. Their red and orange skin is dirty, and their dark hair looks greasy. Some are naked, but most are dressed in ragged loincloths that do little to hide their groins. They all have a " + monster.cockDescript(0) + " as long and thick as a man's arm, far oversized for their bodies. Watching an imp trip over its " + monster.cockDescript(0) + " would be funny, if you weren't surrounded by a horde of leering imps closing in from all sides...\n\n");
			...
			if (player.isTaur()) {
				if (rand(2) == 0 && (player.cockTotal() == 0 || player.gender == 3)) {
					...
					//(Low Corruption)
					if (player.cor < 50) outputText("You shake your hips, trying to escape the demonic tongue. The imp grips your " + player.hipDescript() + " and pulls his face further into your cunt, sliding his nimble tongue over your lips. You grit your teeth, trying to ignore the warmth spreading from your " + player.vaginaDescript(0) + ".");
					//(High Corruption)
					else outputText("You let out a shuddering sigh as the heat from your cunt spreads into the rest of your body. Your " + player.hipDescript() + " tremble as the tongue slides over the folds of your " + player.vaginaDescript(0) + ". The imp grips your flanks harder and dives his nimble tongue into your fuck-hole.");
					outputText("\n\n");
					...
					if (player.biggestTitSize() > 1) {
						...
						//(Low Corruption)
						if (player.cor < 50) outputText("You can't escape the tongues lapping and pulling at your " + player.nippleDescript(0) + ", matching the one in your cunt. You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.");
						//(High Corruption)
						else outputText("The tongues squeezing and tugging your nipples match the tongue working your " + player.vaginaDescript(0) + ", flooding your body with lust. You moan and arch your back, offering your tits to the imps. You can hear your pulse pounding in your ears as you pant with desire.");
					...
					outputText("Dimly through your haze of lust and pain you see a large imp step forward from the mob. " + capitalizeFirstLetter(UnitSystem.displayHeight2EstimateTextually(4)) + " tall and broader and stronger looking than any imp you've seen before, with a face as much bull as imp, this new imp has mottled grey skin, broad purple demon wings, two curving bull-horns on his head, and a " + Appearance.cockNoun(CockTypesEnum.HORSE) + " big enough to choke a minotaur. The mushroom-like head of it bobs just below his mouth, and his snake-tongue darts out to flick a bit of pre-cum off the head and onto your face. You shudder as the hot fluid stings the sensitive skin of your lips. His " +  monster.ballsDescriptLight() + " are each the size of your fist and slick with sweat. He slaps his sweaty cock-head against your cheek, nearly scalding you with the heat.  ");
					//(Low corruption)
					...
					//(If the character has breasts)
					if (player.biggestTitSize() > 2) outputText("The other imps reclaim your aching breasts, sucking your " + player.nippleDescript(0) + " and mauling your " + player.allBreastsDescript() + " so hard their fingers disappear into your swelling flesh. ");
					...
					outputText("You squirm and twist against the imps holding you down as the hot " + Appearance.cockNoun(CockTypesEnum.HORSE) + " almost burns your sensitive cunt. You can smell the sweat steaming off his shaft, and your pussy-fluids start to steam as well as he forces his cock-head into your " + player.vaginaDescript(0) + ". His huge cock-head bulges your groin, and you moan");
					//(Low corruption)
					if (player.cor < 50) outputText(" in helpless terror as you feel the bulge work up from the base of your groin towards your stomach. You let out a shuddering moan of pain as " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of monstrous " + Appearance.cockNoun(CockTypesEnum.HORSE) + " stretches your belly");
					//(High corruption)
					...
					outputText(" in your stuffed " + player.vaginaDescript(0) + ". The big imp sneers and flexes his cock again, watching ");
					//(If the character has breasts)
					...
					if (player.cockTotal() > 0) outputText("Your " + player.multiCockDescriptLight() + " bounces as the bulge passes over it.  ");
					outputText("You moan as the mushroom-head reaches the entrance of your " + player.vaginaDescript(0) + ", your stretched pussy-flesh slowly returning to normal. The master imp pushes forward again, reclaiming your pussy for his monstrous cock. ");
					//(Low corruption)
					...
					//(If the character has a cock)
					if (player.cockTotal() > 0) outputText("over your " + player.multiCockDescriptLight() + " where it ");
					...
					//(If the character has breasts)
					if (player.biggestTitSize() > 1) outputText("Imps gulp milk from your bloated " + player.biggestBreastSizeDescript() + ". As one imp drinks his fill and staggers away with a sloshing belly, another steps up to pump your milk-spewing udders.  ");
					//(If the character has a dick)
					if (player.cockTotal() > 0) outputText("Your " + player.multiCockDescriptLight() + " swell painfully as the rough fucking pumps blood into your groin.  ");
					outputText("The big imp's snake tongue flicks out and slides around your " + player.vaginaDescript(0) + ", pulling at your pussy lips. He moves his tongue back and forth along the sides of your steaming cunt, alternating between stretching and flicking the lips. ");
					//(If the character has a dick)
					if (player.totalCocks() > 0) outputText("He draws his tongue back and wraps it around your " + player.cockDescript(0) + ", sliding its length along your shaft and flicking his tongue over your cock-head.  ");
					...
					//(If the character has breasts)
					if (player.biggestTitSize() > 3) outputText("Your udders shake back and forth under your chest in time to the rough fucking. You arch your back to press your " + player.nippleDescript(0) + " into eager mouths, moaning as your rider milks your distended " + player.allBreastsDescript() + ". ");
					//(Low Corruption).
					...
					//(If the character has breasts)
					if (player.biggestTitSize() > 2) outputText("As orgasms wrack your body your breasts pump out even more milk, too much for the imps below to handle. Milk pours down your chest in great streams, soaking the imps and splashing onto the ground below you. The milk gushing through your tender " + player.nippleDescript(0) + " pushes you to another orgasm. You shake your tits as you cum, mooing in mindless pleasure, spraying jets of milk everywhere. Your rider cums, soaking your " + player.hairDescript() + " with jets of imp-jism that run down your scalp and over your cheeks. ");
					//(High corruption)
					if (player.cor >= 50) outputText("You lap eagerly at the salty cum, licking up and drinking as much as you can.");
					outputText("\n\n");
					outputText("Imp-jism rains down on your helpless spasming body. The imps spew cum into your hair, across your back and " + player.hipDescript() + ", over your face");
					//(If the character has breasts)
					...
					//(If the character has breasts)
					if (player.biggestTitSize() > 2) outputText("milk spurts from your " + player.nippleDescript(0) + " and ");
				...
				//Scene number 2 - male possible.
					...
					//(If the player has breasts)
					if (player.biggestTitSize() > 0) outputText("Your unbound " + player.biggestBreastSizeDescript() + " bounce out over the weathered log. ");
					outputText("The imps spread your arms wide, forcing your chest out, and tie them to the log with sweaty loincloths. Your " + player.hipDescript()  + " are stuck high in the air. Imps rub their sweaty cocks and " + monster.ballsDescriptLight()  + " over your legs and grope your crotch. The two imps riding your back start stroking and licking each other. ");
					//(Low Corruption)
					...
					outputText("An imp quickly climbs up your body, planting his feet on your shoulders and grabbing your " + player.hairDescript()  + " with one hand for support. He rubs his " + monster.ballsDescriptLight()  + " over your mouth, smearing your lips with musky sweat, while he pries at your jaw with his other hand. ");
					//(If the player has breasts)
					...
					if (player.cockTotal() == 1) outputText("An imp ducks under your body and grabs your " + player.cockDescript(0) + ". His nimble tongue flicks over your cock-head while he pricks the shaft with his tiny claws.  ");
					//(If the player has a MULTI cock)
					if (player.cockTotal() > 1) outputText("Two imps duck under your body and seize your " + player.multiCockDescriptLight() + ", licking the tips with their inhumanly flexible tongues while they stroke the shafts.  ");
					//(Low Corruption)
					...
					//(If the player has breasts)
					if (player.biggestTitSize() > 2) outputText("The imp fucking your " + player.biggestBreastSizeDescript() + " handles your soft flesh roughly, pressing and pulling your tits into a fuck-canal for his demon cock. Other imps slap your " + player.allBreastsDescript() + " and laugh as you cry out.  ");
					//(Low Corruption)
					...
					outputText("\n\n");
					outputText(images.showImage("gangbang-loss-centaur"));
					outputText("With a loud sucking sound, the imp pulls his balls out of your mouth. Spit and ball-sweat drip over your cheeks as he repositions himself, bending almost completely over on your shoulders to rub his cock-head against your lips. You nearly choke as pre-cum dribbles into your mouth and runs down the back of your throat. The " + monster.cockDescriptShort(0) + " blocks most of your vision, but in the corners of your eyes you see the master of this imp horde step forward. " + capitalizeFirstLetter(UnitSystem.displayHeight2EstimateTextually(4)) + " tall and broader and stronger than any imp in the pack, with a face as much dog as imp, this new imp has black fur, broad red demon wings, two long demon-horns on his head, and a " + Appearance.cockNoun(CockTypesEnum.DOG) + " big enough to choke a minotaur. He leers at your helpless body and grabs ");
					//(If the player has breasts)
					if (player.biggestTitSize() > 2) outputText("one of your sore " + player.biggestBreastSizeDescript() + " in his calloused hand, brutally pressing his fingers into your flesh");
					...
					outputText("The master imp walks back to your hips, lightly dragging his sharp claws over your flanks. He kicks another imp out of the way and takes position behind your " + player.hipDescript()  + ". He pulls his monstrously long " + Appearance.cockNoun(CockTypesEnum.DOG) + " down and rubs the tip over your ");
					...
					outputText(".  ");
					//(If the player has a cock)
					if (player.cockTotal() > 0) outputText("Pre-cum drips from the broad tip of it, dripping down to the base of your " + player.multiCockDescriptLight() + ".  ");
					...
					outputText(" wide open. You moan around the cock fucking your throat as the corrupted wolf-cock pushes deeper into your hole. The painfully hot shaft claims " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of your flesh, forcing its way deeper into you than any normal human could bear. Bound to the log you can only shake in agony as the big imp's thick dog-knot hits your ");
					//(If the player has a vagina)
					if (player.hasVagina()) outputText(player.vaginaDescript(0));
					//(If the player doesn't have a vagina)
					else outputText(player.assholeDescript());
					outputText(".");
					//(If the player has breasts)
					if (player.biggestTitSize() > 2) outputText("  The imp fucking your aching " + player.biggestBreastSizeDescript() + " paints your tits with a massive load of cum. He falls off the log and another imp jumps up to take his place.");
					outputText("\n\n");
					
					outputText("The big imp fucks you roughly, clenching your " + player.hipDescript()  + " in his clawed hands as he hammers his " + Appearance.cockNoun(CockTypesEnum.DOG) + " into you. The head of his mutated shaft pounds ");
					//(If the player has a vagina)
					...
					//(If the player has a cock)
					if (player.cockTotal() > 0) outputText("  Your " + player.multiCockDescriptLight() + " shoots cum across the ground and into the waiting mouths of the imps licking your crotch.");
					outputText("\n\n");
					
					outputText("Another imp-cock spasms in your throat as its owner rams deep into you. He floods your already swollen stomach with inhuman amounts of cum. Again you feel yourself about to black out as the demon pumps jism into you. He pulls out and again you orgasm as you wheeze for air. Another imp forces his cock down your throat as you moan and gasp. Your body shakes in pleasure on the big imp's " + Appearance.cockNoun(CockTypesEnum.DOG) + ".  Tightening his grip on your " + player.hipDescript() + " the master imp howls and slams his shaft into your ");
					//(If the player has a vagina)
					if (player.hasVagina()) outputText(player.vaginaDescript(0));
					//(If the player doesn't have a vagina)
					else outputText(player.assholeDescript());
					...
					//(If the character has breasts)
					if (player.biggestTitSize() > 2) outputText("The imp riding your " + player.biggestBreastSizeDescript() + " cums, his load lost in the flood of jism dripping off your abused fuck-udders. ");
					...
					outputText("You wake up later, still trembling with small orgasms. Cum burbles in your mouth as you breathe, and your " + player.hairDescript() + " is soaked with jism. You haven't moved since you passed out. Your arms are still tied to the log, ");
					//(If the player has breasts)
			...
			//NOT CENTAUR
				...
				if (rand(2) == 0 && (player.cockTotal() == 0 || player.gender == 3)) {
					...
					//(If the player has breasts)
					if (player.biggestTitSize() > 0) outputText("Imps surround your chest, slapping their " + monster.cockDescriptShort(0) + "s on your " + player.allBreastsDescript() + " and rubbing their slippery pre-cum into your " + player.nippleDescript(0) + ".  ");
					outputText("Others stand over your head, their cocks bobbing " + UnitSystem.literalInches() + " from your face as they jack off. A thick musk wafts off their cocks, and the smell of it makes your sinuses tingle. Two more imps take position between your legs, sliding their cocks along your thighs while stroking your " + player.vaginaDescript(0) + " and flicking your " + player.clitDescript() + ".");
					//(If the player has a cock)
					if (player.cockTotal() > 0) outputText("An imp rubs his hand across his cock-head, smearing it with his pre-cum. He rubs his hand over your " + player.multiCockDescriptLight() + ", making your cock-skin tingle as his fluid soaks into you.");
					outputText("\n\n");
					outputText("The " + monster.short + " snickers lewdly as your nipples harden and your pussy moistens. One of the imps between your legs slides his shaft along your pussy lips, teasing your " + player.clitDescript() + " with the tip of his cock.  ");
					//(Low corruption)
					...
					//(High corruption)
					else outputText("You buck your hips, trying to capture his " + monster.cockDescriptShort(0) + " with your " + player.vaginaDescript(0) + ".  ");
					outputText("Before he can thrust into you, the imp is shoved aside by the biggest imp you've ever seen.\n\n");
					outputText(images.showImage("gangbang-loss-human"));
					outputText(capitalizeFirstLetter(UnitSystem.displayHeight2EstimateTextually(4)) + " tall and broader and healthier looking than any imp you've seen before, with a face as much bull as imp, this new imp has mottled grey skin, broad purple demon wings, two curving bull-horns on his head, and a " + Appearance.cockNoun(CockTypesEnum.HORSE) + " big enough to choke a minotaur. The mushroom-like head of it bobs just below his mouth, and his snake-tongue darts out to flick a bit of pre-cum off the head and onto your groin. You shudder as the hot fluid stings the sensitive skin of your " + player.vaginaDescript(0));
					//(If the player has a dick)
					if (player.cockTotal() > 0) outputText(" and " + player.multiCockDescriptLight());
					outputText(". His " + monster.ballsDescriptLight() + " are each the size of your fist and slick with sweat. He slaps his sweaty balls against your " + player.vaginaDescript(0) + " nearly scalding you with the heat.  ");
					//(Low corruption)
					...
					outputText(" as the inhumanly hot cock-head stretches your pussy lips, your cries vanishing into the dark skies above. Two imps grab your hair and pull your head up, forcing you to watch as their master pushes his corrupted cock into you. Other imps spread your [legs] even wider, leaving you helpless as the big imp slides his swollen meat into your " + player.vaginaDescript(0) + ". You squirm and twist against the imps holding you down as the hot flesh almost burns your sensitive cunt. You can smell the hot sweat steaming off his shaft, and your pussy-fluids start to steam as well as he forces his cock-head into your " + player.vaginaDescript(0) + ". His huge cock-head bulges your groin, and you watch ");
					//(Low corruption)
					if (player.cor < 50) outputText("in helpless terror as the bulge inches up from the base of your groin towards your stomach. You let out a shuddering moan of pain as " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of monstrous " + Appearance.cockNoun(CockTypesEnum.HORSE) + " stretches your belly");
					//(High corruption)
					else outputText("panting in lust as the monstrous " + Appearance.cockNoun(CockTypesEnum.HORSE) + " pushes your flesh aside to make room for itself");
					outputText(". ");
					//(This is a good place for the virginity-loss message, if needed)
					...
					outputText(" in your stuffed " + player.vaginaDescript(0) + ". The big imp sneers and bounces his cock again, watching ");
					//(If the character has breasts)
					...
					outputText(".  ");
					//(If the character has a cock)
					if (player.cockTotal() > 0) outputText("Your " + player.multiCockDescriptLight() + " slaps against your distended belly as you shake.");
					outputText("\n\n");
					...
					//(If the character has a cock)
					if (player.cockTotal() > 0) outputText("Your " + player.multiCockDescriptLight() + " bounces as the bulge passes under it.  ");
					outputText("You moan as the mushroom-head reaches the entrance of your " + player.vaginaDescript(0) + ", your stretched pussy-flesh slowly returning to normal. The master imp pushes forward again, reclaiming your pussy for his monstrous cock. ");
					//(Low corruption)
					...
					outputText("The big imp grinds his hips as he thrusts and pulls, rubbing his cock-ridges against every part of your " + player.vaginaDescript(0) + ".  While sliding his mutated " + Appearance.cockNoun(CockTypesEnum.HORSE) + " in and out of you the imp rubs his hands along your mound, pulling it open or forcing it tight as he takes you. Your pussy juices steam off his cock as he pumps, and hot pre-cum dribbles down your crack to your " + player.assholeDescript() + ". ");
					//(Low corruption)
					...
					//(If the character has a dick)
					if (player.totalCocks() > 0) outputText("Your " + player.multiCockDescriptLight() + " swell painfully as the rough fucking pumps blood into your groin.  ");
					outputText("The big imp's snake tongue lashes out to incredible length and wraps around one of your " + player.nippleDescript(0) + "s, pulling at it and stretching the flesh under it. He moves his tongue back and forth between your nipples, alternating between stretching and flicking them. ");
					//(If the character has a dick)
					if (player.cockTotal() > 0) outputText("He draws his tongue back and wraps it around your " + player.cockDescript(0) + ", sliding its length along your shaft and flicking his tongue over your cock-head.");
					//(If the character doesn't have a dick)
					else outputText("His tongue flicks down to your " + player.clitDescript() + ", the split ends of it teasing your clit.");
					outputText("  You gasp in time to the big imp's thrusts, whimpering when his cock or tongue hit a sensitive point.  ");
					//(Low Corruption)
					...
					//(If the character has a cock) 
					if (player.cockTotal() > 0) outputText(", and cum-dripping " + player.cockDescript(0));
					//(If the character has breasts)
					...
					//(If the character has breasts)
					if (player.biggestTitSize() >= 3 && player.biggestLactation() > 1) outputText("milk spurts from your " + player.nippleDescript(0) + " and ");
					...
					outputText("You wake up later, body still twitching as tiny orgasms spark in your " + player.vaginaDescript(0) + ". It's still dark out. You lie in a pool of cooling cum and pussy juice. Your body is covered in long ropes of drying imp-cum, and your hair is plastered to the ground. There's no sign of the horde of imps or their big master. Your belly is as tight and distended as a woman on the verge of giving birth. It quivers as the flesh of your " + player.vaginaDescript(0) + " spasms. Over the swollen curve of your belly you can see steam rising from between your legs. You start to slip back into unconsciousness. ");
					//(Low corruption)
					if (player.cor < 50) outputText("Your last coherent thought is to find a way to better hide your camp, so this never happens again.");
					//(High corruption)
					...
					//(If the player has a SINGLE cock)
					if (player.cockTotal() == 1) outputText("Your " + player.cockDescript(0) + " is seized by an imp, who licks the tip with his inhumanly nimble tongue while he strokes the shaft.  ");
					//(If the player has a MULTI cock)
					if (player.cockTotal() > 1) outputText("Two imps seize your " + player.multiCockDescriptLight() + ", licking the tips with their inhumanly nimble tongues while they stroke the shafts.  ");
					//(Low Corruption)
					...
					if (player.biggestTitSize() > 0) {
						...
						//(Low Corruption)
						if (player.cor < 50) outputText("You can't escape the tongue lapping and pulling at your " + player.nippleDescript(0) + ". You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.");
						//(High Corruption)
					...
					outputText(", leering down at your trapped body. " + capitalizeFirstLetter(UnitSystem.displayHeight2EstimateTextually(4)) + " tall and broader and stronger than any imp in the pack, with a face as much dog as imp, this new imp has grey fur, broad black demon wings, two long demon-horns on his head, and a " + Appearance.cockNoun(CockTypesEnum.DOG) + " big enough to choke a minotaur. Pre-cum drips from the broad tip of it, dripping down onto your ");
					//(If the player has a cock)
					if (player.cockTotal() > 0) outputText(player.multiCockDescriptLight());
					...
					outputText(". ");
					//(Low Corruption)
					if (player.cor < 50) outputText("You screech in agony as the big imp forces his mutated wolf-cock into your hole, brutally shoving thick " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of painfully hot " + Appearance.cockNoun(CockTypesEnum.DOG) + " deeper into you than anything should ever go.  ");
					//(High Corruption)
					...
					outputText(". ");
					//(If the character has breasts)
					if (player.biggestTitSize() > 1) outputText("The rough fucking shakes your breasts, and the imp sucking your nipples clings tightly to your monstrously swollen " + player.allBreastsDescript() + ". Your " + player.biggestBreastSizeDescript() + " have grown three cup sizes since the imp pumped his venom into you.  ");
					...
					outputText(" spasms on the steaming pole that impales it. Another imp shoves his cock in your mouth as you scream, throat convulsing around his cock-head.");
					//(If the player has a cock)
					if (player.cockTotal() > 0) outputText("  Your " + player.multiCockDescriptLight() + " shoots cum across your belly and into the waiting mouths of the imps licking your crotch.");
					outputText("\n\n");
					//(If the character has breasts)
					if (player.biggestTitSize() > 0) {
						...
						if (player.hasFuckableNipples()) {
							...
							else outputText("You feel the unnaturally large erection spear the fat filled depths of your " + player.biggestBreastSizeDescript() + " until at last the imp has shoved himself in to his hilt. He smiles at the sensation of having his manhood completely engulfed in your " + player.biggestBreastSizeDescript() + ".");
					...
					//(If the character has breasts)
					if (player.biggestTitSize() > 0) outputText("The imp riding your " + player.biggestBreastSizeDescript() + " finally cums, painting your distended fuck-udders with his massive load.  ");
					outputText("Your master isn't done with you yet, churning his " + Appearance.cockNoun(CockTypesEnum.DOG) + " knot in your ");
					//(If the player has a vagina)
					if (player.hasVagina()) outputText(player.vaginaDescript(0));
					//(If the player doesn't have a vagina)
					...
					outputText("You wake up later, still trembling with small orgasms. Cum burbles in your mouth as you breathe. You haven't moved since you passed out. Your hips are still propped up over the log, and you rest in a cooling pool of cum, your " + player.hairDescript() + " plastered to the ground with drying jism. You couldn't move even if your " + player.legs() + " felt stronger. Your hideously bloated belly weighs you down, ");
					//(If the player has breasts)
		...
		public function impRapesYou():void {
			...
			if (player.lust >= player.maxLust()) {
				...
				if (player.gender == 2 || player.gender == 3) {
					player.slimeFeed();
					outputText(images.showImage("imp-loss-female-fuck"));
					outputText("You sink to the ground, too overcome by lust and desire to fight.  The imp smiles, a wicked look glinting in his eyes.  He drops his loincloth to reveal a hardening cock.  Your eyes bulge a bit as it grows...and grows...and grows!  That imp has a " + UnitSystem.displayInchEstimateWithHyphenTextually(12) + " cock..and he's walking towards you.   Your " + player.vaginaDescript(0) + " practically juices itself in anticipation, and you find yourself spreading your " + player.legs() + " in preparation.");
				...
				outputText(images.showImage("monster-imp"));
					...
					if (player.gender == 1 && rand(2) == 0) {
						...
						if (player.isTaur()) outputText("hocks and knees, your " + player.cockDescript(0) + " hurting from the massive blood pressure caused by your unbridled lust. He approaches you and stops about " + UnitSystem.displayHeight2EstimateTextually(2) + " in front of you, watching with delight your helpless state");
						else outputText("knees, pull out your " + player.cockDescript(0) + " and begin mindlessly stroking yourself as the imp approaches you, a wicked grin on his face. Your mind races with thoughts and images of sucking the imp's cock. He approaches you and stops about " + UnitSystem.displayHeight2EstimateTextually(2) + " in front of you, watching with delight as you succumb to your own lust");
						...
						outputText("You fall to the ground gasping, exhausted and unable to move, the demon cum on your face and inside you still burning with intense heat and corruption. You lose consciousness, your " + player.cockDescript(0) + " still firmly erect, your lust not sated.");
		...
		private function noogaisNippleRape():void {
			...
			//(If corrupt: 
			if (player.cor >= 66) {
				outputText("\"<i>Mmm, wouldn't you just love to stick your fat cock into this sopping wet hole, and cum deep inside my " + player.chestDesc() + "?</i>\"  You whisper huskily into his ear, sliding your fingers away from his balls and up along the underside of his aching dick, teasing every " + UnitSystem.literalInch() + " of it until you reach his swollen head and start rubbing your finger around his glans in small circles.  The imp is panting heavily, his eyes firmly locked on your ");
				//(if normal)
		...
		public function impWarlordEncounter():void {
			...
			clearOutput();
			outputText(images.showImage("monster-impwarlord"));
		...
		public function impOverlordEncounter():void {
			...
			outputText(images.showImage("monster-impoverlord"));
			...
			if (monster.HP < 1) {
				...
				if (flags[kFLAGS.SFW_MODE] <= 0) {
					...
					if (player.lust >= 33) {
						...
						if (player.hasVagina() && player.biggestTitSize() >= 4 && player.armor is LustyMaidensArmor) addButton(4, "B.Titfuck", (player.armor as LustyMaidensArmor).lustyMaidenPaizuri).hint("Take advantage of the properties of your special armour and get the imp to give you a good titfuck.", "Bikini Titfuck");
					} else {
						outputText("\n\n<b>You aren't horny enough to rape him.</b>");
					}
				}
		...
		private function getBlownByAnImpLord():void {
			...
			if (player.cocks[player.biggestCockIndex()].cockThickness < 7) {
				outputText("The imp begins to swallow more and more of your length, taking several " + UnitSystem.literalInches() + " before pulling back.  He twists his head and tongue worships your [cock biggest] with his mouth.  His tight mouth tightens more and relaxes as he swallows more of your precum.");
		...
		private function impLordRapeMale(overlord:Boolean):void {
			...
			outputText("\n\n");
			if (player.cockTotal() === 1)  {
				outputText("You feel your " + player.cockDescript(0) + " grow harder than usual and throb. You go to stroke yourself but it's far too sensitive. ");
			} else if (player.cockTotal() > 1) {
				outputText("You feel your " + player.multiCockDescriptLight() + " grow harder than usual and throb. You go to stroke yourself but they are far too sensitive. ");
			}
			
			if (player.cockTotal() > 0) {
				outputText("Any stroking you can do is far too little stimulation and anything else is too painful to withstand. ");
			}
			
			outputText("You whimper and curse in desperation. Your lust clouded mind can only think of one solution; you bend over and reveal your " + player.assholeDescript() + " to the grinning imp.  The humiliation keeps you from looking back to see the imp's reaction, but you can tell by his chuckle that this is exactly what he wanted.");
			...
			outputText("\n\nThe hot seed filling your belly wakes you from your lust induced daydream and you howl in discomfort.  Your belly begins to swell with the thick seed, coating every " + UnitSystem.literalInch() + " of your insides with the burning, arousing sensation.  This pushes you over the edge and you orgasm.  ");
			if (player.balls > 0) {
		...
		private function impLordRapeFemale(overlord:Boolean):void {
			clearOutput();
			if      (doSFWloss())  return;
			else if (overlord)     outputText(images.showImage("impoverlord-loss-female"));
		...
		private function addKillPetrifyButtons(killButtonPos:Number = NaN, petriButtonPos:Number = NaN):Boolean
		{
			...
			if (killButtonPos >= 0 && monster.HP < 1) {
				addButton(killButtonPos, "Kill Him", killImp).hint("Kill the imp and get over with it. That will be one less threat.");
				buttonCount++;
			}

			if (isNaN(petriButtonPos)) petriButtonPos = killButtonPos + 1;
			if (impPetrifyable()) {
				addButton(petriButtonPos, "Petrify Him", petrifyImp).hint("You could put your basilisk eyes to a good use and turn the imp into a statue. This would make a good camp decoration.");
				buttonCount++;
			}
		...
		private function petrifyImp():void
		{
			...
			menu();
			addButton(0, "Take Statue", curry(combat.cleanupAfterCombat, takeStatue)).hint("Carry the statue back to your camp to place it on or in front of your wall.", "Carry the statue to the camp");
		...
		private function takeStatue():void
		{
			...
			clearOutput();
			outputText(images.showImage("event-impStatue"));
		...
		private function killImp():void {
			...
			if (player.cor < 25) dynStats("cor", -0.5);
			menu();
			addButton(0, "Take Skull", takeSkull).hint("You could take the horned skull as a trophy. Maybe it'll make a nice camp decoration...");
			addButton(1, "Leave", combat.cleanupAfterCombat);
		}
