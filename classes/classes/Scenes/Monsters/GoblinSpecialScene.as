/**
 * Original Goblin Assassin created by Fenoxo on Jan 18th 2014.
 * Goblin Shaman and Warrior added by Kitteh6660, based on modified Assassin scenes.
 */
package classes.Scenes.Monsters 
{
	...
	//This is the file to unify the special goblins scenes (Assassin, Warrior, Shaman) to eliminate duplicate codes.
	public class GoblinSpecialScene extends BaseContent
	{
		public function GoblinSpecialScene() {}
		 ...
		 */
		public function goblinSpecialEncounter(forceType:int = -1):void {
			clearOutput();
			//Set up the dice and roll!
			var dice:Number = Math.min(Math.floor(player.level / 2) + 3, 12) + Math.min(Math.floor(time.days / 10) + 3, 12) + 4;
			if (dice > 20) dice = 20;
			var chooser:Number = rand(dice);
			//Shift the roll towards higher level goblins.
			if (player.level >= 12) chooser++;
			if (player.level >= 16) chooser++;
			//Let's choose which goblin we shall encounter, eh?
			if ((chooser < 8 && forceType < 0) || forceType == 0) {
				displayGoblinSprite("goblin assassin");
				//First Time Intro
				...
				flags[kFLAGS.TIMES_ENCOUNTERED_GOBLIN_ASSASSIN]++; //[Initiate combat encounter – goblin assassin]
			}
			else if ((chooser < 14 && forceType < 0) || forceType == 1) {
				displayGoblinSprite("goblin shaman");
				//First Time Intro
				...
				//Repeat Intro - Cell Chambers
					...
					outputText("As you walk, the familiar sight of a mystical goblin catches your eye. You have encountered the goblin shaman again! You ready your " + player.weaponName + " for a fight!");
				}
				outputText("\n\n\"<i>I'm going to cast you into submission,</i>\" she says.");
				startCombat(new GoblinShaman()); //[Initiate combat encounter – goblin shaman]
				flags[kFLAGS.TIMES_ENCOUNTERED_GOBLIN_SHAMAN]++;
			}
			else if ((chooser >= 14 && forceType < 0) || forceType == 2) {
				displayGoblinSprite("goblin warrior");
				//First Time Intro
				...
				outputText("\n\n\"<i>I'm going to beat you so I can fuck you,</i>\" she says.");
		...
		private function goblinClassActions(assassin:String, shaman:String, warrior:String):String { //Allows for variants.
			switch(goblinClassName()) {
				case "assassin":
					return assassin;
					break;
				case "shaman":
		...
		private function displayGoblinSprite(gobtype:String = ""):void {
			if (gobtype == "") gobtype = monster.short;
			switch(gobtype) {
		...
		public function gobboSpecialBeatYaUp():void {
			...
			if (doSFWloss()) return;
			var x:int;
			if (player.lust100 <= 99) outputText("You collapse, utterly beaten. To make sure you'll be easier to handle, the victorious " + goblinClassName() + " saunters up, a pair of fluid filled needles in her hands. She jams them into your [player.legs], emptying the contents into you before you can so much as stammer a protest. Burning lust pours through your veins unbidden, and you moan out loud as the chemicals have their way with you as easily as the goblin soon will.\n\n");
			...
			if (player.cockThatFits(monster.vaginalCapacity()) >= 0 && player.hasCock() && (!player.hasVagina() || rand(2) == 0)) 
			{
				...
				outputText("for a brief moment but vanishes before you can gain any satisfaction from the act.  \"<i>I’m sorry, that was too slow,</i>\" the diminutive " + (goblinClassName() != "shaman" ? "fighter" : "spellcaster") + " teases, a moment before she " + goblinClassActions("slams a pair of needles into your [legs]", "casts another Arouse spell towards your nether regions", "slams another Lust Draft into your mouth and forcibly making you swallow the sweet-tasting liquid") + ".  [EachCock] rises and immediately spurts a trickle of white pre-cum, unable to resist the potent " + goblinClassActions("chemical concoction flooding your bloodstream", "potent spell muddling your mind", "chemical concoction flooding your bloodstream") + ", so perfectly tailored to inflame your passions past the point of reason.  You fail to notice the removal of the spent plungers, but you sure as hell notice the return of the lithe green woman’s probing caresses.  She whispers, \"<i>Now tell me how other girls don’t matter.  This was all just a ruse to get you some green love, wasn’t it?</i>\"");
				outputText("\n\nYour response is as immediate as it is enthusiastic.  You tell her exactly what she wants and then some more, panting and moaning as the " + goblinClassName() + "’s nimble fingers tickle every throbbing-hard " + UnitSystem.literalInch() + " of your over-filled fuck-tool.  It spews another rope of pre so thick and white it may as well be the real deal as you finish, begging, \"<i>Yes, I came here for your delicious, juicy cunny.  Please, fuck my cock.  I need it so bad!  I’ll do anything!</i>\"");
				outputText("\n\n\"<i>Awww, you’re squirting for me already.  I just hope there’s enough left to pay your fine.  If not, I can always " + goblinClassActions("put some succubi’s delight in a few of these needles", "cast more of these arousing spells", "put some succubi’s delight in a few of these potions") + ",</i>\" muses the emerald slut.  She steps over you, the jiggly, rounded curves of her ass hanging over you for a moment before she drops her musky, squishing box straight down onto your nose and lips.  Droplets of fem-spunk roll down your cheeks as the goblin grinds her swollen mound against you, her fragrant pussy driving your neglected cock");
			...
			//iii. Lust Loss - Female
				...
				outputText("\n\nLooking on in confusion, you can only stare as the goblin steps over you, straddling your prone form and easily batting away every attempt you make to touch yourself.  " + goblinClassActions("She pulls off her belt of needles and casually removes a set of small leather thongs from its inner lining.", "She carefully moves her fur loincloth to expose her cunny.", "She carefully moves her leather straps to expose her cunny, not even bothering to take her armor off.") + "  With practiced skill, she binds your hands together with a narrow leather leash.  You don’t resist, expecting the kinky viridian bitch is planning some perverse sex game to get you both off that much harder.  She smirks down and says, \"<i>I don’t see much point in fucking a two-bit twat like yourself, but you got me horny, so by Lethice’s drippy tits, you’re gonna get me off.</i>\"");
				outputText("\n\nBefore you can mutter a protest, the goblin pivots and drops atop your face, plugging your mouth-hole with her wet gash.  Musky fem-drool slimes over your cheeks, running down to the nape of your neck where it drips on the ground.  The goblin utters a quiet, appreciative moan, rocking her hips ever so slightly to drag the bump of her clitty across your parted lips.  She doesn’t do anything for your needs at all!  You try to protest, but all that gets you is a mouth full of slime, tangy and arousing though it may be.  The " + goblinClassActions("leather thong", "fur loincloth", "leather thong") + " binding your hands goes taut as you try to pull your fingers back into your aching cunt, but in your compromised position, you cannot will up the strength to overpower the tiny, dominating minx.");
		...
		//[WIN RAEPZ]
		public function gobboSpecialRapeIntro(hpVictory:Boolean):void
		{
			clearOutput();
			displayGoblinSprite();
			switch(monster.short) {
				case "goblin assassin":
		...
		private function gobboGetsRapedFem():void
		{
			...
			//Goblin victory rape, female naga:
				...
				outputText("You pull it out and examine it. Not much more than " + UnitSystem.literalAnInch() + " long, it's filled with a white fluid. There's a label, but the lettering is far too small to read. You think you can make out the word \"<i>wake</i>\", but aren't entirely sure.\n\n");
