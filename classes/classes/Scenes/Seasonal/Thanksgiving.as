...
package classes.Scenes.Seasonal {
	...
	public class Thanksgiving extends BaseContent {
		...
		public function datTurkeyRumpMeeting():void { //Introduction: -McGirt
			...
			flags[kFLAGS.TURKEY_FUCK_YEAR_DONE] = date.fullYear;
			outputText(images.showImage("thanksgiving"));
		...
		public function shooTurkeyAway():void { //Shoo her Off -McGirt
			clearOutput();
			outputText(images.showImage("thanksgiving-enough"));
		...
		public function letZeTurkeyApproach():void { //Let her Approach
			...
			clearOutput();
			outputText(images.showImage("thanksgiving-approach"));
		...
		public function basteThatTurkeyBooty():void { //Baste Her -McGirt, reluctantly
			clearOutput();
			...
			outputText(images.showImage("thanksgiving-baste"));
		...
		public function basteThatTurkeyBootyII():void {
			clearOutput();
			outputText(images.showImage("thanksgiving-basteII"));
			outputText("\"<i>G-gobble?</i>\"");
		...
		public function stuffDatTurkeyWithSpoo():void { //Stuff Her -McGirt, this is awful by the way.
			clearOutput();
			...
			outputText("\n\n\"<i>G-gobble?</i>\"  the girl asks plaintively, looking over the massive mounds of her breasts to watch the final " + UnitSystem.literalInches() + " slide into her.  Oh, those do look like inviting targets....");
			outputText("\n\nYou begin to move your hips, slowly rocking a few " + UnitSystem.literalInches() + " of your cock out and back into the turkey's vice-like sheathe.  Yet your attention turns to the huge, soft orbs of her tits, so large that all but the top of her head is obscured by titflesh.  You reach up, sinking your fingers into one of the huge mounds, angling the large, pink nipple towards you.  A moment passes, your fingers kneading her sensitive breast as you push your cock into her to the hilt.");
			...
			outputText("\n\n...Gravy?");
			outputText(images.showImage("thanksgiving-stuff"));
			...
			outputText("\n\nSoft moans begin to echo through the air as you thrust harder and faster into the turkey-girl, lapping up " + UnitSystem.display("half a gallon", UnitSystem.displayGallonsEstimateTextually(0.5)) + " of gravy from one of her huge tits before switching to the other, giving her neglected nip the attention it deserves.  You hammer into her, thrusting with enough force to let loose spurts and trickles of femcum and your own pre as the turkey-girl throws her head back in rapture, overwhelmed by the dual sensations.");
		...
		public function spitRoastThatTurkey():void { //SPITROAST HER! GangHel Style / (Needs friendly/lover Hel)
			clearOutput();
			...
			outputText("\n\nWhen you've given her all the lubrication you can, you slowly withdraw your finger and grab your [cock], pushing it up between her full buttcheeks.  The turkey-girl makes a pitiful squeak of a \"<i>Gobble!?</i>\" as you press the [cockHead] against her lightly-stretched anus, giving it an extra gob of spittle on the tip as you start to push into her. She's incredibly tight, her poor, defenseless sphincter clamping down hard around your intruding cock as you press the first " + UnitSystem.literalInches() + " into her, gently guiding yourself into her backdoor.");
			outputText(images.showImage("thanksgiving-spit-roast"));
		...
		public function thatllDoTurkey():void {
			clearOutput();
			outputText(images.showImage("thanksgiving-ending"));
		...
		public function femaleLetZeTurkeyGalApproach():void { //Let her Approach
			clearOutput();
			outputText(images.showImage("thanksgiving-approach"));
		...
		public function turkeyAppetizer():void { //Appetiser
			clearOutput();
			...
			outputText(images.showImage("thanksgiving-appetizer"));
		...
		public function femaleAndTurkeyMainCourse():void { //Main Course
			clearOutput();
			...
			outputText(images.showImage("thanksgiving-main-course"));
		...
		public function haveTurkeyGirlDesertInYourCunt():void { //DESERT
			clearOutput();
			...
			outputText(images.showImage("thanksgiving-dessert"));
		...
		public function turkeyGirlTwoTheTurkeningBySavinWhatADickInAButt():void { //Turkey Girl II: Return of the Cockgobbler (Cockwielders) / {Thanksgiving Day event, nat. Cockmeisters only.}
			clearOutput();
			outputText(images.showImage("thanksgiving"));
		...
		public function helloAgain():void {
			...
			outputText(images.showImage("thanksgiving-hello-again"));
			...
			menu();
			addButton(0,"Next",gravyBoatDatTurkturk); //Gravy boat?
		}

		public function gravyBoatDatTurkturk():void { //Gravy Boat
			clearOutput();
		...
		public function seasonHerDatTurkeyBitch():void { //Season Her
			clearOutput();
			...
			outputText(images.showImage("thanksgiving-season"));
		...
		public function turkeyDesertBitches():void { //Dessert
			clearOutput();
			...
			outputText(images.showImage("thanksgiving-dessert-bitches"));
			...
			outputText("\n\nThe squawking turkey seems to be depthless, easily swallowing up every " + UnitSystem.literalInch() + " of your " + player.multiCockDescriptLight() + " until ");
			...
			outputText(" begin");
			if (player.cockTotal() == 1) outputText("s");
			outputText(" to drool spunk, pooling onto the hungry ground with the " + UnitSystem.literalGallons() + " of spilt gravy and fem-spooge.");
		...
		public function getARoastPiggueOinkOinkOinkMotherfucker():void { //{Return to The Wet Bitch menu} / Ham Roast -> In For A Gem
			clearOutput();
			player.gems -= 1;
			...
			outputText(images.showImage("thanksgiving-pigslut"));
		...
		public function morePigRoast():void {
			clearOutput();
			outputText(images.showImage("thanksgiving-pigslut-more"));
		...
		public function tenderizeDatPigSloot():void { //[ Tenderize Her ] / Ham Roast -> Tenderize Her (Female and Genderless versions)
			clearOutput();
			outputText("\"<i>Turn over.</i>\" you say rather plainly, eager to see that jiggling booty of hers.");
			outputText(images.showImage("thanksgiving-pigslut-tenderize"));
		...
		public function tenderizeHerHardcore():void { //Ham Roast -> Tenderize Her -> Hardcore
			clearOutput();
			outputText(images.showImage("thanksgiving-pigslut-tenderize-hardcore"));
			...
			dynStats("lus", 25, "scale", false);
			menu();
			addButton(0,"Next",spankEpilogueColonYoureAHorriblePersonForAbusingADumbPigGirl); //{Route to: here}
		}

		public function hamRoastTenderizeHerSoftcore():void { //Ham Roast -> Tenderize Her -> Softcore
			clearOutput();
			outputText(images.showImage("thanksgiving-pigslut-tenderize-softcore"));
		...
		public function spankEpilogueForRizzles():void {
			clearOutput();
			outputText(images.showImage("thanksgiving-pigslut-spank-epilogue"));
			outputText("You look around the room once again, searching for more toys you could use on the masochistic piggy slut. Your eyes rest on a jumbo-sized, horsecock dildo sticking up like a sore thumb out of the rest of the stuff. Someone even had the audacity to put a hat on it! You grab the monstrous thing, mottled black and pink all over with an artificial sheath near the base. You reason it's over " + UnitSystem.displayFeetEstimateTextually(2) + " long and incredibly thick, its huge weight making it so that you have to carry it with both hands!");
			...
			outputText("\n\nIt easily sinks in a couple of " + UnitSystem.literalInches() + " into her ass. A lusty, sex-dripping \"<i>Oink...</i>\" escapes her mouth as you shove the dildo in deeper, effortlessly reaching the " + UnitSystem.displayFootEstimateWithHyphenTextually(1) + " mark. You chuckle to yourself; she's probably had a couple minotaurs in her booty today... especially judging by the squishing sounds you hear from her spunk-filled insides.");
		...
		public function getDoneEatingOutPigSlut():void {
			clearOutput();
			outputText(images.showImage("thanksgiving-pigslut-ending"));
