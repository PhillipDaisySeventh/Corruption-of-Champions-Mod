...
public function keltEncounter():void {
	...
	if (player.statusEffectv3(StatusEffects.Kelt) <= 1) keltFirstTime(); //If First Encounter
	else { //Repeated encounter
		if (player.statusEffectv3(StatusEffects.Kelt) <= 3) { //Second/Third Events - Normal
			keltMainEncounter();
			return;
		}
		if (player.statusEffectv2(StatusEffects.Kelt) >= 130) { //Bad Ends
			if (((player.lib + player.lust) < 30 && player.inte >= 50) || !player.hasStatusEffect(StatusEffects.KeltBadEndWarning)) {
				...
				outputText(images.showImage("kelt-dream"));
		...
		if (player.isTaur() && player.statusEffectv2(StatusEffects.Kelt) >= 100 && player.gender > 1) { //Centaur bad end
		...
		if (player.statusEffectv3(StatusEffects.Kelt) == 4 && !player.hasStatusEffect(StatusEffects.NakedOn)) { //Naked event if its time for it
			keltRequiresNakedness();
			return;
		}
		if (player.statusEffectv2(StatusEffects.Kelt) >= 40 && !player.hasStatusEffect(StatusEffects.KeltBJ)) { //60+ Submissiveness—First Time Blowjob Requirement
			keltRequiresBlowjobs();
			return;
		}
		//Remaining events
		if (player.statusEffectv3(StatusEffects.Kelt) > 4) //75+ Submissiveness, 60+ Lust—Lust Encounter
			keltMainEncounter();
	}
}
//Introduction
private function keltFirstTime():void {
	...
	outputText("Wariness growing, you slow down.  But it is too late... the centaur has seen you.  With a snort, he gallops closer, imposingly tall.  You drop into a fighting stance, ready if he should attack you, and the centaur draws to a halt " + UnitSystem.aFewFeet() + " away.\r\rHe looks scornfully at you, and laughs, ");
...
private function youBadEndKeltForGood():void {
	clearOutput();
	outputText(images.showImage("item-kelt'sBow"));
...
//	player.removeStatusEffect(StatusEffects.Kelt);
	player.createStatusEffect(StatusEffects.KeltOff, 0, 0, 0, 0); //Kelt never encountered again
	dynStats("cor", 10);
...
private function keltRequiresNakedness():void {
	...
	if (player.cor + player.lib + (player.lust * 100 / player.maxLust()) >= 180) { //Corruption higher than 60 automatically chooses eagerly
	...
	if (player.inte > 40 && (player.isCorruptEnough(70) || player.findPerk(PerkLib.Sadist) >= 0 || player.findPerk(PerkLib.Pervert) >= 0) && !player.isTaur()) {
		outputText("\n\n<b>If you fight back and take him down a peg, you might never see him again...</b>");
		addButton(3, "Turn Tables", keltResistance);
	}
	else addDisabledButton(3, "Turn Tables", "Someone hard and smart enough could probably catch him off-guard... Being a taur defenitely doesn't help, though.");
...
private function keltEagerlyGetNaked():void {
	...
	outputText("You " + player.clothedOrNaked("remove your top first, slowly revealing your " + player.allBreastsDescript(), "show off your " + player.allBreastsDescript()) + ".  Kelt is pacing around you, eyes locked on your chest hungrily.  As you let your top fall to the ground, he laughs mockingly, though, ");
	if (player.gender == 1 && player.biggestTitSize() < 1) outputText("\"<i>What are you, a girl?  Get on with it so I can see what I'm dealing with.</i>\"\r\r");
		...
		else if (player.biggestTitSize() <= 5) //Too small, A to DD:
			outputText("\"<i>Ha! No wonder you cover up like a sniveling human!  Any centaur maiden would be ashamed to go out in public with " + num2Text(player.totalBreasts()) + " mosquito bites like that!  What do they call you back home... 'Tiny Tits'?  Or maybe they all just assume you're a guy!</i>\"\r\r");
		else if (player.biggestTitSize() <= 11) //Medium, E to HHH:
			outputText("\"<i>Uh oh, boys... look out!  Miss Melons here has been putting on a little weight!  How did someone like you become an adventurer?  From the waist up, you're good for whoring and not much else!</i>\"\r\r");
		else //Big, Watermelon and beyond:
			outputText("\"<i>Oooh, doesn't that look yummy?  Be honest... you wanna learn to shoot?  You'll be slapping yourself in the tits with every shot!  Heh... I bet that's the way you like it too!  Time to change your profession, slut.  Wet nurse and milk cow... they're just about all you're suited for!</i>\"\r\r");
	}
	if (player.gender > 1) outputText(images.showImage("kelt-gn-eager-female"));
	else outputText(images.showImage("kelt-gn-eager-male"));
	...
	if (player.butt.rating < 6) //Too small, Firm to Shapely:
		outputText("\"<i>Well, someone works out!  Got a nice, tight little ass there! Probably a little too tight, huh?  Don't worry, you can tell me... do the mean old monsters hurt when they fuck that poor little ass of yours? HA!</i>\"\r\r");
	else if (player.butt.rating < 13) //Medium, Large to Heavy:
		outputText("\"<i>Hey, nice little cushion you got back here!  Do you get that from sitting around all day?  Nah, can't be that.  I'm sure you get more than a workout on this baby... of course, cock is no substitute for a little exercise, you know!</i>\"\r\r");
	else //Big, Voluminous and beyond:
		outputText("\"<i>Now there's a rear end I could get behind!  For someone who claims to be a hero, you sure are built like a fucktoy!  With an ass like that, you remind me of my last lay... of course, she was a horse!</i>\"\r\r");
	...
	if (player.totalCocks() == 0) //No Cock:
		outputText("\"<i>Well, aren't you a pretty little thing, all together.  You and I will get along real nice, that's for sure!</i>\"\r\r");
	else {
		outputText("and almost inevitably, his eyes drop down to your " + player.cockDescript(0) + ".  He snorts, "); //Any Cock:
		if (player.cocks[0].cockType == CockTypesEnum.HUMAN) //Human Cock:
			outputText("\"<i>Heh.  Just sporting the normal model, huh?  You know there's a phrase around here... hung like a human!  Trust me... it's not a compliment.  ");
		if (player.cocks[0].cockType == CockTypesEnum.DOG) //Dog Cock:
			outputText("\"<i>Oh, lookie here!  We got ourselves a little doggie dong!  Well, cock or not, you'll always be a bitch to me, mutt.  ");
		if (player.cocks[0].cockType == CockTypesEnum.HORSE) //Horse Cock:
			outputText("\"<i>Now that's just sad.  Getting a little envious of me, huh?  Had to go out and get a nice horsecock all your own?  Should have asked.  I'd have given you a taste of mine!  ");
		if (player.cocks[0].cockType.Index >= 3) //Tentacle Cock or other weirdness:
			outputText("\"<i>Hah!  I'll bite... what the fuck are you supposed to be?  That a cock, or just an ugly, misplaced tail?  HA!  ");
		if (player.cocks[0].cockLength <= 10) //Small Size, ?-10 inches:
			outputText("\"So, when you poke a woman with that, does she notice?  Or do you have to tell her when to start faking her orgasm?</i>\"\r\r");
		else if (player.cocks[0].cockLength <= 20) //Medium Size, 11-20 inches:
			outputText("\"Still, at least you've got a decent sized cock.  You know, for a colt.  Maybe you'll get lucky, and the real women will take pity on you!</i>\"\r\r");
		else //Big Size, 21 inches and beyond:
			outputText("\"Now if only they could find a way to get you to stop stepping on your own cock every other step, maybe you'd be a real man!  Can you even get it up, at this point?  Or would that tip you over?  Ha!</i>\"\r\r");
	}
	...
	player.createStatusEffect(StatusEffects.NakedOn,0,0,0,0); //Naked on. Every visit, player will automatically strip
	player.addStatusValue(StatusEffects.Kelt,2,10); //+10 Submissive
...
private function keltReluctantlyGetNaked():void {
	...
	outputText("You " + player.clothedOrNaked("remove your top first, slowly revealing your " + player.allBreastsDescript(), "show off your " + player.allBreastsDescript()) + ".  Kelt is pacing around you, eyes locked on your chest hungrily.  As you let your top fall to the ground, he laughs mockingly, ");
	if (player.gender == 1 && player.biggestTitSize() < 1) outputText("\"<i>What are you, a girl?  Get on with it so I can see what I'm dealing with.</i>\"\r\r");
	else {
		if (player.biggestTitSize() == 0) outputText("\"<i>Oops!  Could have sworn I'd find a pair of tits on you.  That's okay... I'm sure you'll hit puberty some day!</i>\"\r\r");
		else if (player.biggestTitSize() <= 5) //Too small, A to DD:
	...
	outputText(images.showImage("kelt-gn-reluctant"));
	outputText("Even with his harsh comments, you can see the lust in Kelt's eyes.  He is obviously enjoying seeing your naked flesh.  Despite the embarrassment, you are determined to not let his remarks get to you.  His critique only becomes more lewd as you continue, though.  " + player.clothedOrNaked("As you remove the lower half of your clothes, stripping completely naked", "As you perform a pelvic thrust and slowly spin around") +", he eyes your ass and lets out a crude hoot of scornful delight, ");
	if (player.butt.rating < 6) //Too small, Firm to Shapely:
		outputText("\"<i>Well, someone works out!  Got a nice, tight little ass there! Probably a little too tight, huh?  Don't worry, you can tell me... do the mean old monsters hurt when they fuck that poor little ass of yours? HA!</i>\"\r\r");
	else if (player.butt.rating < 13) //Medium, Large to Heavy:
		outputText("\"<i>Hey, nice little cushion you got back here!  Do you get that from sitting around all day?  Nah, can't be that.  I'm sure you get more than a workout on this baby... of course, cock is no substitute for a little exercise, you know!</i>\"\r\r");
	...
	player.addStatusValue(StatusEffects.Kelt,2,7); //+7 Submissive
	dynStats("lus", int(player.lib/10) +5);
	keltMainEncounter2();
}
//Blowjob Requirement
private function keltRequiresBlowjobs():void {
	...
	if (player.cor + player.lib100 + player.lust100 >= 200 && player.inte < 60) { //Corruption higher than 80 automatically chooses Eagerly
...
private function keltBlowjobRequirementShamefully(newl:Boolean = true):void {
	...
	if (player.gender > 1) outputText(images.showImage("kelt-bj-shameful-female"));
	else outputText(images.showImage("kelt-bj-shameful-male"));
	...
	player.addStatusValue(StatusEffects.Kelt,2,7); //+7 Submissiveness
	doNext(camp.returnToCampUseOneHour);
}
//Blowjob Requirement, Eagerly
private function keltBlowjobRequirementEagerly(newl:Boolean = true):void {
	spriteSelect(SpriteDb.s_kelt);
	if (newl) clearOutput(); //This is never called with arguments, so newl will always be true. Can this be simplified?
	player.slimeFeed();
	...
	if (player.gender == 3) outputText(images.showImage("kelt-bj-eager-herm"));
	else if (player.gender == 2) outputText(images.showImage("kelt-bj-eager-female"));
	else outputText(images.showImage("kelt-bj-eager-male"));
	outputText("\"<i>Ahhh, yeah... fuck, bitch, you sure feel pretty good.  I could get used to this.  How about you, huh?  You like the taste of my cock, little whore?</i>\"\r");
	...
	outputText("Your diligence is soon rewarded.  With a grunt of need, Kelt shoves forward one last time, burying almost half of his nearly " + UnitSystem.displayFootEstimateWithHyphenTextually(3) + " length deep inside your throat as his cock tenses, then explodes.  You are paralyzed with ecstasy, split open by the massive cock as it begins unloading thick blasts of salty cream deep into your stomach.  Your throat is spread so tightly you can feel every splurt pulse down the length of his erection, again and again, ceaselessly.  Your stomach is bloated moments later, but still his orgasm continues, your belly visibly bulging periodically as another stream of seed is planted deep inside you.");
	player.refillHunger(50);
	...
	player.orgasm('Lips',false);
	player.createStatusEffect(StatusEffects.BlowjobOn,0,0,0,0); //Blowjob On
	player.addStatusValue(StatusEffects.Kelt,2,15); //+15 Submissiveness
	doNext(camp.returnToCampUseOneHour);
}
//Normal Encounter
private function keltMainEncounter():void {
	...
	if (player.statusEffectv2(StatusEffects.Kelt) <= 30) //Submissive 0-30:
		outputText("You grind your teeth in irritation, but swallow your pride enough to ask him for help.  ");
	else if (player.statusEffectv2(StatusEffects.Kelt) <= 70) //Submissive 30-70:
		outputText("You nod reluctantly, and Kelt grins.  He may be arrogant, but he is rather good.  ");
	else //Submissive 70-100:
		outputText("You nod enthusiastically, almost begging him to teach you.  ");
	if (player.statusEffectv3(StatusEffects.Kelt) <= 3) //Before Naked Requirement
		...
		doNext(keltMainEncounterAfterNakedReq);
		return;
	}
	doNext(keltMainEncounter2); //Continue encounter stuff
}
//After naked requirement
private function keltMainEncounterAfterNakedReq():void {
	if (player.hasStatusEffect(StatusEffects.NakedOn)) { //Naked On
		...
		player.addStatusValue(StatusEffects.Kelt,2,3); //+5 Submissive
	}
	else { //Naked Off
		if (rand(10) <= 5) { //40% Chance
			...
			if ((player.cor + player.lib100 + player.lust100 >= 180 && player.inte < 30) || player.statusEffectv2(StatusEffects.Kelt) >= 60) {
				...
				outputText("\r\r");
				player.addStatusValue(StatusEffects.Kelt,2,7); //+5 Submissive
			}
			//Otherwise
			else {
				...
				menu();
				addButton(0, "Yes", keltReluctantlyGetNaked); //+5 Submissive
				addButton(1, "No", keltRefuseNakedness); //Never event
				if (player.inte > 40 && (player.isCorruptEnough(70) || player.findPerk(PerkLib.Sadist) >= 0 || player.findPerk(PerkLib.Pervert) >= 0) && !player.isTaur()) {
					outputText("\n\n<b>If you fight back and take him down a peg, you might never see him again...</b>");
					addButton(2, "Turn Tables", keltResistance);
				}
				else addDisabledButton(2, "Turn Tables", "Someone hard and smart enough could probably catch him off-guard... Being a taur defenitely doesn't help, though.");
				addButton(4, "FIGHT!", kelly.fightToBeatKelt);
				return;
			}
		}
...
private function keltMainEncounter2():void {
	...
	if (player.hasKeyItem("Bow") < 0) { //No bow equipped
		outputText(images.showImage("item-bow"));
		...
		outputText(images.showImage("item-bow"));
		outputText("Kelt sneers as he looks at your bow, \"<i>You're still using that rotten old thing?  Well, it will do.  We'll just have to shoot for the close targets.</i>\"\r\r");
		outputText("Together, the two of you head off to the practice field.\r\r");
	}
	if (player.statusEffectv2(StatusEffects.Kelt) >= 60 && rand(4) == 0 && player.hasStatusEffect(StatusEffects.KeltBJ)) { //IF BLOWJOB HAS HAPPENED ALREADY, chances to repeat
...
private function keltMainEncounter3():void {
	clearOutput();
	var temporary:Number = 0;
	if (!player.hasStatusEffect(StatusEffects.NakedOn)) { //Clothed
		if (player.gender > 1) outputText(images.showImage("kelt-archery-female"));
		else outputText(images.showImage("kelt-archery-male"));
...
//		player.addStatusValue(StatusEffects.Kelt,1,5+rand(4));
		...
		if (player.inHeat && player.gender > 1) { //Naked, Player in Heat
			...
			if (player.statusEffectv2(StatusEffects.Kelt) <= 30) //Submissive, 0-30:
				outputText("You struggle as best as you can, but Kelt weighs a good deal more than you do.  As his thrusting hips anxiously press his cock to your nether-lips, you realize this is going to happen, whether you want it to or not.  The thought fills you with an undeniable shiver of pleasure.\r\r");
			else if (player.statusEffectv2(StatusEffects.Kelt) <= 70) //Submissive, 30-70:
				outputText("You put forth a token effort to escape, but it is obvious from the beginning that there is no way to get out from under the heavy weight of the centaur.  Besides, the desire running through you is palpable... in a way, you want this to happen.  So much so that as Kelt is thrusting, trying to line up his cock, you raise your hips to help him out, silently longing for penetration.\r\r");
			else //Submissive, 70-100:
				outputText("The masculine scent of him is overpowering, desperate.  Your body wants nothing more than to submit to Kelt's ferocious desires, and your mind agrees.  Eagerly, you thrust your hips up, reaching back with one hand to guide his cock to your wet pussy.  Kelt snorts his approval, and you feel a shivering tremor of lust run through you.\r\r");
			outputText("The centaur's cock has truly equine proportions, being easily " + UnitSystem.display("two and a half feet", UnitSystem.displayFeetEstimateTextually(2.5)) + " long and over " + UnitSystem.displayInchesEstimateTextually(3) + " thick.  ");
			if (player.vaginalCapacity() <= 16) //Vagina, Small
				outputText("The fit seems impossible, but Kelt could care less about your comfort, and shoves the flared head into you without hesitation.  You let out a scream, feeling like you are being torn apart by his cock as it rudely presses against your insides, spreading you wide open.  Kelt thrusts with growing frustration, but can fit no more than half his cock inside of you, despite his best efforts.  Every twitch makes you cry out as your tight pussy squeezes and milks that massive organ.\r\r");
			else if (player.vaginalCapacity() <= 40) //Vagina, Medium
			...
			player.addStatusValue(StatusEffects.Kelt,2,5); //+5 Submissive
			player.knockUp(PregnancyStore.PREGNANCY_KELT, PregnancyStore.INCUBATION_CENTAUR, 50); //Pregnancy Chance
...
//			if (player.pregnancyType == PregnancyStore.PREGNANCY_KELT) trace("PLAYER GOT KNOCKED UP BY KELT");
		...
		if (temporary <= 2) { //Naked, 60% Chance
			outputText(images.showImage("kelt-archery-naked"));
			...
			if (player.statusEffectv2(StatusEffects.Kelt) <= 30) //Submissive, 0-30:
				outputText("You try to ignore the foul remarks, telling yourself that this is simply the way he is.  It does not help, though, that at times you feel Kelt's eyes wandering across you lustfully.  At least some of his comments are not mockeries, but suggestions.  The entire experience makes you feel a little more uncomfortable around the abusive centaur.");
			else if (player.statusEffectv2(StatusEffects.Kelt) <= 70) //Submissive, 30-70:
				outputText("Despite yourself, some of his cruder comments make you blush.  By now, you're getting used to the oft times depraved sexuality of the demon world... but it is a little humiliating to subject yourself to this kind of treatment... and, to your shame, sometimes it's a little arousing.  Though Kelt is insulting, cruel, and crude, you also notice real lust in some of his glances.  By the end of the lesson, you are flushed with arousal as well as exertion.");
			else //Submissive, 70-100:
				outputText("Of course, Kelt's words only distract you even more from hitting the target.  Not because you are angry... but because you are aroused.  Somehow, his lewd comments and crude jibes make you shiver with anticipation.  He's just so powerful, so masculine.  Kelt seems well aware of the effect he has on you, and once reaches out to slap your ass heartily.  By the end of the training, you feel intensely horny.");
			dynStats("lus", 10);
//			player.addStatusValue(StatusEffects.Kelt,1,4);
		...
		if (player.biggestTitSize() == 0 && temporary == 3) //No Breasts—Do standard Naked event
			temporary = 4;
		if (temporary == 3) { //Naked, 20% Chance
			outputText(images.showImage("kelt-boobs"));
			...
			outputText("\"<i>Isn't it obvious?</i>\" he asks, grinning down at you in sardonic amusement. \"<i>You were way off balance.  Of course, I would be too, if I had a couple of extra " + UnitSystem.literalPounds() + " of fat hanging off my chest.</i>\"\r");
			...
			outputText("To your surprise, he suddenly leans forward, fist moving towards your head.  You raise your arms to deflect the blow, but he changes tactics suddenly and grabs one of your " + player.breastDescript(0) + " instead.  You stiffen, but before you can react further, he squeezes them brutally, mauling your breasts roughly with his hands.\r\r");
			if (player.biggestTitSize() <= 5) //Small Size, A-DD:
				outputText("\"<i>Ha!  Even with your itty-bitty-titty, you have to admit to a certain... weakness, is it?  Awfully sensitive, aren't they?  Ooh, am I making the little girl wet?  Naughty slut!</i>\"\r\r");
			else if (player.biggestTitSize() <= 11) //Medium Size, E-HHH:
				outputText("\"<i>Just look at these mommy melons!  You want to be an archer?  I'm amazed you don't slap yourself in the tits with every shot!  Easy target to grab onto.  But hey, I bet you like it that way.  Like it when people grope these fat titties of yours?</i>\"\r\r");
			else //Big Size, Watermelon and beyond:
				outputText("\"<i>Look at you! You're a miracle of science, you are... any other creature would break their back, trying to haul these milk bags around!  Tell the truth, now... which one of your parents was actually a cow?</i>\"\r\r");
			outputText("Despite his cruel words, you can't help but groan a little bit as he brutalizes your sensitive chest.  Kelt seems to take a good deal of pleasure in how helpless you are, pinching and flicking your " + player.nippleDescript(0) + "s.\r\r");
			if (player.biggestLactation() > 1) { //Milk
...
//			player.addStatusValue(StatusEffects.Kelt,1,4);
		...
		if (temporary == 4) { //Naked, 20% Chance
			outputText(images.showImage("kelt-archery-naked"));
			outputText("Nothing you do today seems to please your tutor, however.  He roars at you for every mistake, explodes at every misfire, and merely sneers contemptuously at every actual hit.  While his contempt for your efforts is nothing new, he seems particularly agitated today.\n\nHis aggressive behavior begins to distract you, and finally, you make a huge mistake.  Your shot goes flying, nowhere near the target, and Kelt is suddenly right behind you.\r\r");
			outputText("\"<i>You stupid slut!</i>\" he roars out, backhanding you with casual contempt.  You stumble to the ground, dazed but uninjured, as he rants.  \"<i>Hit the FUCKING target! \nIt's not hard, you dirty whore!  Maybe if you weren't so focused on FUCKING DICK you'd be able to get a single shot in!  Stand up... I said stand up, fucker!</i>\"\r\r");
			...
			outputText("Groaning, distracted and unnerved, you hurriedly line up the shot as best as you can.  Kelt's fingers flex, forcing you to stand up straighter.  Your arm shakes as you take aim, and you can feel Kelt's hot breath on your hair, and then the arrow is flying!  Ten " + UnitSystem.literalYards() + "... five... bullseye!\r\r");
...
private function keltMainEncounterPostBlowjob():void {
	clearOutput();
	if (player.hasStatusEffect(StatusEffects.BlowjobOn)) { //Blowjob Requirement On
		...
		if (player.lust100 >= 75 || player.statusEffectv2(StatusEffects.Kelt) >= 90 && rand(2) == 0) {
			...
			if (player.totalCocks() > 0) //Penis:
				outputText("  Your " + player.cockDescript(0) + " is rock hard, but he ignores it almost contemptuously.  You almost feel ashamed of it, compared to the slowly-growing manhood between his legs.");
			if (player.hasVagina()) //Vagina:
				outputText("  For a moment, his fingers trace the line of your exposed " + player.vaginaDescript(0) + ", giving you the slightest of warnings before he crudely jams two fingers deep inside, as though scouting out your depths.  You whimper urgently at the treatment, and more so as Kelt removes his fingers, licking with obvious pleasure.");
			...
			outputText("\r\r");
			if (player.gender > 1) outputText(images.showImage("kelt-farm-smallbarn-female"));
			else outputText(images.showImage("kelt-farm-smallbarn-male"));
			...
			if (player.totalCocks() > 0) //Penis:
				outputText("  Beneath you, pressed firmly into the harsh wood of the mounting board, your " + player.cockDescript(0) + " erupts, splattering your stomach with your own cum.  As the thick semen slides down towards your face, you begin slipping on your own warm seed, rocking back and forth with each harsh pounding Kelt delivers to your backside.");
			if (player.hasVagina()) //Vagina:
				outputText("  Your poor, neglected pussy quivers with delight, convulsing without even being touched.  Thick juices run freely down your leg, dripping off to splatter the hay below you as you moan like a bitch in heat.");
			if (player.gender == 0) //Genderless:
				outputText("  Never before has a complete lack of genitalia been so frustrating... or pleasurable.  Not being able to physically cum, the explosions of climax simply rack up within your body... waves of cascading pleasure with no release and no mercy.");
			outputText("\r\rIf anything, your orgasm only seems to spur Kelt on, causing him to push deeper and faster.  The idea of fitting his massive, nearly " + UnitSystem.displayFootEstimateWithHyphenTextually(3) + " cock into your bowels seems ludicrous, but the aggressive centaur obviously has no intention of taking 'no' for an answer.  With each powerful thrust of his hips, he sinks more cock deep within your ass, filling you, stretching you, ripping you open with still more to come.  It is almost a terrible relief when you feel his massive testicles collide with your ass, buried at last to the hilt. Even in victory, Kelt is snide.  ");
			...
			player.addStatusValue(StatusEffects.Kelt,2,10); //+10 Submissiveness
			player.orgasm('Anal');
			dynStats("cor", 1);
			return;
		}
		//Otherwise
			...
			player.slimeFeed();
			player.addStatusValue(StatusEffects.Kelt,2,5); //+5 Submissiveness
//			player.addStatusValue(StatusEffects.Kelt,1,3);
		...
		//Otherwise
			...
			doNext(keltTryToResist);
			return;
		}
	}
	keltMainEncounter3();
}
private function keltTryToResist():void {
	spriteSelect(SpriteDb.s_kelt);
	clearOutput();
	outputText(images.showImage("kelt-dream"));
...
private function keltResistGivingBJ():void { //Resist
	spriteSelect(SpriteDb.s_kelt);
	clearOutput();
...
private function keltSubmitGivingBJ():void { //Submit
	spriteSelect(SpriteDb.s_kelt);
	player.slimeFeed();
	clearOutput();
...
private function keltReluctantGivingBJ():void {
	...
	if (player.gender > 1) outputText(images.showImage("kelt-bj-reluctant-female"));
	else outputText(images.showImage("kelt-bj-reluctant-male"));
	outputText("\"<i>Alright, bitch.  Swallow my cock.</i>\"\r");
...
//Bad Ends
private function keltCentaurBadEnd():void { //Centaur
	spriteSelect(SpriteDb.s_kelt);
	clearOutput();
	...
	if (player.gender == 1) { //Male
		...
		outputText(images.showImage("kelt-lost-male-taur"));
	...
	if (player.gender == 0) { //Genderless
		...
		outputText("His legs rear suddenly, wrapping around the barrel of your body.  You let out a startled protestation, but do not move... your body won't let you.  In fact, if anything, you push back against him, eager and ready.  Part of you is joyous, in fact.  What you wanted wasn't to fuck Kelt!  ");
		outputText("<b>... It was for him to fuck you!</b>\r\r");
		outputText(images.showImage("kelt-lost-genderless-taur"));
	...
	if (player.gender >= 2) { //Female, Hermaphrodite
		if (player.gender == 3) { //Hermaphrodite
			clearOutput();
		...
		outputText("<b>... It was for him to fuck you!</b>\r\r");
		outputText(images.showImage("kelt-lost-female-taur"));
...
private function keltSubmissiveBadEnd():void {
	...
	if (player.gender > 1) outputText(images.showImage("kelt-lost-female"));
	else outputText(images.showImage("kelt-lost-male"));
...
private function keltBadEndEpilogue():void {
	...
	outputText(images.showImage("badend-kelt"));
	outputText("\"<i>Enjoying the show, fucker?  She's not bad, for a quick fuck now and then.  A little clingy, but makes for a good cumdump.  What... you jealous?</i>\"\r");
...
private function keltResistance():void {
	...
	if (player.hasMuzzle()) outputText("a low growl building in the back of your throat");
...
private function keltResistancePussyOut():void {
	...
	player.addStatusValue(StatusEffects.Kelt,2,2); //+2 submission
	doNext(camp.returnToCampUseOneHour);
}
...
private function fuckKeltsShitUp():void {
	spriteSelect(SpriteDb.s_kelt);
	clearOutput();
	if (player.face.type == Face.SNAKE_FANGS && player.tongue.type == Tongue.SNAKE && player.lowerBody.type == LowerBody.NAGA) { //If naga folks
		...
		if (player.hasCock()) {
			...
			outputText(images.showImage("kelt-farm-subkelt-naga-female"));
			outputText("Girl cum begins to drip down Kelt's chin.  He is now entirely engaged with your " + player.vaginaDescript(0) + ", moving his head back and forth as his tongue slathers your clit with attention, perhaps in the hope that the sooner he gets you off the sooner this nightmare will end.  You smirk and grip his hair, forcing his head this way and that so that no corner of your pink opening goes without attention.\r\r");
		...
		if (player.gender == 0) {
			...
			if (player.isTaur()) outputText(images.showImage("kelt-farm-subkelt-male-taur"));
			else outputText(images.showImage("kelt-farm-subkelt-male"));
