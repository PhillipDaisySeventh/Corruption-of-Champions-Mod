...
private function auntNancyPoonPartII():void {
	...
	if (player.tail.type > Tail.NONE) outputText(" to the top of your tail");
	outputText(".\n\n");
	...
	//[If you have balls:] 
	if (player.balls > 0) {
		outputText("She smiles at the sight of your " + player.ballsDescriptLight() + ", gently raising two of her hands to fondle and squeeze them as her soft lips press against them, ");
		//[If BallSize allows]
	...
	if (player.hasCock()) {
		...
		if (player.cockTotal() >= 2) {
			if (player.cocks[y].cockLength >= 22) {
				...
				outputText("Still rising and falling atop your body as she rides you, Aunt Nancy grips her breasts with two of her black-shelled hands and begins to slide them up and down on either side of your tall, dripping " + player.cockDescript(y) + ", ");
				...
				outputText("The sensation, both of her soft, pillowy bosom wrapped around your " + player.cockDescript(y) + " and of her skillful tongue");
		...
		if (!titFucked || (titFucked && player.cockTotal() <= 2)) {
			outputText("Her remaining hands gently work their way across your body, alternating between rubbing and squeezing your " + player.buttDescript() + ", even sometimes slipping fingers inside to probe the insides, ");
			//[If the player has balls]
			if (player.balls > 0) outputText("gently kneading and caressing your " + player.ballsDescriptLight() + " as the cum inside churns and boils and ");
			//[If the player has a vagina]
			else if (player.hasVagina()) outputText("exploring the folds and creases of your " + player.vaginaDescript(0) + " and ");
	...
	//[If player has vagina but no dicks: (Nancy prefers dicks and will always choose them over lesbian sex)]
		...
		if (player.tail.type > Tail.NONE) outputText("tail before rising to your ");
...
private function auntNancyPoonPartIII():void {
	...
	if (player.hasCock()) {
		var x:Number = player.shortestCockIndex();
		var y:Number = player.longestCock();
		outputText("You can feel it when the spider-lady's pussy begins to quiver in delight. Aunt Nancy soon moans, then cries out in incredible pleasure.  The lower half of her eyes all close, while the others all roll up into her head in ecstasy.  \"<i>Yes!  YEEEEESSSSSSS!</i>\" she hisses with throaty, feminine indulgence.  The tiny feelers in her otherwise silky-smooth vagina suddenly begin to squeeze and pull at your " + player.cockDescript(x) + ", and you let out a bellow of pleasure of your own as you feel your ");
		//[If PC has balls]
		if (player.balls > 0) outputText(player.ballsDescriptLight() + " tense, churning and tightening.");
		//Else:]
		...
		if (player.hasVagina()) {
			outputText("Your " + player.vaginaDescript(0) + " clenches, tightly, ");
	...
	//[Else:]
		...
		//[If PC has a vagina:] 
		if (player.hasVagina()) outputText(player.vaginaDescript(0) + " and ");
		outputText(player.assholeDescript() + ".  You feel worn out, like you just ran a long mile, even though Aunt Nancy was doing most of the work, and you let your arm fall back down against the sticky surface of her web.  However, she seems fine, smiling down at you with a big, happy grin.");
		//[If PC has a vagina:] 
