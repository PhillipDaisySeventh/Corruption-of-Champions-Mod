...
private function useTheSexMachine():void {
	...
	//[If you decide to mess with the machine, female:]
		...
		//[Female repeat scene]
			...
			outputText("You moan lustfully as the members slip into your " + player.vaginaDescript() + " and " + player.assholeDescript() + ", filling you deeply.");
			player.cuntChange(30,true,true,false);
			player.buttChange(30,true,true,false);
	...
	//[Hermaphrodite]
		...
		if (flags[kFLAGS.TIMES_USED_SEX_MACHINE_AS_HERM] == 0) {
			...
			if (player.biggestTitSize() >= 2) outputText("  Well, not every part of you, but that is rapidly fixed as a final cock rubs against your " + player.chestDesc() + ", starting to stimulate your chest as well.");
			outputText("\n\n");
			...
			if (player.biggestTitSize() >= 2) outputText("  The cock on your chest pumps cum onto your " + player.chestDesc() + ", as y");
			else outputText("  Y");
