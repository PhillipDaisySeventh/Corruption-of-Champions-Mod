...
public function edrynBarTalk():void {
	...
	if (player.isTaur() && player.totalCocks() > 0) {
		...
		outputText("A soft hand slides under your belly, hefting your " + player.cockDescript(0) + " and smearing the bead of pre over it with long slick strokes.  She coos, \"<i>");
		dynStats("lus", 70);
		edrynOffer();
		return;
	}
	...
	if (player.isTaur() && player.totalCocks() > 0) {
		...
		//Normal Wingdangdoodle
			...
			if (player.cockArea(x) >= 24) {
				...
				outputText("Edryn keeps talking, \"<i>I do ALL kinds of mercenary work, even the sloppy wet kind, and lucky for you, I'm more than wet enough to let you satisfy your " + player.cockDescript(0) + " in me.  ");
				if (cost > 0) outputText(Num2Text(cost) + " gems ");
				else outputText("An hour of your time ");
...
private function fuckEdrynTaur():void {
	...
	outputText("Edryn looks over her shoulder at you and winks as she relaxes her internal muscles.  Unprepared for the sudden release, you slide a few " + UnitSystem.literalInches() + " out of her before your hind-legs can steady you.  The centauress teases, \"<i>Are you sure you can handle fucking this way?  Like a rutting beast?  You look like you're about to fall over.  Here, grab onto the rings hanging from the ceiling and steady yourself.  You'll need all the leverage you can get to satisfy a horny girl like me.</i>\"\n\n");
...
public function helAppearance():void {
	outputText("\n\nTo your surprise, you see Hel the salamander sitting in a corner table, a pair of foxy fox-morph girls sitting on her lap.  When she sees you enter, the pretty reptile lifts her tankard and shouts, \"<i>Hey! " + player.short + "! Over here!</i>\" over the loud noises of the bar.");
...
public function fuckPregEdryn():void {
	...
	//NONTAUR
	if (!player.isTaur()) {
		outputText("Edryn lurches forwards as soon as the door closes behind you, slamming her powerful frame into you with enough force to propel you several " + UnitSystem.literalFeet() + " back onto a large pile of pillows in the corner.  The feeling of her massive, milk-drooling teats ");
		...
		outputText("You tear off your " + player.armorName + " in a flash, fully exposing your ");
		...
		outputText("Edryn sinks down, her whole body shaking and shuddering while milk fountains from her swollen breasts.  Her hands reach up to grab her nipples and tug on the milk-spouts.  They look longer now than before her pregnancy.  Her bloated milk-spouts are at least " + UnitSystem.anInchOrTwo() + " long!  ");

		if (player.cumQ() < 100) {
			outputText("Your " + player.cockDescript(x) + " dumps the last of its seed into her hungry cunt and softens slightly.  Her warm cunt's constant milking motions continue, still pleasurable, but they prevent you from going soft until the centaur's orgasm has concluded.");
		}
		else if (player.cumQ() < 500) {
			outputText("Your " + player.cockDescript(x) + " spurts thick flows of seed into her hungry cunt, flooding her snatch with an abnormally large load of semen.  Her warm cunt's constant milking motions continue, forcing more and more of your spunk inside her until her own orgasm has concluded.");
		}
		else {
			outputText("Your " + player.cockDescript(x) + " erupts, pouring a thick wave of seed into the centaur's hungry cunt.  The constant milking motions don't let up while Edryn's orgasm continues, helping you flood her with even more spooge.  Even after your pregnant lover's orgasm has concluded, your " + player.cockDescript(x) + " keeps dumping more cum into her until it's soaking and dripping from her huge, furry ass.");
		}
...
private function jizzFromEatingPregdrynOut():void {
	...
	outputText("your " + player.multiCockDescriptLight() + " in a steady stream.  Your urethra bulges and flexes, forcing you to waste your seed all over Edryn's pillows.");
	if (player.cumQ() < 1000) {
		...
		else outputText("ever greater amounts of liquid pleasure.  Jism drips through the floorboards as you create a puddle a few " + UnitSystem.literalInches() + " deep.");
	}
	outputText("\n\n");
