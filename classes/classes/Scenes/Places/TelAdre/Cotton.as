...
private function cottonFucksYou():void {
	...
	if (player.gender == 1) {
		...
		outputText("She reaches between her legs and lifts her cock, letting it rest on the small of your back while she wets it down with water from the shower and strokes it to its full length. You feel it inching across your back, growing hotter and harder. With a glance back, you estimate it must be at least " + UnitSystem.displayFeetEstimateTextually(2) + " long! You gulp and put your head down, clenching your teeth for the inevitable.\n\n");

		outputText("\"<i>Oh now pet, don't be so scared,</i>\" Cotton whispers into your ear, then lifts her cock from your back and places it at your " + player.assholeDescript() + ". She reaches forwards and you hear the squeak of the temperature nozzle being turned. Seconds later, the water pouring down on you gets hotter, causing your whole body to heat up in response. Using this time, Cotton presses forwards, her equine cock invading your " + player.assholeDescript() + " like a charging army. She thrusts in and out slowly, being careful not to hurt you.");
		...
		outputText("After a few minutes of this, neither of you can take much more. Both of you give a deep moan of orgasmic pleasure as your " + player.assholeDescript() + " clenches and you feel your equine lover's cock twitch and spasm within you, flooding your hole with her hot seed. Your own limp member shudders with orgasm, but instead of spurting, it leaks a small torrent of cum right down onto the floor.");
	...
	//(Get fucked, as Female)
		...
		outputText("\"<i>Oh now pet, don't be so scared,</i>\" Cotton whispers into your ear, then lifts her cock from your back and places it at your " + player.vaginaDescript(0) + ". She reaches forwards and you hear the squeak of the temperature nozzle being turned. Seconds later, the water pouring down on you gets hotter, causing your whole body to heat up in response. Using this time, Cotton presses forwards, her equine cock invading your " + player.vaginaDescript(0) + " like a charging army. She thrusts in and out slowly, being careful not to hurt you.");
	...
	//(Get fucked, as Herm)
		...
		outputText("\"<i>Oh now pet, don't be so scared,</i>\" Cotton whispers into your ear, then lifts her cock from your back and places it at your " + player.vaginaDescript() + ". She reaches forwards and you hear the squeak of the temperature nozzle being turned. Seconds later, the water pouring down on you gets hotter, causing your whole body to heat up in response. Using this time, Cotton presses forwards, her equine cock invading your " + player.vaginaDescript() + " like a charging army. She thrusts in and out slowly, being careful not to hurt you.");
		player.cuntChange(72,true,true,false);
		outputText("\n\n");
...
private function serviceFirstTimeCotton():void {
	...
	outputText("Leaning forward you give it a tentative lick along the side, and, to your surprise, find it to be quite sweet and pleasant. You lick again, then again, stroking it dutifully as you do so. It's not long until it's fully erect, at least " + UnitSystem.displayFeetEstimateTextually(2) + " long by your estimate. Rather than be intimidated, you're spurred on! After all, you've got a lot of cock to suck and only so much time.\n\n");
...
private function fuckCottonInShowerRepeat():void {
	...
	*/
	...
	if (player.isTaur()) {
		...
		if (player.hasVagina()) outputText("  Meanwhile, your " + player.vaginaDescript(0) + " behind you moistens both from the steam and from arousal, and your " + player.clitDescript() + " aches, craving attention.");
		outputText("\n\n");

		//(If PC has breasts)
		if (player.biggestTitSize() >= 2) {
			outputText("Cotton leans down, groping your " + player.biggestBreastSizeDescript() + ", taking one " + player.nippleDescript(0) + " into her mouth and sucking it greedily.");
			...
			else outputText("  Cotton soon switches to the other breast, teasing your " + player.nippleDescript(0) + " with her talented tongue.");
			outputText("\n\n");
		}
		...
		//(if player is too small (under 4</i>\")
			...
			outputText("</i>\" Letting a little smile cross your lips, you push your " + player.cockDescript(x) + " fully into her, not a big accomplishment for one of your size, but still enough to make you feel good.");
		}
		...
		//(If player is too big, add)
		if (player.cockArea(x) > 70) outputText("  Even with all the pounding you've been doing, you can't fit your entire girth into her, but she doesn't care. Her eyes seem to roll back and forth like it's hard to focus, and her tongue drips water, sweat and saliva onto her body. She's panting for breath and you are able to make out faint, \"<i>Oh gods, oh gods</i>\" over the running water. Were you able to see her toned stomach, you're sure you could see the outline of your " + player.cockDescript(x) + " in it. Her back arches under the shower, her red hair messily draped around her face and bench, and she bites her lip as another orgasm rocks her body.");
		outputText("\n\n");
		...
		//(If PC has a pussy)
		if (player.hasVagina()) outputText("  Your " + player.cockDescript(x) + " erupts inside Cotton's warm cunt which clenches you tightly, while your own femme sex twinges and clenches on air, drooling juices down your legs.");
		//(else)
		else outputText("   Your " + player.cockDescript(x) + " erupts inside Cotton's warm cunt which clenches you tightly.");
	...
	//(Repeat fuck her, for nagas)
		...
		//(If PC has a pussy, add the following)
		if (player.hasVagina()) outputText("Your " + player.vaginaDescript(0) + " meanwhile moistens both from the water and from arousal, and your " + player.clitDescript() + " aches, craving attention.");
		outputText("\n\n");
		...
		//(if player is too small (under 4</i>\") add)
			...
			outputText("</i>\" Giving her a little smile, you push your " + player.cockDescript(x) + " fully into her, not a big accomplishment for one of your size, but still enough to give you a little smile.");
		}
		//(otherwise add)
	...
	//(Repeat Fuck Her, for humanoid bodies)
		...
		if (player.hasVagina()) outputText("  Your " + player.vaginaDescript(0) + " moistens both from the water and from arousal, and your " + player.clitDescript() + " aches, craving attention.");
		outputText("\n\n");
...
private function cottonFucksYouInShowerRepeat():void {
	...
	if (player.isTaur()) {
		...
		//(If PC has breasts)
		if (player.biggestTitSize() >= 2) {
			outputText("Cotton's kisses lead down to your " + player.biggestBreastSizeDescript() + ", where she takes one " + player.nippleDescript(0) + " into her mouth, sucking it greedily and teasing it masterfully.");
			//(and if PC is lactating)
			if (player.biggestLactation() >= 1) {
...
public function cottonTantricSex():void {
	...
	if (player.hasCock()) outputText("  Meanwhile, your own " + player.cockDescript(0) + " hangs limply in front of you, tingling with arousal, but unable to harden. Cotton gives it a pat and says, \"<i>Worry not, pet, there's only room for one cock right now, but I'll want it some other time.</i>\"");
	outputText("\n\n");
	...
	//(If PC has a penis)
	if (player.hasCock()) outputText("  Your " + player.cockDescript(0) + " twitches and tingles, releasing its pent up seed in a dribbling torrent down your belly and chest, right into your face and pooling around your head.");
	//(If PC has a vagina)
	if (player.hasVagina()) outputText("  Your " + player.vaginaDescript(0) + " shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your womb.");
	//(If PC doesn't have a vagina)
	else outputText("  Your " + player.assholeDescript() + " shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your intestines.");
