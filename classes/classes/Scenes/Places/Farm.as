...
public function farmExploreEncounter():void {
	...
	//Repeat Offender
		...
		if (flags[kFLAGS.KELT_KILLED] >= 1) {
			outputText("As soon as you approach the farm, Whitney comes storming up to meet you. \"<i>What the fuck did you do?!</i>\"");
		...
		if (flags[kFLAGS.WHITNEY_FLIPPED_OUT_OVER_KELLY] != 1 || flags[kFLAGS.WHITNEY_GONE] > 0) {
			...
			if (player.hasStatusEffect(StatusEffects.Marble)) {
				addButton(4, "Marble", meetMarble).hint("Pay Marble the cowgirl a visit.").disableIf(flags[kFLAGS.MARBLE_WARNING] > 0, "You have resolved not to see Marble. There is still hope if you want to meet her again though.").disableIf(player.hasStatusEffect(StatusEffects.MarbleRapeAttempted), "Marble isn't too pleased that you tried to rape her. Best not to seek her at the moment.").disableIf(player.hasStatusEffect(StatusEffects.NoMoreMarble), "Unfortunately, Marble is gone so you won't be able to find her.");
			}
			if (player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") >= 0) {
				addButton(5, "Get Milked", getMilked).hint("Use the milker to drain your " + player.breastDescript(0) + ". Try not to use the milker too frequently.").disableIf(player.hasStatusEffect(StatusEffects.Milked), "Your " + player.nippleDescript(0) + "s are currently too sore to be milked. You'll have to wait a while.");
			}
			if (player.hasKeyItem("Cock Milker - Installed At Whitney's Farm") >= 0 && player.cockTotal() > 0) {
				addButton(6, "Milk Cock", cockPumping).hint("Use the cock milker to release your tension. Try not to overuse the milker though!");
			}
		}
...
public function getMilked():void {
	...
	//Repeat
		...
		if (player.hasStatusEffect(StatusEffects.LactationReduction) && player.totalCocks() == 0) outputText("  Your " + player.nippleDescript(0) + "s are engorged and ready to be taken care of.");
		//If cocks
		else if (player.totalCocks() > 0) {
			outputText("Your " + player.multiCockDescriptLight() + " erect");
		...
		if (player.hasStatusEffect(StatusEffects.LactationReduction) && player.cockTotal() > 0) {
			outputText("Your " + player.nippleDescript(0) + "s and " + player.multiCockDescriptLight() + " grow");
	...
	if (!player.hasStatusEffect(StatusEffects.BreastsMilked) || application == 0) {
		...
		outputText("You manage to grab the suction cups in spite of your constrictive bindings and pull them to your " + player.nippleDescript(0) + "s.  They latch on immediately, ");
		...
		outputText("There is a mechanical lurching noise as the suction builds rapidly.  Your nipple swells out to " + UnitSystem.displayInches(player.nippleLength*1.5) + " of length, turning purplish from the strain.   You can feel something welling up inside your " + player.allBreastsDescript() + ", building as it moves towards your " + player.nippleDescript(0) + "s.\n\n");
	}
	//Apply repeat alternate
	else if (application == 1) {
		outputText("You stretch down and grab onto the suction cups, pulling them up to your eager nipples.  They latch on, slapping tight against you as the vacuum pressure seals them tightly against your body.  You can feel your " + player.nippleDescript(0) + "s pulling tight, nearly doubling in size from the intense pressure.  ");
	...
	if (player.lactationQ() < 50) {
		dynStats("lus", 15);
		if (milksplosion == 0) {
			outputText("A few drops of milk bud on the tips of your " + player.nippleDescript(0) + "s, growing larger as they roll down to the edge of the tube.  It feels as if a ");
	...
	outputText("The machinery displays " + liters + " " + UnitSystem.liters() + " of milk");
	//If already at cap
	...
	if (liters > 2) {
		outputText("\n\nYou feel woozy and lightheaded from the intense milking, and have difficulty focusing on anything but the residue of fluids coating your " + player.allBreastsDescript() + ".");
...
public function cockPumping():void {
	...
	if (player.cumQ() >= 50 && player.fatigue >= player.maxFatigue() && flags[kFLAGS.USED_MILKER_TODAY] > 0) {
		...
		if (cumQ < 1000) {
			...
			outputText("as the machine measures your contribution, but it barely registers in the back of your mind as your " + player.multiCockDescriptLight() + " harden again from the unending, expertly-given stimulation of the animate tubes.  You try to pull the milker off your " + player.multiCockDescriptLight() + ", but with ");
			//[(big dix)
		...
		//(big skeet, ordinarily would be helper-assisted)
			...
			outputText("Jism splatters on the floor below you as you discharge rivers of spunk.  " + capitalizeFirstLetter(UnitSystem.liters()) + " upon " + UnitSystem.liters() + " of the stuff spill out of your throbbing " + player.multiCockDescriptLight() + ", coating the outside of the ejected suction tubes to match the inside.  Eventually you spend your reserve and the cum flow reduces to a trickle.  Stupid with pleasure, you don't notice the suction tube");
			if (player.cockTotal() > 1) outputText("s");
			...
			outputText("A loud 'ding' sounds and a clatter of gems drop into the plate as the machine measures your contribution, but it barely registers in the back of your mind as " + player.sMultiCockDesc() + " hardens again from the unending, expertly-given stimulation of the animate tube");
			if (player.cockTotal() > 1) outputText("s");
			outputText(".  You try to pull the milker off your " + player.multiCockDescriptLight() + ", but the outside of the tubing is slick with your semen");
			//[(big dix)
			if (player.biggestCockArea() > 70) {
	...
	//Medium Scene – output less than 250 mLs
		...
		if (player.cockTotal() == 1) {
			outputText("An orgasm hits you like an ocean wave, shutting down any remaining conscious thoughts and replacing them with one overriding emotion: relief.   You spurt helplessly into the tube, wracked with pleasure as your ");
			if (player.balls > 0) outputText("bloated " + player.ballsDescriptLight() + " are");
			else outputText("sensitive " + player.cockDescript(0) + " is");
			...
			outputText("An orgasm hits you like an ocean wave, shutting down any remaining conscious thoughts and replacing them with one overriding emotion: relief.   You spurt helplessly into the tube, wracked with pleasure as your ");
			if (player.balls > 0) outputText("bloated " + player.ballsDescriptLight() + " are");
			else outputText("sensitive " + player.multiCockDescriptLight() + " are");
	...
	cumQ = int(cumQ);
	outputText(UnitSystem.displayLitersShort(cumQ) + ".  ");
...
private function milkerBadEnd1():void {
	...
	//[(if vag)  
	if (player.hasVagina()) {
		outputText("  Another tube, likewise capped with a thick, firm end instead of a suction cup, reaches your " + player.vaginaDescript(0) + " and pushes inside, secreting more of the fluid.  ");
		player.cuntChange(15,true);
		//[(bigclit)
		if (player.getClitLength() > 3) outputText("A smaller suction tube fastens over your " + player.clitDescript() + " as it rouses from the vaginal stimulation.");
	}
	//[(lactation, no nipplecunts)
	if (player.biggestLactation() >= 1 && !player.hasFuckableNipples()) {
		outputText("  Some tubes of the cup-type latch onto your " + player.nippleDescript(0) + "s, then begin suction of their own. You feel your breastmilk shift toward the nipples, then begin to dribble into the tubes.");
	}
	//[(nipplecunts)
	if (player.hasFuckableNipples()) {
		outputText("  More tubes snake up to your " + player.biggestBreastSizeDescript() + " and work their way into your " + player.nippleDescript(0) + "s.");
	}
	...
	outputText("Under the effects of this new drug, " + player.sMultiCockDesc() + " inflates, engorging with blood until it's much larger than usual");
	//[(balls)
	if (player.balls > 0) outputText(" and your " + player.sackDescript() + " stretches as your " + player.ballsDescriptLight() + " nearly double in size");
	...
	if (player.hasVagina()) {
		...
		//[(big clit)
		if (player.getClitLength() > 3) outputText(", and your clitoris twitches under the cup, threatening to black you out from the pleasure");
		outputText(".");
	}
	//[(lactation) 
	if (player.biggestTitSize() > 1) {
		outputText("  Your milk sprays out of your " + player.nippleDescript(0) + "s");
		//[(nipplecunts)
	...
	if (player.cor < 90) {
		...
		outputText(" pushing massive loads of semen down the tubes and out of the release valves in a firework pattern. Whitney shrinks back as some of the spooge squirts across the barn and comes to a stop less than " + UnitSystem.aFoot() + " from her. The canine woman screams volubly enough to wake the entire farm. \"<i>Dammit, " + player.short + "; just... DAMMIT!</i>\" You spare her not even a thought as you anticipate your next injection of the tainted chemicals and work up another batch of jizz.");
...
private function centaurToysHoooooo():void {
	...
	outputText("\"<i>Oh, hey there, " + player.short + ",</i>\" Whitney says, leaning heavily on her pitchfork.  \"<i>Not much, just trying to figure out... Hey, now!</i>\" she says, eying up your powerful centaur frame.  ");
