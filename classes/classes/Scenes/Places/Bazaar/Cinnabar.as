package classes.Scenes.Places.Bazaar
{
	...
	public class Cinnabar extends BazaarAbstractContent 
	{
		...
		private function cinnabarGreeting():void {
			...
			if (flags[kFLAGS.CINNABAR_NUMBER_ENCOUNTERS] == 0) {
				...
				//(Big enough Cock) 
					...
					outputText("You walk over towards the rat-morph, and as you near she places her hands on her hips, sizing you up with her intense, blood-red eyes. She sidles up alongside you, caressing your body and giving a happy hum as her fingers trace the huge, rounded bulk of your bulge. The slutty rat coos, \"<i>Such a big... hot fuck-stick you've got there. You make me drip, " + player.mf("stud","baby") + ". Normally I'd charge some gems, but fuck it – I want ");
					//Multicocks if the PC has 2 big enough
					if (player.cockTotal() > 1 && player.biggestCockArea2() >= 75) outputText("those things");
					else outputText("that thing");
					outputText(" inside me... the name's Cinnabar, but I'd rather you plug me and call me whatever you want.</i>\" Her voice trails off into a husky, barely heard vibration as she awaits your response.");
					//[Fuck] [Leave]
					menu();
			...
			//Not first time
				...
				//(Big enough Cock) 
					...
					if (player.cockTotal() > 2 && player.biggestCockArea2() >= 75) outputText("those things");
					else outputText("that thing");
					outputText(" inside me... We could sit here and talk, but I'd rather you plug me and call me your slut.</i>\" Her voice trails off into a husky, barely heard vibration as she awaits your response.");
					//[Fuck] [Leave]
					menu();
					addButton(0, "Fuck Her", cinnabarGetsFUKKKKED);
					if (player.cockTotal() > 2 && player.biggestCockArea2() >= 75) {
		...
		private function cinnabarNonHugeDickings():void {
			...
			player.gems -= 50;
			statScreenRefresh();
			outputText("Once you give your assent, Cinnabar's paws dive into your " + player.armorName + ", gliding over your " + player.skin.desc + " before re-emerging above your gem-pouch. She deftly grabs her payment and snakes back through, tickling your belly with her soft, well-groomed fur. You look at her, then yourself, then her again. How the hell was she flexible enough to do that? Cinnabar giggles and curls her tail around your " + player.legs() + ", looping it like a long, flexible rope. She hugs you and murmurs, \"<i>Where will my " + player.mf("master","mistress") + " take " + player.mf("his","her") + " slutty, fuck-hungry companion?</i>\" Her tongue flicks your ear like a snake tasting its prey, and though her attitude sounds submissive and weak, your horny 'companion' is clearly hiding a fiery, aggressive streak.\n\n");
			...
			else outputText("slip two digits into your " + player.assholeDescript() + ", pulling it apart far enough to cause you discomfort");
			...
			if (player.hasCock() && !player.hasVagina()) outputText(" Unbidden, " + player.sMultiCockDesc() + " begins to harden, stiffening with every brush against your prostate.");
			...
			outputText(" You rock back, slamming your " + player.buttDescript() + " against the wall and moaning with need. She's quite skilled, and ");
			...
			if (player.hasCock() && !player.hasVagina()) outputText("You gasp and moan when you feel her other hand wrap around your " + player.cockDescript(0) + ", pulling and tugging on it until it starts to leak a trail of squeezed-out man-slime on the mattress. ");
			...
			if (player.hasCock() && !player.hasVagina()) {
				outputText(" Her soft pads grip your " + player.cockDescript(0) + " tightly while she presses on your prostate, milking your cum from you with professional skill. Rather than coming in its usual spurts, it drips in a long, liquid chain of white-goo that drops into a submissive puddle on the bed.");
				if (player.cumQ() >= 250) {
			...
			if (player.hasVagina()) {
				outputText(" She licks your " + player.clitDescript() + ", and your " + player.vaginaDescript(0) + " convulses around the intruding digits, making you shiver and moan. Not wanting to be left out, your " + player.assholeDescript() + " clamps and puckers with it, wringing the rat's ");
				if (player.vaginalCapacity() < 30) outputText("fingers");
		...
		private function fuckCinnabarHugerAndCorruptedEr():void {
			...
			outputText(", squirting juices from the callous, cunt-piercing thrust. The " + player.cockHead(x) + " of your " + player.cockDescript(x) + " can be seen just below her neck");
			...
			else outputText(", and only now do you realize you blew past her cervix long ago, the tight ring hugging your " + player.cockDescript(x) + " in a clutching embrace.");
			outputText("\n\n");
			...
			outputText("Cinnabar pulls her dirty paw out and grins like a mouse with a chunk of cheese, massaging her distorting neck while you empty every ounce of cum from your " + player.ballsDescriptLight() + " into her over-stretched womb-filled body. She squeaks, \"<i>No-ohhhh-ormally I LIKE being a mewling slut for " + player.mf("studs","bitches") + " like you, but if you won't treat a cum-sleeve like me with respect, then I'm going to teach it to you.</i>\" You nod dumbly, still locked in a climax that ");
			...
			outputText("Cinnabar tosses your " + player.armorName + " and you realize the nude rat doesn't show any signs of your tryst");
		...
		private function cinnabarHuger():void {
			...
			outputText("True to her words, she starts squeezing you through her fur, massaging your " + player.cockDescript(x) + " by dragging her stretchy puss along your length with her fingertips. It's too much, her body is fucking perfect for your " + player.cockDescript(x) + ", and you feel warmth building inside you. Her tail twists around your ");
		...
		private function cinnabarHuge():void {
			...
			outputText("your " + player.legs() + ", and ");
		...
		private function cinnabarMultiCockPortalFuckFest():void {
			...
			outputText("Cinnabar's hands fly through your " + player.armorName + ", diving through your undergarments to squeeze " + player.sMultiCockDesc() + ". ");
			...
			outputText(" Your " + player.legs() + " go wobbly from the delicate touches dancing through your " + player.multiCockDescriptLight() + ", making it hard to stand. Cinnabar slides ");
			...
			outputText("The sensation is absolutely divine, but primal need takes over. You begin to rock your " +  player.hipDescript() + " in and out, triple-penetrating the whore");
			...
			outputText("An incessant licking reaches your ears, getting your attention. You stand up, careful about your " + player.multiCockDescriptLight() + " while you look towards the bed. The portals are closed down, and Cinnabar is bent double, folded in half like a contortionist");
