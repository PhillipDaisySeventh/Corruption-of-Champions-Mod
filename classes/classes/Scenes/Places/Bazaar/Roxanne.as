	...
	public class Roxanne extends BazaarAbstractContent implements TimeAwareInterface 
	{
		...
		public function RoxanneAppearance():void {
			...
			if (getGame().time.hours > 12 && getGame().time.hours < 19) {
				//(Not Met) 
				if (flags[kFLAGS.ROXANNE_MET] == 0) outputText("\n\nThere's a table with a half-dozen oddly-dressed lizans not too far from the fire. A keg is set up " + UnitSystem.aFewFeet() + " away and they seem to be having a good time.");
				//Met) 
		...
		public function RoxanneChooseApproachOrRepeat():void {
			...
			addButton(2, "Lose", roxanneDrinkingContestLoseDeliberately).hint("Why win when the lizan's cock is so tempting and appealing? Lose on purpose.");
		}
		...
		protected function roxanneDrinkingContest():void {
			...
			{
				outputText("\"<i>You call that drinking? Watch and learn, " + player.mf("brother","sister") + "!</i>\" shouts Roxanne triumphantly as she holds her mug aloft overhead. The frothy beverage begins to pour out, an amber waterfall of intoxicant raining down towards the lizan's face, but the canny pirate is ready for it. She opens her jaw and extends her tongue, over " + UnitSystem.displayFeetEstimateTextually(2) + " of the pink-hued organ, catching the alcoholic downpour and funneling it past her smiling lips. Finished, she belches loudly and pumps her hips at you rudely. \"<i>I hope you're ready to get fucked!</i>\"\n\n");
			}
		...
		private function roxanneGivesABlowjob():void {
			...
			outputText(", squeezing you softly while she steadies herself on your " + player.cockDescript(x) + ". She titters, letting a few " + UnitSystem.literalInches() + " of tongue slip through her lips to envelop your sensitive " + player.cockHead(x) + ", circling the engorged cock-flesh with drunken slobbers. ");
			if (player.balls > 0) outputText("A set of warm, long-nailed fingers squeeze your " + player.ballsDescriptLight() + ", dragging sharp nail-tips along the underside of your sack to tease the poor, cum-packed orbs. ");
			...
			outputText(". \"<i>Jeeze, pent up much?</i>\" she laughs, struggling to stand while giggling at her own supposed cleverness. Happy with how the contest worked out, you slip " + player.sMultiCockDesc() + " into your " + player.armorName + " and wobble off back to camp.");
			//(-100 lust, -1 int)
			player.orgasm('Dick');
			dynStats("int", -1);
		...
		private function roxanneCunnilingus():void {
			...
			outputText("The drunken lizan puts her soft, lightly scaled fingers around you to squeeze at your " + player.buttDescript() + " and leans in to give your " + player.clitDescript() + " a tender kiss. Her massive tongue ");
			if (player.getClitLength() < 16) outputText("dwarfs");
			else outputText("envelops");
			outputText(" the ");
			if (player.getClitLength() < 1) outputText("little pleasure buzzer");
			else if (player.getClitLength() < 3) outputText("swollen love-button");
			...
			if (player.hasFuckableNipples()) {
				outputText("Roxanne pulls her hands off your ass and reaches up, swaying unsteadily until she catches your " + player.nippleDescript(0) + "s. A second later she slips her digits inside the welcoming tit-pussies, finger-fucking your breasts as ");
			...
			if (player.hasCock()) outputText(player.SMultiCockDesc() + " bounces on her head, between her horns, spilling a trail of sticky seed down her back. ");
		...
		private function roxanneRimjob():void {
			spriteSelect(SpriteDb.s_poisontail);
			clearOutput();
			outputText("You shimmy out of your " + player.armorName + " and smirk, turning around to lean onto the table");
			...
			if (player.hasCock()) {
				outputText("Surprisingly, one of the pirate's hands comes off your " + player.buttDescript() + " to tug on ");
			...
			outputText("The long, wondrous tongue pushes forward, wriggling inside you. At first it's just the tip, but the drunken lizard forces " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " inside you with constant intensity. You can feel her spit frothing around your " + player.assholeDescript() + ", bubbling while she fills your backdoor with ever larger quantities of saliva-covered tongue. Seeming to go on forever, the penetration robs you of strength, filling you with " + UnitSystem.display("dozens", num2Text(UnitSystem.lengthInInchesEstimate(12))) + " more " + UnitSystem.literalInches() + ". Roxanne hums at your dark hole, vibrating her tongue inside you while it arches to caress the most sensitive spots.\n\n");
			...
			if (player.hasCock()) {
				...
				outputText("while she teases your " + player.clitDescript() + " with those terribly-slow touches.");
			}
			outputText("\n\n");
		...
		private function roxanneReamsYouNormal():void {
			spriteSelect(SpriteDb.s_poisontail);
			clearOutput();
			outputText("A foot interposes itself between you and the table, and a shove sends you flopping onto the booze-soaked planks face-down, the last few " + UnitSystem.literalInches() + " of descent slowed by Roxanne's firm hands. A scaled digit digs under your " + player.armorName + ", pulling and tugging. As each piece is forcibly removed, you're left more and more naked, totally exposed to the heat of the fire and the smooth scales of the lizan's body. The room lurches when you to try to move and stop her, and you flop back down, ignoring the drunken spin of your surroundings. Laughing heartily, the pirate leans over you to pin your hands to the table. While her position has the intended effect of restraining you, it also gives you the enviable ability to feel Roxanne's iron-hard nipples pressing into your back.\n\n");
			...
			else if (player.hasVagina()) outputText("against your " + player.vaginaDescript() + ", the sweaty orbs growing slick with the moisture of your growing need");
			...
			outputText("You shake your head no, and are rewarded for your disobedience with a violent, butt-jiggling tail-whip. Gasping from the pain, your " + player.assholeDescript() + " contracts involuntarily, milking Roxanne's member for a few more drops of pre. The aggressive lizard really gets into it, her massive, DD-cup tits pinning you to the table while she smacks your cheeks with her flexible lizan tail.  Pounding and slapping you, she abuses you for every dick-milking squeeze you'll give her, ");
			...
			//(JUST CUNTS:  
				...
				outputText("The constant pressure of her thrusts seems to flow straight to your " + player.vaginaDescript() + ", inflaming your moistening labia. You can feel her mass compressing your feminine organs, rubbing your inner walls against each other. The sublime internal friction releases your natural lubricants, ");
			...
			if (player.hasCock()) {
				outputText(" ");
				if (player.cumQ() >= 1500) {
					outputText("A torrential outpouring of spunk pours from " + player.sMultiCockDesc() + ", leaking in a steady, submissive flow to grow a ");
					if (player.cumQ() <= 2500) outputText("puddle");
					else outputText("lake");
					outputText("below the table.");
				}
				else outputText("An outpouring of spunk drools from " + player.sMultiCockDesc() + ", spurting submissively under the table.");
			}
			if (player.biggestLactation() >= 2) outputText(" Bursts of milk erupt from your pinned " + player.nippleDescript(0) +"s, blasting out while you cum to stain the table white.");
			outputText(" As it winds down, you fall into slumber, snoring happily after your violation.\n\n");
			
			outputText("<b>LATER...</b>\n");
			outputText("You wake in the lizan's bed<b> with a nasty hangover</b>, her arm curled around your gurgling belly. From how sore your rear feels, she kept 'winning' at least two or three more times. Your head is pounding, your " + player.legs() + " are weak, and you dribble cum with every movement. It takes some doing to extricate yourself from Roxanne's slumbering form, but you find your equipment and leave, hanging your head in shame under the leering eyes of the caravan-goers.");
		...
		private function roxanneFucksYourAssOHGODITSHUGE():void {
			...
			outputText("Suddenly, all traces of her hesitation or restraint evaporate; the lizan pushes forwards, burying the first few " + UnitSystem.literalInches() + " of her massive cock-head into your clutching, gaped backdoor. ");
			...
			outputText("Roxanne stops and pants lustily, her " + UnitSystem.displayFootEstimateWithHyphenTextually(2) + " prehensile tongue hanging into her cavernous cleavage while she strokes the exposed half of her member. She traces one of her fingers over the swell of your left cheek, letting her nail leave behind ");
			if (player.hasScales()) outputText("a roughly-scratched 'X' on your scales");
		...
		private function bigBootyRoxanneContestLoss():void {
			...
			outputText("\n\nReflexively, Roxanne's penis grows once you look down at it, gaining " + UnitSystem.displayInchesRangeTextually(4, " or ", 5) + " in seconds. It pauses for the split-second before its owner's next heartbeat, and then it expands again. This time, it lurches upward, the sensitive glans bouncing off your [hips] before coming to rest against your belly. The inflating cockflesh slides up to your [chest], leaving a trail of glistening pre-cum as it goes. Roxanne coos, \"<i>Oooh, let's get you inside before we make too much of a scene!</i>\"");
			...
			outputText("\n\nYou whimper as Roxanne thrusts herself further forward, burying another few " + UnitSystem.literalInches() + " of potent lizard-cock inside you. Its sheer size shifts your innards slightly, and you're sure there must be a visible bulge on your tummy by now. The shemale sighs blissfully and continues to work her dick further and further inside you. Finally, when you feel you can fit no more, her large, quaking testes press against your rump. If you didn't have such a round ass, she'd probably still be trying to go further inside, but your plump derriere actually helps to distance her from your vulnerable pucker - by a bit.");
			outputText("\n\nSighing with relief, your body shudders softly as you adjust to the meat-spear inside you, slowly relaxing until your stretched butt-cunt more comfortably fills the role of Roxanne's fuck-toy. She exhales happily and places her smooth palms on your shoulders, gripping you firmly before she pushes again. You squeal in distress as at least another " + UnitSystem.literalInch() + " of the pirate's gigantic dong slides inside you, her hips and balls smushing your jiggly cheeks out to the side. Her sack must be painfully pressing into you by now, but Roxanne doesn't relent. She holds you there, forcing you to hold even more than before.");
