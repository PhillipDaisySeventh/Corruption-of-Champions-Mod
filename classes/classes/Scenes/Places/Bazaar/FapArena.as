	...
	public class FapArena extends BazaarAbstractContent
	{
		...
		private function fapResults(place:Number = 3):void {
			...
			if (place == 3) {
				...
				if (player.hasVagina()) {
				outputText(" Your thighs tense a few seconds before your " + player.vaginaDescript(0) + " squirts too, forming a ");
			...
			//[else if the player doesn't lose, but doesn't win either - he cums neither first nor last]
				...
				outputText(" the floor with your sexual juices and yell in a whorish voice, \"<i>NUMBER " + (num+4) + " CUMMING!</i>\"; you're almost immediately followed by the scoring announcement from your left neighbor. With a half-closed eye you notice the referee nodding in acknowledgement and you point your " + player.cockDescript(x) + " at her, trying to soak her too. She hardly dodges it and some lands on her arms, but she doesn't seem to notice.");
				if (player.hasVagina()) {
					outputText(" Your thighs tense a few seconds before your " + player.vaginaDescript(0) + " squirts too, forming a ");
					if (player.wetness() >= 4) outputText("large puddle");
					else if (player.wetness() >= 2) outputText("puddle");
			...
			//[else, the player wins]
				...
				outputText(" the floor with your sexual juices and yell in a whorish voice: \"<i>NUMBER " + num+ " CUMMING!</i>\" With a half-closed eye you notice the referee nodding in acknowledgement and you point your " + player.cockDescript(x) + " at her, trying to soak her too. She hardly dodges it and some lands on her arms, but she doesn't seem to notice.");
						if (player.hasVagina()) {
					outputText(" Your thighs tense a few seconds before your " + player.vaginaDescript(0) + " squirts too, forming a ");
					if (player.wetness() >= 4) outputText("large puddle");
					else if (player.wetness() >= 2) outputText("puddle");
			...
			//[else, corr >50]
			else outputText("Your body tenses and " + player.sMultiCockDesc() + " hardens at the perspective of a collective anal orgy. You yell in lust like a minotaur in rut, \"<i>BUTT-FUCK! BUTT-FUCK! BUTT-FUCK! BUTT-FUCK! BUTT-FUCK!</i>\" along with the others. ");
			//end of global condition about PC's corr, display the following
			...
			if (place <= 1) {
				...
				if ((player.cockTotal() > 2) || (!tent && player.cockTotal() > 1)) {
					...
					outputText("own, unhindered by the tight colon holding your first " + player.cockDescript(x) + ".");
				}
			...
			//[else if you didn't win]
				...
				tentacleFapSpecial(place);
				outputText("A pressure builds at your crotch as a milky torrent of seed finds its way into your urethra, down your sodomite partner's colon. Almost simultaneously, you feel an equally thick spray of spooge being released inside your own " + player.buttDescript() + ". At the peak of your orgasm, you are unable to think of anything but the steady flow of jism coming in and out of you.");
				tent = tentacleFapCum();
				//[if player has multicocks] 
				if ((player.cockTotal() > 2) || (!tent && player.cockTotal() > 1)) {
					if (player.cockTotal() > 3 || (!tent && player.cockTotal() > 2)) outputText(" Your other cocks also spill ");
			...
			//[else, if you lost]
				...
				outputText("You suddenly hear a muffled moan from behind. Someone is probably being penetrated at the back of the train. You brace yourself for the imminent anal-fucking... and here it comes. The dong is projected at full force into your " + player.buttDescript() + " then all the way inside your colon; you're protected only by the copious glaze of pre-cum its bearer had been applying.");
