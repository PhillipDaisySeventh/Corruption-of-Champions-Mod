package classes.Scenes.Places
{
	...
	public class Owca extends BaseContent 
	{
		...
		private function modOwcasAttitude(change:int):int {
			flags[kFLAGS.OWCAS_ATTITUDE] += change;
			if (flags[kFLAGS.OWCAS_ATTITUDE] > 100) flags[kFLAGS.OWCAS_ATTITUDE] = 100;
			if (flags[kFLAGS.OWCAS_ATTITUDE] < 0) flags[kFLAGS.OWCAS_ATTITUDE] = 0;
			return flags[kFLAGS.OWCAS_ATTITUDE];
		}
		...
		private function declineRebeccsPlea():void {
			...
			//[Attitude is set to 50]
			if (flags[kFLAGS.OWCAS_ATTITUDE] > 5) modOwcasAttitude(-5);
		...
		private function intoTheDemonPit(sacrifice:Boolean = true):void {
			...
			menu();
			addButton(0, "Fight", fightZeDemons, sacrifice).hint("Break free from the chains and engage in combat with the demons!");
			addButton(1, "Submit", loseOrSubmitToVapula).hint("Let the demons have their way with you. " + (flags[kFLAGS.VAPULA_SUBMISSIVENESS] >= 100 ? "Given your submissiveness towards the demons, this will lead to a bad end." : "This will make you more submissive towards the demons."));
		}
		//Submit/Fight
		...
		public function loseOrSubmitToVapula():void {
			...
			if (player.hasCock()) {
				...
				outputText(", as if every " + UnitSystem.literalInch() + " had to be taken care of at every instant.  As you slide back into the pussy, the hands return to crawling over your body, caressing your groin and staining your belly with pre-cum - yours and others.");
		...
		private function wakeUpAfterDemonGangBangs():void {
			...
			getGame().time.days++;
			outputText("When you wake up, you are alone, and your restraints are broken.  You are sloshing in a pool of stinky juices; your mouth and ears are still full of it.  Your whole body is covered with a thin white layer that must certainly be dried spooge.  Underneath, you're nothing but bruises and every movement seems to hurt.  A few " + UnitSystem.literalYards() + " away, outside the pit, you notice your items and your gear.  The village itself appears to be empty... your best assumption is that the residents are hiding, either from shame at having sacrificed you or from awkwardness at the prospect of talking to a sloshing, crusty cumdumpster.  Wearily, you head back to your camp.");
		...
		private function rapeZeVapula():void {
			...
			if (player.hasCock()) {
				...
				outputText(player.balls > 0 ? "balls" : "[skin]");
			...
			//Victory rape using vagina/anus [Take Dicks in All Holes](Z)
				...
				outputText("\n\nWhen you walk up to Vapula, she is barely recognizable.  Her will is utterly broken and she seems considerably mollified.  Your repeated denials made her mind collapse; she's but an empty shell driven crazy by her burning unsatisfied lust.  With a content smirk, you lean down and give her a tender kiss, enjoying the taste of secondhand sexual juices coating her lips.  You whisper to her, \"<i>Good night, beauty.</i>\"  She doesn't seem to hear your taunt.");
				outputText("\n\nA few " + UnitSystem.literalYards() + " away, outside the pit, you notice your items and your " +player.armorName+ ". The village appears to be deserted. Considering these demons will have a hard time threatening anyone with white goo oozing out of their absurdly milked urethras, you let them rest, wallowing in their pool of mixed spunk.  You pick up your belongings and head back to camp.");
		...
		private function owcaMainScreenOn():void {
			...
			if (getGame().time.hours >= 16 && flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0) {
				...
				addButton(0, "Pit", zePit).hint("Volunteer to be the sacrifice tonight and earn the town's trust.");
			} else {
				addDisabledButton(0, "Pit").hint("You cannot volunteer to be the sacrifice at the moment. " + (flags[kFLAGS.OWCA_SACRIFICE_DISABLED] > 0 ? "The demons will no longer trouble the village of Owca again." : "Come back at four in the evening or later."));
			}
			if (flags[kFLAGS.OWCAS_ATTITUDE] >= 50) {
				...
				addButton(1, "Herds", herds).hint("Take the opportunity to work at the sheep herd.");
			} else {
				addDisabledButton(1, "Herds").hint("The village doesn't seem to trust you at the moment. Consider earning more trust by being the sacrifice.");
			}
			...
			if (flags[kFLAGS.OWCAS_ATTITUDE] >= 10) {
				...
				addButton(3, "Tavern", owcaTavern).hint("Give the tavern a visit and see what kinds of food and beverages are there. Rebecc also hangs out there.");
			} else {
				addDisabledButton(3, "Tavern").hint("The tavern appears to be closed at this time.");
			}
		...
		private function rebeccMenu():void {
			...
			menu();
			addButton(0, "Appearance", rebeccAppearance).hint("Get a good look at the sheep-girl's appearance.");
			addButton(1, "Bath", rebeccBathScene).hint("Relax your tension with a nice milk bath. This could get a bit teasing though.");
			if (player.isCorruptEnough(66) || player.findPerk(PerkLib.Sadist)) {
				...
				addButton(3, "Surprise Sex", rapeRebecc, false).hint("Molest the sheep girl? This might not be a good idea.");
			} else {
				addDisabledButton(3, "Surprise Sex").hint("You're not corrupt or sadistic enough to consider doing that.");
			}
			addButton(14, "Back", gangbangVillageStuff);
		}
		//Rebecc Appearance (Z)
		...
		public function beatUpOwca():void {
			...
			if (!player.isGenderless() && player.lust >= 33 && (player.isCorruptEnough(60) || player.findPerk(PerkLib.Sadist) >= 0)) {
				addButton(0, "Rape Rebecc", rapeRebecc, true).hint("Serve the sheep-girl right for setting you up.");
			} else {
				addDisabledButton(0, "Rape Rebecc").hint("You need to have either sexes and be of sufficient lust to consider doing that. In addition, you need to be corrupt or sadistic enough.");
			}
			if (player.hasStatusEffect(StatusEffects.KnowsWhitefire) || player.hasStatusEffect(StatusEffects.KnowsBlackfire) || player.findPerk(PerkLib.FireLord) >= 0 || player.findPerk(PerkLib.Hellfire) >= 0 || player.findPerk(PerkLib.EnlightenedNinetails) >= 0 || player.findPerk(PerkLib.CorruptedNinetails) >= 0) {
				addButton(1, "Torch Village", torchOwcaMotherFuckers).hint("Burn baby burn! Set the village on fire and you can be confident it will be destroyed by the time you return to camp.");
			} else {
				addDisabledButton(1, "Torch Village").hint("Unfortunately, you lack the spells or tools to start a fire.");
			}
			addButton(4, "Leave", leaveOwcaAfterWhupping);
		}
		...
		private function torchOwcaMotherFuckers():void {
			...
			menu();
			if (!player.isGenderless() && player.lust >= 33 && (player.isCorruptEnough(60) || player.findPerk(PerkLib.Sadist) >= 0)) {
				addButton(0, "Abuse Her", rapeRebecc, true).hint("Serve the sheep-girl right for setting you up.");
			} else {
				addDisabledButton(0, "Abuse Her").hint("You need to have either sexes and be of sufficient lust to consider doing that. In addition, you need to be corrupt or sadistic enough.");
			}
			addButton(1, "Leave", torchUpVillagersAndLeave);
		}
		...
		private function morningAfterRape():void {
			...
			menu();
			addButton(0, "Forgive", forgiveOwca).hint("Show the sheep some MERCY. Grudges are not to be held after all.");
			addButton(1, "Rape", rapeRebecc, false).hint("Whether the sheep-girl did it or not, teach her a lesson.");
			addButton(2, "Leave", fuckThisShit).hint("You don't really want to have anything with the sheep-girl.");
		}
		//Option: Forgive (Z)
		...
		private function rebeccsLastPlea():void {
			...
			menu();
			addButton(0, "Rape Her", rapeRebecc, true).hint("You don't give a fuck about what's happening but you could take advantage of her helplessness. However, you'll no longer be able to return to the village.");
			addButton(1, "Face Them All", faceDownHordes).hint("Take on the horde of demons to make up for it.");
			addButton(2, "Leave", leaveRebeccToBeRaped).hint("Leave the sheep-girl to her fate and choose not to return to the village.");
		}
		//Option: Leave (Z)
		...
		private function subdueVapula():void {
			...
			menu();
			addButton(0, "Disband", disbandHorde).hint("Permanently disband the horde. The demons shall bother the village no more.");
			if (!player.isGenderless() && (player.isCorruptEnough(66))) {
				addButton(1, "EnslaveVapula", enslaveVapulaWithYourWang).hint("Make the succubus your slave and bring her to your camp. This will also disband her horde permanently.");
			} else {
			...
			addButton(3, "Skip Out", combat.cleanupAfterCombat).hint("None of the options seem that appealing. Just leave the horde.");
		}
		//Option: Disband (Z)
