...
//Lose against Marae
	...
	public function winAgainstMarae():void {
		...
		if (flags[kFLAGS.FACTORY_SHUTDOWN] == 2) {
			...
			if (player.hasStatusEffect(StatusEffects.KnowsWhitefire) || player.hasStatusEffect(StatusEffects.KnowsBlackfire) || player.findPerk(PerkLib.FireLord) >= 0 || player.findPerk(PerkLib.Hellfire) >= 0) {
	...
	private function maraeStealLethicite(deliberate:Boolean = false):void {
		...
		//(FAIL)
			...
			//FEM)
				...
				outputText("You are dropped from the tree, and with little choice, you waddle to your boat, doing your best to cover up your violated " + player.vaginaDescript(0) + ".");
	...
	public function level2MaraeEncounter():void {
		...
		outputText(".  The old dinghy's cutting through the water with amazing speed, leaving " + UnitSystem.footCompound() + "-high waves in its wake.  Marae's island grows larger with each passing second, almost taunting you with your inability to get away.   By now the root-like vines have crept over the gunwales, and they wriggle at you, seemingly in warning.  Getting in the water is definitely not a good idea right now.	The bone-chilling scrape of sand on wood grates at your ears.  You've arrived.  There's no point in putting off the inevitable.  You straighten up your " + player.armorName);
	...
	private function MaraeIIStageII():void {
		...
		//Marae uses a tree-tentacle to 'milk' male PC's.  Oral or Vajayjay? Not sure.
			...
			outputText("Vines whip out, sliding under your " + player.armorName + ", undoing clasps, and removing it until you stand naked and exposed to Marae.  She smirks and crooks her finger at you in a 'come hither' gesture.  You lurch forward, as if pulled on an invisible string until you're standing " + UnitSystem.literalInches() + " away from her, your " + player.cockDescript(0) + " rubbing her belly");
		...
		//[Hermz]  Marae grows vinecawks for DP under her flowercunt and sexes.
			...
			outputText("She's easy to push down into the soft grasses of the island, and her legs part to allow you better access.  Your " + player.cockDescript(0) + " doesn't need to be told what to do, and it slips into her waiting wetness as if it was made for her.  ");
			...
			else outputText("She feels perfect.  A velvet vice of hot, slippery wetness clutches tightly around your " + player.cockDescript(0) + ".  It almost feels like it's actually gripping you, cradling your cock in her ambrosia-slicked box.");
			if (player.cockTotal() > 1) {
				if (player.cockTotal() > 2) outputText("  Another ");
				else outputText("Your other ");
				outputText(Appearance.cockNoun(CockTypesEnum.HUMAN) + " prods at her tight pucker, and with a slight adjustment, you're able to line it up.  It's wet!  " + capitalizeFirstLetter(UnitSystem.literalInch()) + " after " + UnitSystem.literalInch() + " slides in with incredible ease, violating her slippery butthole until you've completely double-penetrated her.");
			}
			outputText("\n\n");

			if (player.vaginas[0].vaginalWetness < Vagina.WETNESS_WET) outputText("Sticky wetness glistens between your thighs");
			else if (player.vaginas[0].vaginalWetness < Vagina.WETNESS_DROOLING) outputText("Drops of feminine arousal run down your thighs");
			...
			outputText("You happily thrust forwards, ramming your " + player.cockDescript(0) + " into her cunt with such force that a wet slap echoes over the lake and her fluids splatter your abdomen.  ");
			if (player.cockTotal() > 1) outputText("Her asshole is squelching and dripping from your " + player.cockDescript(1) + ", actually squirting more lubricant than her pussy from the violent fucking!  ");
			...
			if (player.cockTotal() > 1) outputText("The " + player.cockDescript(1) + " in her ass spasms and explodes with its brother, glazing her slippery colon with a coating of syrupy spunk.  ");
...
//Only procs when you have both perks. Rare.
	...
	public function get encounterObject():Encounter {
		return Encounters.group("marae", {
			name: "default",
			when: function ():Boolean {
				return flags[kFLAGS.MARAE_QUEST_COMPLETE] <= 0 && flags[kFLAGS.MET_MARAE_CORRUPTED] <= 0;
			},
			call: encounterMarae
		}, {
			name: "aboutMinerva",
			...
			call: encounterPureMaraeEndgame
		}, {
			name: "level2",
			when: function ():Boolean {
