...
package classes.Scenes.Explore {
	...
	public class Giacomo extends BaseContent implements TimeAwareInterface {
		...
		public function giacomoEncounter():void {
			...
			addButton(1, "Erotica", eroticaMenu);
			addButton(2, "Books", bookMenu);
			if (player.hasStatusEffect(StatusEffects.Infested))
				addButton(3, "Worm Cure", wormRemovalOffer);
		...
		private function potionMenu():void {
			...
			menu();
			addButton(0, "Vitality T.", pitchVitailtyTincture, undefined, undefined, undefined, "This potent tea is supposedly good for the strengthening the body.", "Vitality Tincture");
			addButton(1, "Scholars T.", pitchScholarsTea, undefined, undefined, undefined, "This powerful brew supposedly has mind-strengthening effects.", "Scholars Tea");
			if (!player.isFemale()) {
				addButton(2, "Cerulean P.", pitchCeruleanPotion, undefined, undefined, undefined, "This is a mysterious bottle filled with a sky-blue liquid that sloshes gently inside.  Supposedly it will make you irresistible, though to what or who you cannot say.", "Cerulean Potion");
			}
			else {
				addDisabledButton(2, "Cerulean P.", "This potion is useless for females.", "Cerulean Potion");
			}
		...
		private function eroticaMenu():void {
			...
			if (player.hasKeyItem("Dildo") < 0) {
				addButton(0, "Dildo", pitchDildo, undefined, undefined, undefined, "Pick a hole, stick it in.");
			} else {
				addDisabledButton(0, "Dildo", "You already own a Dildo.");
			}
			if (player.hasVagina()) {
				if (player.hasKeyItem("Self-Stimulation Belt") < 0) {
					addButton(1, "Stim-Belt", pitchSelfStimulationBelt, undefined, undefined, undefined, "A mechanic sex toy for females.", "Self-Stimulation Belt");
				} else {
					addDisabledButton(1, "Stim-Belt", "You already own a Self-Stimulation Belt.", "Self-Stimulation Belt");
				}
				if (player.hasKeyItem("All-Natural Self-Stimulation Belt") < 0) {
					addButton(2, "AN Stim-Belt", pitchAllNaturalSelfStimulationBelt, undefined, undefined, undefined, "A biomechanical sex toy for females.", "All-Natural Self-Stimulation Belt");
				} else {
					addDisabledButton(2, "AN Stim-Belt", "You already own an All-Natural Self-Stimulation Belt.", "All-Natural Self-Stimulation Belt");
				}
			} else {
				addDisabledButton(1, "Stim-Belt", "It is designed for females.");
				addDisabledButton(2, "AN Stim-Belt", "It is designed for females.");
			}
			if (player.hasCock()) {
				if (player.hasKeyItem("Plain Onahole") < 0) {
					addButton(3, "Onahole", pitchOnahole, undefined, undefined, undefined, "A simple sex toy for males.");
				} else {
					addDisabledButton(3, "Onahole", "You already own an Onahole.");
				}
				if (player.hasKeyItem("Deluxe Onahole") < 0) {
					addButton(4, "D Onahole", pitchDeluxeOnahole, undefined, undefined, undefined, "A simple sex toy for males. Looks and feels a lot like real vagina.", "Deluxe Onahole");
				} else {
					addDisabledButton(4, "D Onahole", "You already own a Deluxe Onahole.");
				}
				
			} else {
				addDisabledButton(3, "Onahole", "It is designed for males.");
				addDisabledButton(4, "D Onahole", "It is designed for males.");
			}
			if (player.hasCock() && player.hasVagina()) {
				if (player.hasKeyItem("Dual Belt") < 0) {
					addButton(5, "Dual Belt", pitchDualStimulationBelt, undefined, undefined, undefined, "A mechanic sex toy for herms.", "Dual Stimulation Belt");
				} else {
					addDisabledButton(5, "Dual Belt", "You already own a Dual Stimulation Belt.");
				}
				if (player.hasKeyItem("All-Natural Onahole") < 0) {
					addButton(6, "AN Onahole", pitchAllNaturalOnahole, undefined, undefined, undefined, "An organic sex toy for herms.", "All-Natural Onahole");
				} else {
					addDisabledButton(6, "AN Onahole", "You already own an All-Natural Onahole.");
				}
			} else {
				addDisabledButton(5, "Dual Belt", "It is designed for herms.");
				addDisabledButton(6, "AN Onahole", "It is designed for herms.");
			}
		...
		private function pitchVitailtyTincture():void {
			...
			menu();
			if (player.gems >= 15) {
				addButton(0, "Yes", buyVitailtyTincture);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", potionMenu);
		}
		...
		private function pitchScholarsTea():void {
			...
			menu();
			if (player.gems >= 15) {
				addButton(0, "Yes", buyScholarsTea);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", potionMenu);
		...
		private function pitchCeruleanPotion():void {
			...
			menu();
			if (player.gems >= 75) {
				addButton(0, "Yes", buyCeruleanPotion);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", potionMenu);
		...
		public function pitchCondom():void {
			...
			menu();
			if (player.gems >= 10) {
				addButton(0, "Yes", buyCondom);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", eroticaMenu);
		}
		...
		private function pitchDangerousPlantsBook():void {
			...
			menu();
			if (player.gems >= 10) {
				addButton(0, "Yes", buyDangerousPlantsBook);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", bookMenu);
		}
		...
		private function pitchTravellersGuide():void {
			...
			menu();
			if (player.gems >= 1) {
				addButton(0, "Yes", buyTravellersGuide);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", bookMenu);
		...
		private function pitchHentaiComic():void {
			...
			menu();
			if (player.gems >= 10) {
				addButton(0, "Yes", buyHentaiComic);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", bookMenu);
		...
		private function pitchDildo():void {
			...
			outputText("Giacomo takes out a slender tube roughly over " + UnitSystem.halfAFoot() + " in length.  \"<i>Since you seek pleasure, this is as simple and effective as it gets.  This dildo is a healthy " + UnitSystem.displayInchesEstimateTextually(7) + " long and is suitable for most women and even adventurous men.  Pick a hole, stick it in and work it to your heart's content or your partner's pleasure.  The single-piece construction makes it solid, sturdy and straightforward.  For 20 gems, you can take matters into your own hands.  How about it?</i>\"");
			menu();
			if (player.gems >= 20) {
				addButton(0, "Yes", buyDildo);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", eroticaMenu);
		...
		private function pitchSelfStimulationBelt():void {
			...
			menu();
			if (player.gems >= 30) {
				addButton(0, "Yes", buySelfStimulationBelt);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", eroticaMenu);
		...
		private function pitchAllNaturalSelfStimulationBelt():void {
			...
			menu();
			if (player.gems >= 40) {
				addButton(0, "Yes", buyAllNaturalSelfStimulationBelt);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", eroticaMenu);
		...
		private function pitchAllNaturalOnahole():void {
			...
			menu();
			if (player.gems >= 150) {
				addButton(0, "Yes", buyAllNaturalOnahole);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", eroticaMenu);
		...
		private function pitchDualStimulationBelt():void {
			...
			menu();
			if (player.gems >= 50) {
				addButton(0, "Yes", buyDualStimulationBelt);
			} else {
				addDisabledButton(0, "Yes", "You can't afford this!");
			}
			addButton(1, "No", eroticaMenu);
		...
		private function wormRemoval():void {
			...
			outputText("The merchant puts away his prize and comes back to help you up. \"<i>Here.</i>\", he says as he shoves a bottle into your hand. \"<i>This is on the house. You probably need it after the shock of getting those things out.</i>\"\n\n");
		...
		private function ceruleanSuccubusEncounterPart4():void {
			...
			if (player.gender == 1) {
				...
				if (player.tallness < 80)
					outputText("you see that she towers over you.  She must stand well over " + UnitSystem.displayHeight2EstimateTextually(7) + " in height.  ");
				if (player.tallness >= 80 && player.tallness < 90)
					outputText("you see she is about as tall as you - around " + UnitSystem.displayHeight2EstimateTextually(7) + " in height.  ");
				if (player.tallness >= 90)
					outputText("you see she's definitely shorter than you, only about " + UnitSystem.displayHeight2EstimateTextually(7) + " tall.  ");
		...
		private function nightSuccubiRepeat():void {
			...
			if (player.gender == 1) {
				if (player.cor < 66) {
					...
					if (player.isTaur()) outputText("She moans inhumanly, and reflexively digs her claws into your forelegs. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You press your " + player.cockDescript(0) + " into her");
				...
				if (flags[kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 10 && player.cor > 50) {
					...
					if (player.horseScore() + player.dogScore() + player.nagaScore() + player.goblinScore() + player.sharkScore() + player.minoScore() + player.cowScore() > 5) outputText("  All of the other corruptions and changes to your body have faded away as your new form has taken shape.");
