...
public function lumiLabChoices():void {
	...
	if (flags[kFLAGS.LUMI_MET] == 0) { //First time meeting
		...
		outputText("She's about " + UnitSystem.displayHeight2Estimate(3) + " tall, with yellow-green skin, and wears her orange hair in a long ponytail that reaches to her knees.  Her breasts are about B cup, with average nipples that have been colored orange. All of her nails have been painted orange to match. She doesn't seem to ever stop moving, and while the majority of her face looks cute, it's a little hard to be sure while she's wearing those thick goggles.  The solid black lenses of her goggles make her look quite unsettling, stifling any appeal her form could inspire in you.\n\n");
...
public function lumiEnhance():void {
	...
	if (player.hasItem(consumables.GROPLUS))
		 addButton(btn++, consumables.GROPLUS.shortName, lumiEnhanceGo, consumables.GROPLUS, undefined, undefined);
...
private function lumiEnhanceGo(itype:ItemType):void {
	...
	else if (itype == consumables.GROPLUS) nextItem = consumables.GIRTHOP;
