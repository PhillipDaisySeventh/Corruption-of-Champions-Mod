...
//const KIHA_CHOKED_OUT_PC:int = 432;
	...
	public function kihaSparIntensity():int {
		var amount:int = Math.max(0, flags[kFLAGS.KIHA_SPAR_VICTORIES]);
...
internal function kihaLossIntro():void {
	...
	//Rape texts if horny
		...
		outputText("Leaving her axe to rest " + UnitSystem.aFewFeet() + " away, stuck on the ground, Kiha strips off your " + player.armorName + " revealing your ");
...
private function kihaMilkTitHumiliation():void {
	...
	outputText("The scaly woman leans down to tug on your " + player.nippleDescript(0) + "s in turn, laughing every time a spurt of milk erupts from one of your tits.  Each rough squeeze sends shivers of erotic sensation through your body, and you quickly grow aroused from the rough treatment.  Kiha keeps going, ");
...
private function tsundereMasturbationChristmasCarol():void {
	...
	spriteSelect(SpriteDb.s_kiha);
	outputText("You walk towards the draconic woman, removing your " + player.armorName + " as you observe her firm, ");
...
private function kihaVictoryPomfTail():void {
	...
	outputText("  Now nude, you sway your " + player.hipDescript() + " back and forth sensually, forcing Kiha to watch your body before you use her.  ");
	...
	outputText("Satisfied that it is sufficiently lubricated, you squeeze the now-slippery tail and point its nubby tip at your entrance.  It bumps your " + player.clitDescript() + " as you wrangle with the struggling hind-limb, only turning you on all the more.  Feeling the tip circling your tunnel's opening, you push it up and seat the tentacle-like tail deep inside yourself.  ");
	player.cuntChange(1,true,false,true);
	outputText("Gods she's so... warm, and the ridges, they're stroking every " + UnitSystem.literalInch() + " of your interior, even your g-spot!  Your " + player.legs() + " wobble unsteadily, but spurred by lust, you keep an iron grip on your new-found toy.  Kiha doesn't stop resisting either.\n\n");
...
private function victoryDickKiha():void {
	...
	if (player.lust100 > 60) outputText("only makes " + player.sMultiCockDesc() + " harder.");
	else outputText("quickly makes " + player.sMultiCockDesc() + " hard");
	outputText(".  Her rear-most extremity yanks out of your grip to cover her femininity once more and you let it, for now.\n\n");
	
	outputText("Pausing for a moment, you set to removing your " + player.armorName + " and unleashing " + player.sMultiCockDesc() + " ");
	if (player.hasVagina()) outputText("and " + player.vaginaDescript() + " ");
	...
	if (y != -1 && player.cockTotal() > 1) {
		...
		outputText(" aching so much that you may as well try to double your pleasure and fun.  Without hesitation, you slide two cocks inside your unsuspecting partner.  The top one smoothly sheaths itself inside Kiha's velvety tunnel.  Her tightly-clenched pucker proves more resistant to your " + player.cockDescript(y) + "'s sudden intrusion, pushing back with near painful force as " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " penetrates her delicious rear passage.  One last, glorious push completely buries both your bones in their draconic sockets, much to the delight of your captive, if her moans are to be believed.\n\n");
	...
	//(SINGLE DICKING VARIANT)
		...
		outputText("Eventually, you become accustomed to the feel of the reptilian tunnel's simmering juices, and eager for more stimulation, you begin to saw your " + player.cockDescript(x) + " in and out of sodden box.  The draconic woman beneath you lets out whimpers and moans of pleasure mixed with discontent, unable to resist you and unable to allow herself to enjoy it.  She squirms and wriggles beneath your forced affections, growing ");
...
private function kihaRapesLittleGirlsISawItOnTheNews():void {
	...
	outputText("You moan, weakly begging her to stop, or at least to slow down and be gentler.  The pleasure of her relentless stimulus sets your cunny drooling, while the pain of the dragon's brutal indifference forces saliva to bubble in your mouth as you hold your tongue, trying not to give her the satisfaction of watching you scream. She stretches your netherlips and brutally thrusts into your depths, watching your face intently, relishing your weakness. When, at last, you cry out in orgasmic throes, she purses her lips in satisfaction, the faintest trace of an affectionate flush creeping across her dusky cheeks.\n\n");
...
private function analRapuzulaKiha():void {
	...
	if (y >= 0) {
		...
		outputText("The dragoness slumps and falls to the ground, drawing your spent shaft from her ass's depths.  It appears that she has fainted from overwhelming pain, pleasure, and exhaustion. Deciding that you're done here, you retrieve your " + player.armorName + " and leave the girl in a mixed puddle of your cum and hers.  Perhaps now she'll treat you with a little more respect.  You return to camp.");
	}
	else {
		...
		if (player.hasVagina()) outputText("Your " + player.vaginaDescript() + " pulses with pleasure, but it's an empty orgasm, leaving you wishing you had something to fill that aching void.  Still, you cannot help but marvel at the superior feeling of cumming with a multitude of sexual organs.  ");
		...
		outputText("Deciding that you are done here, you retrieve your " + player.armorName + " and leave the girl. Perhaps now she'll treat you with a little more respect. You return to camp.");
	}
	player.orgasm('Dick');
	combat.cleanupAfterCombat();
}
