...
public function canKihaGetPregnant():Boolean {
	if (getGame().time.days % 15 == 0 && flags[kFLAGS.KIHA_PREGNANCY_POTENTIAL] > 0 && flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] <= 0 && !pregnancy.isPregnant) return true;
	else return false;
}
...
internal function kihaFriendlyGreeting(output:Boolean = true):void {
	...
	if (output && flags[kFLAGS.KIHA_TALK_STAGE] == 6 && player.isPureEnough(50)) {
		...
		if (getGame().time.hours < 20) outputText("clouds");
...
internal function sparWithFriendlyKihaLose():void {
	clearOutput();
	if (!followerKiha()) {
		outputText("You can't take it anymore!  You stumble away from the dragoness, but only make it " + UnitSystem.aFewFeet() + " before toppling over, landing right on your ass.  Dazed, you can only sit there as Kiha casually walks over and presses the haft of her axe into your throat.");
...
private function lovinHugKiha():void {
	...
	outputText("[pg]The warrioress's axe stands " + UnitSystem.aFewFeet() + " back, like a silent sentinel.  It's been forgotten in the heat of moment.  Kiha's prodigious, normally suppressed libido reveals itself when she forces a long tongue into your mouth, tying up your own oral organ while her mischievous, clawed fingers gently remove your " + player.armorName + ", one piece at a time.");
...
private function lovingHugsForRetards():void {
	...
	outputText("[pg]You tell her to shut up and shove it deeper.  Kiha gives a whimsical smile and obliges your crude request by forcing another " + UnitSystem.literalInch() + " of thick dragon-tail into your rectum. It stretches you so wide - ");
...
private function warmLoverKihaIntro(output:Boolean = true):void {
	...
	if (output) {
		...
		if (followerKiha()) {
			...
			if (getGame().time.days % 15 == 0 && flags[kFLAGS.KIHA_PREGNANCY_POTENTIAL] == 0) {
				kihasFirstClutch();
				return;
			}
...
private function hangOutWithKiha():void {
	...
	//Hang Out 3
		...
		if (!followerKiha()) {
			...
			if (getGame().time.hours < 21) outputText("particularly cloudy day above you, with great pink and purple clouds drifting across the demonically-tainted horizon");
			...
			if (getGame().time.hours < 21) outputText("day around your camp - thick pink and purple clouds are rolling lazily over the wasteland, shimmering behind the great maw of the portal");
		...
		if (getGame().time.hours < 21) outputText("day");
		...
		if (getGame().time.hours < 21) outputText("clouds");
		else outputText("stars");
...
private function boneTheShitOutofKihaHolesWithHorsecock():void {
	...
	outputText("[pg]Your reptilian lover bobs haltingly on your shaft.  Even with her efforts, she's struggling to slide more than a " + UnitSystem.halfInch() + " at a time through the lip-lined gateway to her mouth.  Consternated, she begins to pump you faster, ");
	...
	outputText(" to spear through Kiha's entrance and into her womanly canal.  She groans appreciably, and the wordless exultations of your mate are all the encouragement you need to push deeper, shoving " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of ");
	...
	outputText(" member.  It slurps up " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of pulsating cock, devouring your dick with a hunger born of the female's instinctive drives and desires.");
	...
	outputText("to seal itself inside your fuck-broken lover's baby-hole.  What feels like " + UnitSystem.aGallon() + " of seed lurches out from your cock.  Kiha's innards turn white from the creamy deluge");
...
internal function kihaReactsToHorseDicking():void {
	clearOutput();
	outputText("Kiha folds her arms across your chest when you approach, her wordlessness letting you know that she clearly remembers the events of your last meeting.  You stop " + UnitSystem.aFewFeet() + " away and cross your arms, waiting to see what she's going to do.  In a flash, she's on you!  Not punching, kicking, or biting you, but hugging you, sniffling into your shoulder.  She whimpers, \"<i>I... I didn't mean for that to happen!  I didn't!  I swear!</i>\"");
...
private function ghostboobiesKiha():void {
	...
	outputText("[pg]Kiha's protests swiftly fade to gasps of poorly-disguised delight as you focus both hands on her oversized F-cup, knocking your knees at how good the slutty tit feels in its engorged state.  Her surprisingly soft skin yields to her slender fingers as you knead into the flesh, exploring every square " + UnitSystem.literalInch() + " of her new expanse.  Your manipulations nearly cause her to forget her predicament... nearly.  \"<i>Hey hey, HEY!  Deal with this!</i>\" Kiha demands, one of your hands quivering in her novel attempt to point at her other, unaltered breast.  \"<i>You're not just going to... leave me like this?</i>\"");
...
private function kihaCampAppearance():void {
	...
	outputText("Kiha is a " + UnitSystem.displayHeight2Estimate(6) + " tall dragoness, with dark skin and blood-red scales covering much of her body.  She is " + (flags[kFLAGS.KIHA_UNDERGARMENTS] > 0 ? "wearing " + (flags[kFLAGS.KIHA_UNDERGARMENTS] == 1 ? "a pair of spider-silk panties" : "a spider-silk loincloth") : "naked, shameless of her nudity,") + " and carries a tremendous enchanted greataxe, the head of which blazes with heat.  She has a sharp, predatory face with dark red eyes bearing black, reptilian slits.  Long red hair grows from her scalp, reaching down past her shoulders.  She has strong, child-bearing hips and a squishy bubble-butt.  She has two reptilian legs adorned with scales and claws, ending in soft, leathery soles.");
	outputText("[pg]She has a pair of dusky, soft D-cup tits, with a single " + UnitSystem.displayInches(0.5, 1) + " nipple on each breast.");
		...
		private function kihaPregUpdate():Boolean
		{
			switch (pregnancy.eventTriggered()) {
				...
				case 7: 
						...
						return true;
				case 8: 
						outputText("\nKiha has grown to the point that she can barely stand, much less walk, so she uses her wings to float several " + UnitSystem.literalInches() + " above the ground. She has grown very weary as her pregnancy advances, pleading to the gods that her plight will be over soon. \n");
						return true;
				default:
