...
public function approachLatexy():void {
	...
	outputText("\n\n");
	//{Dick}
	if (flags[kFLAGS.GOO_DICK_LENGTH] > 0) outputText("Below her waist, a " + UnitSystem.displayInchWithHyphenTextually(flags[kFLAGS.GOO_DICK_LENGTH], 1) + ", " + gooCock() + " hangs, soft and shiny.  ");
...
private function latexyEatsADickItem(item:ItemType):void {
	...
	if (flags[kFLAGS.GOO_DICK_LENGTH] == 0) {
		...
		outputText("\n\n\"<i>You're making it harder!</i>\" " + flags[kFLAGS.GOO_NAME] + " whines, trying to cover it with her hands.  Of course, that only makes it harder, and a bead of oily pre-cum beads at the tip.  You could get used to this.  <b>" + flags[kFLAGS.GOO_NAME] + " now has a " + UnitSystem.displayInchWithHyphenTextually(flags[kFLAGS.GOO_DICK_LENGTH]) + " ");
		...
		if (item == consumables.GROPLUS) {
			outputText("Her " + gooCock() + " shivers, and starts to sprout outward from the base, lengthening before your very eyes.  " + capitalizeFirstLetter(num2Text(UnitSystem.lengthInInches(1))) + "... " + num2Text(UnitSystem.lengthInInches(2)) + "... " + num2Text(UnitSystem.lengthInInches(3)) + "... new " + UnitSystem.literalInches() + " of gleaming prick reveal themselves to you!\n\n" + flags[kFLAGS.GOO_NAME] + " giggles, \"<i>You sure do like them big!</i>\"");
...
private function feedLatexyCumIndirectly():void {
	...
	if (player.cumQ() >= 4000) outputText("  That puddle swiftly fills into " + UnitSystem.footCompound() + " deep lake of spunk, a muddy morass of your own virile semen.");
...
private function feedLatexyGirlCumDirect():void {
	...
	outputText("\n\nYou confidently approach, stopping just in front of your latex pet.  She immediately drops to her knees, placing her nose a scant few " + UnitSystem.literalInches() + " from your vulva.  Her " + flags[kFLAGS.GOO_EYES] + " eyes lock onto the ");
...
private function tryToAssertMaleDomWhileLatexGooDrains():void {
	...
	outputText("\n\nOnce your [cock biggest] is nice and slick with her vaginal excretions, though, you pull back again and dig your fingers back into " + flags[kFLAGS.GOO_NAME] + "'s ass, spreading her lush cheeks apart to reveal the tight little star of her backdoor.  " + flags[kFLAGS.GOO_NAME] + " gasps as she feels your crown press against her asshole, her mouth opening wide as your oh-so-wet prick plunges the first " + UnitSystem.literalInch() + " into her without resistance.  You groan with satisfaction as her vice-tight hole squeezes down around your slick shaft, desperately, reflexively trying to repel the intruding cockmeat; but you're so slick, so covered in liquid latex that " + flags[kFLAGS.GOO_NAME] + "'s stunned body can't resist you, and you begin to slide " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " into her, burying yourself to the hilt in a single long, painfully slow push.");
	...
	outputText("\n\nWhen your second orgasm passes, you roll limply off of " + flags[kFLAGS.GOO_NAME] + ", utterly sated.  Your [chest] heaving, you sigh peacefully as you close your heavy lids, eager to rest.  A moment later, though, you feel slick skin press against your own as " + flags[kFLAGS.GOO_NAME] + " curls up beside you, her face " + UnitSystem.literalInches() + " from your own.  With a wan smile, you reach out and run your fingers through her hair before pulling her into a long kiss that leaves your own taste on your lips.");
	//{Low happiness}
	if (gooHappiness() < 33) outputText("\n\n\"<i>If it's always going to be like that, maybe... being here with you isn't going to be so bad after all,</i>\" " + flags[kFLAGS.GOO_NAME] + " whispers.");
	//{Moderate happiness}
	else if (gooHappiness() < 66) outputText("\n\n\"<i>I'm sorry I got so forceful, [name].  I enjoy serving you... I do.</i>\"");
...
private function femalePCDomFucksLatexGooFuta():void {
	...
	if (flags[kFLAGS.LATEX_GOO_TIMES_FEMDOMMED_BY_PC] > 0) outputText(", and obediently, she does.  To reward her for being a good pet, you give her a deep tongue kiss.");
		...
		outputText(", but the order goes unheeded.  " + flags[kFLAGS.GOO_NAME] + "'s nose disappears into the slimy mess that your pussy has become, pressing deeper and harder to gluttonously devour your juices.  Tired of her insubordination, you shove her back, shuddering when " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of slithering tongue slides free.  The sensuously-skinned slut immediately tries to get back up, but you force her down and slap her, right across the face.  She stops cold, slowly raising a hand to her cheek.  Glittery black tears escape from her eyes as she apologizes, \"<i>I'm sorry!  I'm sorry!  I'll be good, I'll be good!  I promise!</i>\"");
...
private function malePCDomFucksLatexGoo():void {
	...
	outputText(".  She whines, but wiggles back in your direction, staying locked on her target in spite of the punishment.  An ebony slit, so warm and wet that it feels like it's blowing steam onto your erection, closes the few " + UnitSystem.literalInches() + " of separating air.");
	...
	outputText("\n\nYou're breathing hard and more than a little exhausted from cumming back to back.  Still, there's a bubbly latex butt just " + UnitSystem.aFewFeet() + " away, bouncing back and forth hypnotically.  You did say you were going to cream all three holes, didn't you?");
	...
	outputText("\n\nLike hell!  You stagger up from your impromptu brace and grab hold of your sensuous slut once again, this time lining up with her asshole.  You're in charge here, and by gods, you're going to outfuck her!  You spank her bottom on the cheek you've ignored up to now, setting it bouncing.  Before the waves of enticing jiggles can settle, you lurch your [hips] forward and stuff your " + player.cockDescript(0) + " straight into the crinkled latex asshole.  The ebony pucker stretches obscenely around your girth, much tighter than the other two holes you've plumbed today.  " + capitalizeFirstLetter(UnitSystem.literalInch()) + " after " + UnitSystem.literalInch() + " of semi-turgid dick-flesh passes into the goo-woman's anus, but not without some difficulty.");
