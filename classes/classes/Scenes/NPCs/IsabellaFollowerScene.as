	...
	public class IsabellaFollowerScene extends NPCAwareContent 
	{
		...
		internal function isabellaMoovesInGreeting():void {
			...
			if (flags[kFLAGS.ISABELLA_TIMES_OFFERED_FOLLOWER] == 0) {
				...
				else outputText("squeeze against your " + player.chestDesc() + ", the four hard nipples capping each milk-flooded breast pressing on your own.");
			...
			//Repeat Offer
				...
				addButton(3, "Fight 4 Rape", isabellaScene.fightIsabella).hint("Fight to rape the cow-girl. She won't be too happy with what you've done afterwards though.");
				addButton(5, "Accept Offer", moveTheBitchIn).hint("Accept the offer of having Isabella moving into your camp.");
			}
		...
		public function callForFollowerIsabella():void {
			...
			if (flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0)
			{
				...
				if (isabellaAccent()) outputText("Hi " + player.short + "!  Vat do you need from Izabella?");
				else outputText("Hiya " + player.short + "!  Anything I can help you with?");
				outputText("</i>\"");
				if (player.hasCock()) {
		 ...
		 */
		public static function getDescAppearEventNotPregnant():String {
			return "The cow-girl is about " + UnitSystem.display("seven and a half feet", UnitSystem.displayHeight2EstimateTextually(7.5)) + " tall. Instead of feet, she has hooves, complete with fur that grows part-way up her legs. Her olive skirt only covers the upper portion of her dusky, spotted thighs, and it flares out deliciously from her swaying hips. Isabella's top is sheer, white silk that barely hides anything from you, least of all her exotic, quad-tipped nipples. Unlike most of the rest of her, her face is not spotted with dark and white patches. Instead it is pure, unbroken chocolate in color. Two small, bovine horns sprout from her head, emerging from the tangle of her unruly, red curls. She even has a pair of cow ears that flick back and forth from time to time.";
		}
		/**
		 * Description of isabella if she is in the first event stage of pregnancy
		 */
		public static function getDescAppearEventFirstStagePregnant():String {
		 ...
		 */
		public static function getDescAppearEventLastStagePregnant():String {
			...
			if (isabellaScene.pregnancy.isPregnant) {
				switch(isabellaScene.pregnancy.event) {
					...
					case 1:
						...
						break;
					case 2:
						outputText(getDescAppearEventFirstStagePregnant());
				...
				outputText(getDescAppearEventNotPregnant());
			}
		...
		protected function campIzzySexMenu():void {
			...
			menu();
			if (player.hasCock()) {
				addButton(0, flags[kFLAGS.ISABELLA_BLOWJOBS_DISABLED] == 0 ? "No BJs" : "Plz BJs", toggleIsabellasMorningWoodChopping);
			}
			outputText("\n\n");
			outputText("You could get a drink of Isabella's delicious milk, straight from the tap.");
			addButton(1, "Drink Milk", isabellaScene.nomOnMommaIzzysTits).hint("Get your fill of some delicious cow-girl milk, straight from the tap!");
			outputText("  There's always the opportunity to climb under her skirt and service her as well.");
			addButton(2, "Service Her", isabellaScene.volunteerToSlurpCowCunt).hint("Put that tongue of yours to a good use servicing that cow pussy of hers.");
			if (player.lust >= 33) {
				if (player.hasCock()) {
					addButton(3, "Hotdog", repeatGermanBratwurstInCamp).hint("Make good use of the cow's oh-so-soft, ample rump.");
					if (player.shortestCockLength() <= 9) {
						outputText("  Or you could have her suck you off now, since she seems quite taken with the size of your penis.");
					...
					else addButtonDisabled(6, "TentacleSex").hint("Requires you to have at least 3 tentacle cocks of considerable length.");
				}
			}
			else {
				outputText("  <b>You aren't aroused enough to initiate sex.</b>");
				addDisabledButton(3, "Hotdog", "You're not aroused enough to consider doing that.");
				addDisabledButton(4, "Get Sucked", "You're not aroused enough to consider doing that.");
				addDisabledButton(5, "Fuck Her", "You're not aroused enough to consider doing that.");
				addDisabledButton(6, "TentacleSex", "You're not aroused enough to consider doing that.");
			}
		...
		public function isabellaMorningWakeupCall():void {
			...
			outputText("\"<i>Oh fuck!</i>\" you groan without meaning to, lifting your " + player.hipDescript() + " to press your ");
			if (player.balls > 0) outputText(player.ballsDescriptLight() + " against her chin, the cum-bloated orbs bouncing and shuddering with your " + player.cockDescript(x) + " as release nears.");
			else {
				outputText(player.cockDescript(x) + " the whole way into her mouth, the slippery, hot cow-girl's lips sealing tightly around your ");
			...
			outputText("milk-puddle around your " + player.legs() + ".  She sighs and says, \"<i>");
			if (flags[kFLAGS.ISABELLA_MORNING_FELLATIO_COUNT] == 1) {
				...
				if (isabellaAccent()) outputText("Yum!  You cum so hard for momma Izabella, don't you?</i>\"  The sated cow-girl gives your " + player.cockDescript(x) + " an affectionate squeeze before murmuring, \"<i>So yummy...");
				else outputText("Yum!  You cum so hard for momma Isabella, don't you?</i>\"  The sated cow-girl gives your " + player.cockDescript(x) + " an affectionate squeeze before murmuring, \"<i>So yummy...");
			}
			else {
		...
		private function repeatGermanBratwurstInCamp():void {
			...
			outputText("You ask Isabella if she would mind helping you blow off some pressure before you go back out.  She glances down at " + player.sMultiCockDesc() + " and ");
			if (flags[kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING] < 5 && flags[kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING] > 0) {
				...
				if (isabellaAccent()) outputText("gleefully muses, \"<i>Oooh, I see.  Of course I vill help you, " + player.short + "!</i>\"  She traces a finger around " + player.oMultiCockDesc() + "' outline and continues, \"<i>It vould be mein pleasure to squeeze it all out into mein caboose.  Zat is what you vant, nein?");
				else outputText("gleefully muses, \"<i>Oooh, I see.  Of course I'll help you, " + player.short + "!</i>\"  She traces a finger around " + player.oMultiCockDesc() + "' outline and continues, \"<i>It would be my pleasure to squeeze it all out into my caboose.  That is what you want, no?");
			}
			outputText("</i>\"\n\n");
			...
			outputText("You nod, catching a whiff of her musky pussy's aroma without meaning to.  It's not like you could avoid it, with her body mere " + UnitSystem.literalInches() + " in front of your " + player.faceDescript() + ", shaking so perfectly and so closely that an occasional droplet catches on your nose.  Your hands clamp around the chair with an iron grip as you fight not to rise and rape her tender, teasing asshole on the spot.  " + player.SMultiCockDesc() + " is trickling and dribbling, bouncing with your heartbeats as your " + player.legs() + " shiver and shake with denied lusts.  You NEED to do something... to fuck, thrust, or just fap... anything - your " + player.multiCockDescriptLight() + " can't take it anymore!\n\n");
			
			outputText("Drawn up by your dick, you let go of your grip on the cow-crafted chair and start to rise into Isabella.  She expertly drops her bouncy butt into your " + player.chestDesc() + ".  It knocks you back down into place while she scolds, ");
			...
			outputText("The curvaceous cow-girl's stance shifts slightly, her teasing, shaking dance coming to a stop for just a moment as she adjusts to rest her rump against your " + player.cockDescript(x) + ".  ");
			...
			if (player.cockArea(x) <= 38) {
				...
				//(if libido <=15:  
				if (silly() && (player.lust100 < 50 || player.lib <= 20)) {
		...
		private function tentacleBoneFollowerIzzy():void {
			...
			outputText("You grin at Isabella as you nonchalantly remove your " + player.armorName + ", exposing your monstrous tentacle-cocks, fully erect, to her frightened face.  Crossing your arms, you stand before her, dicks squirming all over.  \"<i>");
			...
			if (player.canFly() && player.str >= 90) {
				outputText("\"<i>Time for a wild ride, sweetheart.</i>\"  You flap your [wings], mobilizing all your muscles in order to lift yourself off.  The weight you are burdened with is absurdly heavy, and you groan and pant in your attempts at taking off; eventually, you manage to part from the possessive surface little by little, grunting in triumph as you see your redoubled effort paying off; soon you're way up in the sky and Isabella's dickflesh prison is wobbling back and forth as you drag her under you. If it weren't for your colossal tentacle strength, the poor cow-girl would probably crash to the earth in a matter of seconds.  You keep ascending until Isabella realizes how high you both are and shrieks.  \"<i>");
			...
			if (player.canFly() && player.str >= 90) outputText("you descend, setting your " + player.feet() + " on the soil again but still holding Isabella herself in the air; ");
			outputText("you feel your climax coming closer and closer.  Isabella's body is literally swooning in pleasure and her face displays all signs of an intense incoming climax.  The sight of your lover's bliss is enough to launch you over the edge; with an amazingly inhuman might, you cum. The spunk boils furiously as it spurts out of " + player.sMultiCockDesc() + ". You give in to your powerful orgasm as you coat her with white goo, filling her throat");
			if (t4 >= 0) {
				outputText(", ");
			...
			if (t7 >= 0) outputText("; drowning her nipples with your spunk");
			//[if 10th cock]
		...
		private function izzyMilkingMeinMilkersMya():void {
			...
			else outputText("Ahh, " + player.short + "... it's been sooo long since I've been properly milked, my poor breasts are fuller than they've ever been.  I have been trying to release the pressure myself, but there are just not enough hours in the day with everything else I must do, and my nipples get quite sore.");
			outputText("</i>\"\n\n");
		...
		private function izzyMilkingMeinMilkersMya2():void {
			...
			menu();
			addButton(0, "Allow It", AllowIzzyMilkerUse).hint("Allow Isabella to use the milkers. This seems like the right choice.");
			addButton(1, "MINE!", noMilkingMilky).hint("Don't allow Isabella; you'll be the one taking care of her.");
		}
		...
		private function sparring(type:int = 1):void {
			...
			outputText("Isabella lifts her shield and raps her knuckles against the solid steel, making a loud, gong-like sound that carries for some distance.  You raise your " + player.weaponName + " and prepare to fight.  It's on!");
		...
		private function isabellaBurps():void {
			...
			menu();
			addButton(0, "Run", runAwayFromIzzyBurps).hint("Step back and watch Isabella burp the gas out.");
			addButton(1, "Stay", getIzzyBurped).hint("Stay and help Isabella to burp.");
		}
		...
		private function getIzzyBurped():void {
			...
			if (flags[kFLAGS.ISABELLA_PROBOVA_BURP_COUNT] == 1) {
				...
				if (player.horns.value <= 0 || player.horns.type != Horns.COW_MINOTAUR) {
					if (player.horns.value > 0 && player.horns.type > Horns.NONE) outputText("  Your existing horns quiver, then reform into those of a " + player.mf("bull","cow"));
				...
				//([Female/herm: if breasts >G-cup and lactating]
					...
					outputText("  Propped up as you are with your soft " + player.breastDescript(0) + " nestling under your chin, you can easily see the growing puddle of alabaster liquid seeping into the ground.  You heave yourself up, the effort rewarded with a jet of milk shot into the air as your evidently over-laden boobs jostle about inside your " + player.armorName + ". It seems the effects of Isabella's gas has ramped your lactation to overdrive.");
					changed = true;
				}
				...
				if (player.hasCock() && player.longestCockLength() < 18) {
					outputText("  A pressure builds in your crotch, at first subtle, then incredibly obnoxious to the point where you're forced to undo the lower portion of your " + player.armorName + " to seek some sort of relief.  Your shock is total as you stare at your former " + player.cockDescript(0) + ", now measuring at least " + UnitSystem.displayInchesEstimate(18) + " long – a condition that Isabella has already noticed and is clearly formulating a gripe about.");
		...
		internal function receiveAllTheCowTOngues():void {
			...
			outputText("\n\nAs 'punishment', Isabella puts her mouth right up your [pussy] and lets her huge, wide tongue loll out, " + UnitSystem.literalInches() + " and " + UnitSystem.literalInches() + " pouring out of her mouth until your twat is covered by the warm, wet muscle.  You groan as Isabella makes a single long, slow lick from bottom to top, dragging her delightfully soft tongue across your sensitive flesh and finishing with a teasing flick across your [clit] that nearly sends you over the edge."); 
		...
		public function findIzzyMilking():void {
			...
			//[Fuck her] [Leave]
			addButton(0,"Fuck Her",fuckIsabellaInTheBarn).hint("Give Isabella a good surprise sex!");
		...
		private function fuckIsabellaInTheBarn():void {
			...
			outputText("\n\n" + player.clothedOrNaked("Stripping out of your [armor]", "Being naked to start with") + ", you sidle up behind the distracted bovine and grab your " + player.cockDescript(x) + ", giving it a few quiet strokes until it's nice and hard.  Then, with a grin, you let it pop free of your grasp so that it flops down between Isabella's gropable cheeks, quickly buried beneath her yielding flesh.");
		...
		private function isabellaBarnFuckPartII():void {
			...
			//{If big fucking cock}
				...
				outputText("  Isabella bites her lip as your oversized prick stretches her wide, whimpering that you'll tear her apart as your [cock smallest] pushes into her womb, her cervical lips clamping hard around your first " + UnitSystem.literalInches() + " of thick cockflesh.  \"<i>Aaaaahhhhhh, ");
				if (isabellaAccent()) outputText("It iz sehr too big, zo huge?</i>\"");
				else outputText("God it's so huge, I... it's tearing me apart!</i>\"");
			}
			outputText("\n\nOnce you're as far into Isabella as you can go, she relaxes her death grip on your [ass], and you start to move your hips, slowly rocking yourself back and dragging " + UnitSystem.literalInches() + " of thick cockmeat slowly, tantalizingly out of her.  Isabella moans lewdly, arching her back as you leave her box empty, her inner depths left gaping as you withdraw...  before plunging back in.  She cries out in ecstasy as you hammer back in with a single thrust, throwing yourself ");
		...
		public function isabellaGivesBirth():void {
			...
			if (flags[kFLAGS.ISABELLA_COWMOTHER] < 1) { //Not cowmother
				...
				if (babyGender == 1) { //Perfectly normal human baby, always male.
					outputText("\n\nTo your " + (isabellaScene.getIsabellaChildType(IsabellaScene.OFFSPRING_HUMAN_BOYS) > 0 ? "continued" : "") + " surprise, the baby is not a bovine like Isabella, but a perfectly formed and completely human boy.  His skin is pale white, and he seems to take more after you than her, even having your eyes and " + player.hair.color + " hair.  Still you doubt Isabella would ever care; he’s her son, after all.");
				}
				else {
