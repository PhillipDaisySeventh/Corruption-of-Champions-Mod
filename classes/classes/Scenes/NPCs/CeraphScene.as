...
//Ceraph's fluids boost PC libido and sensitivity massively either way.
		internal function maleFuckCeraphsPussy():void
		{
			...
			if (player.cockArea(0) <= monster.vaginalCapacity()) {
				...
				outputText("Ceraph squirms in the dirt as you bring your " + player.cockDescript(0) + " closer and closer to her female hole, her cock twitching with need like some kind of perverse, mutant clit.  Her hips twitch at you, as if eager to devour your maleness.  You don't keep her waiting, ");
				...
				outputText("The " + player.cockHead() + " of your " + player.cockDescript(0) + " squishes flat against her twat, too big for her prodigious demon-cunt to swallow.  Spurred by frustrated desires, you lift her legs and wrap her girlcum-slicked thighs about your ");
				...
				outputText("You grunt, hold the captive by her ankles, and use her like a disposable sex-toy.  The slap of your " + player.hipDescript() + " slapping into her thighs echoes loudly, nearly drowning out the wet squishing of your " + player.cockDescript(0) + " with each violent brush against her cuntlips.  ");
				...
				outputText("Underneath your own mammoth erection, you can feel Ceraph's smaller cock twisting and squirming, unloading its own hot fluids onto her sweat-slicked belly.  She wriggles and twitches under you, her thigh muscles clenching and releasing in time with her spurts as they push you beyond your limit.  You watch your " + player.cockDescript(0) + " as it strains and squirts, dumping your thick load into the demon's ");
...
//[Agree to become Ceraph's Fuckpet Bondage Toy bitch slut]
		private function agreeToBecomeCeraphsFuckpetBondageToyBitchSlut():void
		{
			...
			if (player.hasVagina()) outputText("  Your " + player.vaginaDescript(0) + " tingles as you wonder how many piercings and fetishes she could fit into your labia.");
			else if (player.cockTotal() > 0) outputText("  Your " + player.cockDescript(0) + " grows rock hard as you wonder if she'll give you a Jacob's ladder full of new fetishes and thoughts.");
			...
			outputText("Not seeing the harm and seeking to endear yourself to your mistress, you thrust your lower lip out.  Your obedience is rewarded by a sharp snap of dazzling pain.  Licking experimentally, you feel the smooth band that loops over the center of your lip. ");
			//if pierced there
			...
			if (player.tail.type == Tail.NONE) outputText("backside");
...
//[YES OH YES]
		private function yesOHGODYESPIERCELEVEL3():void
		{
			...
			if (player.totalCocks() > 0) {
				...
				outputText("feminine moisture drools from between your lips and your " + player.clitDescript() + " turns into a hard button.");
			}
			else outputText("your body aches for release.");
			...
			//[cunts]
				...
				outputText("spreads your lips");
				if (player.vaginas[0].clitPierced > 0) outputText(", the old piercing clattering to the ground as it slides out of your flesh,");
				outputText(" and gets ahold of the flesh around the base of your " + player.clitDescript() + ".  With practiced ease, she snaps the piercing closed, attaching the diamond stud to you while the pain fuzzes your vision red.");
				player.vaginas[0].clitPierced = 1;
		...
		public function ceraphRapesYouBADDAWGYODIGGITY():void
		{
			...
			if (player.totalCocks() > 0) {
				...
				outputText("our " + player.multiCockDescript() + " is twitching and dripping pre-cum, on the edge of orgasm.  ");
			}
			if (player.hasVagina()) {
				outputText("Your " + player.vaginaDescript(0) + " twitches, aching with the desire to be touched or filled, and your hips twitch lewdly, desperate for sensation.  ");
			}
...
//(REQ's – HUGE WANG, Exgartuan or Jojo corruption, and !centaur)
		private function hugeCorruptionForceFuckCeraph():void
		{
			...
			outputText("She's quite the sore loser!  You drop your " + player.armorName + " on the ground and stroke your ");
			if (player.cockTotal() > 1) outputText("largest ");
			outputText(player.cockDescript(x) + ", feeling the blood pump into it.  The thick, penile flesh inflates to the ludicrous size it adopts every time you get hard.  Ceraph's eyes widen into dinner plate-sized saucers of shock as she beholds the behemoth that sprouts from your loins.  She actually tries to scrabble away in the dirt, but she fails miserably.  ");
			...
			if (player.statusEffectv1(StatusEffects.Exgartuan) == 1) outputText("You pat your " + player.cockDescript(x) + " and say, \"<i>A little help please, Exgartuan?</i>\"  ");
		...
		public function friendlyNeighborhoodSpiderManCeraph():void
		{
			...
			if (player.cor > 66) outputText("and without a second thought you disrobe, tossing your " + player.armorName + " to the side.  It thumps heavily in the dry mountain dust.");
			...
			outputText("You ready your " + player.weaponName + ", but Ceraph just throws her head back and laughs in your face.  \"<i>");
...
//[Trade Genitals]
		private function tradeCeraphCrotchityBits():void
		{
			...
			//(Cunts) 
				...
				outputText("The demon's gaze drops down between your " + player.legs() + ", and she remarks, \"<i>");
			...
			//(Tits) 
				...
				outputText("Ceraph's hands reach up to cup the swell of your " + player.biggestBreastSizeDescript() + " as she asks, \"<i>");
...
//*Orgy floor Drain
		public function ceraphBodyPartDreams():void
		{
			...
			if (choice == 2) {
				...
				if (subChoice == 0) {
					...
					if (!player.hasVagina()) outputText("You don't have a vagina any more, do you?");
				...
				//PUSSY CLITPUMP
					...
					if (!player.hasVagina()) outputText("Wait... you don't have a clit any more... Ceraph took that away.  ");
					else outputText("Wait... that's not your " + player.clitDescript() + "... it's the one Ceraph took from you.  ");
		...
		internal function buttRapeCeraph():void
		{
			...
			if (monster.HP < 1) outputText("  Cunt-juice splatters over your " + player.legs() + " as Ceraph's now-energetic tail plunges into her sodden box, pumping into it in time with the penetration of her asshole.");
			else outputText("  Cunt-juice dribbles over your " + player.legs() + " as Ceraph's tail slowly and purposefully plows her box, mimicking the penetration of her asshole.");
			outputText("\n\n");
