...
private function acceptBimboSophie():void {
	...
	outputText("Sophie looks up and gasps, \"<i>Oh no!</i>\"  Thankfully, the ditz remembers her own wings and spreads them, slowing the descent to a rapid, controlled glide.  You sigh with relief, supported by her arms and legs " + UnitSystem.display("thousands of feet", "hundreds of " + UnitSystem.meters()) + " in the air.  Sophie titters, \"<i>Like, which way should we go, " + player.short + "?  Everything is all funny up here, and I dunno where you live.  We need a place to fuck!</i>\"\n\n");
...
private function buttFuckBimboSophie():void {
	...
	outputText(" delic... de... yummy!</i>\"  You slap her ass, grab her tail feathers, and pull the harpy's winking asshole down onto your " + player.cockDescript(x) + ". Her voices rises, not in anger, but in delight, and your " + player.cockHead(x) + " pulses pleasantly in the first few " + UnitSystem.literalInches() + " of the harpy's rectum.  The bimbo relaxes and sinks down, giggling and asking, \"<i>Is-ooh-is my butt, like, good for you?  It's soooo tight, and it's making my cunny all wet and stuff!</i>\"\n\n");
...
private function fuckBimboSophiesTits():void {
	...
	outputText(" in warm, smooth skin.  The smell of your " + player.cockDescript(x) + " mere " + UnitSystem.literalInches() + " from her nose seems to startle Sophie from her trance, and she breaks into a smile so big you'll worry it'll break her face.\n\n");
...
private function sophieImpregnationSex():void {
	...
	outputText("  You push on, reveling in the tight clenching of her anal ring as it passes over every " + UnitSystem.literalInch() + " of cock, squeezing you like a slippery cock-ring as you nestle deeper into the harpy's plump bottom");
	...
	if (player.cumQ() >= 500) {
		...
		if (player.cumQ() >= 1500) {
			...
			outputText(".  You've filled every " + UnitSystem.literalInch() + " of her intestines with cum, and there's simply nowhere for your fresh, virile squirts to go but out!");
		}
	}
...
public function sophieFenCraftedSex(morning:Boolean = false):void {
	...
	//{Regular Sex}
		...
		clearOutput();
		sophieSprite();
		outputText("You hastily remove your armor and lie back on your blankets, beckoning Sophie forward with your hand.  The slutty, sex-hungry bimbo prances up on her tiptoes, supporting herself with flapping wings as her flared hips roll hypnotically, ever closer.  In no time, she's straddling your supine form; dripping sex positioned " + UnitSystem.aFoot() + " above.  The moisture her puffy, aroused cleft exudes patters off your ");
	...
	if (player.cumQ() >= 2000) outputText("  The oozing spunk soaks you thoroughly, and Sophie's thighs become matted with the " + UnitSystem.literalGallons() + " of gunk that squirts out of her lips.");
