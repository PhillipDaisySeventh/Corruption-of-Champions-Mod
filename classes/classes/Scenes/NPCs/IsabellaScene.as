package classes.Scenes.NPCs 
{
	...
	public class IsabellaScene extends NPCAwareContent implements TimeAwareInterface 
	{
		...
		public function preFollowerMenus(oralEnabled:Boolean = true):void {
			...
			addButton(2, "Fight", isabellaScene.fightIsabella).hint("Put up a bit of tussle with the cow-girl. She won't like you for a while though.");
			if (oralEnabled && player.shortestCockLength() < 9) {
				addButton(3, "Get Licked", isabellaScene.izzyGivesSmallWangsFreeOral).hint("Put the cow's tongue to a good use pleasuring your dick!");
			} else {
				addDisabledButton(3, "Get Licked", oralEnabled ? "She likes to lick small cocks." : "She doesn't seem interested in that for the moment.");
			}
			addButton(4, "Offer Oral", isabellaScene.volunteerToSlurpCowCunt).hint("Offer to eat the cow-girl out and get a taste!");
		...
		public function isabellaGreeting():void {
			...
			if (flags[kFLAGS.ISABELLA_CAMP_APPROACHED] == 0) {
				...
				menu();
				addButton(0, "Approach", isabellaGreetingFirstTime).hint("Investigate the source of the voice.");
			...
			if (flags[kFLAGS.ISABELLA_MET] == 0) {
				...
				if (player.tallness > 78) {
					...
					//[Talk] [Fight] [Leave]
					menu();
					addButton(0, "Try To Talk", tryToTalkDownAngryCow).hint("Try to convince the cow-girl not to get violent towards you.");
					addButton(1, "Fight", unwelcomeFightCowGal).hint("Defend yourself against the angry cow-girl.");
					addButton(4, "Leave", leaveAngryIzzy).hint("Of course, the best action is to leave the cow-girl alone for now, give her time to calm down.");
				} 
				//(Shorter PC's) 
				else {
			...
			//Camp Meeting – Repeat Unwelcome
				...
				menu();
				addButton(0, "Try To Talk", tryToTalkDownAngryCow).hint("Try to convince the cow-girl not to get violent towards you.");
				addButton(1, "Fight", fightIsabella).hint("Defend yourself against the angry cow-girl.");
				addButton(4, "Leave", leaveAngryIzzy).hint("Of course, the best action is to leave the cow-girl alone for now, give her time to calm down.");
				return;
			} 
			...
			//Camp Meeting – Standard Repeat
				...
				if (player.hasCock()) {
					outputText("  She takes a long sniff and glances between your " + player.legs() + " at your groin");
		...
		public function unwelcomeFightCowGal():void {
			clearOutput();
			outputText("You ready your " + player.weaponName + " and adopt a fighting pose.  No cow is going to chase you away!");
		...
		public function tryToTalkDownAngryCow():void {
			...
			//(Else) 
				...
				if (player.weaponName != "fists") outputText("You toss aside your " + player.weaponName + " and");
				else outputText("You");
		...
		public function nomOnMommaIzzysTits():void {
			...
			if (player.tallness > 78 || player.isTaur()) {
				...
				if (player.hasCock()) {
					outputText("Your " + player.armorName + " tents ");
					if (player.biggestCockArea() > 40) outputText("hard, barely constraining " + player.sMultiCockDesc() + ".");
					else outputText("visibly from " + player.sMultiCockDesc() + ".");
					outputText("  ");
				}
				if (player.hasVagina()) {
				...
				outputText("You're roughly yanked away from the milk spouting breast and pulled up to Isabella's face.  The cow-girl's bronzed visage is flushed darker, and even one of the white patches on her neck is crimson-tinged with lust.  She licks the creamy milk from your lips with an exceptionally wide, smooth tongue and then passionately french-kisses you, squirting more of her milk onto your " + player.chestDesc() + " the entire time.");
				if (player.hasCock()) outputText("  " + player.SMultiCockDesc() + " twitches and drips from the intensity of the kiss, but you had completely forgotten about your sexual needs while you were drinking.");
				else if (player.hasVagina()) outputText("  Your " + player.vaginaDescript(0) + " aches with need and desire from the intensity of the kiss, but you had completely forgotten it during the feeding.");
				...
				if (player.hasCock() && x >= 0) {
					...
					if (flags[kFLAGS.ISABELLA_SLEEP_RAPE_COUNTER] == 0) outputText("  S-she fucked you in your sleep?  That explains how good your dreams were.  On one hand you feel a little violated, but on the other you have to wonder how long this woman has held her camp against the demons with nothing to please her.");
			...
			//Voluntary Nursing (Small Characters)
				...
				outputText("You stop and look at them, just look, leaving nothing in the air but a long, pregnant pause that seems to go on and on.  Isabella coughs, snapping you from your reverie – you're standing " + UnitSystem.aFoot() + " away from those sweat-glazed orbs, and you jerk your head back.  If you leaned any further forward, her prominent, quad-tipped areola would be in your mouth.  The cow-girl laughs and scoops you up in her arms before you can hesitate further.  Your cheek is crushed against a white spot on the side of her left tit, and your " + player.buttDescript() + " comes to rest on the short fur that sprouts from her thick thighs.  Isabella coos, \"<i>Relax, " + player.short + ", and let Izabella sate your thirst.  You vill love it.</i>\"\n\n");
				
				outputText("She forcefully shifts your position, angling her left arm to cradle your back as you're dragged off the creamy part of her chest and onto the darker portions of her mounds.  A " + UnitSystem.displayInchWithHyphenTextually(3) + " wide nipple looms at the bottom of your vision, and moist sweetness brushes over your lips.  You're struck by how very much this entire situation is out of your control.  Confident hands have you locked in their embrace while you curl on Isabella's lap in the most helpless way, and there's not a lot you can do to stop it, even if you wanted to.  Your mouth yields to her insistently pressing nipples, letting all four tips slide through your puckered gateway and press together over your tongue.\n\n");
				...
				if (player.hasCock() && x >= 0) {
					...
					if (flags[kFLAGS.ISABELLA_SLEEP_RAPE_COUNTER] == 0) {
		...
		public function izzyGivesSmallWangsFreeOral():void {
			...
			else outputText("pose lewdly and trace a finger over your bulge as you inform Isabella just how happy you'd be to feel her tongue on your " + player.cockDescript(x) + ".");
			...
			outputText("Isabella goes to work immediately, undoing the lower portions of your " + player.armorName + " with strong, forceful motions that shake your " + player.assDescript() + " as she forcibly disrobes you.  Free at last, your " + player.cockDescript(x) + " flops out");
			...
			outputText("The cow-girl pulls down on her neckline, giving you a tantalizing view of her cream and caramel cleavage.  She leans forwards and presses her milk-swollen tits against your " + player.legs() + ", rocking up and down so that you can feel each of the soft orbs squeezing around you.  Isabella's tongue slides out... and out... and out, until you see at least " + UnitSystem.displayInchesEstimateTextually(7) + " of tongue hovering over your " + player.cockDescript(x) + ".  Her warm spittle drips from the pulsing, smooth pink exterior of her tantalizing tongue while it lashes back and forth, less than " + UnitSystem.literalAnInch() + " away from your " + player.cockHead(x) + ".  Each drop of fallen cow-girl spit that lands on your " + player.cockHead(x) + " only turns you on more, until you're grunting and panting at her, begging like an animal with a needy expression on your face.\n\n");
			...
			if (player.sens < 50) {
				...
				outputText("The cow-girl returns to her task with gusto, snaring your " + player.cockDescript(x) + " with her tongue, but instead of going all-out with her corkscrew technique, she's pumping it, sliding her hot wetness up and down your shaft with practiced ease.  ");
				if (player.balls > 0) outputText("She cups your " + player.ballsDescriptLight() + " and begins caressing the twitching orbs, giving them gentle squeezes each time the pleasure forces them to involuntarily contract towards your groin.");
				else if (player.hasVagina()) {
					outputText("She probes your " + player.vaginaDescript() + " with her fingers, running them over your engorged lips and giving your " + player.clitDescript() + " ");
					if (player.getClitLength() < 3) outputText("gentle squeezes");
					...
					outputText(" of your " + player.cockDescript(x) + " to your " + player.assholeDescript() + ", teasing the sensitive skin with light touches of her fingernail.");
				}
				...
				outputText("Isabella pulls back and licks her lips, leaving you to realize that your " + player.legs() + " have been completely soaked with the cow-girl's own sweet cream.  ");
			...
			//(High sensitivity fail!) 
				...
				if (player.balls > 0) outputText("her hand stroking and gently squeezing at your " + player.ballsDescriptLight() + " as if she could milk a bigger load from them that way.");
				else if (player.cockTotal() > 1) outputText("her hand stroking and squeezing your neglected " + player.cockDescript(1) + " almost as an afterthought.");
		...
		public function volunteerToSlurpCowCunt():void {
			...
			if (player.cor < 33) outputText("Hesitantly, you start to lean closer and closer until you're mere " + UnitSystem.literalInches() + " from the cow-girl's slick box.  The air practically fogs with her anticipation.  A strong, impatient hand grabs you by the back of the head and pushes you forward, burying your nose and lips into her wet, squelching lips.");
			else if (player.cor < 66) outputText("Eagerly, you start to lean further and further forward until you're less than " + UnitSystem.literalAnInch() + " from the cow-girl's slick folds, and the air fogs with lusty anticipation.  Before you can dive in, her hand grips the back of your head and FORCES you inside her.  Her greedy lips swallow your nose and lips with one wet squelch.");
			else outputText("Unabashedly, you dive right into the cow-girl's lust-slicked snatch.  Her greedy lips swallow your nose and lips with one wet squelch, but the cow-girl doesn't seem quite satisfied until her hand is on the back of your head, mashing your " + player.faceDescript() + " roughly over her sex.");
			...
			if (!isabellaFollower() || !player.hasVagina() || player.biggestTitSize() < 1) {
				...
				//[Leave] [Get Cowlicked]
				menu();
				addButton(0, "Get Licked", isabellaFollowerScene.receiveAllTheCowTOngues).hint("You've done the service, now let Isabella return the favour.");
		...
		public function IsabellaWinsAndSpanks():void {
			...
			if (player.tallness <= 78) {
				...
				outputText("  A hand grabs hold of the back of your " + player.armorName + " and lifts you up, placing you firmly over the cow-girl's fur-covered knee.  You can feel the transition from fur to skin underneath your belly, at the midpoint of her thigh.  ");
				if (player.lust >= player.maxLust()) outputText("You start trying to hump and grind, but the angry cow will have none of it.  ");
				outputText("SMACK!  A powerful impact slams into your " + player.assDescript() + ", making you gasp out in pain");
				...
				if (player.findPerk(PerkLib.Masochist) < 0) outputText("draws a scream from your lips, pushing you past the bounds of consciousness.  Isabella hefts your limp form like a wet noodle and grasses sway behind you as you're carried off.");
					...
					if (player.hasVagina()) {
						outputText("  Your " + player.vaginaDescript() + " quivers and ");
			...
			//[Lose And Get Spanked – Tall]
				...
				outputText("  A hand grabs hold of the back of your " + player.armorName + " and props you up, ass in the air.  ");
		...
		public function isabellaRapesYouWithHerAss():void {
			...
			if (player.balls > 0) outputText(" or the tightening of your " + player.sackDescript() + " as it pulls your " + player.ballsDescriptLight());
			...
			if (player.cockThatFits(38) >= 0) {
				outputText("She slides her plump ass back down, arching her back to press your " + player.cockHead(x) + " firmly against her tight, pre-moistened anus.  The cow-girl relaxes slowly, letting her muscles dilate to allow your member inside. The tight ring of muscle slides over your tip, clenching just under the head for a moment before the tension oozes back out of her muscles.  Her large, rounded ass-cheeks flex involuntarily as " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of your " + player.cockDescript(x) + " is devoured by her anal passage.  The process is excruciating, but eventually the redhead is pressing her plush bottom against your groin");
				...
				if (player.balls > 0) outputText("Involuntary muscles contract, tugging your " + player.sackDescript() + " up against your crotch while your " + player.ballsDescriptLight() + " visibly contract, loading your body full of liquid lust that's about to explode.");
				...
				if (player.cumQ() >= 1000) {
					...
					if (player.cumQ() >= 1500) outputText(", but your " + player.cockDescript(x) + " won't let up.  She cries and moans, jism pouring out around your shaft");
			...
			//[TOOBIG]
				...
				if (player.balls > 0) outputText("Your " + player.sackDescript() + " pulls tight against your body, each of your " + player.ballsDescriptLight() + " quivering and pushing its load through you.");
				...
				outputText("  Warm pressure builds higher and higher, and then at once you're shooting, spraying ropes of jism a " + UnitSystem.display("half-dozen feet", "two " + UnitSystem.meters()) + " into the air.  Your urethra bulges, and you spurt out the next batch to splatter on Isabella's back.  Some of it lands on her black leather corset, glazing it with an off-white sheen.");
				if (player.cockTotal() > 1) {
				...
				outputText(", pooling around your " + player.legs() + " on the ground.");
		...
		public function IsabellaPostSpankFeedSex():void {
			...
			else outputText("in between your " + player.allBreastsDescript());
			outputText(", pushing your torso so hard it sinks " + UnitSystem.anInchOrTwo() + " into mud that reeks of Isabella's sex-juices.  You lie there, immobilized and defeated while you're forcibly raped, used like a small, disposable dildo.\n\n");
			
			outputText("The cow-girl lets some of the pressure off in order to tweak one of your " + player.nippleDescript(0) + "s, but as you gasp, her tongue is forced into your mouth, smothering your ");
			...
			else if (player.cumQ() < 1000) outputText("bursting into Isabella's constricting cunny and soaking every " + UnitSystem.literalInch() + " of her passage with your copious spooge.");
		...
		public function defeatIsabella():void {
			...
			menu();
			addDisabledButton(0, "Buttsex").hint("You're not aroused enough to consider doing that.");
			addDisabledButton(1, "Vaginal").hint("You're not aroused enough to consider doing that.");
			addDisabledButton(2, "Big Titfuck").hint("You're not aroused enough to consider doing that.");
			addDisabledButton(3, "Small Titfuck").hint("You're not aroused enough to consider doing that.");
			addDisabledButton(5, "Sixty-Nine").hint("You're not aroused enough to consider doing that.");
			addDisabledButton(6, "Lactation69").hint("You're not aroused enough to consider doing that.", "Lactation Sixty-Nine");
			...
			if (player.hasCock()) {
				if (player.cockThatFits(monster.analCapacity()) != -1) addButton(0, "Buttsex", PCVictoryOnIsabellaButtsex); 
				else addButtonDisabled(0, "Buttsex").hint(player.cockTotal() == 1 ? "Your dick is too big for the cow-girl's rear." : "None of your dicks would fit inside the cow-girl's rear.");
				if (player.cockThatFits(monster.vaginalCapacity()) != -1) addButton(1, "Vaginal", vaginalProdNPokeIsabella);
				else addButtonDisabled(1, "Vaginal").hint(player.cockTotal() == 1 ? "Your dick is too big for the cow-girl's cooter." : "None of your dicks would fit inside the cow-girl's cooter.");
				if (player.cockArea(player.biggestCockIndex()) > 70) addButton(2, "Big Titfuck", tooBigVictoryTittyFuckingFuntimesWithMilk);
				else addButtonDisabled(2, "Big Titfuck").hint("You need to have a large penis for this one!");
				if (player.cocks[player.shortestCockIndex()].cockLength < 9) addButton(3, "Small Titfuck", tinyVictoryTittyFuckingFuntimesWithMilk);
				else addButtonDisabled(3, "Small Titfuck").hint("You need to have a small penis for this one!");
			}
			if (player.gender > 0) addButton(5, "Sixty-Nine", victoryAgainstIzzzzzySixtyNine);
			else addButtonDisabled(5, "Sixty-Nine").hint("Sixty-nine is less fun when your crotch is featureless.");
			if (player.biggestLactation() >= 1) addButton(6, "Lactation69", victoryLactation69).hint("");
			else addButtonDisabled(6, "Lactation69").hint("You need to be lactating for this one.");
		}
		...
		public function victoryLactation69():void {
			...
			if (player.hasCock()) outputText("squeeze " + player.sMultiCockDesc() + " gently, feeling the heavy, turgid mass pulsating with each beat of your heart.");
			...
			outputText("With your bellies filling, the conflict that started it all seems like a distant dream that's washing away with each burst of sexual relief and swallow of pearlescent milk.  Isabella's moos of pleasure vibrate your " + player.nippleDescript(0) + ", adding to your own");
			if (player.breastRows.length > 1) outputText(", while your other " + player.breastDescript(1) + " plasters her crimson hair into her scalp");
			...
			else if (player.hasCock()) outputText("  Of course, you're pumping at " + player.sMultiCockDesc() + " just as hard, but the sounds of your fapping are easily drowned out by the enthusiastic cow-girl.");
			else outputText("  Of course, you're fingering your " + player.assholeDescript() + " just as hard, but that muscular hole barely makes a sound as you violate it.");
			...
			outputText("After a few moments of guzzling Isabella's orgasmic cream, your own climax starts to build, radiating from your squirting " + player.nippleDescript(0) + " and ");
			...
			//(Cuntnips) 
			if (player.hasFuckableNipples()) outputText("  Isabella thrusts her tongue at your " + player.nippleDescript(0) + ", burrowing it deep into the unusual passage to get at the last of your liquid bounty and inadvertently intensifying your pleasure.  Your arm gives out and you drop on top of her, burying her in tit while hers serve as your pillows.");
			//(Cock & Puss)
			if (player.gender == 3) {
				if (player.wetness() == 5) outputText("  The juices that splatter from your " + player.vaginaDescript() + " almost seems an afterthought, but when you cum, you cum <i>allll</i> over.");
				else if (player.wetness() > 3) outputText("  The juices that drip from your " + player.vaginaDescript() + " almost seems an afterthought, but when you cum, you cum all over.");
				else outputText("  The clenching of your " + player.vaginaDescript() + " almost seems an afterthought, but when you cum you cum all over.");
			}
			//(Cock)
			if (player.hasCock()) {
				outputText("  With a few more solid pumps " + player.sMultiCockDesc() + " goes rock hard");
				if (player.balls > 0) outputText(", your " + player.ballsDescriptLight() + " pulling tight against your groin");
			...
			//(Puss only)
			if (player.gender == 2) {
				outputText("  The constant fingering of your " + player.vaginaDescript() + " accomplishes its goal, making your velvet tunnel clench vise-tight against the intruding digit.  ");
				...
				outputText("  Your " + player.clitDescript() + " pulses and jumps with each brush of your thumb, and you nearly lose yourself in the hedonistic self-pleasure.");
			}
			//(butt)
			if (player.gender == 0) outputText("  The constant fingering of your " + player.assholeDescript() + " finally comes to fruition, sending bolts of pleasure from your anal passage.  The star clenches tight around your digits, contracting involuntarily around the invader.");
			outputText("\n\n");
			...
			outputText(" You nod as the blissed-out cow-girl closes her eyes, contented and cradling the slight paunch you've given her.  A moment later you've regained your strength, but you have trouble fitting back into your " + player.armorName + " with all the milk in your system.");
		...
		public function PCVictoryOnIsabellaButtsex():void {
			...
			outputText("You shrug out of your " + player.armorName + " and expose " + player.sMultiCockDesc() + " to the warm, breezy air");
			if (player.lust100 < 70) {
				outputText(", allowing ");
				if (player.cockTotal() == 1) outputText("it ");
				else outputText("them ");
				outputText("to harden precipitously.  The turgid flesh of " + player.sMultiCockDesc() + " grows harder and harder, egged on by the sight of Isabella's dark pucker and juicy, leaking cunt");
			} else {
			...
			outputText("You advance and grab her tail, yanking the bovine appendage back to pull those glorious ass-cheeks closer to your " + player.cockDescript(x) + ".  ");
			if (player.cockTotal() > 1) {
				outputText("It's probably the best suited to the task, out of your " + player.multiCockDescriptLight());
				if (y >= 0) outputText(", and your " + player.cockDescript(y) + " may as well go into that sloppy cow-cooch");
				outputText(".  ");
			}
			...
			outputText(", and she voices a moo of discomfort when you slap her moist backside.  You ignore it and pull harder on her tail, forcing her anus to devour your entire " + player.cockDescript(x) + " in a single stroke");
			...
			if (y >= 0) outputText("contracts around your " + player.cockDescript(y) + " and squeezes a flow of heated pussy-juice over the penis, cocooning it with enough warm heat and wetness to make you feel as if you're melting.");
			else outputText("spews out a few strands of girl-juice that cling to your " + player.legs() + ", hanging and stretching with every sticky slap against her body.");
			...
			outputText(" and feeling the cum build up inside your " + player.ballsDescriptLight() + ".");
			...
			if (y >= 0) outputText("Your " + player.cockDescript(y) + " emulates its brother, shooting ropes of semen into the cow-girl's drippy pussy.  ");
			...
			if (player.cumQ() >= 750) {
				...
				outputText(" with more spunk, enough to balloon her back-door and remove most of the sensation from your " + player.cockDescript(x) + ".");
				if (player.cumQ() < 2000) {
		...
		public function victoryAgainstIzzzzzySixtyNine():void {
			...
			if (player.gender == 2) outputText("nuzzle your " + player.vaginaDescript(0) + ", smearing your vulva with her lips even she spreads the folds to access your " + player.clitDescript());
			else if (player.cockThatFits(38) != -1) outputText("swallow your " + player.cockDescript(x) + " whole, plying her smooth, spit-covered tongue over every " + UnitSystem.literalInch() + " of it");
			else outputText("lick your " + player.cockDescript(x) + ", fondling it with her long, smooth tongue in ways that no human could");
			outputText(".\n\n");
			...
			//(VAGOOZLES) 
			if (player.gender == 2) {
				outputText("Isabella returns to her task with gusto, muff-diving deep and hard to plant her exquisitely long tongue far inside you.  Her lips circle your " + player.clitDescript() + ", ");
				if (player.getClitLength() >= 5) outputText("struggling with the cock-like appendage");
				else if (player.getClitLength() >= 2) outputText("suckling the large button with slow, measured inhalations");
			...
			//(WANGS – no fit)
				...
				outputText(" numerous times before bobbing her head up and down.  She's using her tongue like some kind of onahole, and the warm, slippery pressure makes your " + player.cockDescript(x) + " want to melt with pleasure.  The cow-girl slides all the way to the tip");
			...
			//(Dick + No fit)
				...
				outputText("Even as you exult in victory, the humming pleasure-squeals of Isabella's orgasm vibrate through her lips and tongue, spreading down your shaft.  The effect is profound and immediate, and even as Isabella loses control of her oral onahole, orgasm wracks your " + player.cockDescript(x) + ".  It trembles and pulsates a second before the first load bursts from the tip and splatters on Isabella's thin, white blouse.  The next doesn't fly as far, and drops on her neckline.  After that, you pour a few bursts onto her face.");
				if (player.cumQ() > 250) {
			...
			if (player.cockTotal() > 1) {
				outputText("  Throughout it all you feel like a one-man bukkake show, spurting jism from your " + player.multiCockDescriptLight() + " over the cumming cow-girl.");
			}
			outputText("\n\n");
		...
		public function tooBigVictoryTittyFuckingFuntimesWithMilk():void {
			...
			clearOutput();
			outputText("You toss aside your " + player.armorName + " to reveal your " + player.cockDescript(x) + " to the ");
			...
			if (player.lust100 > 70) outputText("pulsate with your raging lust");
			...
			if (player.cocks[x].cockLength < 50) outputText("burying the top few " + UnitSystem.literalInches() + " of your length into her mouth");
			...
			if (!isabellaFollower()) isabellaFollowerScene.isabellaAffection(3);
			outputText("Shrugging, you wipe " + player.sMultiCockDesc() + " off on her lips and get dressed.  This cow is one marvelous cum-dump.");
			player.orgasm('Dick');
			combat.cleanupAfterCombat();
		}
		...
		public function vaginalProdNPokeIsabella():void {
			...
			outputText("In spite of the distracting milk-fountains, your main focus remains on her cunt, and how wonderful that hot little box feels around your " + player.cockDescript(x) + ".  ");
			if (player.totalCocks() > 1) {
				outputText("You wish you had room for ");
				if (player.totalCocks() > 2) outputText("another " + player.oMultiCockDesc());
		...
		public function tinyVictoryTittyFuckingFuntimesWithMilk():void {
			...
			outputText(" cow-girl.  She squeals in delight at the sight of your " + player.multiCockDescriptLight());
			...
			outputText("The cow-girl grabs hold of your " + player.cockDescript(x) + " for emphasis and begins to stroke it enthusiastically.  Having her large (but still feminine) hand wrapped around you makes your dick look positively tiny in comparison, ");
			...
			outputText("Pulling herself up to lean against your groin, Isabella holds her hefty breasts in her hands and presses them around your " + player.cockDescript(x) + ".  The squishy-soft mammaries utterly envelops your penis in their warm, sweat-lubricated embrace");
			if (player.cockTotal() > 1) {
		...
		public function talkWithIsabella():void {
			spriteSelect(SpriteDb.s_isabella);
			clearOutput();
			flags[kFLAGS.ISABELLA_TIMES_TALKED]++;
			if (flags[kFLAGS.ISABELLA_TIMES_TALKED] == 1) {
