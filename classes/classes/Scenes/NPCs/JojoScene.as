...
private function jojoMutationOfferNo():void {
	jojoSprite();
	clearOutput();
	outputText("There are some lines even you won't cross.  Besides, having a sex-addled mouse with a constantly drooling " + UnitSystem.footCompound() + "-long cock is all the fun you can handle.\n\nWith that decided you prepare to call on your slut.");
...
public function useTentacleJojo():void {
	...
	outputText("You marvel at just how hard it is to dominate someone who can fuck you from " + UnitSystem.aFewFeet() + " away and wrap you up with a half-dozen appendages before you just give in and decide to enjoy it.\n\n"); 
	...
	if (player.totalCocks() > 0) {
		outputText("Upside down, you're forced to watch as his tail-tentacles curl out from between his legs, and extend towards your vulnerable " + player.multiCockDescriptLight() + ".  All three of them pause a few " + UnitSystem.literalInches() + " away for what seems an eternity, revealing circular dripping orifices filled with slimy wriggling cilia.\n\n");
	...
	if (player.totalCocks() == 0 && player.hasVagina()) {
		outputText("One of Jojo's tail-tentacles slides underneath his body, shooting forwards to seek out your " + player.clitDescript() + ".  It pauses " + UnitSystem.anInchOrTwo() + " away and opens up a horrific orifice full of wriggling tentacles and slick fluids.  The next moment it lunges forwards and sucks it inside.");
		//(If big clit – 
		if (player.getClitLength() >= 4) outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.");
...
private function repeatMilkJojo(tentacle:Boolean = false):void {
	...
	//TENTACLE JOJO MILKING (edited)
		...
		outputText("Blinking the stars from your eyes, you look up to see Jojo hanging there, panting and shaking.  The ever-present mechanical hum incrementally increases in pitch, climbing all the way to a keening wail that would put a banshee to shame.  Swollen bulges of cum are coursing through the tentacle-slut's members, distending the shafts and the tubes struggling to encapsulate the newfound girth.  Jojo's eyes roll back while frothy spit drips from his open maw and dangling tongue.  His body strains at the harness' bindings, the huge, dangling orbs between his legs visibly contracting as their cargo is released.  Semen bubbles from the seals at the edges of hoses as your pet drowns the cock-milker in spooge.  Gauges on the wall rapidly switch to 'F', one after another.  The last " + UnitSystem.literalYard() + " tops out, and then the pressure is blasting the suckers off his dicks, sending the sperm-packed machinery flying from the sheer, cummy pressure.\n\n");
		
		outputText("Jojo sags down in the harness while his members finish releasing their load.  The leftovers from his orgasm easily fill the stall with at least " + UnitSystem.aFoot() + " of cum.  You sigh at the realization that both you and your clothes are utterly drenched.  The harness unceremoniously dumps Jojo into the puddle of his own making, and as he rises, he hugs your " + player.legs() + " and says, \"<i>Thank you.  Did... did I cum enough?  I'll do more next time, I promise!</i>\"\n\n");
...
public function jojoFollowerMeditate():void {
	...
	doNext(camp.returnToCampUseOneHour);
		...
		private function jojosSecondRape():void {
			clearOutput();
			outputText("The poor mouse is already hard... his cock is throbbing eagerly as it protrudes through the opening in his robe, looking nearly " + UnitSystem.displayInchesEstimateTextually(8) + " long.  You're pretty sure it wasn't that big last time.\n\n");
			flags[kFLAGS.JOJO_STATUS] += 1;
			player.orgasm('Generic');
			dynStats("lib", -10, "cor", 4);
			if (player.gender == 1) {
		...
		private function jojosThirdRape():void {
			clearOutput();
			outputText("It's no wonder the monk's body has betrayed him so thoroughly, his " + monster.cockDescriptShort(0) + " is nearly " + UnitSystem.displayInchesEstimateTextually(10) + " long, pulsing with hot need.\n\n");
			if (player.gender == 1) {
		...
		private function jojosFourthRape():void {
			clearOutput();
			outputText("Jojo flops down, eyes filled with anticipation.  His self-control has really slipped away.  The corrupted and horny mouse on display here is anathema to the studious monk you met before.  His cock is close to " + UnitSystem.aFoot() + " long and over " + UnitSystem.displayInchesTextually(2) + " thick, veiny with arousal.\n\n");
			//Male Version
			if (player.gender == 1) {
		...
		private function jojosFifthRape():void {
			clearOutput();
			outputText("Jojo smiles serenely, pleased at the outcome, " + UnitSystem.aFoot() + " of tumescent mouse-meat bobbing at attention.\n\n");
		...
		private function corruptJojoAnalGentle():void {
			...
			if (player.cocks[x].cArea() < 10)
				outputText("You lift the mouse's balls out of the way and spit down onto the crinkled star of his anus, then lever your tip to the well used hole. There is little ceremony or foreplay, but his cock is already straining erect, and a blush colors his cheeks as you push into his ass, " + UnitSystem.literalInch() + " by " + UnitSystem.literalInch() + ". You set a slow and tender pace at first, but as your orgasm nears, your thrusts become more animal and needy.  ");
			else if (player.biggestCockArea() < 36)
				outputText("You slide your thick and drooling cockhead beneath the mouse's balls, working the musky drool of your pre-cum against the well used crinkle of his ass before forcing the thick vein-lined length of your " + player.cockDescript(x) + " into him. You watch as " + UnitSystem.literalInch() + " after thick, vulgar " + UnitSystem.literalInch() + " disappears into his body, grinning as his face contorts in a mix of pain and pleasure from it, and then start to fuck him in earnest, watching as his belly bulges with each thrust of your massive prick.  ");
			else outputText("You force your " + player.cockDescript(x) + " against the mouse's ass and watch as he shakes his head, silently begging you not to do it. You smile and grip his hips, then press forward hard, forcing his body to adapt to your girth, stretching his ass and belly dangerously. You can barely get more than " + UnitSystem.aFoot() + " of your " + player.cockDescript(x) + " into him before bottoming out against his diaphragm, so you just fuck him with what you can, churning his insides with each thrust.  ");
...
private function beeEggsInCorruptJojo():void {
	...
	outputText("\n\nYou withdraw one insectile spear from the slut-mouse's rump and replace it with another, thicker prong.  Your honeyed ovipositor slides right into Jojo's welcoming anus, his rectum stretching to welcome the egg-pipe's healthy width.  A trickle of lubricating, honey-like fluid dribbles from the tip to aid the penetration, and in no time flat, the mouse's ass is a slick fuck-tunnel that sweetly squishes with each pump of your abdomen.  His thick, " + UnitSystem.footCompound() + "-long mouse-cock actually softens slightly and flops around with each thrust, trailing trickles of stringing pre-cum as you squeeze it out of him.");
...
public function apparantlyJojoDOESlift():void
{
	...
	else if (player.findPerk(PerkLib.ControlledBreath) >= 0 && player.findPerk(PerkLib.CleansingPalm) >= 0 && player.findPerk(PerkLib.Enlightened) < 0)
	{
		...
		outputText("As the two of you give each other decidedly shaky bows, Jojo says, “<i>Great effort [name], you are... wow... I need a rest. I’ve earned it.</i>”  The two of you share a laugh and end your training.\n\n");
