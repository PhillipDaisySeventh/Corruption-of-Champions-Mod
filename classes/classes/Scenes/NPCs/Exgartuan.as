							...
							if (getGame().time.hours % 9 == 0) { //Only once every 9 hours or so.
								if (rand(3) == 0) { //lactation messing with!
									...
									if (rand(2) == 0)
										...
										outputText("Your hands knead and caress your " + player.breastDescript(0) + ", eagerly touching every " + UnitSystem.literalInch() + " of soft flesh.  You gasp when you realize what you're doing and pull them away");
...
private function drinkFountainEndowment():void {
	...
	if (rand(3) == 0 && player.biggestTitSize() > 1) {
		outputText("\n\nYour " + player.allBreastsDescript() + " balloon, each growing about four bra-sizes larger... they feel so... jiggly and sensitive.  Even your nipples seem to grow with them!  Your " + player.armorName + " feels tighter than ever!");
	...
	if (rand(3) == 0 && player.totalCocks() > 0) {
		outputText("\n\nYour " + player.multiCockDescriptLight() + " feels tighter inside your " + player.armorName + ", even when flaccid.  You shudder and realize you've probably gained more than a few " + UnitSystem.literalInches() + " in total length, and who knows how your thickness has changed.");
	...
	if (rand(4) == 0 && player.hasVagina()) {
		outputText("\n\nYour " + player.clitDescript() + " plumps up, visibly parting your lips even when you aren't turned on.  It probably ");
		player.changeClitLength(2);
		if (player.getClitLength() < 6) outputText("gets as big as a cock");
...
private function exgartuanInfestDick():void {
	...
	if (player.cocks[0].cockType != CockTypesEnum.DEMON) {
		...
		outputText("\n\nYou look down, and find your demonic cock pointing directly at you, and shaking with... indignation?  The ground hits your " + player.buttDescript() + " hard as you fall backwards, too surprised to maintain your footing.  Is your dick talking to you?");
	...
	//(+Demon dick possession – not demondicked)
		...
		outputText("\n\nYour " + player.cockDescript(0) + " puffs up, growing even larger as it absorbs the fountain's essence.  It dribbles pre-cum and twitches about as if sniffing the air, feeling very warm and sensitive.");
		outputText("\n\nA voice suddenly splits the air, demanding, \"<i>Satiate me mortal, or I'll make you find someone who will!</i>\"");
		outputText("\n\nWhat the hell was that?  You look around, but cannot find the source of the voice.  It speaks again, \"<i>Down here.  What are you, deaf!?</i>\"");
		outputText("\n\nYou look down, and find your demonic cock pointing directly at you, and shaking with... indignation?  The ground hits your " + player.buttDescript() + " hard as you fall backwards, too surprised to maintain your footing.  Is your dick talking to you?");
		outputText("\n\n\"<i>Yes I am.  You should consider yourself lucky – you're now the host of the great demon Exgartuan, and you'd best please me every few hours, or I'll make sure your body finds someone to relieve my building pressure.  But I think you'll do fine.  Come now, I can see a wonderful camp in your mind that we can paint white,</i>\" it suggests.");
...
private function exgartuanInfestTits():void {
	outputText("\n\nYour " + player.allBreastsDescript() + " jiggle as they grow MUCH larger, turning into obscene mounds that shake with every motion of your body.  All your " + player.nippleDescript(0) + "s puff up with them, gaining volume to match their new, larger homes.  They feel hot and ache to be touched.");
	temp = player.breastRows.length;
	while(temp > 0) {
		temp--;
	...
	outputText("\n\nThe ground hits your " + player.buttDescript() + " hard as you fall backwards, too surprised to maintain your footing.  Are your breasts really talking to you?");
	outputText("\n\n\"<i>Yes I am,</i>\" mutters Exgartuan, spurting a trickle of milk from your " + player.nippleDescript(0) + "s for emphasis, \"<i>and you had better take me back to that lovely camp I can see in your memories and give me a nice long massage.</i>\"");
	outputText("\n\nWell now... this was certainly unexpected.  Perhaps there's a way to be rid of this thing?");
...
public function exgartuanMasturbation():void {
	...
	if (player.statusEffectv1(StatusEffects.Exgartuan) == 1) {
		...
		if (player.cor < 33) {
			...
			if (player.countCocksOfType(CockTypesEnum.TENTACLE) > 0) outputText("  A tentacle-cock curls up, tightening around the base like an organic cock-ring.  It makes each of the corrupt nodules grow bigger, harder, and even more sensitive.");
		...
		//Medium corruption characters
			...
			if (player.biggestTitSize() >= 5) outputText("shove the bloated demon between your " + player.allBreastsDescript() + ", surrounding it in a home of jiggling flesh.  The wet demon-pre soaks into the skin of your tits, lubricating the passage as you mash them around, vigorously tit-fucking the source of your unholy desires.");
		...
		//high corruption characters
			...
			if (player.biggestTitSize() >= 5) outputText("your hands smashing your " + player.allBreastsDescript() + " around the " + player.cockDescript(0) + " and vigorously titfucking it.");
			else outputText("your hands repositioning your " + player.cockDescript(0) + " so it lays against your chest.");
	...
	//TITURBATION
		...
		//(ALT 1st PG)
			...
			outputText("You shrug off your top eagerly, ready to cooperate with the demon inside your " + player.allBreastsDescript() + " and enjoy a relaxing tit-massage.  You slide the offending material to the side and marvel at the wondrously large orbs on your chest.  Truly any place that can give you such wonderful endowments can't be evil.  You lean back, enjoying the warmth in the air as it flows over every extra-sensitive " + UnitSystem.literalInch() + " of your mounds, more than ready to get started.\n\n");
		}
		outputText("Both hands rise unbidden and begin to caress your " + player.breastDescript(0) + ".  They slide over every sensitive " + UnitSystem.literalInch() + " of ");
		if (player.hasPlainSkin()) outputText("flesh");
		else if (player.hasFur()) outputText("furry-covered flesh");
		else if (player.hasScales()) outputText("soft scaly flesh");
		else outputText("gooey surface");
		...
		outputText("At last, fingers wrap themselves about your " + player.nippleDescript(0) + "s, squeezing them gently and forcing happy gasps from your mouth.  ");
		if (player.hasVagina()) {
			outputText("Juice");
		...
		if (player.biggestLactation() > 1) outputText("As you calm down you realize your " + player.nippleDescript(0) + "s are dribbling streams of milk, and judging from the pools of whiteness in the soil, you turned into quite the little milk-sprinkler.  ");
...
public function exgartuanBored():void {
	...
	if (player.statusEffectv1(StatusEffects.Exgartuan) == 1 && player.cockArea(0) >= 100) {
		...
		if (select == 0) {
			...
			outputText("A tiny voice mutters, \"<i>How can you live with so few orgasms?  We need some quality time, " + player.short + ".  You've got a LOT of tension building up...</i>\"");
		}
		else if (select == 2) {
			outputText("You feel a stirring inside your " + player.armorName + " as it rubs against the material.  A muffled voice says, \"<i>Hey!  Don't forget me!  I need some air!</i>\"");
		}
		else if (select == 3) {
		...
		else if (select == 4) outputText("You feel your " + player.cockDescript(0) + " slowly expand and contract, air escaping its urethra and tickling your nether regions.  Did... did your dick just sigh?  Your demon is growing impatient.");
		else if (select == 5) outputText("Your " + player.armorName + " suddenly bulges, a stifled voice picking up along with it, \"<i>There has to be a hole out there for me to fuck.  A mouth, a cunt... stop slacking off and get on it!</i>\"");
		else if (select == 6) {
			...
			outputText("Your " + player.cockDescript(0) + " has started rubbing against your thighs as a familiar voice chimes in, \"<i>I've got a hot, steaming batch of cum ");
			if (player.balls > 0) outputText("boiling in your " + player.ballsDescriptLight() + ".");
		...
		switch(select) {
			case 0:
			 	outputText("You feel an ominous jiggling in your " + player.breastDescript(0) + " as the demon inside you stirs back to wakefulness.  The sound of a yawn being muffled by your " + player.armorName + " confirms it.");
				break;
			case 1:
				outputText("After a few hours of peace, Exgartuan begins to stir.  You can feel a desire to be touched building within your " + player.allBreastsDescript() + ".  What a needy demon.");
				break;
			case 2:
			...
			case 4:
				...
				break;
			case 5:
				outputText("Your " + player.chestDesc() + " fidget ominously, a muffled female voice piping up, \"<i>No cloth, metal, latex, or gel can protect you from me, champion.  Give in and caress these cans!</i>\"");
				break;
			case 6:
				outputText("The silent alone time you've been enjoying is coming to an end as the demoness dwelling within your bosom shifts in your " + player.armorName + ".  She wastes no time in stuffing you full of chest-focused temptations.");
				break;
			case 7:
		...
		//[FETISH CULTISTS TURNED ON BY DEMON-COCK]
			...
			switch(select) {
				case 0:
					...
					break;
				case 1:
					outputText("Openly touching herself, the fetish cultist's gaze never leaves your crotch, locked onto the outline of Exgartuan in your " + player.armorName + ".  What a slut.");
					break;
				case 2:
		...
		//(+Lust in combat) for girls
			...
			switch(select) {
				case 0:
					...
					if (player.cor < 50) outputText("  You glance down at the sensitive bulge in your " + player.armorName + " and sigh.  Damned demons.");
					break;
				case 1:
				...
				case 2:
					...
					break;
				case 3:
					outputText("You really want to end this fight and bend " + monster.a + monster.short + " over... Why is this taking so long, your " + player.cockDescript(0) + " does NOT want to wait!");
					if (player.cor < 50) outputText("  You snap back to reality and curse yourself for falling to the demon's desires.");
					break;
				case 4:
					outputText("This fight has gone on long enough; all you really want is to shove your " + player.cockDescript(0) + " into the " + monster.a + monster.short + "'s... Wait, what? Your possessed pecker is getting the better of you.");
					break;
				case 5:
					outputText("For a moment, you find it hard to concentrate, visions of impaling the " + monster.a + monster.short + " with your " + player.cockDescript(0) + " etching their way into your mind.  Curse this horny demon!");
					break;
				case 6:
					outputText("Pleasure begins to radiate through your being, suddenly.  You snap back to your senses in time to see your hands stroking the bulge in your " + player.armorName + ", answering to the whims of an unholy force.");
					break;
				case 7:
					...
					break;
				case 8:
					outputText("Tight loving assholes and gaping vaginas, slick long red tongues and breasts like fine china.  Succubi and the hot sexings they bring, these are a few of your favorite things!  ...Wait, what?  Was that musing your own?  And why are you absently stroking your " + player.cockDescript(0) + "?");
					break;
				case 9:
		...
		//(Taunts Male Foes -enemy lust)
			...
			switch(select) {
				...
				case 1:
					...
					break;
				case 2:
					outputText("Your " + player.cockDescript(0) + " squirms around, pushing your " + player.armorName + " out of the way.  Exgartuan barks, \"<i>Why are you bothering my host, fool?  Can't you see my magnificence?  You're nothing compared to me.  Go wank in shame and get out of our way.  We have cunts to fill.</i>\"");
					break;
				case 3:
					outputText("The waist of your " + player.armorName + " expands as your " + player.cockDescript(0) + " flies skyward fully erect, \"<i>Tremble in fear, peon! My " + UnitSystem.displayInchesTextually(player.cocks[0].cockLength) + " of might shame that feeble nub between your legs.  We've no time for your slack-jawed awe; go home and be a family man.</i>\"");
					break;
				case 4:
		...
		if (monster.totalCocks() > 0) {
			...
			switch(select) {
				...
				case 2:
					...
					break;
				case 3:
					outputText("Your " + player.chestDesc() + " begin to playfully bounce up and down, making it difficult for you to focus on your fight.  You aren't too worried, however, as the " + monster.a + monster.short + " seems to be hypnotized by the movement.");
					break;
				case 4:
					outputText("A sultry woman's voice teases the " + monster.a + monster.short + ", \"<i>There's no need for hostilities, is there?  Merely lay down your arms and come help fondle these marvels of creation.</i>\"  Your " + player.chestDesc() + " shimmy for emphasis.");
					break;
				case 5:
			...
			switch(select) {
				case 0:
					outputText("A brief fantasy of " + monster.a + monster.short + " brutally squeezing and caressing your chest fills your mind.  You break free of the twisted daydream and pull your hands away from your " + player.breastDescript(0) + ".  Damnit you're in combat!  There's no time for such foolishness!");
					break;
				case 1:
...
public function exgartuanArmorShift():void {
	...
	if (player.armor == armors.BEEARMR) {
		...
		outputText("The green gel-plate protecting your groin thins and presses tightly against you, molding around your " + player.cockDescript(0) + " in an incredibly lewd way.");
		player.modArmorName = "crotch-hugging glistening gel-armor plates";
		changed = true;
	}
	else if (player.armor == armors.LEATHRA) {
		outputText("Your leather armor shifts, pressing tightly against your upper " + player.legs() + " and molding itself around your " + player.cockDescript(0) + " to prominently display it.");
		player.modArmorName = "crotch-hugging leather armor segments";
		changed = true;
	}
	else if (player.armor == armors.INDECST){
		outputText("The chainmail bikini of your indecent steel armor rearranges and bends its interlocking rings to best shape itself around your " + player.cockDescript(0) + ", leaving very little else to the imagination.");
		player.modArmorName = "crotch-hugging practically indecent steel armor";
		changed = true;
	}
	else if (player.armor == armors.R_BDYST){
		outputText("The thin, transparent material of your red bodysuit begins to firmly press against your groin, perfectly shaping to your " + player.cockDescript(0) + " and every last one of its nubs and nodules.");
		player.modArmorName = "crotch-hugging red, high-society bodysuit";
		changed = true;
	}
	else if (player.armor == armors.SSARMOR){
		outputText("The fine silk that makes up your armor suddenly undoes itself around your crotch, exposing your " + player.cockDescript(0) + " to the open air. The thin strands in the air begin to re-weave themselves around your enormous member, forming a prominent new addition to your protection.");
		player.modArmorName = "crotch-hugging spider-silk armor";
		changed = true;
	}
	else if (player.armor == armors.S_SWMWR){
		outputText("The miniscule piece of swimwear that doubles as a tent to your " + player.cockDescript(0) + " begins to grow and encapsulate it, molding itself perfectly to your manhood.");
		player.modArmorName = "crotch-hugging slutty swimwear";
		changed = true;
	}
	else if (player.armor == armors.FULLCHN){
		outputText("You\'re taken by surprise as the binds of your chainmail begin to flatten and rearrange themselves, doing their best to match the curves of your " + player.cockDescript(0) + " and make its presence known.");
		player.modArmorName = "crotch-hugging full-body chainmail";
		changed = true;
	}
	else if (player.armor == armors.CHBIKNI){
		outputText("Your chainmail bikini rearranges and bends its interlocking rings to best shape itself around your " + player.cockDescript(0) + ", leaving very little else to the imagination.");
		player.modArmorName = "crotch-hugging revealing chainmail bikini";
		changed = true;
	}
	else if (player.armor == armors.FULLPLT){
		outputText("You begin to clench your fists as your steel platemail heats up around your " + player.legs() + " and crotch. Slowly it begins to press itself against your " + player.cockDescript(0) + " and match its every feature.");
		player.modArmorName = "crotch-hugging full platemail";
		changed = true;
	}
	else if (player.armor == armors.SCALEML){
		outputText("The steel scales that make up your armor begin to flap wildly around your crotch. They bend and shift as they attempt to match the profile of your " + player.cockDescript(0) + ".");
		player.modArmorName = "crotch-hugging scale-mail armor";
		changed = true;
	}
	else if (player.armor == armors.LTHRROB){
		outputText("Your leather armor shifts, pressing tightly against your upper " + player.legs() + " and molding itself around your " + player.cockDescript(0) + " to prominently display it through your robes.");
		player.modArmorName = "crotch-hugging black leather armor surrounded by voluminous robes";
		changed = true;
	}
	else if (player.armor == armors.RBBRCLT){
		outputText("You begin to feel your rubber outfit compressing itself against your upper " + player.legs() + " and " + player.cockDescript(0) + ", eliminating any pockets of air or wrinkles that may have existed before.");
		player.modArmorName = "crotch-hugging rubber fetish clothes";
		changed = true;
	}
	else if (player.armor == armors.ADVCLTH){
		outputText("The layer beneath your tunic begins to compress against your " + player.cockDescript(0) + ", highlighting every curve and nodule while lifting your package to be clearly visible beneath your outer layers.");
		player.modArmorName = "crotch-hugging green adventurer's clothes";
		changed = true;
	}
	else if (player.armor == armors.OVERALL){
		outputText("The denim of your overalls begins to press tightly against your " + player.cockDescript(0) + ", molding itself around your member and its every facet.");
		player.modArmorName = "crotch-hugging white shirt and overalls";
		changed = true;
	}
	else if (player.armor == armors.C_CLOTH) {
		outputText("Your clothing shifts, tightening up about your crotch until every curve and nodule of your " + player.cockDescript(0) + " is visible through the fabric.");
...
public function exgartuanLactationAdjustment():void {
	...
	if (player.biggestLactation() > 1) {
		...
		if (rand(2) == 0 || player.hasStatusEffect(StatusEffects.Feeder)) {
			outputText("Your nipples grow warm and sensitive, then start dripping milk into your " + player.armorName + ".  Exgartuan appears to be having some fun with you again...");
			player.boostLactation(player.breastRows.length);
		}
		//(Stops) 
		else {
			outputText("Your " + player.nippleDescript(0) + "s tighten up.  What's that demon up to?  Realization dawns on you when you realize your " + player.allBreastsDescript() + " no longer feel so 'full'.  Your lactation has stopped!");
	...
	//(Not lactating)
		...
		//(START)
		if (rand(2) == 0) {
			outputText("Your chest feels cold.  You touch your " + player.armorName + " experimentally and discover a few drops of milk have leaked from your " + player.nippleDescript(0) + "s!  The demon has made you start lactating!");
			player.boostLactation(player.breastRows.length);
		}
		//Nipple stuff
		else {
			//(Bigger Nipples!)
			if (rand(2) == 0) {
				outputText("The inner surface of your " + player.armorName + " arouses you as it rubs against your " + player.nippleDescript(0) + "s.  You think about it and realize they've never been like this before.  A quick check reveals each nipple has grown about a " + UnitSystem.halfInch() + " longer.  Damn demons.");
				player.nippleLength += .5;
			}
			//(SHORTER NIPPLES!)  
			else if (player.nippleLength > .5) {
				outputText("As time passes you realize something feels different about your chest.  A quick glance confirms your suspicious – your nipples have somehow been shortened by about " + UnitSystem.halfAnInch() + ".  You've no doubt Exgartuan is responsible.");
				player.nippleLength -= .5;
			}
		}
	}
...
private function exgartuanSleepSurprise():void {
	...
	if (player.cor <= 20 && player.findPerk(PerkLib.BulgeArmor) >= 0) {
		...
		if (player.cor < 33) {
			...
			if (player.biggestTitSize() >= 2) outputText(" and bouncing your " + player.allBreastsDescript() + " around it");
			...
			//(MED+COR, DONE A LOT:
				...
				if (player.biggestTitSize() >= 2) {
					outputText("  Your biceps squeeze around your " + player.allBreastsDescript() + ", mashing them into your " + player.cockDescript(0) + " and adding to the pleasure.");
					...
					if (player.hasFuckableNipples() || player.hasFuckableNipples()) {
						outputText(" from your " + player.nippleDescript(0) + " puddle in the bouncing cleavage and turn your chest into a slip-n'-slide for dicks.");
					}
				}
				outputText("\n\n");
			}
		...
		outputText(" with obscene, liquid sloshing.  Your " + player.hipDescript() + " and " + player.assDescript() + " rise off the ground with your back as your muscles lock");
		if (player.balls > 0) outputText(" and your " + player.ballsDescriptLight() + " pull tight against your groin from the effort");
		outputText(".");
		temp = player.cumQ();
		if (temp > 500) {
			...
			if (temp > 2500) outputText("  Back-pressure blasts your " + player.cockDescript(0) + " free from your imprisoning oral cavity along with a fountain of seed while you weakly try to cough out enough spooge to breathe.");
		}
		outputText("\n\n");
...
private function exgartuanBulgeTortureII():void {
	...
	outputText("  A little more pushing, pulling, knocking, groping, and stroking confirms what you feared: your " + player.cockDescript(0) + " has been cordoned off from you");
...
private function exgartuanBulgeTortureIII():void {
	...
	//[if cocks ==2]
	if (player.cockTotal() == 2) outputText("  Your " + player.cockDescript(1) + " remains flaccid beside this bronze adonis, as if sapped of all its life.");
	//[if cocks ≥3]
	...
	//[if breasts present]
	if (player.biggestTitSize() > 1) outputText("  Your " + player.chestDesc() + " work just as hard, smothering Exgartuan in all of their jiggly glory.");
	...
	//[if scrotum present]
	if (player.balls > 0) outputText("  Your " + player.ballsDescriptLight() + " churn with mighty force, roiling in anticipation.");
	outputText("\n\n");
...
private function boobgartuanSurprise2():void {
	...
	if (flags[kFLAGS.BOOBGARTUAN_SURPRISE_COUNT] == 0) {
		...
		//[if corruption <30]
		if (player.cor < 33) outputText("Your " + player.chestDesc() + " brush against the imp's enormous scrotum, causing him to eventually tumble over from the sensation, blocking the impeding entourage of imps behind you.");
		//[if corruption ≥30 & <70]
...
private function boobgartuanSurprise3():void {
	...
	outputText("Thankfully you were more prepared this time, your reflexes heightened enough to keep you from slamming your noggin yet again.  You also felt just where the shove originated from: your " + player.chestDesc() + ".  ");
	//[if occurrence ==0]
	if (flags[kFLAGS.BOOBGARTUAN_SURPRISE_COUNT] == 0) outputText("As you sit up and begin to piece the puzzle together, you're interrupted by a familiar voice, \"<i>You have time to fuck everyone in Mareth silly, but can't be bothered to spend an instant of it with these magnificent mounds?</i>\" You feel your " + player.chestDesc() + " jiggle as Exgartuan speaks, only to have them yank you forward, planting your face back in the ground.  \"<i>I'll just have to make sure you'll never forget again.</i>\"");
	//[if occurrence ≥1]
	else if (flags[kFLAGS.BOOBGARTUAN_SURPRISE_COUNT] < 4) {
		outputText("It's Exgartuan again; you haven't fondled her in a while.  \"<i>Rarin' for another round, eh, champion?</i>\" your " + player.chestDesc() + " ask you, jiggling furiously for emphasis, \"<i>I had hoped the previous lesson");
		//[if occurrence ≥2]
	...
	//[if occurrence ≥8]
	else outputText("Before another thought can cross your mind, your " + player.chestDesc() + " fly up and smack you in the gob! \"<i>" + player.short + ", it's that time again,</i>\" the eager demoness teases you.  You think she's grown to enjoy the torture she puts you through.  \"<i>If you insist on going day in and day out without spending quality time with these tits, then I'll just insist on instructing you otherwise.</i>\" Your beautiful bosom launches forward, throwing your mug right back in the dirt.");
	...
	//[if occurrence ≥1]
	else if (flags[kFLAGS.BOOBGARTUAN_SURPRISE_COUNT] < 4) outputText("\"<i>Ready for your final, eh, " + player.short + "?</i>\" Exgartuan teases, a faint lustful waver in her voice contrasting against her dominating demeanor, \"<i>Let's make this one to remember.</i>\"");
	//[if occurrence ≥4]
	...
	outputText("  Your solid hold around your " + player.chestDesc() + " slowly loosens, your tits perking up as they prepare for their much craved attention.  ");
	//[if corruption <50]
	...
	outputText("  Your hands waste no time, caressing every square " + UnitSystem.literalInch() + " of your flesh.  They gently brush with the softest of grace; they squeeze with the exceptional skill of any expert masseur.  Any particularly sensitive spots get extra attention, eliciting soft moans from you as they only emanate more warmth throughout your body.  You feel as if you're melting into your supporting rock, the pleasure only mounting as your absent gaze goes skyward.");
	//[if cock or vagina present]
	if (player.gender > 0) {
		outputText("  Your " + player.hipDescript() + " begin grinding together, moist with ");
		//[if cock >0]
	...
	//[if not lactating]
	if (player.biggestLactation() < 1 || player.lactationQ() == 0) outputText("  <b>Your " + player.chestDesc() + " have started to lactate profusely.</b>");
	...
	//[if occurrence ≥8]
	else outputText("Well, this has been fun, " + player.short + ".  I look forward to beating you senseless and enjoying the wonderful makeup massage should you forget to see to my means yet again.");
	outputText("</i>\"\n\n");
	
	outputText("Your " + player.chestDesc() + " begin to rumble and quake, filling you with a mix of anxiety, dread, and passion.  One moment later, an enormous force bursts forth against your hands; your nipples have popped back out, soaking and ready to - wait a second...");
	...
	if (player.averageNipplesPerBreast() > 1) {
		...
		if (player.nippleLength < 4) {
			outputText(", each swollen to at least " + UnitSystem.displayInchesEstimateTextually(4) + " in length and around " + UnitSystem.displayInchesEstimate(3) + " in diameter!");
		}
		else outputText(".");
	}
	//[if nipplesPerBreast <1 AND nippleLength <4]
		...
		if (player.nippleLength < 4) {
			outputText("  Your nipples have become much larger than before, each having swollen to at least " + UnitSystem.displayInchesEstimateTextually(4) + " in length and around " + UnitSystem.displayInchesEstimate(3) + " in diameter!");
		}
		else outputText("  Your nipples are much thicker than you remember, increasing to at least " + UnitSystem.displayInchesEstimate(3) + " in diameter.");
	}
	...
	if (player.gender > 0) {
		...
		outputText(" already coating your " + player.hipDescript() + ".");
	}
	//else
	else outputText(".");
...
public function exgartuanNagaStoleMyMasturbation():void {
	...
	//[if corruption ≥30 & <60]
	else if (player.cor <= 50) outputText("You happily remove your " + player.armorName + ", eager to pleasure yourself.  Your possessed cock pulses happily as you remove the last article.");
	//[if corruption ≥60 & <80]
	...
	outputText("Your dastardly dick moves on to phase two of its evil plan: stroking.  It's slow at first; you can feel each little nub react as each slick coil passes over them, the sensation just as maddeningly satisfying as everything else.  You feel as if your midsection is being stretched when your tail lifts as high as it can, exposing the base of your sinister shaft to the elements.  The roller coaster's been nothing but anticipation.  You enjoy just the mounting pleasure of the journey as it makes the first ascension.  Now you're hanging hundreds of " + UnitSystem.literalFeet() + " in the air, your ride inching closer to the precipice.  You can see everything that you're in for laying ahead of you, but you can never perceive how it'll actually play out in action.  All you can do is dig your nails into your harness and- great goblins' ghosts!  The demon's already gone to work, your tail bottoming out on you one instant and teasing only your head the next.  The sound of scales smacking a wide surface area fills the air.  You would begin to wonder how the repeated blows to your groin haven't resulted in injury, but your mind is enthralled by the feel of your super-sized shaft's stimulation.  Even if you weren't orgasmically paralyzed, you know better than to try and make sense of the dealings of your possessed pecker.\n\n");
