...
//	import classes.Scenes.NPCs.*;
	...
	public class Masturbation extends BaseContent {
		...
		public function setMasturbateButton(inDungeon:Boolean = false, index:int = 8):void {
			if (player.lust >= 30) {
				addButton(index, "Masturbate", inDungeon ? masturbateGo : masturbateMenu).hint("Selecting this option will make you attempt to manually masturbate in order to relieve your lust buildup.");
				if ((player.hasPerk(PerkLib.HistoryReligious) && player.isPureEnough(66) || player.hasPerk(PerkLib.Enlightened) && player.isPureEnough(10)) && (!player.hasStatusEffect(StatusEffects.Exgartuan) || player.statusEffectv2(StatusEffects.Exgartuan) != 0) || flags[kFLAGS.SFW_MODE] >= 1) {
					addButton(index, "Meditate", masturbateMenu).hint("Selecting this option will make you attempt to meditate in order to reduce lust and corruption.");
				}
			}
			else {
				addButtonDisabled(index, "Masturbate", "You are not horny enough to consider doing that.");
			}
		}
		...
		private function fappingItems(menus:Boolean = true):Boolean {
			...
			if (player.hasKeyItem("All-Natural Onahole") >= 0 && canReachCock) {
				if (menus) addButton(button, "AN Onahole", allNaturalOnaholeUse).hint("An all-natural onahole, this device looks more like a bulbous creature than a sex-toy. Nevertheless, the slick orifice it presents looks very inviting.", "All-Natural Onahole");
				button++;
			}
			if (player.hasKeyItem("Deluxe Onahole") >= 0 && canReachCock) {
				if (menus) addButton(button, "D Onahole", deluxeOnaholeUse).hint("This is a deluxe onahole, made of exceptional materials and with the finest craftsmanship in order to bring its user to the height of pleasure.", "Deluxe Onahole");
				button++;
			}
			if (player.hasKeyItem("Plain Onahole") >= 0 && canReachCock) {
				if (menus) addButton(button, "Onahole", onaholeUse).hint("This is what is called an 'onahole'. This device is a simple textured sleeve designed to fit around the male anatomy in a pleasurable way.", "Onahole");
				button++;
			}
			if (player.hasKeyItem("Self-Stimulation Belt") >= 0 && player.vaginas.length > 0 && !player.isTaur()) {
				if (menus) addButton(button, "Stim-Belt", stimBeltUse).hint("This is a self-stimulation belt. Commonly referred to as stim-belts, these are clockwork devices designed to pleasure the female anatomy.", "Self-Stimulation Belt");
				button++;
			}
			if (player.hasKeyItem("All-Natural Self-Stimulation Belt") >= 0 && player.vaginas.length > 0 && !player.isTaur()) {
				if (menus) addButton(button, "AN Stim-Belt", allNaturalStimBeltUse).hint("This is an all-natural self-stimulation belt. The methods used to create such a pleasure device are unknown. It seems to be organic in nature.", "All-Natural Stimulation Belt");
				button++;
			}
			if (player.hasKeyItem("Dual Belt") >= 0 && player.gender == 3 && !player.isTaur()) {
				if (menus) addButton(button, "Dual Belt", dualBeltMasturbation).hint("This is a strange masturbation device, meant to work every available avenue of stimulation.", "Dual Belt");
				button++;
			}
			if (player.hasKeyItem("Fake Mare") >= 0 && player.hasCock() && player.isTaur()) {
				if (menus) addButton(button, "Fake Mare", centaurDudesGetHorseAids).hint("This fake mare is made of metal and wood, but the anatomically correct vagina looks as soft and wet as any female centaur's.", "Fake Mare");
				button++;
			}
			if (player.hasKeyItem("Centaur Pole") >= 0 && player.hasVagina() && player.isTaur()) {
				if (menus) addButton(button, "C. Pole", centaurGirlsGetHorseAids).hint("This 'centaur pole' as it's called appears to be a sex-toy designed for females of the equine persuasion. Oddly, it's been sculpted to look like a giant imp, with an even bigger horse-cock.", "Centaur Pole");
				button++;
			}
			...
			if (player.cocks.length == 1) {
				...
				if (player.cocks[0].cockLength < 12) {
					if (player.countCocksOfType(CockTypesEnum.HUMAN) >= 1)
						...
						outputText("You stroke quickly, pleasuring the supple length of your tentacular endowment, fondling every inhuman nodule as you slide along every " + UnitSystem.literalInch() + " of twisting length. ");
					...
					if (player.hasFuckableNipples() && player.biggestTitSize() >= 3) {
						...
						if (player.cor > 60) {
							if (player.countCocksOfType(CockTypesEnum.HUMAN) >= 1)
								outputText("The heady aroma of your cock fills your nostrils as you force " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " into your mouth, deepthroating as much of yourself as you can. ");
							else if (player.countCocksOfType(CockTypesEnum.HORSE) >= 1)
								outputText("The thick animalistic scent fills your nostrils as you force " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " into your mouth, deepthroating as much of yourself as you can. ");
							else if (player.dogCocks() >= 1)
								outputText("The strong animal scent of your cock fills your nostrils as you force " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " into your mouth, deepthroating as much of yourself as you can. ");
							else if (player.countCocksOfType(CockTypesEnum.TENTACLE) >= 1)
								outputText("The sweet scent of your " + player.cockDescript() + " fills your nostrils as " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of tentacle forces its way down your throat. You struggle briefly with it, wrestling to keep it from pushing the whole way into your gut, swooning with the pleasure that fighting with your own cock induces. ");
							else if (player.countCocksOfType(CockTypesEnum.DEMON) >= 1)
								outputText("The spicy demonic odor your " + player.cockDescript() + " radiates fills your nostrils as you force " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of tainted meat down your throat. You struggle briefly, but your " + player.cockDescript() + " quickly overwhelms your resistance and your gag reflex momentarily seems to disappear. ");
							else if (player.countCocksOfType(CockTypesEnum.CAT) >= 1)
								outputText("The sweet, slightly tangy scent of your cock fills your nostrils as you force " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " into your mouth, deepthroating as much of the prickly shaft as you can. The soft spines that coat its surface actually feel quite pleasant as you force it deeper, denying your gag reflex. ");
							else if (player.countCocksOfType(CockTypesEnum.LIZARD) >= 1)
								outputText("The salty, dry odor of your " + player.cockDescript() + " fills your nostrils as you force " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " into your mouth, swallowing as much of the bulgy shaft as you can. ");
							else if (player.countCocksOfType(CockTypesEnum.ANEMONE) >= 1)
								...
								outputText("The strong animal scent of your cock fills your nostrils as you force " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " into your mouth, deepthroating as much of yourself as you can. ");
							else outputText("The heady aroma of your cock fills your nostrils as you force " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " into your mouth, deepthroating as much of yourself as you can. ");
						}
						autofellatio = true;
					}
					if (player.canTitFuck() && player.biggestTitSize() > 3 && !nippleFuck) {
				...
				//Tittyfuck if D cup+
					...
					outputText("You look down at your trembling members, mere " + UnitSystem.literalInches() + " away, and tentatively lick at a cocktip. Both dicks begin oozing pre in response. ");
					//Try to stick it in a titty!
				...
				//Tittyfuck if D cup+
					...
					//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
					outputText("You look down at your trembling members, mere " + UnitSystem.literalInches() + " away, and tentatively lick at a cocktip. Your dicks begin oozing pre in response. ");
					//Try to stick it in a titty!
					if (player.hasFuckableNipples()) {
						//check for dogcocks
						if (player.dogCocks() > 0) {
			...
			if (player.vaginas.length > 0) {
				...
				if (player.vaginas.length == 1) {
					...
					//Big clits
						...
						if (player.vaginas[0].vaginalWetness > Vagina.WETNESS_DRY) outputText("glistening ");
		...
		private function genderlessMasturbate():void {
			clearOutput();
			outputText(images.showImage("masti-genderless-fuck"));
			//Early prep
			doStripCheck();
			//Tit foreplay
			titForeplay();
		...
		*/
		...
		private function orgazmo(selfSucking:Boolean, nippleFuck:Boolean):void {
			...
			if (player.vaginas.length > 0) {
				...
				if (player.vaginas.length == 1) {
					...
					//Wet orgasms
					if (player.vaginas[0].vaginalWetness == Vagina.WETNESS_SLAVERING) outputText("A veritable gush of fluid explodes from your nethers, pulsing in time with the ripples of your " + player.vaginaDescript() + ". ");
				}
		...
		public  function titDrink():void {
			...
			outputText(images.showImage("masti-tit"));
		...
		private function deluxeDildo():void {
			...
			//[USE FEMALE]
			if (player.hasVagina()) {
				outputText(images.showImage("masti-dildo-vag"));
				//(highcor)
				if (player.cor > 66)
		...
		private function deluxeDildoAnal():void {
			player.slimeFeed();
			clearOutput();
			outputText(images.showImage("masti-dildo-anal"));
			//(highcor)
			if (player.cor > 66)
				outputText("You retrieve the floppy pink dildo from your possessions and strip down in the middle of your camp, shivering with the sexual thrill of your exhibitionism.");
			//(medcor)
			...
			//(Pretty wet)
			else if (player.ass.analWetness < Ass.WETNESS_SLIME_DROOLING) {
				outputText("The toy's girth seems to pulse and swell within you, spreading you wide open as it sops up your unnatural wetness and grows larger. You grab it in a two-handed grip and begin working it in and out of your [asshole], gasping and twitching as every ridge and feature of the dildo rubs you just right. Every " + UnitSystem.literalInch() + " of your [ass] tingles with a desire to be touched, rubbed, and squeezed. ");
				if (player.cocks.length > 0) {
					outputText("Even your " + player.multiCockDescript() + " ache");
					if (player.totalCocks() == 1) outputText("s");
					outputText(" and pulse");
		...
		private function onaholeUse():void {
			...
			clearOutput();
			outputText(images.showImage("masti-Onahole"));
		...
		private function onaholeFutaContinuation():void {
			...
			outputText(UnitSystem.displayFeet(Math.random() * player.str / 12 + player.str / 6, 1));
			outputText(" away from you. Delirious with pleasure, you continue your 'impression' of a semen volcano, covering yourself and the area with your seed. ");
		...
		private function allNaturalOnaholeUse():void {
			clearOutput();
			outputText(images.showImage("masti-Onahole-allNatural"));
			//First use!
		...
		private function stimBeltUse():void {
			...
			if ((player.hasKeyItem("Self-Stimulation Belt") >= 0)) {
				outputText(images.showImage("masti-stimBelt"));
		...
		private function catAutoLick():void { //Male cat masturbation
			...
			if (flags[kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY] == 0) {
				...
				if (player.countCockSocks("gilded") > 0 && flags[kFLAGS.GILDED_JERKED] < player.countCockSocks("gilded")) {
					...
					outputText(" along with a sudden chill from your Gilded cock sock causing you to reflexively pull off your " + player.cockDescript() + "'s tip just as the complete bliss of orgasm fills your body. Your face is less than " + UnitSystem.literalAnInch() + " from your cock head as you watch your cum shoot up into the air. Caught in the light of the golden cocksock, it beads and twists in the light, crystallizing into a glittering shower. A ");
					if (player.cumQ() < 25)
			...
			//[Repeatable]
				...
				if (player.countCockSocks("gilded") > 0 && flags[kFLAGS.GILDED_JERKED] < player.countCockSocks("gilded")) {
					...
					if (flags[kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY] > 10 && player.balls > 1 && rand(5) == 0) {
						...
						outputText("\n\nFinally satiated, you begin untangling yourself and realize how sweaty and sticky you are. Again, remembering the cats from your village, you begin to lick ever square " + UnitSystem.literalInch() + " clean you can reach just like they do and you discover a new form of pleasure. After you lick yourself clean, you stretch out into the spread-eagle position to get a few small kinks out and to admire your naked body glistening in your spit. As you begin to doze off, <b>you think your balls feel a little denser.</b>");
		...
		public function meditate(description:String = "rock"):void {
			clearOutput();
			if (player.gender == 2) outputText(images.showImage("masti-meditate-female"));
			else outputText(images.showImage("masti-meditate-male"));
		...
		private function dualBeltMasturbation():void {
			clearOutput();
			outputText(images.showImage("masti-stimBelt-play"));
			outputText("You look at the thing in front of you and sigh, your " + player.multiCockDescriptLight() + " and " + player.vaginaDescript() + " dripping with fluids. With a nervous sigh you step into the underwear looking device and slip it up your legs, the cold metal feeling so good on your " + player.skin.desc + ", sending a rush of feelings up your body. You slip your " + player.cockDescript() + " down and into a hole on the front of the belt, the extra length sticking out on the other side of it. Underneath the hole and right above your pussy is another metal square with what feels like a small nub on the inside rubbing against your clit. Shivering from the feeling of it, you stay there for a moment, waiting in anticipation. Finally, you reach down to the side of the belt and flick the switch to the on position.\n\n");
			...
			if (player.tail.type > Tail.NONE) outputText("covering up your tail, ");
		...
		private function centaurNippleCunt():void {
			...
			outputText(images.showImage("masti-centaur-nipple"));
			outputText("Shivering with pleasure, you kneel down to prevent your legs from going out from underneath you.  Your " + player.faceDescript() + " flushes hotter as you toy with the slippery nipple-cunts, feeling them growing hard and puffy as you turn yourself on more and more. ");
		...
		private function centaurHugeCock():void {
			...
			outputText("\n\n");
			outputText(images.showImage("masti-centaur-huge"));
			...
			if (player.cumQ() < 50)
				...
				if (player.cumQ() > 3000) outputText(" As you finish cumming, the puddle deepens. It's " + UnitSystem.literalInches() + " thick and splattering everywhere whenever you take a step.");
			}
		...
		private function centaurCantMasturbate():void {
			outputText(images.showImage("masti-centaur-fail"));
		...
		private function centaurDudesGetHorseAids():void {
			...
			outputText(images.showImage("masti-centaur-cPole-male"));
		...
		private function centaurGirlsGetHorseAids():void {
			...
			outputText(images.showImage("masti-centaur-cPole-female"));
		...
		private function getHugeEggsInCawk():void {
			clearOutput();
			outputText(images.showImage("masti-cock-eggs"));
		...
		private function layEggsInYerTits():void {
			clearOutput();
			outputText(images.showImage("masti-eggs-in-tits"));
		...
		private function tentacleSelfFuck():void {
			...
			outputText(images.showImage("masti-tentacle-vag"));
			outputText("You arch your back as you try to contain the unexpected waves of desire that flood your groin, but it's no use. In the span of a few seconds, you decide to accept that you need to fuck something now. Foreplay is no longer an option.");
			if (player.balls > 0)
		...
		private function tentacleGoesUpYerPooperNewsAtEleven():void {
			...
			doStripCheck();
			outputText(images.showImage("masti-tentacle-butt"));
		...
		private function gildedCockTurbate():void {
			...
			outputText(images.showImage("masti-cocksock"));
			outputText("Your fingers slide easily over the smooth metallic fabric and you begin masturbating. The gold sheath shifts up and down against your shaft as you stroke faster and faster. The gold casing blocks the familiar feel of your hand, instead feeling like the hand of some eager stranger on your dick.");
