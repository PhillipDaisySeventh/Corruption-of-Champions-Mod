	...
	public class DeepCave extends DungeonAbstractContent
	{
		...
		*/
		...
		public function getSwordAndGetTrapped():void {
			...
			if (player.spe < 50) outputText("fall flat on your " + player.assDescript() + ", slipping on the slick, shifting surface.");
		...
		public function getTrappedContinuation():void {
			...
			outputText("Meanwhile, a number of tentacles have sprung up from below, and are crawling up your " + player.legs() + ".  It's becoming fairly clear how the skeleton wound up in this cave...  You've got to escape!");
		...
		public function loseToVala():void {
			...
			if (player.gender == 0) {
				clearOutput();
				outputText("Vala forces a bottle into your throat before your defeated form has a chance to react, and you grunt with pleasure as a new gash opens between your " + player.legs() + "!");
				player.createVagina();
		...
		public function loseToValaAsMale():void {
			...
			outputText("You are powerless to stop the fairy as she drags you to the south wall and up to the wooden rail secured " + UnitSystem.display("a couple of feet", "half a " + UnitSystem.meter()) + " off the ground. \"<i>When she was still growing, Bitch was too small and tight for the masters,</i>\" your captor tells you. \"<i>They blessed her with this ladder to make us big enough. You will feel their generosity.</i>\" Gripping you under the arms, the fairy's lust-fuelled strength lifts you off the ground and flies you directly over the bristling peg ladder.\n\n");
			//[Next]
		...
		//	if (player.ass.analLooseness <= 1) {
			outputText("\"<i>It will never please them like that,</i>\" she scolds. \"<i>You must be made more to their liking or they will never grant you endless joy.</i>\" She grinds her button-stiff clit against your abdomen as she lowers you toward the smallest peg on the rail, an uncarved, lacquered wooden nub " + UnitSystem.anInch() + " wide and " + UnitSystem.displayInchesTextually(3) + " long, barely larger than a finger. You try to attack the fairy before she can plug you in, but she simply drops you the rest of the way, and what should've been a relatively painless insertion becomes agonizing as you hit the peg and " + UnitSystem.displayInchesTextually(3) + " of hardened wood fill your " + player.assholeDescript() + ". You gasp and try to get off the device, but the fairy has already grabbed you again and pulls you back into the air. You clench your muscles as you look at the far end of the ladder in horrified fascination at a wooden carving that would shame a minotaur. The fairy moves up a couple of notches.");
		...
		public function impGangGetsRapedByFemale():void {
			...
			outputText("At last you feel it bottom out, bumping against your cervix with the tiniest amount of pressure.  Grinning like a cat with the cream, you swivel your hips, grinding your " + player.clitDescript() + " against him in triumph.  ");
			if (player.getClitLength() > 3) outputText("You stroke the cock-like appendage in your hand, trembling with delight.  ");
		...
		public function loseToImpMob():void {
			...
			if (player.biggestTitSize() > 1) {
				...
				if (player.biggestLactation() > 1) outputText("  Milk starts to squirt from the pressure being applied to your " + player.breastDescript(0) + ", which only encourages the imp to squeeze even harder.");
				outputText("\n\n");
			}
			//(NIPPLECUNTS!)
			if (player.hasFuckableNipples()) {
				outputText("A rough tweak on one of your nipples startles you, but your grunt of protest is turned into a muffled moan when one of the imp's tiny fingers plunges inside your " + player.nippleDescript(0) + ".  He pulls his hand out, marveling at the sticky mess, and wastes no time grabbing the top of your tit with both hands and plunging himself in.");
			...
			if (player.biggestTitSize() > 1) {
				...
				if (player.hasFuckableNipples()) {
					...
					outputText(" of cocks buried in your " + player.nippleDescript(0) + " pull free before they cum, dumping the spooge into the gaping holes they've left behind.  It tingles hotly, making you quiver with pleasure.");
				}
			}
			...
			if (player.cockTotal() == 0) outputText("and you clench tightly around the uncomfortable intrusion.");
				...
				outputText("and " + player.sMultiCockDesc() + " unloads, splattering the many demons with a bit of your own seed.  You'd smile if your mouth wasn't so full of cock.  At least you got to make a mess of them!");
			}
			if (player.hasVagina()) {
		...
		public function femaleZetazOver():void {
			...
			outputText("Your lips start to tingle, and you run your tongue over them reflexively.  A shiver of pleasure worms through your body, and you instinctively press your " + player.legs() + " against the straps in an effort to spread them further.  Worse yet, your lips feel much plumper and fuller than a few moments ago.  ");
		...
		public function femaleZetazOverPtII():void {
			...
			if (player.biggestTitSize() < 1) outputText("tweak your " + player.nippleDescript(0) + "s roughly, pulling and yanking on them as");
			else outputText("maul at your " + player.allBreastsDescript() + ", slapping and squeezing them as");
		...
		public function malesZetazOverPtII():void {
			...
			outputText("While you observe the room, you realize that the waves of pleasure sliding up your spinal cord haven't stopped, and that your entire body is being shaken rhythmically.  You look down with a look of incredible, still-drugged confusion and behold the last thing you expected to see.  Somehow " + player.sMultiCockDesc() + " has been shrunk to less than half of its previous size");
		...
		public function badEndValaNumber2Pt2():void {
			...
			outputText("The space between your shoulder blades feels like it's been torn open and your muscles reknit themselves as gossamer wings burst from your skin, thin as a dragonfly's and nearly as long as you are tall, settling against their rubber counterparts. Every " + UnitSystem.literalInch() + " of your skin seems to blister as a feeling of molten glass pouring over you causes you to tremble with agonized shudders, your pores sealing and skin gaining a glossy sheen.\n\n");
		...
		public function loseToThisShitPartII():void {
			...
			//[OPTIONAL CUM ESCAPE]
			if (player.cumQ() > 3500) {
				outputText("Your orgasm drags on for so long that you begin to feel pressure from the cum-slime surrounding you.  It doesn't seem to matter to " + player.sMultiCockDesc() + ", which is too busy sending bliss to your brain and squirting cum for the tentacles to care.  It actually kind of hurts.  The oscillating purple ambiance flashes brighter in protest for a second, and then everything releases all at once.  The pressure is gone and you're sliding down on a wave of fungal-slime cum, feeling the tentacles being pulled from you by the sudden shift of position.  Moist cave air tickles at your " + player.skin.desc + " as you come to rest on another spongy petal and begin to cough out the sludge.\n\n");
			...
			if (player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
				...
				outputText("The vine-like stalks currently hugging " + player.sMultiCockDesc() + " constrict the base and begin to swirl around it in a circular motion.  Warm fungi-flesh and viscous, drugged ooze work together to send hot spikes of pleasure up your spinal-cord.  Despite your recent orgasm, you aren't being given any chance to recover or refill your ");
		...
		public function roomFungusCavern():void {
			...
			outputText("This cavern is huge!  Though you can see the edge of a large stalactite to the west, the rest of the cave disappears into darkness beyond " + UnitSystem.displayFeetEstimateRangeTextually(20, " or ", 30) + " away.  The floor is covered in spongy, leaf-shaped fungus.  They're huge, shiny, and purple, and they cover the cavern floor for as far as the illumination will reach.  ");
			if (flags[kFLAGS.ZETAZ_FUNGUS_ROOM_DEFEATED] == 0) {
				outputText("A strange, sweet smell hangs in the cavern's humid air, probably coming from the copious fungal flora.  At the edge of your vision you can see a humanoid skeleton propped up against a stalagmite.  There's a rapier laying " + UnitSystem.aFewFeet() + " in front of it, and it still looks as good as new.  What do you do?");
			}
		...
		public function roomZetazChamber():void {
			...
			if (flags[kFLAGS.DEFEATED_ZETAZ] == 0) {
				outputText("A familiar imp is looking at you with a bewildered expression painted across his face.  You recognize his face immediately – this is Zetaz!  Oddly, he seems to have grown much larger in the time since your previous meeting.  He's over " + UnitSystem.displayHeight2EstimateTextually(4) + " tall and much more solidly built!\n\n");
