package classes.Scenes.Dungeons.Factory 
{
	...
	public class SecretarialSuccubusScene extends BaseContent
	{
		...
		public function encounterSuccubus():void {
			...
			if (player.demonScore() >= 4 && player.isCorruptEnough(75)) {
		...
		private function goDemon():void {
			...
			if (player.gender == 1) {
				...
				if (player.cockTotal() > 1) {
					...
					if (player.balls > 0) outputText("your " + player.ballsDescriptLight() + " and ");
					...
					if (player.countCocksOfType(CockTypesEnum.DEMON) > 0) outputText("growing");
					else outputText("new");
			...
			//[female]
				...
				if (player.getClitLength() >= 2) outputText("Your " + player.clitDescript() + " pushes free, nuzzling against her hairless cunt and slipping inside, as if drawn in by its desire.  She openly moans, and begins rocking on top of you.  You gasp in delight as she rides your " + player.clitDescript() + ", fucking and grinding against it.  ");
			...
			//[HERM ENDING]
				...
				outputText("She effortlessly swings her lissome legs onto the table as she pulls herself up, mounting you in a single swift motion.  You can feel waves of heat rolling off her sex, bathing your " + player.cockDescript(0) + " in her warmth.  ");
				if (player.getClitLength() >= 2) outputText("Your " + player.clitDescript() + " pushes free, nuzzling against her tight asshole and slipping inside, as if drawn in by its desire.  She openly moans, and begins rocking on top of you.  You gasp in delight as she rides your " + player.clitDescript() + ", fucking her ass and grinding against it.");
		...
		public function doRapeSuccubus():void {
			...
			if (player.cocks.length > 0 && (player.gender != 3 || rand(2))) {
				...
				if (player.hair.length > monster.hair.length) outputText("lengthens");
				else outputText("shortens");
				outputText(", shifting to " + player.hair.color + ".  The bone structures of her cheeks, nose, and face shift ever so slightly, and you suddenly realize you are looking down at a slutty version of yourself!  You aren't sure if it's the growing pool of succubus fluid below you or how hot your female twin is, but your " + player.cockDescript(0) + " is as hard as a rock.\n\n");
				...
				if (player.cockArea(0) > monster.vaginalCapacity()) {
					...
					if (player.cumQ() >= 50 && player.cumQ() < 500) {
						...
						if (player.balls > 0) outputText("Your " + player.ballsDescriptLight() + " practically glow with relief as they begin contracting.  Their entire surface is covered with black veins that radiate from your demonic prick, sharing the corruption with your sperm factories.  ");  
				...
				//((IT FITS))
					...
					outputText("You plunge in to her velvety depths and feel her rippling cunt-muscles contract tightly around you for a perfect fit.  She gasps as each cock-distorting nodule bounces her " + UnitSystem.displayInchWithHyphenTextually(2) + " clitty, making your mirror image moan like a bitch in heat.  The corrupted fluids dripping from her snatch squelch loudly, making your groin burn with pleasure.  ");
					...
					//New PG
					outputText("Each of the new bumps and rounded spines of your " + player.cockDescript(0) + " flood with blood, doubling in size as orgasm overtakes you.  ");
					outputText("Your hips bury your entire length inside her, acting on their own as your " + player.cockDescript(0) + " clenches, pouring more and more spunk inside her");
					if (player.balls > 0) outputText(" as your balls empty their load queue");
					outputText(".  ");
					//Big cum
					if (player.cumQ() >= 50 && player.cumQ() < 400) outputText("You inhale as black cum spatters from her entrance, her belly distending slightly as you empty what feels like " + UnitSystem.aGallon() + " inside her.  ");
					//((Ginormohuge)) 
			...
			//FEMSAUCE
				...
				outputText("Her budding clit rises from between her folds, hardening like a tiny " + UnitSystem.displayInchWithHyphenTextually(3) + " dick.\n\n");
				menu();
		...
		public function doScissorSuccubus():void {
			...
			if (player.getClitLength() >= 3) outputText("You groan with wanton desire as your " + player.clitDescript() + " continues to grow and grow until reaching full size and slipping inside the defeated slut's sloppy pleasure-hole.  ");
			outputText("It takes only a few seconds to get the succubus' juices really flowing, the sounds of your grinding hips dissolving into a cacophony of liquid squelches.  The gooey corrupt fem-cum tingles, spreading warmth through every patch of skin it touches.  Your locked hips writhe and twist with her's, eliciting pants and squeals from the both of you.  In no time flat, you find yourself cumming and feel your " + player.vaginaDescript(0) + "'s muscles clench hungrily with an unquenchable desire to be filled.  The succubus shivers in pleasure, probably feeding off your orgasm.  You back off, fingering your " + player.vaginaDescript(0) + " languidly and denying her a full meal.  Pouting, the succubus dips her fingers back in, determined to cum.");
		...
		public function doForcefeedSuccubus():void {
			clearOutput();
			outputText("You chuckle as you decide to release some of the pent up pressure in your " + player.allBreastsDescript() + ".  Laying down over your conquest, you grasp her wrists and pin them to the floor as you shove your tits in her face");
		...
		public function doLossSuccubus():void {
			...
			if (player.cocks.length > 0) {
				...
				if (player.balls > 0) {
					//[[balls]]
					if (player.ballSize < 6) outputText("Your sultry captor leans low over you, her luscious tits wobbling enticingly as she reaches down and caresses your " + player.ballsDescriptLight() + " with soft touches.  Almost immediately you feel them clench with boiling heat, growing heavy and churning with a load big enough to satisfy a thirsty succubus."); 
					//[[huge balls]]
					else outputText("Your sultry captor leans low, marveling at the size of your " + player.ballsDescriptLight() + " as she reaches down to caress them.  Her tits swing lewdly above you, bouncing in hypnotic motions. Her hands work gently, taking each one of your " + player.ballsDescriptLight() + " and hefting it gently.  Almost immediately you feel them fill with an unnatural heat that spreads everywhere her slender fingers touch.  They begin to feel full and heavy, practically sloshing as the pent up need inside you is channeled into liquid form.  \"<i>So ripe... and full,</i>\" she whispers to herself as she caresses them, her silken foot still sliding all over your " + player.cockDescript(0) + ", pumping stroke after stroke of pleasure into your lust-weakened form.");
				}
				else {
					//[[no balls no pussy]]
				...
				outputText(" as she releases your " + player.cockDescript(0) + ", shouting, \"<i>CUM!  Feed me!</i>\"  You are all too happy to oblige.  ");
				//[[normal volume]]
				...
				if (player.lust >= player.maxLust()) {
					outputText("Driven half mad with lust, you shake yourself free from the trappings of your " + player.armorName + ", first revealing your " + player.allBreastsDescript() + ", then " + player.hipDescript() + " and finally your ");
				...
				if (player.vaginas.length > 0) outputText("your " + player.clitDescript() + " parts your folds, growing harder.");
				else outputText("a fleshy growth erupts from your pale flesh, growing harder.");
				outputText("  In seconds you're playing with it, tugging the sensitive button as it fills up with more and more blood, growing bigger and harder than ever before.  Your legs give out as you begin stroking it with feverish intensity, barely registering as it grows to nearly " + UnitSystem.displayInchesEstimateTextually(18) + " in length, not noticing the increasingly veiny surface or different texture at the tip.  You force yourself to stop as a sudden truth asserts itself upon your consciousness - you need to shove your clit-like cock into a pussy.  You need to cum inside that hungry slut's blue spunk-receptacle.\n\n");
		...
		public function doBadEndSuccubusPart1():void {
			...
			//Tits – regular
			if (player.biggestLactation() < 1) outputText("Your " + player.nippleDescript(0)  + "s begin prodding painfully against your " + player.armorName + ", every touch serving to make them harder and more erect.  ");
			//Tits – lactating
			if (player.biggestLactation() >= 1 && player.biggestLactation() < 3) outputText("Your " + player.nippleDescript(0) + "s get painfully hard as you feel milk begin backing up inside your " + player.allBreastsDescript() + ".   The succubus glances down mischievously as her hands begin to grope you through your " + player.armorName + ", squeezing out a few drops of milk.  ");
			//Tits – megalactating
			...
			if (player.vaginas.length > 0) {
				//'uge clit
				if (player.getClitLength() >= 4.5) outputText("Popping from between your thighs, your " + player.clitDescript() + " responds to the sheer hotness of the situation by making itself known.   You squeeze your legs tightly together, hungry for additional sensation.  ");
				//big clit
			...
			if (player.cocks.length == 0) {
				...
				if (player.vaginas.length > 0) outputText("a vacuum pump to your clit, and a pump many times bigger than your " + player.cockDescript(0) + " to it.  ");
		...
		private function doBadEndSuccubusPart2():void {
			...
			//Nipples
			outputText("The suction pulls squirt after squirt of milk from your breasts as your " + player.allBreastsDescript() + " start to grow, swelling heavier as they enlarge to produce more milk.  You squeal with delight as your nipples turn black, tainted by corruptive chemicals that are slowly dripped into you.  ");
			//Dick  
			if (player.cocks.length > 0) {
				...
				//either or:
				outputText("Dimly, you feel a needle lodged in your taint, pumping your prostate full of something.  Your " + player.cockDescript(0) + " begins growing mid-orgasm, the skin turning a deep purple even as small nodule-like bumps form all over it, rapidly becoming a bloated parody of it's demonic counterparts.  ");
			}
			//Puss Orgasmz
