	...
	public class IncubusMechanicScenes extends BaseContent
	{
		...
		public function doLossIncubus():void {
			...
			//Tentacle gangbang
				...
				if (player.lust >= player.maxLust()) outputText("Molten arousal pumps through your veins, burning away your reason with an unquenchable desire to mate. You drop your top, exposing your " + player.allBreastsDescript() + " to your foe in a submissive display, ");
				...
				if (player.biggestTitSize() >= 2) outputText("They make sure to rub each of your breasts, spending a few seconds smearing slick pre-cum into your " + player.nippleDescript(0) + ".  ");
				outputText("A pair of them slides into your undergarments, pressing against your needy crotch and teasing your " + player.assholeDescript() + " with more slick demonic cum.  ");
				...
				if (player.vaginas.length > 0) outputText("squeezing your legs around them and grinding your " + player.clitDescript() + " against the oddly textured demon-cock.");
				else {
				...
				if (player.vaginas.length > 0 && (player.cocks.length == 0 || rand(2))) {
					...
					outputText("You then giggle and try to wiggle your " + player.hipDescript() + " forward, begging for him to take you and quench the fire burning in your " + player.vaginaDescript(0) + ".  The incubus obliges, pressing forth with both drooling members and simultaneously plugging your front and back doors.  ");
					...
					//FemCum
					if (player.getClitLength() > 3) outputText("You nearly cum on the spot when the cock fucking your pussy loops its length around your " + player.clitDescript() + ", the cum-slickened coils driving you mad with pleasure as they coil, slide, and jerk around your clit as if it was a cock.  ");
					else outputText("You nearly cum on the spot when the cock fucking your pussy curves up to rub its textured nodules against your " + player.clitDescript() + ".  ");
				...
				//Male/Genderless
					...
					if (player.cocks.length > 0) outputText("  As your body's orifices fill with more and more dripping demonic dick-flesh, your " + player.cockDescript(0) + " becomes painfully hard.  Another tentacle wastes no time in wrapping itself tightly around the base like a cock-ring.  The rest of the demon's prehensile tool slides along your shaft, curling around to squeeze and jerk you off.");
		...
		public function doRideIncubusAnally():void {
			...
			outputText("You awaken moments later beside a sleeping, limp, and drained incubus. You have definitely come out on top from the encounter. Though you feel stretched, sticky and a little sore, for the moment at least the burning desire to fill your " + player.assholeDescript() + " is satisfied.");
			player.buttChange(monster.cockArea(0), true);
			player.slimeFeed();
			player.orgasm('Anal');
			dynStats("cor", 2);
		...
		public function doRapeIncubus():void {
			...
			if (player.cocks.length > 0) {
				...
				outputText(".  The demon underneath you grunts in an attempt to sound displeased, but it's plain to see the pre-cum he's dripping all over the floor.  What a slut!  You slap his ass and begin roughly butt-fucking him, panting with each plunge into the depths of his ridged passage, gradually increasing your tempo until your " + player.hipDescript() + " fill the room with loud slapping noises.");
				if (player.balls > 0) outputText("  Your " + player.ballsDescriptLight() + " swing freely, smacking into the demon's own and making both of you squeal and dribble more pre-cum.");
				outputText("\n\n");
				//(CUM)  
				if (player.balls > 0) outputText("You feel your " + player.ballsDescriptLight() + " draw up tight against your body.  ");
				outputText("Warm heat begins to build inside your groin, pooling under the base of your " + player.cockDescript(0) + ".  You realize you're about to paint this demon's gut with white, the thought only turning you on more.  ");
			...
			//(VAGINAL - Requires Vagina)
				...
				outputText("Relaxing the muscles in your " + player.legs() + ", you let a few " + UnitSystem.literalInches() + " of his length slip inside you, every nub and nodule of his corrupted prick filling the walls of your love-canal with inhuman pleasures that make your knees weak.  A particularly delightful bump brushes your " + player.clitDescript() + ", causing your " + player.legs() + " to finally give out. The incubus' nubbly cock plunges entirely inside you.\n\n");
		...
		public function doOralIncubus():void {
			...
			if (player.hair.length > 10) outputText("and brushing a strand of " + player.hair.color + " out of the way ");
		...
		public function suckIncubusDick():void
		{
			...
			outputText("\n\nA hand rubs through your [hair], and an affectionate voice encourages, \"<i>Ohhhh, that hits the spot.</i>\" With the corners of your mouth curling upward excitedly, you close your lips around that heady member, just behind the nubby tips, slipping down its length slowly, letting it ooze its pre-ejaculate in a line down the center of your tongue on its way into your throat. You get about " + UnitSystem.displayInchesEstimateTextually(4) + " before pulling back, gasping in a quick breath and swallowing his juice. It's so delicious that you find your tongue slipping back out to circle the head, lapping up the excess from his crown; it truly does seem to be a crown with the way the nubs adorn its perimeter, protruding like ornamental designs for your tongue to weave and slide around, sloppily polishing each to a mirrored spit-shine.");
			...
			outputText("\n\nYou lean in toward him once more, taking the incubus's first " + UnitSystem.displayInchesTextually(4) + " in with almost feverish eagerness, his throbbing thickness filling your mouth with its sinful heat, leaking its sticky lust into your throat. You whimper and move forward a bit, slowly pushing the head of his cock towards your throat. It would normally be difficult to take so much dick into your mouth, but he slides in easily. He's so wet and slick with spit. So very slippery. He doesn't bother your gag reflex at all, honestly. It’s like his cock was meant to fill your face, to slip through your lips and touch to your throat... to slide into your tight, constricting esophagus. You gulp another " + UnitSystem.literalInch() + " in, drawing him into your throat. It's so easy that you begin to do it almost reflexively, swallowing " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of dripping demon cock.");
		...
		private function rideMechanicsCockII(cType:int):void
		{
			...
			else if (cType == MECHANIC_HORZ_GOG)
			{
				...
				outputText("\n\nThe incubus squirms under you and lashes his tail to and fro. His whole body is quaking. Suddenly, his cock stops shrinking. There's so much skin at the base that it actually bulges out from his body in a decidedly equine sheath! He grunts, twisting this way and that, his movements mostly arrested by the weight of your [butt] atop him, and grows. The once-demonic member swells, pouring " + UnitSystem.literalInch() + " after " + UnitSystem.literalInch() + " of additional length out of his sheath. You gasp when you see a textured ring emerge from hiding. It's his medial ring.... which means the " + UnitSystem.displayInchWithHyphenTextually(11) + " monster sticking out is only half of his total length! While the rest of him lazily sprouts from the musky-smelling sheath, the width of his tool begins to expand, noticeably bulging and retracting, each time keeping a little bit more of its bulged-out size.");
			...
			{
				tightFit = true;
				outputText(" and feel the girthy boner slowly pry your lips further apart. He's so goddamn big that it feels like it's barely fitting inside. The first " + UnitSystem.literalInch() + " eventually slips into your tightly-clinging lips, forcing an inadvertent clench to run up the length of your channel. Your [vagina] noisily squelches around the slowly inserting member. You're stretched so full, and you're loving every moment of it. His "+ monster.cockDescriptShort() +" is so goddamn filling, and you've only taken the first few " + UnitSystem.literalInches() + "! The feel of him sliding through you is so fulfilling, so all-consuming, that you barely notice your [butt] touching down against his crotch");
				if (cType == MECHANIC_HORZ_GOG) outputText(", your belly bulging with his equine shape");
				else if (cType == MECHANIC_DOG_COCK) outputText(", your lips slurping noisily about his knot");
				outputText(".");
			}
		...
		private function maleLossToMechanic(hpVictory:Boolean):void
		{
			...
			else
			{
				...
				outputText("\n\nPure, biological need overwhelms you. The ache from your wounds is nothing next to the mountains of lust crushing your crotch with fuck-driven impetus. You forget your attempts at resistance. Though there are no pussies present, there is a hard, musky cock being dangled a scant few " + UnitSystem.literalFeet() + " away, its nubby, corruption-textured surface promising you a long, slow cum of your own if you only give into it. You lick your lips without meaning to, already falling to the idea.");
			}
