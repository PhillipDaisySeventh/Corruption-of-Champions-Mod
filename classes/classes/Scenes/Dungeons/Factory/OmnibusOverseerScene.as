	...
	public class OmnibusOverseerScene extends BaseContent
	{
		...
		private function chooseBreasts():void {
			...
			//Singular row - cup size + 3.  Nipple size to 1" if smaller.
				...
				outputText("Your " + player.allBreastsDescript() + " tingle pleasantly as the magic takes effect.  You watch with fascination as they begin to swell up, like sponges exposed to water.  The top of your " + player.armorName + " is pulled tight by change, until your chest seems ready to burst free.  <b>You've gained 3 cup sizes!</b>  ");
				player.breastRows[0].breastRating += 3;
				if (player.nippleLength < .75) {
					player.nippleLength += .5;
					outputText("Your " + player.nippleDescript(0) + "s grow hard and sensitive, becoming much more noticeable inside your " + player.armorName + ".  It appears your nipples are have grown larger to match.");
				}
			}
			//Multiple Rows...
			else {
				...
				outputText("The next row of " + player.breastDescript(1) + " jiggle and tingle with even more pleasure than the first.  They pulsate for a few seconds, shrinking and growing rapidly until they settle at a size just below your top " + player.breastDescript(0) + ".  ");
		...
		private function normalChest():void {
			...
			if (player.breastRows.length > 1) {
				...
				outputText("Your chest tingles and begins to feel lighter.  You hastily pull open your " + player.armorName + " and realize you only have " + player.allBreastsDescript() + " now!  ");
		...
		private function normalGroin():void {
			...
			//Remove multiple.
			if (player.cocks.length > 1) {
				outputText("Your " + player.multiCockDescriptLight() + " shiver and retract back towards your body.  When the process finishes you are left with only your " + player.cockDescript(0) + ".  ");
			...
			if (player.ballSize > 5) {
				temp++;
				outputText("The " + player.ballsDescriptLight() + " that constantly pull so heavily on your groin tingle and shrink down to a more managable size.  ");
		...
		private function doBadEndOmnibusPart1():void {
			...
			else outputText("Forgetting about the fight, you lean on the desk and slide your free hand under your " + player.armorName + ", seeking any pleasure you can get.  ");
		...
		private function doBadEndOmnibusPart2():void {
			...
			if (player.cocks.length > 1) {
				...
				outputText("Overloaded with pleasure, you feel your " + player.multiCockDescriptLight() + " pulse and cum, creating translucent green cum balloons the size of ");
			...
			if (player.cocks.length == 1) {
				...
				if (player.averageCockLength() < 15) outputText("With one swift motion, the beast engulfs your " + player.cockDescript(0) + " in its slimy maw.  ");
				else outputText("Distending obscenely, the beast starts engulfing your " + player.cockDescript(0) + " in its slimy maw, progressing along its entire length until you can no longer see your pleasure tool.  ");		
				outputText("The slimy tentacles waste no time, massaging you with mechanical precision.  You groan in helpless pleasure, growing to painful hardness within the squirming confines of the creature.  Three protrusions sprout from the creature's core, dripping with slime of their own, and covered on the inside with the same wriggling protrusions that now massage your trapped member.  Two curl around your " + player.hipDescript() + ", while the last one");
